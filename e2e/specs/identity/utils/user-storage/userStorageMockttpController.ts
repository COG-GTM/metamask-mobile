...
export interface UserStorageMockRequest {
  url?: string;
  path?: string;
  body?: { getJson: () => Promise<unknown> };
}

type UserStorageRequest = UserStorageMockRequest | CompletedRequest;
...
export interface UserStorageMockOverrides {
...
export const pathRegexps: Record<UserStorageGenericFeatureName, RegExp> = {
...
export class UserStorageMockttpController {
  ...
  async onPut(
    ...
    const data = (await request.body?.getJson()) as PutBody | undefined;
  ...
  async setupPath(
    path: UserStorageGenericFeatureName,
    server: Mockttp,
    overrides?: UserStorageMockOverrides,
  ) {
