...
// Loose types for the mutable fixture shape. The fixture object is a deeply
// nested state tree used by Detox tests; assignments happen at many levels.
// We keep top-level structure typed where it matters and fall back to
// `Record<string, unknown>` for sub-trees that vary heavily across tests.
export interface FixtureRecord { [key: string]: unknown }

export interface NetworkControllerFixtureState {
  networkConfigurationsByChainId: { [chainId: string]: FixtureRecord };
  selectedNetworkClientId: string;
  [key: string]: unknown;
}

export interface BackgroundFixtureState {
  NetworkController: NetworkControllerFixtureState;
  PermissionController: FixtureRecord;
  PreferencesController: FixtureRecord;
  KeyringController: FixtureRecord;
  TokensController: FixtureRecord;
  TransactionController: FixtureRecord;
  [key: string]: FixtureRecord | NetworkControllerFixtureState;
}

export interface FixtureStateRoot {
  engine: { backgroundState: BackgroundFixtureState };
  fiatOrders: {
    selectedRegionAgg: unknown;
    selectedPaymentMethodAgg: unknown;
    [key: string]: unknown;
  };
  [key: string]: unknown;
}
...
export interface FixtureData {
  state?: FixtureStateRoot;
  asyncState: FixtureRecord;
  [key: string]: unknown;
}

const getState = (fixture: FixtureData): FixtureStateRoot => {
  if (!fixture.state) {
    throw new Error('FixtureBuilder.fixture.state has not been initialized');
  }
  return fixture.state;
};

const getBackgroundState = (fixture: FixtureData): BackgroundFixtureState =>
  getState(fixture).engine.backgroundState;
...
class FixtureBuilder {
   ...
   */
  withAsyncState(asyncState: FixtureRecord): this {
   ...
   * @returns The FixtureBuilder instance for method chaining.
   */
  withState(state: FixtureStateRoot): this {
   ...
   * @returns The FixtureBuilder instance for method chaining.
   */
  withPermissionController(data: FixtureRecord = {}): this {
    merge(getBackgroundState(this.fixture).PermissionController, data);
    return this;
  }

  /**
   * Merges provided data into the background state of the AddressBookController.
   * @param data - Data to merge into the AddressBookController's state.
   * @returns The FixtureBuilder instance for method chaining.
   */
  withAddressBookController(data: FixtureRecord = {}): this {
    const state = getBackgroundState(this.fixture);
    if (!state.AddressBookController) {
      state.AddressBookController = {} as FixtureRecord;
    }
    merge(state.AddressBookController, data);
    return this;
  }
  ...
  withNetworkController(
    ...
    const networkController = getBackgroundState(this.fixture).NetworkController;
  ...
  createPermissionControllerConfig(
    additionalPermissions: FixtureRecord = {},
  ): FixtureRecord {
    const caip25Endowment = additionalPermissions[
      Caip25EndowmentPermissionName
    ] as { caveats?: { type: string; value: FixtureRecord }[] } | undefined;
        ...
        caveats: [
          {
            ...
            value: setEthAccounts(
              caip25CaveatValue as Parameters<typeof setEthAccounts>[0],
              [DEFAULT_FIXTURE_ACCOUNT],
            ),
          },
        ],
  ...
  withRampsSelectedRegion(region: FixtureRecord | null = null): this {
    const defaultRegion = {
    ...
    getState(this.fixture).fiatOrders.selectedRegionAgg = region ?? defaultRegion;
    return this;
  }
  withRampsSelectedPaymentMethod(): this {
    ...
    // Use the provided region or fallback to the default
    getState(this.fixture).fiatOrders.selectedPaymentMethodAgg = paymentType;
    return this;
  }

  /**
   * Adds chain switching permission for specific chains.
   * @param chainIds - Array of chain IDs to permit (defaults to ['0x1']), other nets like linea mainnet 0xe708
   * @returns The FixtureBuilder instance for method chaining.
   */
  ...
  withGanacheNetwork(_permissions?: unknown): this {
    const fixtures = getBackgroundState(this.fixture);
  ...
  withPopularNetworks(): this {
    ...
    for (const key in PopularNetworksList) {
      const network = (
        PopularNetworksList as { [k: string]: { providerConfig: { rpcUrl: string; chainId: string; ticker: string; nickname: string } } }
      )[key];
      const {
        rpcUrl: rpcTarget,
        chainId,
  ...
  withPreferencesController(data: FixtureRecord): this {
    merge(getBackgroundState(this.fixture).PreferencesController, data);
    return this;
  }

  withKeyringController(): this {
    merge(getBackgroundState(this.fixture).KeyringController, {
      keyrings: [
        {
          type: 'HD Key Tree',
  ...
  withTokens(tokens: unknown): this {
    merge(getBackgroundState(this.fixture).TokensController, {
      allTokens: {
        [CHAIN_IDS.MAINNET]: {
  ...
  withIncomingTransactionPreferences(
    ...
    merge(getBackgroundState(this.fixture).PreferencesController, {
  ...
  withTransactions(transactions: unknown): this {
    merge(getBackgroundState(this.fixture).TransactionController, {
      transactions,
    });
    return this;
  }
