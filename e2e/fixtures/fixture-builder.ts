...
class FixtureBuilder {
   ...
   */
  withAsyncState(asyncState: unknown) {
   ...
   * @returns {FixtureBuilder} - The FixtureBuilder instance for method chaining.
   */
  withPermissionController(data: unknown) {
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  withNetworkController(data: any) {
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  createPermissionControllerConfig(additionalPermissions: any = {}) {
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  withPermissionControllerConnectedToTestDapp(additionalPermissions: any = {}) {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  withRampsSelectedRegion(region: any = null) {
    const defaultRegion = {
  ...
  withPopularNetworks() {
    ...
    for (const key in PopularNetworksList) {
      const network = (PopularNetworksList as Record<string, { providerConfig: { type: string; chainId: `0x${string}`; rpcUrl: string; nickname: string; ticker: string; } }>)[key];
      const {
        rpcUrl: rpcTarget,
        chainId,
        ticker,
  ...
  withPreferencesController(data: unknown) {
    merge(
  ...
  withIncomingTransactionPreferences(incomingTransactionPreferences: unknown) {
  ...
  withTransactions(transactions: unknown) {
