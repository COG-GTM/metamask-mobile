...
function normalizeLocalNodeOptions(localNodeOptions: any): any[] {
  if (typeof localNodeOptions === 'string') {
    ...
    return localNodeOptions.map((node: unknown) => {
      const n = node as { type?: string; options?: Record<string, unknown> };
      ...
      if (typeof node === 'object' && node !== null) {
        ...
        const type = n.type || 'ganache';
        return {
          type,
          options:
            type === 'ganache'
              ? { ...defaultGanacheOptions, ...(n.options || {}) }
              : type === 'anvil'
  ...
  if (typeof localNodeOptions === 'object' && localNodeOptions !== null) {
    ...
    return [
      {
        type: 'ganache',
        options: { ...defaultGanacheOptions, ...(localNodeOptions as Record<string, unknown>) },
      },
    ];
  }
...
export const loadFixture = async (
  ...
  const state =
    (fixture as Record<string, unknown> | undefined) ||
    (new FixtureBuilder({ onboarding: true }).build() as unknown as Record<
      string,
      unknown
    >);
  await fixtureServer.loadJsonState(state, undefined);
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export async function withFixtures(options: any, testSuite: (ctx: any) => Promise<unknown>) {
  const {
    fixture,
    restartDevice = false,
    smartContract,
    ...
    if (dapp) {
      for (let i = 0; i < numberOfDapps; i++) {
        if (dappServer[i] && dappServer[i].listening) {
          await new Promise<void>((resolve, reject) => {
            dappServer[i].close((error: Error | undefined) => {
              if (error) {
                return reject(error);
              }
              return resolve();
