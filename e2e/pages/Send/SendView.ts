...
class SendView {
  ...
  async tapBackButton() {
    await Gestures.TapAtIndex(this.backButton as Promise<Detox.IndexableNativeElement>, 0);
  }
  ...
  async splitAddressText(){
    const attributes = (await (await this.sendAddressConfirmation).getAttributes()) as { label: string };
