...
export const waitForTestDappToLoad = async () => {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    try {
      ...
      await Assertions.webViewElementExists(TestDApp.testDappFoxLogo as any);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await Assertions.webViewElementExists(TestDApp.testDappPageTitle as any);

      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      await Assertions.webViewElementExists(TestDApp.DappConnectButton as any);
      return; // Success - page is fully loaded and interactive

    } catch (error: unknown) {
      if (attempt === MAX_RETRIES) {
        const message = error instanceof Error ? error.message : String(error);
        throw new Error(`Test dapp failed to load after ${MAX_RETRIES} attempts: ${message}`);
      }
