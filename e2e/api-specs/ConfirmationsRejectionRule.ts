...
interface Driver {
  runScript(script: string | ((...args: unknown[]) => unknown), args?: unknown[]): Promise<unknown>;
}
...
interface RpcMethod {
  ...
  examples?: { name: string; params: { value: unknown }[]; result?: { value: unknown } }[];
}
...
interface RpcCall {
  ...
  valid?: boolean;
  reason?: string;
  attachments?: { data: string; image: string; type: string }[];
}
...
export default class ConfirmationsRejectRule {
  ...
  async beforeRequest(_: unknown, call: RpcCall): Promise<void> {
  ...
  // Need this now?
  getCalls(_: unknown, method: RpcMethod): CallEntry[] {
    ...
    if (isMethodAllowed) {
      if (method.examples) {
        ...
        const p = ex.params.map((entry) => entry.value);
        const params =
          method.paramStructure === 'by-name'
            ? paramsToObj(
                p as Parameters<typeof paramsToObj>[0],
                (method.params ?? []) as Parameters<typeof paramsToObj>[1],
              )
            : p;
        calls.push({
  ...
  async afterRequest(_: unknown, call: RpcCall): Promise<void> {
    await new Promise((resolve, reject) => {
      addToQueue({
        name: 'afterRequest',
        resolve,
  ...
  async afterResponse(_: unknown, call: RpcCall): Promise<void> {
    await new Promise((resolve, reject) => {
      addToQueue({
        name: 'afterResponse',
        resolve,
  ...
  validateCall(call: RpcCall): RpcCall {
    if (call.error) {
