...
interface ConfirmationsRejectRuleOptions {
  driver: { runScript: (script: string) => Promise<unknown> };
  only?: string[];
}
...
interface RpcMethod {
  name: string;
  examples?: { name: string; result?: { value: unknown }; params: { value: unknown }[] }[];
  params: unknown[];
...
export default class ConfirmationsRejectRule {
  ...
  async beforeRequest(_: unknown, call: RpcCall) {
  ...
  // Need this now?
  getCalls(_: unknown, method: RpcMethod) {
    const calls: RpcCall[] = [];
    ...
    if (isMethodAllowed) {
      if (method.examples) {
        ...
        const p = ex.params.map((e: { value: unknown }) => e.value);
        const params =
          method.paramStructure === 'by-name'
            ? paramsToObj(p, method.params as never[])
            : p;
        calls.push({
  ...
  async afterRequest(_: unknown, call: RpcCall) {
    await new Promise((resolve, reject) => {
      addToQueue({
        name: 'afterRequest',
        resolve,
  ...
  async afterResponse(_: unknown, call: RpcCall) {
    await new Promise((resolve, reject) => {
      addToQueue({
        name: 'afterResponse',
        resolve,
