...
import { isValidChecksumAddress } from '@metamask/utils';
import {
  BtcAccountType,
  EthAccountType,
  EthScope,
...
import {
  ...
  selectSolanaAccount,
  selectPreviouslySelectedEvmAccount } from
'./accountsController';
import {
  ...
  createMockUuidFromAddress } from
'../util/test/accountsControllerTestUtils';

import { AGREED } from '../constants/storage';
import {
  MOCK_KEYRINGS,
  MOCK_KEYRING_CONTROLLER } from
'./keyringController/testUtils';
...
describe('Accounts Controller Selectors', () => {
  ...
  describe('selectSelectedInternalAccountFormattedAddress', () => {
    it('returns selected internal account address in checksum format', () => {
      ...
      expect(isValidChecksumAddress(result)).toEqual(true);
...
function getStateWithAccount(account) {
  return {
    engine: {
      backgroundState: {
        AccountsController: {
