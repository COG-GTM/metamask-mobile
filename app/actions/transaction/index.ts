...
} = TransactionTypes as { ASSET: Record<AssetType, AssetType> };
...
/**
 * Asset selected for a transaction (native currency, ERC20 token or NFT).
 * Only the discriminating fields are typed; callers pass richer shapes.
 */
...
/**
 * Transaction payload; contains the standard tx fields (from, to, data, gas,
 * gasPrice, value, ...) plus arbitrary transaction meta data.
 */
...
export interface SetProposedNonceAction {
  ...
  proposedNonce: number | string | undefined;
}
...
export function setNonce(nonce: number | string | undefined): SetNonceAction {
  return {
    type: SET_NONCE,
    nonce,
  };
...
export function setTransactionValue(value: unknown): SetTransactionValueAction {
  return {
