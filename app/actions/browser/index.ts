...
export type BrowserTabUpdate = Partial<Omit<BrowserTab, 'id'>>;

export type AddToViewedDappAction = Action<
  typeof BrowserActionTypes.ADD_TO_VIEWED_DAPP
> & {
  hostname: string;
};

export type AddToHistoryAction =
  Action<BrowserActionType.ADD_TO_BROWSER_HISTORY> & BrowserHistoryEntry;
...
export type UpdateTabAction = Action<BrowserActionType.UPDATE_TAB> & {
  id: number;
  data: BrowserTabUpdate;
};

export type StoreFaviconAction =
  Action<BrowserActionType.STORE_FAVICON_URL> & BrowserFavicon;
 ...
 */
export function updateTab(id: number, data: BrowserTabUpdate): UpdateTabAction {
  return {
...
export function storeFavicon({
  origin,
  url,
}: BrowserFavicon): StoreFaviconAction {
  return {
