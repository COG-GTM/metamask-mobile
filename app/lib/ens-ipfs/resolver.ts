...
export default async function resolveEnsToIpfsContentId({
  ...
  // lookup resolver
  const resolverLookupResult = await Registry.resolver<[string]>(hash);
  const resolverAddress = resolverLookupResult[0];
  if (hexValueIsEmpty(resolverAddress)) {
    throw new Error(`EnsIpfsResolver - no resolver found for name "${name}"`);
  }
  const Resolver = contract(resolverAbi).at(resolverAddress);
  const isEIP1577Compliant = await Resolver.supportsInterface<[boolean]>(
    '0xbc1c58d1',
  );
  const isLegacyResolver = await Resolver.supportsInterface<[boolean]>(
    '0xd8389dc5',
  );
  if (isEIP1577Compliant[0]) {
    const contentLookupResult = await Resolver.contenthash<[string]>(hash);
  ...
  if (isLegacyResolver[0]) {
    // lookup content id
    const contentLookupResult = await Resolver.content<[string]>(hash);
...
function hexValueIsEmpty(value: string | null | undefined) {
  return [
    undefined,
    null,
    '0x',
