...
export interface ResolveEnsToIpfsContentIdArgs {
  provider: unknown;
  name: string;
  chainId: string;
}

export interface EnsContentId {
  type: string;
  hash: string;
}
...
export default async function resolveEnsToIpfsContentId({
  ...
  name,
  chainId,
}: ResolveEnsToIpfsContentIdArgs): Promise<EnsContentId> {
  ...
  if (typeof resolverAddress !== 'string' || hexValueIsEmpty(resolverAddress)) {
  ...
  if (isEIP1577Compliant[0]) {
    ...
    const rawContentHash = String(contentLookupResult[0]);
  ...
  if (isLegacyResolver[0]) {
    ...
    if (typeof content !== 'string' || hexValueIsEmpty(content)) {
      throw new Error(
