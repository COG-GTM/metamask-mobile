 ...
 */
const fetchHtmlSource = async (url) => {
  try {
 ...
 */
const parseHtmlSource = async (htmlSource) => {
 ...
 */
export const getFaviconFromCache = (originUrl) => {
  const cacheKey = originToHost(originUrl);
  if (!cacheKey) return;
  const { browser } = store.getState();
  const cachedFavicon = browser.favicons.find(
    (favicon) => favicon.origin === cacheKey
  );
 ...
 */
export const isFaviconSVG = (imageSource) => {
  if (
  imageSource &&
