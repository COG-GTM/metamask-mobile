...
import NetworkList, {
  ...
  isValidNetworkName } from
'.';
import {
  convertNetworkId,
  deprecatedGetNetworkId,
  fetchEstimatedMultiLayerL1Fee } from
'./engineNetworkUtils';
import {
  MAINNET,
  RPC,
  ...
  LINEA_MAINNET,
  LINEA_SEPOLIA,
  MEGAETH_TESTNET } from
'../../../app/constants/network';
...
import {
  ...
  MOCK_ACCOUNT_BIP122_P2WPKH_TESTNET } from
'../test/accountsControllerTestUtils';
import { BtcScope, SolScope } from '@metamask/keyring-api';
import {
  selectMultichainNetworkControllerState,
  selectSelectedNonEvmNetworkChainId } from
...
describe('network-utils', () => {
  ...
  describe('getNetworkTypeById', () => {
    it.each([getAllNetworks()])('should get network type by Id - %s', (networkKey) => {
      const network = NetworkList[networkKey];
  ...
  describe('deprecatedGetNetworkId', () => {
    ...
    it('throws when provider is not initialized', async () => {
      ...
      Engine.controllerMessenger.call.mockReturnValueOnce(
        undefined
      );
  ...
  describe('findBlockExplorerForNonEvmAccount', () => {
    beforeEach(() => {
      ...
      selectMultichainNetworkControllerState.mockReturnValue({
