 ...
 */
export const getDecimalChainId = (chainId: unknown): string => {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const c = chainId as any;
  if (
    !c ||
    typeof c !== 'string' ||
    !c.startsWith('0x') ||
  ...
  return parseInt(c, 16).toString(10);
};

export const isMainnetByChainId = (chainId: unknown): boolean =>
  getDecimalChainId(String(chainId)) === String(1);

export const isLineaMainnetByChainId = (chainId: unknown): boolean =>
...
export const getTestNetImage = (networkType: string | undefined): unknown => {
  if (
    ...
    return (networksWithImages as Record<string, unknown>)?.[
...
export const getTestNetImageByChainId = (chainId: string): unknown => {
  const images = networksWithImages as Record<string, unknown>;
...
export function getNetworkTypeById(id: string | number): string {
  ...
  const networkList = NetworkList as Record<
    string,
    { networkId?: number; chainId?: string; name?: string; [k: string]: unknown }
  >;
...
export function getDefaultNetworkByChainId(
  ...
  const networkList = NetworkList as Record<string, { chainId?: string; [k: string]: unknown }>;
...
export function findBlockExplorerForRpc(
  ...
  if (networkConfiguration) {
    const idx = networkConfiguration?.defaultBlockExplorerUrlIndex ?? 0;
    return networkConfiguration?.blockExplorerUrls?.[idx];
  }
...
export function findBlockExplorerForNonEvmAccount(
  ...
  if (
    ...
    const networkConfigs = (
      nonEvmNetworks as { multichainNetworkConfigurationsByChainId?: Record<string, { chainId: string }> }
  ...
  const blockExplorerFormatUrls = (
    MULTICHAIN_NETWORK_BLOCK_EXPLORER_FORMAT_URLS_MAP as Record<string, unknown>
  )[scope];
...
export const getNetworkNameFromProviderConfig = (
  ...
  const networkList = NetworkList as Record<string, { name?: string; [k: string]: unknown }>;
  if (providerConfig.nickname) {
    ...
    name =
      networkList?.[networkType]?.name || (networkList[RPC].name as string);
  }
  return name;
};
 ...
 * @param params.networkType - Type of network from the provider (optional).
 * @param params.chainId - ChainID of the network.
 * @returns Image source of the network.
 */
interface NetworkImageSourceArgs {
  networkType?: string;
  chainId: string;
}
...
export const getNetworkImageSource = ({
  networkType,
  chainId,
}: NetworkImageSourceArgs): unknown => {
  ...
  const customNetworkImg = (CustomNetworkImgMapping as Record<string, unknown>)[
    chainId
  ];
...
): unknown => networkOnboardedState[chainId];
