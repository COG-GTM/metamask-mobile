 ...
 */
export function findBlockExplorerForRpc(rpcTargetUrl: string, networkConfigurations: Record<string, { rpcEndpoints?: { url: string }[]; blockExplorerUrls: string[]; defaultBlockExplorerUrlIndex: number }>) {
 ...
 * @returns {string} - Block explorer url or undefined if not found
 */
export function findBlockExplorerForNonEvmAccount(internalAccount: { address: string; scopes: string[] }) {
  let scope;
 ...
 * @returns
 */
export const getIsNetworkOnboarded = (chainId: string, networkOnboardedState: Record<string, boolean>) =>
