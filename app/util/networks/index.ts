 ...
 */
export const NetworkList: Record<string, NetworkEntry> = {
  [MAINNET]: {
...
export function getDefaultNetworkByChainId(
  ...
  let returnNetwork: NetworkEntry | undefined;
...
export function findBlockExplorerForNonEvmAccount(
  ...
  if (
    ...
    const matchingNetwork = Object.values(networkConfigs || {}).find(
      (network) =>
        (internalAccount.scopes as string[]).includes(network.chainId),
    );
  ...
  const blockExplorerFormatUrls = (
    MULTICHAIN_NETWORK_BLOCK_EXPLORER_FORMAT_URLS_MAP as Record<
      string,
      (typeof MULTICHAIN_NETWORK_BLOCK_EXPLORER_FORMAT_URLS_MAP)[keyof typeof MULTICHAIN_NETWORK_BLOCK_EXPLORER_FORMAT_URLS_MAP]
    >
  )[scope];
...
export function blockTagParamIndex(payload: BlockTagPayload): number | undefined {
...
export const getNetworkImageSource = ({
  ...
  const customNetworkImg = (CustomNetworkImgMapping as Record<
    string,
    ImageSourcePropType
  >)[chainId];
