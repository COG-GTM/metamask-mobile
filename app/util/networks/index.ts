...
export const isMainnetByChainId = (
  chainId: string | number | undefined,
): boolean => getDecimalChainId(String(chainId)) === String(1);

export const isLineaMainnetByChainId = (
  chainId: string | number | undefined,
): boolean => getDecimalChainId(String(chainId)) === String(59144);
...
export const isTestNetworkWithFaucet = (chainId: string): boolean =>
  TESTNET_FAUCETS[chainId as Hex] !== undefined;
...
export const isTestNet = (chainId: string | undefined): boolean =>
  (TESTNET_CHAIN_IDS as readonly string[]).includes(chainId as string);

export function getNetworkTypeById(id?: number | string): string {
  if (!id) {
 ...
 */
interface RpcNetworkConfiguration {
...
export function findBlockExplorerForRpc(
  ...
  networkConfigurations: Record<string, RpcNetworkConfiguration>,
): string | undefined {
...
export function findBlockExplorerForNonEvmChainId(
  ...
  const blockExplorerUrls =
    MULTICHAIN_NETWORK_BLOCK_EXPLORER_FORMAT_URLS_MAP[chainId as CaipChainId];
