...
/**
 * The subset of a network configuration used to resolve a block explorer.
 */
interface BlockExplorerNetworkConfiguration {
...
type NetworkListKey = keyof typeof NetworkList;
export type NetworkListEntry = (typeof NetworkList)[NetworkListKey];
 ...
 */
export const getDecimalChainId = (chainId?: string | number): string => {
  if (
    !chainId ||
    ...
    // Non hexadecimal values are returned untouched, as documented above.
...
export const isTestNetworkWithFaucet = (chainId: string): boolean =>
  TESTNET_FAUCETS[chainId as keyof typeof TESTNET_FAUCETS] !== undefined;
...
export function getNetworkTypeById(id?: string | number): string {
  ...
  const filteredNetworkTypes = NetworkListKeys.filter((key) => {
    const network = NetworkList[key as NetworkListKey];
    return 'networkId' in network && network.networkId === parseInt(`${id}`, 10);
  });
...
export function getDefaultNetworkByChainId(
  ...
  getAllNetworks().forEach((type) => {
    const network = NetworkList[type as NetworkListKey];
    if (
      toLowerCaseEquals(
        String('chainId' in network ? network.chainId : undefined),
        chainId,
      )
    ) {
      returnNetwork = network;
    }
  });
 ...
 *
 * @param rpcTargetUrl - The RPC url of the network
 * @param networkConfigurations - The known network configurations
 */
export function findBlockExplorerForRpc(
  ...
  const networkConfiguration = Object.values(
    networkConfigurations as Record<string, BlockExplorerNetworkConfiguration>,
  ).find(({ rpcEndpoints }) =>
...
export function findBlockExplorerForNonEvmAccount(
  internalAccount: Pick<InternalAccount, 'address'> & { scopes?: string[] },
): string | undefined {
  ...
  const blockExplorerFormatUrls =
    MULTICHAIN_NETWORK_BLOCK_EXPLORER_FORMAT_URLS_MAP[scope as CaipChainId];
...
export function compareRpcUrls(rpcOne: string, rpcTwo: string): boolean {
  ...
  if (typeof rpcOne === 'string' && typeof rpcTwo === 'string') {
    const rpcUrlOne = new URLParse(rpcOne);
    const rpcUrlTwo = new URLParse(rpcTwo);
...
export function getBlockExplorerName(
  ...
  const hostname = new URLParse(blockExplorerUrl).hostname;
...
export function blockTagParamIndex(
  payload: Pick<JsonRpcRequest, 'method'>,
): number | undefined {
...
export const getNetworkNameFromProviderConfig = (providerConfig: {
  ...
  if (providerConfig.nickname) {
    ...
    name =
      NetworkList?.[networkType as NetworkListKey]?.name || NetworkList[RPC].name;
  }
  return name;
};
...
export const getNetworkImageSource = ({
  ...
  if (defaultNetwork && 'imageSource' in defaultNetwork) {
  ...
  if (customNetworkImg) {
    // `CustomNetworkImgMapping` holds remote image urls
    return customNetworkImg as unknown as ImageSourcePropType;
  }
...
export const getBlockExplorerAddressUrl = (
  ...
  if (isCustomRpcBlockExplorerNetwork) {
    ...
    const title = new URLParse(rpcBlockExplorer).hostname;
    return { url, title };
  }
...
export const isValidNetworkName = (
  ...
  WHILELIST_NETWORK_NAME[chainId as keyof typeof WHILELIST_NETWORK_NAME] ===
    nickname;
