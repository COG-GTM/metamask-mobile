...
export const getTestNetImage = (networkType?: string) => {
  if (
    ...
    return networksWithImages?.[(networkType as string).toUpperCase()];
  }
};
...
export function getNetworkTypeById(id?: number | string | null): string {
  if (!id) {
...
export function getBlockExplorerName(
  ...
  const hostname = new URL(blockExplorerUrl as string).hostname;
