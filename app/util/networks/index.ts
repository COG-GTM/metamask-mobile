...
/**
 * A single entry in the {@link NetworkList} map describing a supported network.
 */
 ...
 */
export const TESTNET_CHAIN_IDS: Hex[] = [
...
export const isTestNet = (chainId: string): boolean =>
  TESTNET_CHAIN_IDS.includes(chainId as Hex);
...
export function getNetworkTypeById(id?: string | number): string {
  ...
  const filteredNetworkTypes = NetworkListKeys.filter(
    (key) =>
      (NetworkList[key as keyof typeof NetworkList] as NetworkListEntry)
        .networkId === parseInt(id as string, 10),
  );
...
export function getDefaultNetworkByChainId(
  ...
  getAllNetworks().forEach((type) => {
    const network = NetworkList[
      type as keyof typeof NetworkList
    ] as NetworkListEntry;
    if (toLowerCaseEquals(String(network.chainId), chainId)) {
...
/**
 * The minimal shape of a network configuration required to resolve a block
 * explorer URL from an RPC endpoint.
 */
 ...
 *
 * @param rpcTargetUrl - The RPC target URL to find a block explorer for.
 * @param networkConfigurations - The available network configurations.
 */
 ...
 *
 * @param rpcOne - The first RPC URL to compare.
 * @param rpcTwo - The second RPC URL to compare.
 */
...
export const getNetworkNameFromProviderConfig = (
  ...
  if (providerConfig.nickname) {
    ...
    name =
      NetworkList?.[networkType as keyof typeof NetworkList]?.name ||
