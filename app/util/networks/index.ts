...
interface NetworkListEntry {
  ...
  color: string;
  networkType: string;
  // Image sources are `require`d assets
...
const NetworkListByKey = NetworkList as Record<string, NetworkListEntry>;
 ...
 */
// The returned value is a decimal chain id string, but callers historically
// consume it as either a string or a number.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const getDecimalChainId = (chainId?: string | number): any => {
  if (
    !chainId ||
...
export function getNetworkTypeById(id?: string | number): string {
  ...
  const filteredNetworkTypes = NetworkListKeys.filter(
    (key) => NetworkListByKey[key].networkId === parseInt(String(id), 10),
  );
...
export function getDefaultNetworkByChainId(
  ...
  getAllNetworks().forEach((type) => {
    if (toLowerCaseEquals(String(NetworkListByKey[type].chainId), chainId)) {
      returnNetwork = NetworkListByKey[type];
    }
  });
...
export function findBlockExplorerForRpc(
  ...
  // Network configurations reach this helper in several controller shapes.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export function findBlockExplorerForNonEvmAccount(internalAccount: {
  ...
  const blockExplorerFormatUrls = (
    MULTICHAIN_NETWORK_BLOCK_EXPLORER_FORMAT_URLS_MAP as Record<
      string,
      // The block explorer url format map is keyed by CAIP chain id
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      any
    >
  )[scope];
...
export const getNetworkNameFromProviderConfig = (providerConfig: {
  ...
  if (providerConfig.nickname) {
    ...
    name = NetworkListByKey?.[networkType]?.name || NetworkListByKey[RPC].name;
  }
  return name;
};
