...
export const getDecimalChainId = (chainId: unknown): string => {
  // Type narrowing for non-string input
 ...
 */
export const getTestNetImage = (networkType: string): unknown => {
  if (
    networkType === SEPOLIA ||
...
export const isTestNet = (chainId: string): boolean =>
  (TESTNET_CHAIN_IDS as unknown as string[]).includes(chainId);
...
export function getNetworkTypeById(id: string | number): string {
  ...
  const filteredNetworkTypes = NetworkListKeys.filter(
    (key) =>
      (NetworkList as Record<string, { networkId: number }>)[key].networkId ===
...
export function getDefaultNetworkByChainId(
  ...
  getAllNetworks().forEach((type) => {
    const network = (NetworkList as Record<string, { chainId: string }>)[type];
...
export function findBlockExplorerForNonEvmAccount(
  ...
  if (
    ...
    const matchingNetwork = (
      Object.values(networkConfigs || {}) as { chainId: string }[]
    ).find((network) => internalAccount.scopes.includes(network.chainId));
  ...
  return formatBlockExplorerAddressUrl(
    blockExplorerFormatUrls as Parameters<
      typeof formatBlockExplorerAddressUrl
    >[0],
...
export const getNetworkNameFromProviderConfig = (
  ...
  if (providerConfig.nickname) {
    ...
    const networkListMap = NetworkList as Record<string, { name: string }>;
    name =
