...
export const getDecimalChainId = (
  ...
  if (!chainId.startsWith('0x') || isNonEvmChainId(chainId)) {
    return chainId;
  }
...
export function getDefaultNetworkByChainId(chainId: string) {
  ...
  getAllNetworks().forEach((networkType) => {
    const type = networkType as keyof typeof NetworkList;
    if (
      toLowerCaseEquals(
        String((NetworkList[type] as { chainId?: string }).chainId),
        chainId,
      )
    ) {
