...
export interface NetworkProviderConfigLike {
...
export interface NonEvmAccountLike {
  address: string;
  scopes: string[];
}
...
const NetworkListByType: Record<string, NetworkListEntry> = NetworkList;
 ...
 */
export function getDecimalChainId(chainId: string | null | undefined): string;
export function getDecimalChainId(chainId: number | null | undefined): number;
export function getDecimalChainId<T>(chainId: T): T;
export function getDecimalChainId(chainId: unknown): unknown {
  if (
    !chainId ||
...
export function getNetworkTypeById(id?: string | number | null): string {
  if (!id) {
...
export function findBlockExplorerForRpc(
  ...
  if (networkConfiguration) {
    ...
    return index === undefined
      ? undefined
      : (networkConfiguration?.blockExplorerUrls as string[])[index];
  }
...
export function findBlockExplorerForNonEvmAccount(
  internalAccount: NonEvmAccountLike,
): string | undefined {
...
export const getNetworkNameFromProviderConfig = (
  providerConfig: NetworkProviderConfigLike,
): string => {
...
export const getNetworkImageSource = ({
  ...
  if (customNetworkImg) {
    // CustomNetworkImgMapping is declared as Record<Hex, string> but holds require() image sources
  ...
  // Undefined only for unknown testnets; callers treat the result as an image source.
  return getTestNetImage(networkType as string) as ImageSourcePropType;
};
 ...
 * @returns
 */
export const getIsNetworkOnboarded = <T>(
  chainId: string,
  networkOnboardedState: Record<string, T>,
): T => networkOnboardedState[chainId];
