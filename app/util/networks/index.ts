...
export function getNetworkTypeById(id?: string | number): string {
  ...
  const filteredNetworkTypes = NetworkListKeys.filter(
    (key) => NetworkListMap[key].networkId === parseInt(id as string, 10),
  );
 ...
 */
interface FindBlockExplorerNetworkConfiguration {
...
export function findBlockExplorerForRpc(
  rpcTargetUrl: string,
  networkConfigurations: Record<string, FindBlockExplorerNetworkConfiguration>,
): string | undefined {
...
export const getNetworkNameFromProviderConfig = (providerConfig: {
  ...
  if (providerConfig.nickname) {
    ...
    name =
      NetworkListMap?.[networkType as string]?.name || NetworkListMap[RPC].name;
  }
  return name;
};
