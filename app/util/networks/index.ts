...
interface BlockExplorerUrlResult {
  url: string | null;
  title: string | null;
}
 ...
 */
export const getTestNetImage = (networkType: string): ImageSourcePropType | undefined => {
  if (
    networkType === SEPOLIA ||
...
export const getTestNetImageByChainId = (chainId: string): ImageSourcePropType | undefined => {
 ...
 */
export const isTestNet = (chainId: string): boolean => (TESTNET_CHAIN_IDS as string[]).includes(chainId);

export function getNetworkTypeById(id: string): string {
  if (!id) {
    throw new Error(NetworkSwitchErrorType.missingNetworkId);
  }
  const filteredNetworkTypes = NetworkListKeys.filter(
    (key) => (NetworkList as Record<string, NetworkInfo>)[key].networkId === parseInt(id, 10),
  );
...
export function getDefaultNetworkByChainId(chainId: string): NetworkInfo | undefined {
  ...
  getAllNetworks().forEach((type) => {
    const networkListRecord = NetworkList as Record<string, NetworkInfo>;
    if (toLowerCaseEquals(String(networkListRecord[type]?.chainId), chainId)) {
      returnNetwork = networkListRecord[type];
    }
  });
 ...
 */
export function findBlockExplorerForRpc(rpcTargetUrl: string, networkConfigurations: Record<string, NetworkConfiguration>): string | undefined {
...
export function findBlockExplorerForNonEvmChainId(chainId: string): string | undefined {
  const blockExplorerUrls =
    MULTICHAIN_NETWORK_BLOCK_EXPLORER_FORMAT_URLS_MAP[chainId as keyof typeof MULTICHAIN_NETWORK_BLOCK_EXPLORER_FORMAT_URLS_MAP];
...
export const getNetworkNameFromProviderConfig = (providerConfig: ProviderConfig): string => {
  ...
  if (providerConfig.nickname) {
    ...
    name = (NetworkList as Record<string, NetworkInfo>)?.[networkType as string]?.name || NetworkList[RPC].name;
  }
  return name;
};
 ...
 */
export const getNetworkImageSource = ({ networkType, chainId }: { networkType?: string; chainId: string }): ImageSourcePropType | undefined => {
  ...
  const customNetworkImg = (CustomNetworkImgMapping as Record<string, ImageSourcePropType>)[chainId];
...
export const isValidNetworkName = (chainId: string, networkName: string, nickname: string): boolean =>
  networkName === nickname || WHILELIST_NETWORK_NAME[chainId as keyof typeof WHILELIST_NETWORK_NAME] === nickname;
