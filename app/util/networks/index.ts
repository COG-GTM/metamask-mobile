 ...
 */
export const getDecimalChainId = (chainId: string | undefined): string => {
  if (
    !chainId ||
...
export const getTestNetImage = (networkType: string): any => {
  if (
    networkType === SEPOLIA ||
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const getTestNetImageByChainId = (chainId: string): any => {
...
export function getNetworkTypeById(id?: string | number): string {
  ...
  const filteredNetworkTypes = NetworkListKeys.filter(
    (key) => (NetworkList as Record<string, Record<string, unknown>>)[key].networkId === parseInt(String(id), 10),
  );
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function getDefaultNetworkByChainId(chainId: string): any {
  if (!chainId) {
  ...
  getAllNetworks().forEach((type) => {
    if (toLowerCaseEquals(String((NetworkList as Record<string, Record<string, unknown>>)[type].chainId), chainId)) {
      returnNetwork = (NetworkList as Record<string, Record<string, unknown>>)[type];
    }
  });
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function findBlockExplorerForRpc(rpcTargetUrl: string, networkConfigurations: Record<string, any>): string | undefined {
  const networkConfiguration = Object.values(networkConfigurations).find(
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ({ rpcEndpoints }: any) => rpcEndpoints?.some(({ url }: { url: string }) => url === rpcTargetUrl),
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ) as any;
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function findBlockExplorerForNonEvmAccount(internalAccount: any): string | undefined {
  let scope;
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const getNetworkNameFromProviderConfig = (providerConfig: any): string => {
  ...
  if (providerConfig.nickname) {
    ...
    name = (NetworkList as Record<string, { name: string }>)?.[networkType]?.name || NetworkList[RPC].name;
  }
  return name;
};
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const getNetworkImageSource = ({ networkType, chainId }: { networkType?: string; chainId: string }): any => {
  ...
  const customNetworkImg = (CustomNetworkImgMapping as Record<string, string>)[chainId];
...
export const isValidNetworkName = (chainId: string, networkName: string, nickname: string): boolean =>
  networkName === nickname || (WHILELIST_NETWORK_NAME as Record<string, string>)[chainId] === nickname;
