...
import { isCaipChainId, type CaipChainId, type Hex } from '@metamask/utils';
import type { InternalAccount } from '@metamask/keyring-internal-api';
import type { ImageSourcePropType } from 'react-native';
import type { ProviderConfig } from '../../selectors/networkController';
...
} satisfies Record<string, NetworkListEntry>;

export const NetworkList: typeof networkList &
  Record<string, NetworkListEntry | undefined> = networkList;
 ...
 */
export const isDefaultMainnet = (networkType: string | undefined): boolean =>
  networkType === MAINNET;
 ...
 */
export const isMainNet = (chainId: string | undefined): boolean =>
  chainId === '0x1';

export const isLineaMainnet = (networkType: string | undefined): boolean =>
  networkType === LINEA_MAINNET;
export const isLineaMainnetChainId = (chainId: string | undefined): boolean =>
  chainId === CHAIN_IDS.LINEA_MAINNET;

export const isSolanaMainnet = (chainId: string | undefined): boolean =>
 ...
 */
export const getDecimalChainId = <T extends string | number | null | undefined>(
  chainId: T,
): T | string => {
  if (
    !chainId ||
...
export const isMainnetByChainId = (chainId: string | number | undefined): boolean =>
  getDecimalChainId(String(chainId)) === String(1);

export const isLineaMainnetByChainId = (
  chainId: string | number | undefined,
): boolean =>
  getDecimalChainId(String(chainId)) === String(59144);

export const isMultiLayerFeeNetwork = (chainId: string | undefined): boolean =>
...
export const isTestNet = (chainId: string | undefined): boolean =>
  chainId !== undefined && TESTNET_CHAIN_IDS.includes(chainId);
 ...
 */
export interface BlockExplorerNetworkConfiguration {
  [key: string]: unknown;
...
export function findBlockExplorerForRpc(
  ...
  networkConfigurations: Record<string, BlockExplorerNetworkConfiguration>,
): string | undefined {
...
export function findBlockExplorerForNonEvmAccount(
  ...
  if (
    ...
    const matchingNetwork = Object.values(networkConfigs || {}).find(
      (network) => internalAccount.scopes.includes(network.chainId as CaipChainId),
    );
...
export const getNetworkNameFromProviderConfig = (
  providerConfig: Pick<ProviderConfig, 'nickname' | 'chainId' | 'type'>,
): string => {
