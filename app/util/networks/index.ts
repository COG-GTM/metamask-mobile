 ...
 *
 * @param rpcTargetUrl - Provider RPC target URL.
 * @param networkConfigurations - Map of chain ID to network configuration.
 * @returns The block explorer URL or undefined if no match is found.
 */
...
export function findBlockExplorerForNonEvmAccount(
  internalAccount: Pick<InternalAccount, 'scopes' | 'address'>,
): string | undefined {
 ...
 *
 * @param rpcOne - First RPC URL.
 * @param rpcTwo - Second RPC URL.
 * @returns True if both URLs have the same host.
 */
 ...
 * @returns Block explorer name or undefined.
 */
...
interface ProviderConfigForName {
  nickname?: string;
  chainId?: string;
  type?: string;
}
...
export const getNetworkNameFromProviderConfig = (
  providerConfig: ProviderConfigForName,
): string => {
  ...
  if (providerConfig.nickname) {
    ...
    name =
      (networkType ? NetworkList?.[networkType]?.name : undefined) ||
      NetworkList[RPC].name;
  }
  return name;
};

interface NetworkImageSourceParams {
  networkType?: string;
  chainId?: string;
}
...
export const getNetworkImageSource = ({
  networkType,
  chainId,
}: NetworkImageSourceParams): ImageSourcePropType => {
  const defaultNetwork = getDefaultNetworkByChainId(chainId);

  if (defaultNetwork?.imageSource) {
  ...
  const customNetworkImg = chainId
    ? CustomNetworkImgMapping[chainId as Hex]
    : undefined;
...
interface BlockExplorerUrlAndTitle {
  url: string | null;
  title: string | null;
}
 ...
 * @returns Whether the network is onboarded.
 */
