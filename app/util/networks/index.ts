...
export function getDefaultNetworkByChainId(chainId: string): typeof NetworkList[keyof typeof NetworkList] | undefined {
  if (!chainId) {
...
export function findBlockExplorerForRpc(
  rpcTargetUrl: string,
  networkConfigurations: Record<string, { rpcEndpoints?: { url: string }[]; blockExplorerUrls: string[]; defaultBlockExplorerUrlIndex: number }>,
): string | undefined {
...
export function findBlockExplorerForNonEvmAccount(
  internalAccount: { address: string; scopes?: string[] },
): string | undefined {
  let scope;
 ...
 */
export const getNetworkNameFromProviderConfig = (providerConfig: { nickname?: string; chainId?: string; type?: string }): string => {
