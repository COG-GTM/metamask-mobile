...
const NetworkListByType: Record<string, NetworkListItem> = NetworkList;
...
export const isMainnetByChainId = (chainId: unknown) =>
  getDecimalChainId(String(chainId)) === String(1);

export const isLineaMainnetByChainId = (chainId: unknown) =>
...
export function getDefaultNetworkByChainId(chainId?: string) {
  ...
  let returnNetwork: NetworkListItem | undefined;
...
export function findBlockExplorerForRpc(
  ...
  if (networkConfiguration) {
    return (networkConfiguration?.blockExplorerUrls as string[])[
 ...
 */
export function findBlockExplorerForNonEvmChainId(chainId: CaipChainId) {
 ...
 * @param {string} rpcTwo
 */
export function compareRpcUrls(rpcOne: unknown, rpcTwo: unknown) {
