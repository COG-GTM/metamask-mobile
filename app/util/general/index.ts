...
export const tlc = (str: unknown): unknown =>
  (str as { toLowerCase?: () => unknown } | null | undefined)?.toLowerCase?.();
...
export function findRouteNameFromNavigatorState(
  ...
  if ((route as NavigationRoute).state) {
    route = (route as NavigationRoute).state;
  }
...
export const deepJSONParse = ({
  ...
  function parseProperties(obj: JsonObject): void {
    Object.keys(obj).forEach((key) => {
      if (typeof obj[key] === 'string') {
        const isNumber = !isNaN(obj[key] as number);
        ...
        if (!isNumber || (isNumber && !skipNumbers)) {
          try {
            ...
            if (typeof parsed === 'object') {
              parseProperties(parsed as JsonObject);
            }
          } catch (e) {
        ...
        parseProperties(obj[key] as JsonObject);
      }
    });
  }

  // Start parsing from the root object
  parseProperties(parsedObject as JsonObject);
