...
export const tlc = (str?: string | number | null): string | undefined =>
...
export const toLowerCaseEquals = (
  ...
  if (!a && !b) return false;
  return (
    tlc(typeof a === 'string' ? a : undefined) ===
    tlc(typeof b === 'string' ? b : undefined)
  );
};
...
export const getURLProtocol = (url: string): string | undefined => {
  try {
    const { protocol } = new UrlParser(url);
  ...
  function parseProperties(obj: unknown): void {
    ...
    Object.keys(object).forEach((key) => {
      if (typeof object[key] === 'string') {
        const isNumber = !isNaN(Number(object[key]));
        ...
        if (!isNumber || (isNumber && !skipNumbers)) {
          try {
            ...
            const parsed = JSON.parse(object[key] as string);
            object[key] = parsed;
