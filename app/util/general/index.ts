...
export const tlc = (str?: string | null) => str?.toLowerCase?.();
...
export function findRouteNameFromNavigatorState(
  ...
  let route: NavigatorStateRoute | undefined = routes?.[routes.length - 1];
  if (route?.state) {
    route = route.state;
  }
...
export const toLowerCaseEquals = (a?: string | null, b?: string | null) => {
  if (!a && !b) return false;
  return tlc(a) === tlc(b);
};
