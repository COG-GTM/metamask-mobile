...
export const tlc = (str: any): string | undefined => str?.toLowerCase?.();
 ...
 */
export function timeoutFetch(url: string, options: RequestInit | undefined, timeout = 500): Promise<Response> {
  return Promise.race([
    fetch(url, options),
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function findRouteNameFromNavigatorState(routes: Record<string, any>[]): string | undefined {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  let route: Record<string, any> | undefined = routes?.[routes.length - 1];
  if (route?.state) {
    route = route.state;
  }
  ...
  let name: string | undefined = route?.name;
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export const toLowerCaseEquals = (a: any, b: any): boolean => {
  if (!a && !b) return false;
  return tlc(a) === tlc(b);
};
...
export const deepJSONParse = ({ jsonString, skipNumbers = true }: { jsonString: string; skipNumbers?: boolean }): unknown => {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  function parseProperties(obj: Record<string, any>) {
