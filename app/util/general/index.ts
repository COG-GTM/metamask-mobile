...
type StringLike = string | number | boolean | object | null | undefined;

export const tlc = (str: StringLike) => (str as string)?.toLowerCase?.();
...
interface NavigationRouteLike {
  name?: string;
  index?: number;
  state?: NavigationRouteLike;
  routes?: NavigationRouteLike[];
}

export function findRouteNameFromNavigatorState(routes: NavigationRouteLike[]) {
  let route: NavigationRouteLike | undefined = routes?.[routes.length - 1];
  if (route.state) {
    route = route.state;
  }
  // eslint-disable-next-line @typescript-eslint/prefer-optional-chain -- preserving the existing runtime check
  while (route !== undefined && route.index !== undefined) {
    route = route?.routes?.[route.index] as NavigationRouteLike;
    if (route.state) {
      route = route.state;
    }
  }
...
export const toLowerCaseEquals = (a: StringLike, b: StringLike) => {
  if (!a && !b) return false;
  return tlc(a) === tlc(b);
};
