...
export default async (
  ...
  const onConfirmationComplete = async (confirmed: boolean) => {
    if (!confirmed) {
      await handleSignatureAction(onReject, messageParams, signType, undefined, false);
    } else {
      await handleSignatureAction(onConfirm, messageParams, signType, undefined, true);
    }
  };
