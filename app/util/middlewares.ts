...
interface JsonRpcRequestWithOrigin {
  origin?: string;
  params?: unknown;
  isMetamaskInternal?: boolean;
}

interface JsonRpcResponseWithError {
  error?: {
    message?: string;
    code?: number;
    data?: unknown;
...
type NextCallback = (handler: (cb: () => void) => void) => void;
      ...
      if (res.error) {
        ...
        if (error) {
          if (containsUserRejectedError(error.message, error.code)) {
             ...
             * "data": { "code": -32000, "message": "gas required exceeds allowance" } }`
