...
/**
 * A JSON-RPC request as it flows through the mobile middleware stack, which
 * decorates it with the origin of the DApp that issued it.
 */
export interface OriginatedJsonRpcRequest {
  method: string;
...
/**
 * A JSON-RPC response, which may hold an error raised while processing the
 * request.
 */
 ...
 * @returns whether the error was caused by the user rejecting the request
 */
      ...
      if (res.error) {
        ...
        if (error) {
          if (containsUserRejectedError(error.message, error.code)) {
            ...
            const errorParams: {
              message: string;
              orginalError: typeof error;
              res: typeof resWithoutError;
              req: OriginatedJsonRpcRequest;
              data?: Json;
            } = {
