...
interface OriginRequest extends JsonRpcRequest {
  origin?: string;
  isMetamaskInternal?: boolean;
}

interface RpcResponse {
  error?: { message: string; code?: number; data?: Json };
  [key: string]: unknown;
}
...
type Middleware = (
  req: OriginRequest,
  res: RpcResponse,
  next: (cb?: (callback: () => void) => void) => void,
) => void;
      ...
      if (res.error) {
        ...
        if (error) {
          if (containsUserRejectedError(error.message, error.code)) {
            ...
            const errorParams: {
              message: string;
              orginalError: { message: string; code?: number; data?: Json };
