 ...
 * @returns {boolean}
 */
export function containsUserRejectedError(errorMessage: string | undefined | null, errorCode: number | undefined): boolean {
  try {
...
export function createLoggerMiddleware(opts: { origin: string }) {
  return function loggerMiddleware(
    ...
    next((cb: () => void) => {
      if (res.error) {
        const { error, ...resWithoutError } = res as { error?: { message?: string; code?: number; data?: unknown }; [key: string]: unknown };
        if (error) {
