...
// @ts-expect-error @metamask/ethjs-unit does not ship declarations.
import convert from '@metamask/ethjs-unit';
import { add0x, remove0x } from '@metamask/utils';
// @ts-expect-error number-to-bn does not ship declarations.
...
const currencySymbolMap = currencySymbols as Record<string, string>;

interface BNLike {
  add(value: unknown): BNLike;
  div(value: unknown): BNLike;
  isZero(): boolean;
  lt(value: unknown): boolean;
  mod(value: unknown): BNLike;
  mul(value: unknown): BNLike;
  toNumber(): number;
...
interface ConversionOptions {
  ...
  conversionRate?: NumericInput | null;
...
export function BNToHex(inputBn: BNLike): string {
...
> = {
  ...
  BN: (n: NumericInput) => new BigNumber(n.toString(16), 16),
};
...
const baseChange: Record<NumericBase, (value: BigNumber) => string | BNLike> = {
...
export function addHexPrefix(str?: string | null): string | null | undefined;
export function addHexPrefix(
  str?: string | null,
...
export function fromTokenMinimalUnit(
  ...
  let minimal = safeNumberToBN(prefixedInput as NumericInput);
...
export function renderFiatAddition(
  ...
  if (currencySymbolMap[currentCurrency]) {
    return `${currencySymbolMap[currentCurrency]}${renderMinimalUnit}`;
  }
 ...
 */
export function toBN(value: NumericInput): BNLike {
  return new BN4(value as unknown as string | number, 10);
}
...
export const dotAndCommaDecimalFormatter = (value: NumericInput): string => {
...
export function toWei(
  ...
  if (isNumberScientificNotationWhenString(value)) {
    value = (value as BigNumber).toFixed(18);
  }
...
export function addCurrencySymbol(
  ...
  if (currencySymbolMap[currencyCode]) {
    return `${prefix}${currencySymbolMap[currencyCode]}${absAmountStr}`;
  }

  const lowercaseCurrencyCode = currencyCode?.toLowerCase();

  if (currencySymbolMap[lowercaseCurrencyCode]) {
    return `${prefix}${currencySymbolMap[lowercaseCurrencyCode]}${absAmountStr}`;
  }
...
export function weiToFiatNumber(
  ...
  let value = parseFloat(
    (Math.floor((eth as unknown as number) * conversionRate * base) / base) as unknown as string,
  );
 ...
 */
export function safeNumberToBN(value: NumericInput): BNLike {
  try {
...
export function renderFiat(
  ...
  const base = Math.pow(10, decimalsToShow);
  let fiatFixed = parseFloat(Math.round(value * base) as unknown as string) / base;
  fiatFixed = isNaN(fiatFixed) ? 0.0 : fiatFixed;
  if (currencySymbolMap[currencyCode]) {
    return `${currencySymbolMap[currencyCode]}${fiatFixed}`;
  }
 ...
 */
export function renderWei(value: NumericInput): string {
  if (!value) return '0';
  const wei = fromWei(value);
...
const converter = ({
  ...
  roundDown,
}: ConversionOptions & { value: NumericInput }): BigNumber | string | BNLike => {
  let convertedValue: BigNumber | string | BNLike = fromNumericBase
    ? toBigNumber[fromNumericBase](value)
    : (value as BigNumber | string | BNLike);
  ...
  if (fromCurrency !== toCurrency) {
    ...
    if (invertConversionRate) {
      rate = new BigNumber(1.0).div(conversionRate as string | number);
    }
...
export const toHexadecimal = (decimal?: NumericInput): NumericInput | undefined => {
...
export const calculateEthFeeForMultiLayer = ({
  ...
  if (!multiLayerL1FeeTotal) {
    return ethFee as string | number;
  }
  ...
  return new BigNumber(multiLayerL1FeeTotalDecEth as string | number)
...
export const isZeroValue = (value: unknown): boolean => {
  ...
  return (
    value === '0x0' ||
    (isBN(value) && (value as BNLike).isZero()) ||
    isZero(value)
  );
};
...
export const safeBNToHex = (value: BNLike | null | undefined): string | null | undefined => {
  if (value === null || value === undefined) {
    return value;
  }
...
export const localizeLargeNumber = (
  ...
  if (number >= oneTrillion) {
    return `${(number / oneTrillion).toFixed(2)}${translate(
      'token.trillion_abbreviation',
    )}`;
  } else if (number >= oneBillion) {
    return `${(number / oneBillion).toFixed(2)}${translate(
      'token.billion_abbreviation',
    )}`;
  } else if (number >= oneMillion) {
    return `${(number / oneMillion).toFixed(2)}${translate(
