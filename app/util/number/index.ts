...
type NumericValue = BigNumber.Value | BN4;
...
/**
 * The number utilities run on bn.js v4 (bnjs4) at runtime, but callers across
 * the app pass instances created from bn.js v5 (bnjs5) or the plain `bn.js`
 * package. These are structurally distinct to TypeScript, so input parameters
 * that accept "any BN instance" use this union.
 */
type BNInput = BN4 | BN5 | BNJs;
...
export function BNToHex(inputBn: BNInput): string {
...
export function fromWei(
  value: number | string | BNInput = 0,
  unit = 'ether',
): string {
  return convert.fromWei(value as BN4, unit);
}
...
const converter = ({
  ...
  roundDown,
}: ConverterOptions): NumericValue => {
  let convertedValue: NumericValue = fromNumericBase
...
export const toHexadecimal = (
  ...
  if (!decimal) return decimal as string | number;
