...
type NumericValue = string | number | BN4 | BN | BigNumber;
type NumericBase = 'hex' | 'dec' | 'BN';
type EthDenomination = 'WEI' | 'GWEI' | 'ETH';
type ConversionResult = string | BigNumber | BN4;
 ...
 */
export const hexToBN = (inputHex?: NumericValue | null) =>
...
export function BNToHex(inputBn: { toString(base?: number): string }) {
  return add0x(inputBn.toString(16));
}

// Setter Maps
export const toBigNumber = {
  hex: (n: NumericValue) => new BigNumber(stripHexPrefix(n as string), 16),
  dec: (n: NumericValue) => new BigNumber(String(n), 10),
  BN: (n: BN4) => new BigNumber(n.toString(16), 16),
};
...
export function fiatNumberToTokenMinimalUnit(
  ...
  let weiNumber: string | number = floatFiatConverted * base;
  // avoid decimals
 ...
 */
export function isNumberValue(value: string | number | null | undefined) {
...
export function weiToFiat(
  ...
  const value = weiToFiatNumber(wei as BN4, conversionRate, decimalsToShow);
...
export function addCurrencySymbol(
  ...
  if ((currencySymbols as Record<string, string>)[currencyCode as string]) {
    return `${prefix}${
      (currencySymbols as Record<string, string>)[currencyCode as string]
    }${absAmountStr}`;
  }
  ...
  if (
    (currencySymbols as Record<string, string>)[lowercaseCurrencyCode as string]
  ) {
    return `${prefix}${
      (currencySymbols as Record<string, string>)[
        lowercaseCurrencyCode as string
      ]
    }${absAmountStr}`;
  }
...
export function fiatNumberToWei(fiat: string | number, conversionRate: number) {
  ...
  let weiNumber: string | number = Math.trunc(base * floatFiatConverted);
  // avoid decimals
  weiNumber = weiNumber.toLocaleString('fullwide', { useGrouping: false });
  const weiBN = safeNumberToBN(weiNumber);
  return weiBN;
 ...
 */
export function safeNumberToBN(value?: string | number | null) {
  try {
...
const converter = ({
  ...
  let convertedValue: BigNumber = fromNumericBase
    ? toBigNumber[fromNumericBase](value as BN4)
    : (value as BigNumber);
...
export const toHexadecimal = (decimal?: string | number | null): string => {
...
export const safeBNToHex = (value?: BN4 | string | null) => {
  if (value === null || value === undefined) {
    return value;
  }
...
export const convertDecimalToPercentage = (decimal: unknown) => {
