...
/**
 * Minimal shape shared by the BN implementations used across the app
 * (bnjs4, bnjs5, bn.js). These types are nominally incompatible with each
 * other, but all can be serialized to a base-16 string.
 */
interface BNLike {
...
export function BNToHex(inputBn: BNLike): Hex {
 ...
 * @param [isRounding=true] - If true, minimalInput is converted to number and rounded for large numbers.
 * @returns String containing the new number
 */
export function fromTokenMinimalUnit(
  minimalInput: string | number | BN4,
  decimals: number,
  isRounding = true,
): string {
  const rawInput: string | number | BN4 = isRounding
    ? Number(minimalInput)
    : minimalInput;
  const prefixedInput = addHexPrefix((rawInput as number | BN4).toString(16));
 ...
 * @returns BN instance containing the new number
 */
export function toWei(value: NumericValue, unit = 'ether'): BN4 {
 ...
 * @returns BN instance containing the new number
 */
export function toGwei(value: NumericValue, unit = 'ether'): number {
 ...
 */
export function renderToGwei(value: NumericValue, unit = 'ether'): number {
 ...
 */
export function renderWei(value: NumericValue): string {
  if (!value) return '0';
  const wei = fromWei(value);
...
export const toHexadecimal = (decimal?: NumericValue): Hex => {
 ...
 */
export const isZeroValue = (value: NumericValue | null | undefined): boolean => {
  if (value === null || value === undefined) {
    return false;
  }
