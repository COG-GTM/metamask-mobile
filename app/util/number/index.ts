 ...
 */
import { stripHexPrefix, BN as EthereumjsBN } from 'ethereumjs-util';
...
type NumberValue = string | number | BN4 | BN5 | BigNumber | EthereumjsBN;
type ConversionValue = string | number | BigNumber | BN4;
...
interface ConverterInput extends ConverterOptions {
...
export function BNToHex(inputBn: BN4 | BN5 | EthereumjsBN | string) {
  return add0x((inputBn as BN4).toString(16));
}
 ...
 */
export function fromWei(value: NumberValue = 0, unit = 'ether') {
...
export function fromTokenMinimalUnit(
  minimalInput: NumberValue,
  decimals: number,
  isRounding = true,
) {
  minimalInput = isRounding ? Number(minimalInput) : minimalInput;
  const prefixedInput = addHexPrefix((minimalInput as number).toString(16));
 ...
 */
export function toTokenMinimalUnit(tokenValue: NumberValue, decimals: number) {
  const base = toBN(Math.pow(10, decimals).toString());
  ...
  let whole: string | BN4 = comps[0],
    fraction: string | BN4 = comps[1];
  if (!whole) {
    whole = '0';
  }
  if (!fraction) {
  ...
  whole = new BN4(whole as string);
  fraction = new BN4(fraction as string);
...
export function renderFiatAddition(
  ...
  if ((currencySymbols as Record<string, string>)[currentCurrency]) {
    return `${
      (currencySymbols as Record<string, string>)[currentCurrency]
    }${renderMinimalUnit}`;
  }
 ...
 * If value is less than 5 precision decimals will show '< 0.00001'
 */
export function renderFromWei(value: NumberValue, decimalsToShow = 5) {
  // eslint-disable-next-line @typescript-eslint/no-shadow -- preserving the existing local name
  let renderWei = '0';
  // avoid undefined
  if (value) {
 ...
 */
export function isNumber(str: string | null | undefined) {
...
export const dotAndCommaDecimalFormatter = (value: string | number) => {
 ...
 */
export function toWei(value: NumberValue, unit = 'ether') {
 ...
 */
export function toGwei(value: NumberValue, unit = 'ether') {
 ...
 */
export function renderToGwei(value: NumberValue, unit = 'ether') {
...
export function weiToFiat(
  ...
  // eslint-disable-next-line @typescript-eslint/default-param-last -- preserving the existing runtime signature
...
export function addCurrencySymbol(
  ...
  if ((currencySymbols as Record<string, string>)[currencyCode]) {
    return `${prefix}${
      (currencySymbols as Record<string, string>)[currencyCode]
    }${absAmountStr}`;
  }

  const lowercaseCurrencyCode = currencyCode?.toLowerCase();

  if ((currencySymbols as Record<string, string>)[lowercaseCurrencyCode]) {
    return `${prefix}${
      (currencySymbols as Record<string, string>)[lowercaseCurrencyCode]
    }${absAmountStr}`;
  }
 ...
 * @returns {Object} - The converted balance as BN instance
 */
export function fiatNumberToWei(fiat: string | number, conversionRate: number) {
 ...
 */
export function safeNumberToBN(value: string | number | null | undefined) {
  try {
    const safeValue = fastSplit(value?.toString() as string) || '0';
...
export function getCurrencySymbol(currencyCode: string) {
  if ((currencySymbols as Record<string, string>)[currencyCode]) {
    return `${(currencySymbols as Record<string, string>)[currencyCode]}`;
  }
  return currencyCode;
}
 ...
 */
export function renderWei(value: NumberValue) {
  if (!value) return '0';
  const wei = fromWei(value);
  // eslint-disable-next-line @typescript-eslint/no-shadow -- preserving the existing local name
  const renderWei = (wei as unknown as number) * Math.pow(10, 18);
...
const converter = ({
  ...
  roundDown,
}: ConverterInput): ConversionValue => {
  let convertedValue: ConversionValue = fromNumericBase
 ...
 */
export const isZeroValue = (value: unknown) => {
  if (value === null || value === undefined) {
    return false;
  }
...
export const safeBNToHex = (value: BN4 | null | undefined) => {
  if (value === null || value === undefined) {
    return value;
  }
 ...
 */
export const localizeLargeNumber = (i18n: { t: unknown }, number: number) => {
