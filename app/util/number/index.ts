 ...
 */
export const hexToBN = (inputHex: string | number | BN4 | undefined): BN4 =>
  typeof inputHex !== 'string'
    ? new BN4(inputHex ?? 0, 16)
...
export function BNToHex(inputBn: BN4 | BNjs): Hex {
...
export function fromWei(
  value: number | string | BN4 | BNjs | BigNumber = 0,
  unit = 'ether',
): string {
...
export function fromTokenMinimalUnit(
  minimalInput: number | string | BN4 | BNjs,
  decimals: number,
  isRounding = true,
): string {
  const normalizedInput: number | string | BN4 | BNjs = isRounding
    ? Number(minimalInput)
    : minimalInput;
  const prefixedInput = addHexPrefix(
    (normalizedInput as number | BN4).toString(16),
  );
...
export function renderWei(value: number | string | BN4): string {
  if (!value) return '0';
  const wei = fromWei(value);
  const result = Number(wei) * Math.pow(10, 18);
...
const converter = ({
  ...
  roundDown,
}: ConverterInput): BigNumber | BN4 | string => {
  let convertedValue: BigNumber =
    fromNumericBase && value !== null && value !== undefined
      ? toBigNumber[fromNumericBase](value)
      : (value as unknown as BigNumber);
  ...
  if (fromCurrency !== toCurrency) {
    ...
    if (invertConversionRate) {
      rate = new BigNumber(1.0).div(rate);
    }
...
export const toHexadecimal = (decimal?: ConversionValue): Hex => {
...
export const calculateEthFeeForMultiLayer = ({
  ...
  multiLayerL1FeeTotal?: ConversionValue | null;
