...
// @ts-expect-error Module '@metamask/ethjs-unit' has no declaration file
import convert from '@metamask/ethjs-unit';
import { add0x, remove0x } from '@metamask/utils';
// @ts-expect-error Module 'number-to-bn' has no declaration file
 ...
 */
export function fromWei(value: number | string | BN4 = 0, unit: string = 'ether'): string {
...
export function fromTokenMinimalUnit(
  ...
  fraction = (fraction.match(regex.fractions) ?? ['', '0'])[1];
...
export function renderFiatAddition(
  ...
  const currencySymbolsRecord = currencySymbols as Record<string, string>;
  if (currencySymbolsRecord[currentCurrency]) {
    return `${currencySymbolsRecord[currentCurrency]}${renderMinimalUnit}`;
  }
 ...
 * @returns {string}
 */
export function limitToMaximumDecimalPlaces(num: number, maxDecimalPlaces: number = 5): string | number {
...
export function fiatNumberToTokenMinimalUnit(
  ...
  // avoid decimals
  const weiString = weiNumber.toLocaleString('fullwide', { useGrouping: false });
 ...
 * If value is less than 5 precision decimals will show '< 0.00001'
 */
export function renderFromWei(value: number | string | BN4, decimalsToShow: number = 5): string {
  let renderWei = '0';
  // avoid undefined
  if (value) {
 ...
 */
export function calcTokenValueToSend(value: number | string | BN4, decimals: number): string | number {
 ...
 */
export function toBN(value: string): BN4 {
 ...
 */
export function toWei(value: number | string | BN4, unit: string = 'ether'): BN4 {
  // check the posibilty to convert to BN
  // directly on the swaps screen
  if (typeof value !== 'object' && isNumberScientificNotationWhenString(value)) {
    value = (Number(value)).toFixed(18);
  }
  return convert.toWei(String(value) as string, unit);
}
 ...
 */
export function toGwei(value: number | string | BN4, unit: string = 'ether'): number {
 ...
 */
export function renderToGwei(value: number | string | BN4, unit: string = 'ether'): number {
...
export function addCurrencySymbol(
  ...
  let displayAmount: string = String(amount);
  if (extendDecimals) {
    if (isNumberScientificNotationWhenString(amount)) {
      displayAmount = (numericAmount).toFixed(18);
      numericAmount = parseFloat(displayAmount);
    }

    // if bigger than 0.01, show 2 decimals
    if (numericAmount >= 0.01 || numericAmount <= -0.01) {
    ...
    if ((numericAmount < 0.01 && numericAmount > 0) || (numericAmount > -0.01 && numericAmount < 0)) {
      const decimalString = displayAmount.toString().split('.')[1];
      if (decimalString && decimalString.length > 1) {
        const match = decimalString.match(regex.decimalString);
        const firstNonZeroDecimal = match ? decimalString.indexOf(match[0]) : -1;
        if (firstNonZeroDecimal > 0) {
          displayAmount = numericAmount.toFixed(firstNonZeroDecimal + 3);
          // remove trailing zeros
          displayAmount = displayAmount.replace(regex.trailingZero, '');
        }
      }
    }
  }
  ...
  if (currencyCode === 'usd' && !extendDecimals) {
    displayAmount = parseFloat(String(amount)).toFixed(2);
  }

  const amountString = displayAmount.toString();
  const absAmountStr = amountString.startsWith('-')
    ? amountString.slice(1) // Remove the first character if it's a '-'
    : amountString;

  const currencySymbolsRecord = currencySymbols as Record<string, string>;
  if (currencySymbolsRecord[currencyCode]) {
    return `${prefix}${currencySymbolsRecord[currencyCode]}${absAmountStr}`;
  }

  const lowercaseCurrencyCode = currencyCode?.toLowerCase();

  if (currencySymbolsRecord[lowercaseCurrencyCode]) {
    return `${prefix}${currencySymbolsRecord[lowercaseCurrencyCode]}${absAmountStr}`;
  }
 ...
 */
export function weiToFiatNumber(wei: number | string | BN4, conversionRate: number, decimalsToShow: number = 5): number {
  const base = Math.pow(10, decimalsToShow);
...
export function fiatNumberToWei(fiat: number | string, conversionRate: number): string | BN4 {
  ...
  // avoid decimals
  const weiStr = weiNumber.toLocaleString('fullwide', { useGrouping: false });
  const weiBN = safeNumberToBN(weiStr);
  return weiBN;
}
...
export function balanceToFiat(
  ...
  const fiatFixed = balanceToFiatNumber(balance, conversionRate ?? 0, exchangeRate ?? 0);
...
export function getCurrencySymbol(currencyCode: string): string {
  const currencySymbolsRecord = currencySymbols as Record<string, string>;
  if (currencySymbolsRecord[currencyCode]) {
    return `${currencySymbolsRecord[currencyCode]}`;
  }
  return currencyCode;
}
...
export function renderFiat(value: number, currencyCode: string, decimalsToShow: number = 5): string {
  ...
  const currencySymbolsRecord = currencySymbols as Record<string, string>;
  if (currencySymbolsRecord[currencyCode]) {
    return `${currencySymbolsRecord[currencyCode]}${fiatFixed}`;
  }
...
export function renderWei(value: number | string | BN4): string {
  if (!value) return '0';
  const weiStr = fromWei(value);
  const renderWeiVal = Number(weiStr) * Math.pow(10, 18);
 ...
 * false otherwise.
 */
export function isPrefixedFormattedHexString(value: string | unknown): boolean {
...
export const toHexadecimal = (decimal: number | string): string | number => {
...
export const isZeroValue = (value: number | string | BN4 | null | undefined): boolean => {
  ...
  return value === '0x0' || (isBN(value) && (value as BN4).isZero()) || isZero(value);
};
