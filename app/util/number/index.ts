...
type NumericValue = string | number | BN4 | BigNumber;
...
const currencySymbolMap: Record<string, string> = currencySymbols;
...
export function BNToHex(inputBn: BN4 | BigNumber) {
...
export function renderFromWei(value: NumericValue, decimalsToShow = 5) {
  ...
  if (value) {
    ...
    if (weiNumber < 0.00001 && weiNumber > 0) {
      ...
      const base = Math.pow(10, decimalsToShow);
      renderValue = (Math.round(weiNumber * base) / base).toString();
    }
  }
  return renderValue;
}
 ...
 */
export function calcTokenValueToSend(value: number | BN4, decimals: number) {
...
export function toWei(value: NumericValue, unit = 'ether') {
  ...
  if (isNumberScientificNotationWhenString(value as number | string)) {
...
export const toHexadecimal = (decimal?: NumericValue): string => {
...
export const calculateEthFeeForMultiLayer = ({
  ...
  return new BigNumber(multiLayerL1FeeTotalDecEth as BigNumber.Value)
    .plus(new BigNumber((ethFee ?? 0) as BigNumber.Value))
    .toString(10);
};
...
export const safeBNToHex = (value: BN4 | BigNumber | null | undefined) => {
  if (value === null || value === undefined) {
    return value;
  }
