...
type ConversionValue = number | string | BN4 | BigNumber;
...
type BNLike = BN4 | { toString(base?: number | 'hex'): string };
...
export function fromWei(
  value: number | string | BNLike = 0,
  unit = 'ether',
): string {
  return convert.fromWei(value as number | string | BN4, unit);
}
...
export function fromTokenMinimalUnit(
  minimalInput: number | string | BNLike,
  decimals: number,
  isRounding = true,
): string {
...
export function fiatNumberToTokenMinimalUnit(
  ...
  // avoid decimals
  const weiNumberStr = weiNumber.toLocaleString('fullwide', {
    useGrouping: false,
  });
  const weiBN = safeNumberToBN(weiNumberStr);
  return weiBN;
}
...
export function renderFromWei(
  value: number | string | BNLike | null | undefined,
  decimalsToShow = 5,
): string {
...
export function calcTokenValueToSend(
  value: BigNumber | BN4 | number | null | undefined,
  decimals: number,
): string | 0 {
  return value
...
export function weiToFiat(
  wei: BNLike | number | null | undefined,
...
export function addCurrencySymbol(
  ...
  if (extendDecimals) {
    ...
    if (
      (amount as unknown as number) >= 0.01 ||
      (amount as unknown as number) <= -0.01
    ) {
  ...
  if (currencySymbolsMap[currencyCode as string]) {
    return `${prefix}${currencySymbolsMap[currencyCode as string]}${absAmountStr}`;
  }
...
export function weiToFiatNumber(
  wei: number | string | BNLike | null | undefined,
  conversionRate: number,
  decimalsToShow = 5,
): number {
  const base = Math.pow(10, decimalsToShow);
  const eth = fromWei(wei as number | string | BN4).toString();
  let value = parseFloat(
    String(
      Math.floor((eth as unknown as number) * conversionRate * base) / base,
    ),
  );
...
export function balanceToFiatNumber(
  ...
  let fiatFixed = parseFloat(
    String(
      Math.floor(
        (balance as number) * (conversionRate as number) * exchangeRate * base,
      ) / base,
    ),
  );
...
const converter = ({
  ...
  roundDown,
}: ConverterInput): BigNumber | string | BN4 => {
  ...
  if (fromCurrency !== toCurrency) {
    ...
    let rate = toBigNumber.dec(conversionRate as number | string | BigNumber);
    if (invertConversionRate) {
      rate = new BigNumber(1.0).div(conversionRate as number | string);
    }
...
export const toHexadecimal = (decimal?: number | string | null): string => {
...
export const calculateEthFeeForMultiLayer = ({
  ...
  const multiLayerL1FeeTotalDecEth = conversionUtil(
    multiLayerL1FeeTotal as string,
    {
      fromNumericBase: 'hex',
      toNumericBase: 'dec',
...
export const safeBNToHex = (
  value?: BN4 | string | null,
): Hex | null | undefined => {
  if (value === null || value === undefined) {
    return value;
  }

  return BNToHex(value as unknown as BN4);
};
