...
/**
 * A value that can be converted by the utilities in this module.
 */
export type ConversionValue = string | number | BN | BigNumber;

/**
 * The result of a conversion. Depending on the requested `toNumericBase` the
 * value may be returned as a string, a `BN`, a `BigNumber` or the number `0`.
 */
export type ConversionResult = string | number | BN | BigNumber;
...
export type ConversionOptions = Omit<ConverterParams, 'value'>;

export interface AddSubtractOptions extends ConversionOptions {
  aBase?: number;
  bBase?: number;
}

export interface MultiplyOptions extends ConversionOptions {
...
const toBigNumber: Record<NumericBase, (n: ConversionValue) => BigNumber> = {
  ...
  BN: (n) => new BigNumber((n as BN | BigNumber).toString(16), 16),
};
...
const converter = ({
  ...
  let convertedValue: ConversionResult = fromNumericBase
...
const conversionGTE = (
  ...
  const firstValue = converter({ ...firstProps });
  const secondValue = converter({ ...secondProps });
  return (firstValue as BigNumber).isGreaterThanOrEqualTo(
...
const conversionLTE = (
  ...
  const firstValue = converter({ ...firstProps });
  const secondValue = converter({ ...secondProps });
  return (firstValue as BigNumber).isLessThanOrEqualTo(secondValue as BigNumber);
};
