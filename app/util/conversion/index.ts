...
type ConversionResult = string | BigNumber | BN;
...
interface ConversionOptions {
  ...
  conversionRate?: string | number | BigNumber | null;
...
interface CurrencyOperationOptions extends ConversionOptions {
  aBase?: number;
  bBase?: number;
}

interface MultiplyCurrenciesOptions extends ConversionOptions {
...
const getBigNumber = (value: NumericValue, base?: number) => {
  if (!isValidBase(base)) {
...
const conversionGTE = (
  ...
  const firstValue = converter({ ...firstProps }) as BigNumber;
  const secondValue = converter({ ...secondProps }) as BigNumber;
  // `greaterThanOrEqualTo` was removed in bignumber.js v9 and is no longer in
  // its types; the call is kept as-is so behaviour is unchanged.
  return (
    firstValue as unknown as {
...
const conversionLTE = (
  ...
  const firstValue = converter({ ...firstProps }) as BigNumber;
  const secondValue = converter({ ...secondProps }) as BigNumber;
  // `lessThanOrEqualTo` was removed in bignumber.js v9 and is no longer in its
  // types; the call is kept as-is so behaviour is unchanged.
  return (
    firstValue as unknown as { lessThanOrEqualTo(n: BigNumber): boolean }
  ).lessThanOrEqualTo(secondValue);
};

const toNegative = (n: NumericValue, options: MultiplyCurrenciesOptions = {}) =>
