...
import {
  ...
  doENSReverseLookup,
  getCachedENSName,
  isDefaultAccountName } from
'../../util/ENSUtils';
import {
  isMainnetByChainId,
  findBlockExplorerForRpc } from
'../../util/networks';
 ...
 * @returns {String} - String corresponding to full formatted address. EVM addresses are checksummed, non EVM addresses are not.
 */
export function toFormattedAddress(address) {
 ...
 */
export function renderShortAddress(address, chars = 5) {
...
export function isHDOrFirstPartySnapAccount(account) {
  if (
 ...
 * @returns {Boolean} - Returns a boolean
 */
export function isSnapAccount(address) {
  const keyring = getKeyringByAddress(address);
