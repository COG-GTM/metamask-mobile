 ...
 */
export function apiEstimateModifiedToWEI(estimate: number | string): BN4 {
 ...
 * @returns BN instance containing gas fee in wei
 */
 ...
 * @param estimate - Number corresponding to api gas price estimation
 * @param gasLimit - Number corresponding to transaction gas limit
 * @returns String of the gas fee in eth (render format)
 */
 ...
 * @param gasLimit - Number corresponding to transaction gas limit
 * @returns String of the gas fee formatted in the requested currency
 */
...
export async function getGasLimit(
  transaction: Partial<TransactionParams> & Record<string, unknown>,
  ...
  // `@metamask/controller-utils` ships its own `bn.js` typings while the rest
  // of the app uses the `bnjs4` alias. The runtime value is the same instance
  // shape, so cast to align with the surrounding code's BN type.
  const gas = hexToBN(estimation.gas) as unknown as BN4;
  return { gas };
}
