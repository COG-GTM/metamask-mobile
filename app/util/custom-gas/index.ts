 ...
 */
export function apiEstimateModifiedToWEI(estimate: string | number): BN {
...
export function parseWaitTime(min: number): string {
  ...
  if (parsed === '') {
    const secondVal = (Math.round(tempMin * 100) * 3) / 5;
    if (secondVal) {
      parsed += ` ${Math.ceil(secondVal)}${strings('unit.second')}`;
    }
  }
  return parsed.trim();
}
