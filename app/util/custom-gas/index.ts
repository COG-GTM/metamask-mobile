...
export function parseWaitTime(min: number): string {
  ...
  timeUnits.forEach((unit) => {
    ...
    val = Math.floor(tempMin / (unit[1] as number));
    if (val) {
      if (parsed !== '') parsed += ' ';
      parsed += `${val}${unit[0]}`;
    }
    tempMin = min % (unit[1] as number);
  });
  if (parsed === '') {
...
export async function getGasLimit(
  ...
  try {
    ...
    estimation = await estimateGas(newTransactionObj as Parameters<typeof estimateGas>[0], networkClientId ?? '');
  } catch (error) {
    estimation = {
