 ...
 * @param estimate - Number corresponding to api gas price estimation
 * @param gasLimit - Number corresponding to transaction gas limit
 * @returns Renderable ETH gas fee
 */
 ...
 * @param gasLimit - Number corresponding to transaction gas limit
 * @returns Renderable fiat gas fee
 */
...
export function getValueFromWeiHex({
  ...
  // `conversionUtil` still lives in the untyped `app/util/number` module, so its
  // inferred parameter types are narrower than the values it actually accepts.
  return conversionUtil(value, {
    ...
    toDenomination,
    conversionRate,
  } as Parameters<typeof conversionUtil>[1]) as string | number;
}
