...
export async function doENSReverseLookup(
  ...
  if (networkHasEnsSupport) {
    ...
    (this as EnsContext).ens = new ENS({ provider, network: networkId });
    try {
      const name = await ((this as EnsContext).ens as EnsResolver).reverse(
        address,
      );
      const resolvedAddress = await (
        (this as EnsContext).ens as EnsResolver
      ).lookup(name);
