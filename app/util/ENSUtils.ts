 ...
 */
const ENS_SUPPORTED_CHAIN_IDS: Hex[] = [ChainId[NetworkType.mainnet]];
...
export function getCachedENSName(
  ...
  const networkHasEnsSupport = ENS_SUPPORTED_CHAIN_IDS.includes(chainId as Hex);
...
export async function doENSReverseLookup(
  ...
  if (networkHasEnsSupport) {
    const networkId = CHAIN_ID_TO_NETWORK_ID[chainId as Hex];
