...
// @ts-expect-error ethjs-ens does not provide TypeScript declarations.
...
export async function doENSReverseLookup(
  ...
  if (networkHasEnsSupport) {
    const networkId = CHAIN_ID_TO_NETWORK_ID[chainId];
    (this as { ens?: typeof ENS }).ens = new ENS({
      provider,
      network: networkId,
    });
    try {
      const name = await (this as { ens: InstanceType<typeof ENS> }).ens.reverse(
        address,
      );
      const resolvedAddress = await (
        this as { ens: InstanceType<typeof ENS> }
      ).ens.lookup(name);
      ...
      if (
        (error.message as string).includes(ENS_NAME_NOT_DEFINED_ERROR) ||
        (error.message as string).includes(INVALID_ENS_NAME_ERROR)
      ) {
