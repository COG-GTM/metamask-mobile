...
type GasFeeEstimatesMap = Record<string, GasFeeEstimateLevel>;
 ...
 */
export function generateApprovalData(opts: ApprovalOpts) {
...
export const calculateAmountsEIP1559 = ({
  ...
  gasFeeMaxHex: string | number | BN | BigNumber;
  gasFeeMinHex: string | number | BN | BigNumber;
}) => {
  // amount numbers
...
export const calculateEIP1559Times = ({
  ...
  try {
    ...
    const low = gasFeeEstimates?.[LOW];
    const medium = gasFeeEstimates?.[MEDIUM];
    const high = gasFeeEstimates?.[HIGH];

    if (selectedOption && low && medium && high) {
      let hasTime = false;
      if (selectedOption === LOW && low.maxWaitTimeEstimate) {
        timeEstimate = `${strings('times_eip1559.maybe')} ${humanizeDuration(
          low.maxWaitTimeEstimate,
          timeParams,
        )}`;
        timeEstimateId = AppConstants.GAS_TIMES.MAYBE;
        hasTime = true;
      } else if (selectedOption === MEDIUM && low.maxWaitTimeEstimate) {
        ...
        hasTime = true;
      } else if (selectedOption === HIGH && high.minWaitTimeEstimate) {
        timeEstimate = `${strings(
          'times_eip1559.likely_in',
        )} ${humanizeDuration(high.minWaitTimeEstimate, timeParams)}`;
      ...
      if (
        ...
        Number(high.suggestedMaxPriorityFeePerGas)
      ) {
        timeEstimate = `${strings(
          'times_eip1559.likely_in',
        )} ${humanizeDuration(Number(high.minWaitTimeEstimate), timeParams)}`;
...
export const parseTransactionLegacy = (
  ...
  let weiTransactionFee =
    gasLimit?.mul(
      hexToBN(
        decGWEIToHexWEI(selectedGasFee.suggestedGasPrice as string | number),
      ),
    );
  ...
  const suggestedGasPriceHex = decGWEIToHexWEI(
    selectedGasFee.suggestedGasPrice as string | number,
  );
