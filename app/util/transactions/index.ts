  ...
  if (actionKey === SEND_ETHER_ACTION_KEY) {
    ...
    if (tx?.isTransfer) {
      ...
      if (
        ...
        (
          tx.transferInformation as { contractAddress?: string; symbol?: string }
        ).symbol = 'SAI';
      }
...
export const getActiveTabUrl = ({
  ...
  browser?: {
    tabs?: { id: number | string; url?: string }[];
    activeTab?: number | string;
  };
}): string | undefined =>
  (browser.tabs &&
    browser.activeTab &&
    browser.tabs.find(({ id }) => id === browser.activeTab)?.url) as
    | string
    | undefined;
...
export const calculateERC20EIP1559 = ({
  ...
  const tokenAmountConversion = convertTokenToFiat({
    ...
    contractExchangeRate: exchangeRate as number,
  });
...
export const calculateEIP1559Times = ({
  ...
  selectedOption,
  recommended,
  gasFeeEstimates: gasFeeEstimatesParam,
}: {
  ...
  const gasFeeEstimates = gasFeeEstimatesParam as Record<
    string,
    GasFeeEstimateLevel
  >;
...
export const parseTransactionEIP1559 = (
  ...
  const estimatedGasLimitHex = (selectedGasFee.suggestedEstimatedGasLimit &&
    BNToHex(new BN(selectedGasFee.suggestedEstimatedGasLimit))) as
    | string
    | undefined;
...
interface TokenData {
  ...
  } & Record<number, { _hex?: string; toString(): string }>;
}
