 ...
 */
export function generateTransferData(type: string | undefined = undefined, opts: Record<string, string> = {}): string | undefined {
  if (!type) {
 ...
 */
export async function getMethodData(data: string, networkClientId: string): Promise<Record<string, unknown>> {
...
export function getTransactionToName({
  ...
  internalAccounts,
  ensRecipient,
}: { addressBook: Record<string, Record<string, { name: string }>>; chainId: string; toAddress: string; internalAccounts: { address: string; metadata: { name: string } }[]; ensRecipient?: string }): string | undefined {
  if (ensRecipient) {
    return ensRecipient;
  }
 ...
 */
export function validateTransactionActionBalance(transaction: Record<string, unknown>, rate: number, accounts: Record<string, { balance: string }>) {
  try {
...
export function calcTokenValue(value: number | string | BigNumber, decimals: number) {
...
export function getTokenValue(tokenParams: Array<{ name: string; value: unknown }> = []) {
 ...
 */
export function getTransactionById(transactionId: string, transactionController: { state: { transactions: Array<{ id: string; [key: string]: unknown }> } }) {
