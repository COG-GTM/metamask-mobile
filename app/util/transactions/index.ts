...
type ContractExchangeRates = Record<
  string,
...
export async function isCollectibleAddress(
  ...
  const result = Boolean(ownerOf && ownerOf !== '0x');
  CollectibleAddresses.cache[address] = result;
  return result;
}
...
export async function getActionKey(
  ...
  const transactionActionKey = actionKeys[actionKey as keyof typeof actionKeys];
...
export const calculateEIP1559GasFeeHexes = ({
  ...
  gasLimitHex: ConversionValue;
  estimatedGasLimitHex?: ConversionValue;
  estimatedBaseFeeHex: ConversionValue;
  suggestedMaxFeePerGasHex: ConversionValue;
  suggestedMaxPriorityFeePerGasHex: ConversionValue;
}) => {
  // Hex calculations
...
export const parseTransactionLegacy = (
  ...
  if (multiLayerL1FeeTotal) {
    weiTransactionFee = hexToBN(
      sumHexWEIs([BNToHex(weiTransactionFee), multiLayerL1FeeTotal]) as string,
    );
  }
 ...
 */
export const getIsNativeTokenTransferred = (txParams: TxParams) =>
  txParams?.value !== '0x0';
