...
/**
 * Any of the numeric representations accepted by these helpers.
 */
type NumericValue = string | number | BN | BigNumber;

/**
 * Asset parameters accepted by {@link generateTransferData}.
 */
...
/**
 * The transaction parameters read by the helpers in this module.
 */
...
/**
 * A transaction, either in its controller shape (`txParams`) or in the legacy
 * app shape (`transaction`).
 */
interface TransactionLike {
  ...
  txParams?: TransactionParamsLike;
  transaction?: TransactionParamsLike;
}

/**
 * The asset selected in the confirmation views.
 */
interface SelectedAssetLike {
  address?: string;
  symbol?: string;
  decimals?: number;
  isETH?: boolean;
...
/**
 * Token exchange rates as provided by callers: either the controller shape
 * (`{ price }`) or a plain numeric rate.
 */
type ContractExchangeRatesLike = Record<string, unknown>;

interface GasFeeEstimateLike {
  maxWaitTimeEstimate?: number;
  minWaitTimeEstimate?: number;
  suggestedMaxPriorityFeePerGas?: string;
}

interface SelectedGasFeeLike {
...
/**
 * Decoded token transaction data, as produced by the ethers Interface.
 */
interface TokenDataLike {
  args?: {
    _to?: string;
    _value?: { _hex?: string; toString(): string };
    [index: number]: { _hex?: string; toString(): string } | undefined;
  };
}

interface TransactionStateLike {
  selectedAsset: SelectedAssetLike | string;
 ...
 * @param opts.spender - The address of the spender
 * @param opts.value - The amount of tokens to be approved or increased
 * @param opts.data - The data of the transaction
...
export function decodeTransferData(type: string, data: string) {
  switch (type) {
    ...
    case 'transferFrom': {
      ...
      return [
        addHexPrefix(
          rawDecode(['address'], bufferEncodedFromAddress)[0] as string,
        ),
        addHexPrefix(
          rawDecode(['address'], bufferEncodedToAddress)[0] as string,
        ),
...
export async function isSmartContractAddress(
  ...
  return isSmartContractCode(code as string);
}
 ...
 * @returns Wether the given address is an ERC721 contract
 */
export async function isCollectibleAddress(address: string, tokenId: string) {
...
export async function getActionKey(
  ...
  if (actionKey === SEND_ETHER_ACTION_KEY) {
    ...
    if (tx?.isTransfer) {
      const transferInformation = tx.transferInformation as NonNullable<
        TransactionLike['transferInformation']
      >;
      // Third party sending wrong token symbol
      if (transferInformation.contractAddress === SAI_ADDRESS.toLowerCase()) {
        transferInformation.symbol = 'SAI';
      }
      currencySymbol = transferInformation.symbol;
    }

    const txParams = tx.txParams as TransactionParamsLike;
    const incoming =
      safeToChecksumAddress(txParams.to as string) === selectedAddress;
    const selfSent =
      incoming &&
      safeToChecksumAddress(txParams.from as string) === selectedAddress;
    return incoming
      ? selfSent
        ? currencySymbol
  ...
  const transactionActionKey = (actionKeys as Record<string, string>)[
    actionKey
  ];
...
export async function getTransactionReviewActionKey(
  ...
  const transactionReviewActionKey = (
    reviewActionKeys as Record<string, string>
  )[actionKey];
...
export function getTransactionToName({
  ...
  const internalAccountsMap = internalAccounts.reduce<
    Record<string, { address: string; metadata: { name?: string } }>
  >((acc, account) => {
...
export function getNormalizedTxState(state: {
  transaction?: TransactionParamsLike & { transaction?: TransactionParamsLike };
}): NormalizedTxState {
  // Returns `undefined` when there is no transaction in state; call sites read
  // the transaction params directly.
  return (
    state.transaction
...
/**
 * The flattened transaction state read from the redux store. `from` and `to` are
 * always present by the time call sites read them.
 */
...
export const getActiveTabUrl = ({
  ...
  (browser.tabs &&
    browser.activeTab &&
    browser.tabs.find(({ id }) => id === browser.activeTab)?.url) as string;
...
export const calculateAmountsEIP1559 = ({
  ...
  gasFeeMinConversion: NumericValue;
  gasFeeMinNative: NumericValue;
  gasFeeMaxNative: NumericValue;
  gasFeeMaxConversion: NumericValue;
  gasFeeMaxHex: string;
  gasFeeMinHex: string;
}) => {
  // amount numbers
...
export const calculateEthEIP1559 = ({
  ...
  nativeCurrency: string;
  currentCurrency: string;
  totalMinNative: NumericValue;
  totalMinConversion: NumericValue;
  totalMaxNative: NumericValue;
  totalMaxConversion: NumericValue;
}) => {
...
export const calculateEIP1559Times = ({
  ...
  const gasFeeEstimates = gasFeeEstimatesInput as
    | Record<string, GasFeeEstimateLike>
    | undefined;
    ...
    if (
      ...
      if (selectedOption === LOW && gasFeeEstimates[LOW].maxWaitTimeEstimate) {
        timeEstimate = `${strings('times_eip1559.maybe')} ${humanizeDuration(
          gasFeeEstimates[LOW].maxWaitTimeEstimate as number,
          timeParams,
        )}`;
        ...
        timeEstimate = `${strings(
          ...
          gasFeeEstimates[HIGH].minWaitTimeEstimate as number,
          timeParams,
        )}`;
...
export const parseTransactionEIP1559 = (
  {
    ...
    swapsParams,
    contractExchangeRates: contractExchangeRatesInput,
    conversionRate,
    currentCurrency,
    nativeCurrency,
  ...
  const selectedGasFee = selectedGasFeeInput as SelectedGasFeeLike;
  const contractExchangeRates = contractExchangeRatesInput as Record<
    string,
  ...
  const gasLimitHex = BNToHex(
    new BN(selectedGasFee.suggestedGasLimit as string),
  );
  ...
  let { gasFeeMinHex, gasFeeMaxHex, maxPriorityFeePerGasTimesGasLimitHex } =
    calculateEIP1559GasFeeHexes({
      gasLimitHex,
      estimatedGasLimitHex: estimatedGasLimitHex as string | undefined,
      estimatedBaseFeeHex,
      suggestedMaxPriorityFeePerGasHex,
      suggestedMaxFeePerGasHex,
    });
  ...
  const asset = selectedAsset as SelectedAssetLike;
...
export const parseTransactionLegacy = (
  ...
  const selectedGasFee = selectedGasFeeInput as SelectedGasFeeLike;
  const contractExchangeRates = contractExchangeRatesInput as
    | Record<string, { price?: number } | undefined>
    | undefined;
  const gasLimit = new BN(selectedGasFee.suggestedGasLimit as string);
  const gasLimitHex = BNToHex(
    new BN(selectedGasFee.suggestedGasLimit as string),
  );

  let weiTransactionFee = gasLimit?.mul(
    hexToBN(decGWEIToHexWEI(selectedGasFee.suggestedGasPrice as string)),
  );
...
export function validateTransactionActionBalance(
  transaction: { transaction: TransactionParamsLike },
  rate: number,
...
export function calcTokenValue(value: NumericValue, decimals?: number) {
 ...
 */
export function getTokenAddressParam(tokenData: TokenDataLike = {}) {
 ...
 */
export function getTokenValueParamAsHex(tokenData: TokenDataLike = {}) {
 ...
 */
export function getTokenValueParam(tokenData: TokenDataLike = {}) {
  return tokenData?.args?._value?.toString();
}

export function getTokenValue(
  tokenParams: { name?: string; value?: string }[] = [],
) {
...
export const generateTxWithNewTokenAllowance = (
  ...
  transaction: TransactionParamsLike & { txParams?: TransactionParamsLike },
) => {
 ...
 */
export const getIsNativeTokenTransferred = (txParams?: TransactionParamsLike) =>
  txParams?.value !== '0x0';
 ...
 */
export function getTransactionById<Transaction extends { id?: string }>(
  transactionId: string,
  transactionController: { state: { transactions: Transaction[] } },
) {
