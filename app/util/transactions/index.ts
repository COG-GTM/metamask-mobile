...
export function generateTransferData(
  ...
  opts: { toAddress?: string; amount?: string; fromAddress?: string; tokenId?: string } = {},
): string | undefined {
  if (!type) {
...
): Promise<{ name?: string; parsedRegistryMethod?: Record<string, unknown> }> {
...
export function getNormalizedTxState(state: Record<string, unknown>): Record<string, unknown> | undefined {
  return state.transaction
    ? { ...state.transaction, ...state.transaction.transaction }
    : undefined;
}

export const getActiveTabUrl = ({ browser = {} }: { browser?: { tabs?: { id: string; url: string }[]; activeTab?: string } }): string | undefined =>
  browser.tabs &&
  browser.activeTab &&
...
export const parseTransactionEIP1559 = (
  {
    ...
    transactionState: { selectedAsset, transaction: { value, data } } = {
      selectedAsset: {} as Record<string, unknown>,
    ...
    currentCurrency: string;
    nativeCurrency: string;
    transactionState?: { selectedAsset: Record<string, unknown>; transaction: Record<string, unknown> };
    gasFeeEstimates?: Record<string, Record<string, unknown>>;
  },
...
export const parseTransactionLegacy = (
  {
    ...
    currentCurrency,
    transactionState: { selectedAsset, transaction: { value, data } } = {
      selectedAsset: '' as unknown as Record<string, unknown>,
      transaction: {} as Record<string, unknown>,
    },
    ticker,
    selectedGasFee,
...
export function validateTransactionActionBalance(
  transaction: { transaction: Record<string, string> },
  rate: number,
...
export function getTokenAddressParam(
  tokenData: { args?: { _to?: { toString(): string }; [index: number]: { toString(): string } } } = {},
): string | undefined {
...
export function getTokenValueParamAsHex(
  tokenData: { args?: { _value?: { _hex?: string }; [index: number]: { _hex: string } } } = {},
): string | undefined {
...
export function getTokenValueParam(
  tokenData: { args?: { _value?: { toString(): string } } } = {},
): string | undefined {
 ...
 */
export const getIsNativeTokenTransferred = (txParams: { value?: string } | undefined): boolean =>
  txParams?.value !== '0x0';
