...
export function generateApprovalData(opts: ApprovalDataOpts) {
 ...
 */
export function decodeTransferData(type: string, data: string): string[] {
  switch (type) {
    case 'transfer': {
  ...
  if (actionKey === SEND_ETHER_ACTION_KEY) {
    ...
    if (tx?.isTransfer) {
      ...
      const transferInformation =
        tx.transferInformation as NonNullable<typeof tx.transferInformation>;
      if (
        transferInformation.contractAddress === SAI_ADDRESS.toLowerCase()
      ) {
        transferInformation.symbol = 'SAI';
      }
      currencySymbol = transferInformation.symbol as string;
    }

    const txParams = tx.txParams as NonNullable<typeof tx.txParams>;
    const incoming =
...
interface GetTransactionToNameConfig {
  addressBook: Record<string, Record<string, AddressBookEntryLike>>;
  chainId: string;
  toAddress: string;
...
// Permissive input shape for the gas fee estimates object as it is provided by
// callers (e.g. GasFeeController state). It is narrowed internally before use.
type GasFeeEstimatesInput = object;
...
export const calculateEIP1559Times = ({
  ...
  const gasFeeEstimates = gasFeeEstimatesInput as unknown as GasFeeEstimatesLike;
...
interface TokenData {
  args?: {
    _to?: TokenArgValue;
    _value?: TokenArgValue;
    [index: number]: TokenArgValue;
  };
}
...
export function getTokenValue(tokenParams: TokenParam[] = []) {
...
export const generateTxWithNewTokenAllowance = (
  ...
  const approvalData = generateApprovalData({
    ...
    value: uint.gt(UINT256_BN_MAX_VALUE)
      ? (UINT256_BN_MAX_VALUE as InstanceType<typeof BN>).toString(16)
      : uint.toString(16),
