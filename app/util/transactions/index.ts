...
type GasFeeEstimatesLike = Record<string, unknown>;
...
export async function isCollectibleAddress(
  ...
  const isCollectibleAddress = Boolean(ownerOf && ownerOf !== '0x');
...
export async function getTransactionActionKey(
  ...
  if (data && data !== '0x') {
    const { name } = await getMethodData(data as string, networkClientId);
    if (name) return name;
  }
...
export async function getActionKey(
  ...
  if (actionKey === SEND_ETHER_ACTION_KEY) {
    ...
    if (tx?.isTransfer) {
      const transferInformation = tx.transferInformation as {
...
export const calculateEIP1559Times = ({
  ...
  try {
    ...
    if (
      ...
      let hasTime = false;
      const lowEstimate = gasFeeEstimates[LOW] as GasFeeEstimateLevel;
      const highEstimate = gasFeeEstimates[HIGH] as GasFeeEstimateLevel;
      ...
      if (
        ...
        timeEstimate = `${strings(
          ...
          highEstimate.minWaitTimeEstimate as number,
          timeParams,
        )}`;
    ...
    if (
      !times ||
      (times as unknown as string) === 'unknown' ||
...
export const parseTransactionEIP1559 = (
  {
    selectedGasFee: selectedGasFeeParam,
    swapsParams,
  ...
  const selectedGasFee = selectedGasFeeParam as SelectedGasFee;
  value = value || '0x0';
  ...
  const estimatedGasLimitHex = (selectedGasFee.suggestedEstimatedGasLimit &&
    BNToHex(new BN(selectedGasFee.suggestedEstimatedGasLimit as string))) as
    | string
    | undefined;
  ...
  if (selectedAsset.isETH || selectedAsset.tokenId) {
    ...
    const exchangeRate = (
      contractExchangeRates as Record<string, { price?: number }>
    )[address as string]?.price;
...
export const parseTransactionLegacy = (
  ...
  if (selectedAsset.isETH) {
    ...
    const exchangeRate = (
      contractExchangeRates as Record<string, { price?: number }> | undefined
    )?.[address as string]?.price;
...
export const generateTxWithNewTokenAllowance = (
  ...
  spenderAddress: string,
  transaction: { data?: string; txParams?: Record<string, unknown> },
) => {
...
export function getTransactionById(
  ...
  transactionController: { state: { transactions: { id: string }[] } },
