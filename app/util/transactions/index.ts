...
// Transactions, assets and gas fee objects handled here come from controllers,
// redux state and dapps, and are not strictly shaped at this layer.
...
export async function isCollectibleAddress(
  ...
  const isCollectibleAddress = (ownerOf && ownerOf !== '0x') as boolean;
...
export function getTransactionToName({
  ...
  addressBook: LooseObject;
  chainId: string;
  toAddress: string;
  internalAccounts: LooseObject[];
  ensRecipient?: string;
}): string | undefined {
  if (ensRecipient) {
    return ensRecipient;
  ...
  const internalAccountsMap = internalAccounts.reduce(
    (acc: Record<string, LooseObject>, account: LooseObject) => {
...
export function getNormalizedTxState(state: LooseObject) {
...
export const calculateEIP1559Times = ({
  ...
  try {
    ...
    const times: LooseObject = GasFeeController.getTimeEstimate(
      suggestedMaxPriorityFeePerGas,
      suggestedMaxFeePerGas,
    );
...
export function getTransactionById(
  ...
  transactionController: LooseObject,
) {
