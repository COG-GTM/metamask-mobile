...
export interface GasFeeEstimatesByLevel {
...
const getExchangeRatePrice = (
  ...
  const rate = contractExchangeRates?.[address ?? ''];
  if (typeof rate !== 'object' || rate === null || !('price' in rate)) {
    return undefined;
  }
  const { price } = rate as { price?: unknown };
...
const asObject = <T extends object>(value: T | string | undefined): Partial<T> =>
...
export function generateApprovalData(opts: ApprovalDataOptions): string {
  ...
  return (
    functionSignature +
    bufferToHex(rawEncode(['address', 'uint256'], [spender, addHexPrefix(value)]))
  );
}
  ...
  switch (type) {
    ...
    case 'transferFrom': {
      ...
      return [
        ...
        addHexPrefix(String(rawDecode(['address'], bufferEncodedToAddress)[0])),
...
export const calculateAmountsEIP1559 = ({
  ...
  gasFeeMaxHex: ConversionOutputValue;
  gasFeeMinHex: ConversionOutputValue;
}) => {
  // amount numbers
...
export const calculateEIP1559Times = ({
  ...
  selectedOption?: string | null;
  recommended?: string;
  gasFeeEstimates?: GasFeeEstimatesByLevel;
}): {
  timeEstimate: string;
  ...
  try {
    ...
    const lowEstimate = gasFeeEstimates?.[LOW];
    const mediumEstimate = gasFeeEstimates?.[MEDIUM];
    const highEstimate = gasFeeEstimates?.[HIGH];

    if (selectedOption && lowEstimate && mediumEstimate && highEstimate) {
      let hasTime = false;
      if (selectedOption === LOW && lowEstimate.maxWaitTimeEstimate) {
        ...
        hasTime = true;
      } else if (selectedOption === MEDIUM && lowEstimate.maxWaitTimeEstimate) {
        ...
        hasTime = true;
      } else if (selectedOption === HIGH && highEstimate.minWaitTimeEstimate) {
        timeEstimate = `${strings(
          'times_eip1559.likely_in',
        )} ${humanizeDuration(highEstimate.minWaitTimeEstimate, timeParams)}`;
    ...
    if (
      ...
      times.upperTimeBound === undefined ||
...
export const calculateEIP1559GasFeeHexes = ({
  ...
  gasLimitHex: string;
  estimatedGasLimitHex?: string | false | 0;
...
export const parseTransactionEIP1559 = (
  ...
  const selectedAsset = asObject<SelectedAsset>(transactionState.selectedAsset);
  const { data } = transactionState.transaction ?? {};
  const value = transactionState.transaction?.value || '0x0';
  ...
  const gasLimitHex = BNToHex(new BN(selectedGasFee.suggestedGasLimit ?? 0));
  ...
  if (selectedAsset.isETH || selectedAsset.tokenId) {
    ...
    const exchangeRate = getExchangeRatePrice(contractExchangeRates, address);
...
export const parseTransactionLegacy = (
  ...
  const selectedGasFee = asObject<SelectedGasFee>(selectedGasFeeInput);
  const selectedAsset = asObject<SelectedAsset>(transactionState.selectedAsset);
  const { value, data } = transactionState.transaction ?? {};
  const gasLimit = new BN(selectedGasFee.suggestedGasLimit ?? 0);
  const gasLimitHex = BNToHex(new BN(selectedGasFee.suggestedGasLimit ?? 0));

  let weiTransactionFee = gasLimit?.mul(
    hexToBN(decGWEIToHexWEI(selectedGasFee.suggestedGasPrice ?? 0)),
  );
...
export function calcTokenAmount(
  ...
  const divisor = new BigNumber(10).pow(Number(decimals ?? 0));
...
export const generateTxWithNewTokenAllowance = <
  T extends { data?: string; [key: string]: unknown },
>(
  tokenValue: string | number | BN | BigNumber,
...
export const getIsNativeTokenTransferred = (
  txParams: Partial<TransactionParams> | undefined,
): boolean =>
  txParams?.value !== '0x0';
 ...
 */
export function isNFTTokenStandard(tokenStandard: string | undefined): boolean {
  return (
    tokenStandard !== undefined &&
    ([ERC721, ERC1155] as string[]).includes(tokenStandard)
  );
}
 ...
 */
export function getTransactionById<T extends Pick<TransactionMeta, 'id'>>(
