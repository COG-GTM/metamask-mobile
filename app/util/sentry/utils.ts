...
function removeSES(report: SentryReport): void {
  ...
  if (stacktraceFrames) {
    ...
    if (values?.[0]?.stacktrace) {
      values[0].stacktrace.frames = filteredFrames;
    }
  }
}
...
function rewriteReport(report: SentryReport): SentryReport {
  try {
    ...
    const appState = store?.getState() as unknown as
      | Record<string, unknown>
      | undefined;
    const maskedState = maskObject(
      appState,
      sentryStateMask as Record<string | symbol, unknown>,
    );
    if (!report.contexts) {
      report.contexts = {};
    }
...
): SentryTransactionEvent | null | undefined {
  ...
  const init = async () => {
    ...
    Sentry.init({
      ...
      beforeSend: ((report: unknown) =>
        ...
        )) as unknown as Parameters<typeof Sentry.init>[0] extends
        | infer Opts
        | undefined
        ? Opts extends { beforeSend?: infer F }
          ? F
          : never
        : never,
      beforeBreadcrumb: ((breadcrumb: unknown) =>
        ...
        | infer Opts
        | undefined
        ? Opts extends { beforeBreadcrumb?: infer F }
          ? F
          : never
        : never,
      beforeSendTransaction: ((event: unknown) =>
        ...
        | infer Opts
        | undefined
        ? Opts extends { beforeSendTransaction?: infer F }
          ? F
          : never
        : never,
