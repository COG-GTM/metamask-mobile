...
export type MaskValue = boolean | typeof AllProperties | Mask | unknown[];
...
export type MaskedObject = Record<string, unknown>;
...
function removeSES(report: SentryReport): void {
  ...
  if (stacktraceFrames && firstException?.stacktrace) {
    ...
    firstException.stacktrace.frames = filteredFrames;
  }
}
...
export function maskObject(
  ...
  const maskRecord = mask as Mask;
  const sourceRecord = objectToMask as Record<string, unknown>;

  // Include both string and symbol keys.
  const maskKeys = Reflect.ownKeys(maskRecord);
  const allPropertiesMask = maskKeys.includes(AllProperties)
    ? maskRecord[AllProperties]
    : undefined;

  return Object.keys(sourceRecord).reduce<MaskedObject>((maskedObject, key) => {
    ...
    if (maskRecord[key] !== undefined && maskRecord[key] !== AllProperties) {
      maskKey = maskRecord[key];
    }
...
function rewriteReport(report: ErrorEvent): ErrorEvent {
  try {
    ...
    (report.contexts as NonNullable<ErrorEvent['contexts']>).appState =
      maskedState;
  } catch (err) {
...
export function excludeEvents<
  T extends Pick<TransactionEvent, 'transaction' | 'tags' | 'start_timestamp'>,
>(event: T | null): T | null {
