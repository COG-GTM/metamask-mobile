...
/**
 * A mask describing which properties of an object should be reported to Sentry.
 * Values are `true` (include), `false` (exclude), a sub-mask, or the
 * `AllProperties` symbol rule.
 */
export type SentryMask = Record<string | symbol, unknown>;
...
function simplifyErrorMessages(report: SentryEvent) {
...
function removeDeviceTimezone(report: SentryEvent) {
  if (report.contexts?.device)
    (report.contexts.device as Record<string, unknown>).timezone = null;
}

function removeDeviceName(report: SentryEvent) {
  if (report.contexts?.device)
 ...
 */
function removeSES(report: SentryEvent) {
...
export function maskObject(
  ...
  return Object.keys(source).reduce<Record<string, unknown>>((
    ...
    const shouldIterateSubMask =
      ...
      (maskKey as unknown) !== (AllProperties as unknown);
...
function rewriteReport<T extends SentryEvent>(report: T): T {
  try {
    ...
    const appState = store?.getState() as RootState | undefined;
    const maskedState = maskObject(appState, sentryStateMask);
    (report.contexts as NonNullable<SentryEvent['contexts']>).appState =
      maskedState;
  } catch (err) {
 ...
 * @returns the event, or null when it should be dropped
 */
export function excludeEvents<T extends SentryEvent>(
  event: T | null,
): T | null {
...
function sanitizeUrlsFromErrorMessages(report: SentryEvent) {
  rewriteErrorMessages(report, (errorMessage) => {
...
function sanitizeAddressesFromErrorMessages(report: SentryEvent) {
  rewriteErrorMessages(report, (errorMessage) => {
 ...
 * @param metamaskEnvironment - The environment MetaMask is running in (process.env.METAMASK_ENVIRONMENT). It defaults to 'local' if not provided.
