...
/**
 * The subset of a Sentry event this module reads from and rewrites.
 */
...
/**
 * The subset of a Sentry breadcrumb this module rewrites.
 */
interface SentryBreadcrumb {
  data?: { url?: string; to?: string; from?: string };
}

/**
 * A mask, as described by {@link maskObject}. Values are `true`, `false`, an
 * array, or a sub-mask, and may be keyed by {@link AllProperties}.
 */
type Mask = Record<string | symbol, unknown>;
...
function getProtocolFromURL(url: string) {
  return new URL(url).protocol;
}

function rewriteBreadcrumb(breadcrumb: SentryBreadcrumb) {
...
function simplifyErrorMessages(report: SentryReport) {
...
function removeDeviceTimezone(report: SentryReport) {
  if (report.contexts?.device) report.contexts.device.timezone = null;
}

function removeDeviceName(report: SentryReport) {
 ...
 */
function removeSES(report: SentryReport) {
  const values = report?.exception?.values;
  const stacktrace = values?.[0]?.stacktrace;
  const stacktraceFrames = stacktrace?.frames;
  if (stacktraceFrames && stacktrace) {
...
export function maskObject(
  objectToMask: object | undefined | null,
  mask: Mask = {},
): Record<string, unknown> {
  if (!objectToMask) return {};

  const valuesToMask = objectToMask as Record<string, unknown>;
  ...
  return Object.keys(valuesToMask).reduce<Record<string, unknown>>(
    (maskedObject, key) => {
...
function rewriteReport(report: SentryReport) {
  try {
    ...
    (report.contexts as Record<string, unknown>).appState = maskedState;
  } catch (err) {
 ...
 */
export function excludeEvents<Event extends SentryReport>(
  event?: Event | null,
): Event | null {
...
function sanitizeUrlsFromErrorMessages(report: SentryReport) {
  rewriteErrorMessages(report, (errorMessage) => {
...
function sanitizeAddressesFromErrorMessages(report: SentryReport) {
  rewriteErrorMessages(report, (errorMessage) => {
...
export function deriveSentryEnvironment(
  isDev: boolean,
  metamaskEnvironment: string | undefined = 'local',
  metamaskBuildType: string | undefined = 'main',
) {
...
export function setupSentry() {
  ...
  const init = async () => {
    ...
    Sentry.init({
      ...
      beforeSend: (report) => rewriteReport(report) as typeof report,
      beforeBreadcrumb: (breadcrumb) =>
        rewriteBreadcrumb(breadcrumb) as typeof breadcrumb,
