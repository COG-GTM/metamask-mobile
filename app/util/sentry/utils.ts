...
/**
 * A mask entry is either an inclusion flag, a sub-mask, or an empty array
 * placeholder used for collections.
 */
type MaskValue = boolean | unknown[] | Mask | typeof AllProperties | undefined;

interface Mask {
  [key: string]: MaskValue;
  [key: symbol]: MaskValue;
}
...
function rewriteBreadcrumb(breadcrumb: Breadcrumb) {
...
function removeDeviceTimezone(report: SentryEvent) {
  if (report.contexts?.device)
    (report.contexts.device as { timezone?: string | null }).timezone = null;
}

function removeDeviceName(report: SentryEvent) {
  if (report.contexts?.device)
    (report.contexts.device as { name?: string | null }).name = null;
}
...
function removeSES(report: SentryEvent) {
  // The non-null assertions preserve the original behaviour: an exception
  // without `values` threw, and was handled by the caller.
  ...
  return Object.keys(source).reduce<Record<string, unknown>>(
    (maskedObject, key) => {
      ...
      const shouldIterateSubMask =
        ...
        (maskKey as MaskValue) !== AllProperties;
      ...
      if (shouldPrintValue) {
        ...
        maskedObject[key] = maskObject(
          source[key] as object | undefined | null,
          maskKey as Mask,
        );
...
function rewriteReport<ReportEvent extends SentryEvent>(report: ReportEvent) {
  try {
 ...
 * @returns {(event|null)}
 */
export function excludeEvents<TransactionEvent extends SentryEvent>(
