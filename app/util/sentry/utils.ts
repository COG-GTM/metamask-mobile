...
type SentryMaskValue = boolean | unknown[] | SentryMask;
...
function removeSES(report: SentryEvent): void {
  const firstValue = report?.exception?.values?.[0];
  const stacktraceFrames = firstValue?.stacktrace?.frames;
  if (stacktraceFrames && firstValue?.stacktrace) {
    firstValue.stacktrace.frames = stacktraceFrames.filter(
...
export function maskObject(
  objectToMask: object | null | undefined,
  mask: SentryMask = {},
  ...
  return Object.keys(source).reduce<Record<string, unknown>>(
    (maskedObject, key) => {
      ...
      if (mask[key] !== undefined && (mask[key] as unknown) !== AllProperties) {
        maskKey = mask[key];
      }
