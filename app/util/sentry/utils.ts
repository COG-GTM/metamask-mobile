...
type MaskValue = boolean | Mask | unknown[];
...
function rewriteBreadcrumb(breadcrumb: Breadcrumb): Breadcrumb {
  if (breadcrumb.data?.url) {
    breadcrumb.data.url = getProtocolFromURL(breadcrumb.data.url as string);
  }
  if (breadcrumb.data?.to) {
    breadcrumb.data.to = getProtocolFromURL(breadcrumb.data.to as string);
  }
  if (breadcrumb.data?.from) {
    breadcrumb.data.from = getProtocolFromURL(breadcrumb.data.from as string);
  }
...
function removeDeviceTimezone(report: SentryEvent): void {
  const deviceContext = report.contexts?.device as
...
export function maskObject(
  ...
  return Object.keys(source).reduce<Record<string, unknown>>(
    (maskedObject, key) => {
      ...
      let maskKey: MaskValue | typeof AllProperties | undefined =
        allPropertiesMask;

      // If a key-specific mask exists, it overrides the AllProperties mask
      const keyMask = mask[key] as MaskValue | typeof AllProperties;
      if (keyMask !== undefined && keyMask !== AllProperties) {
        maskKey = keyMask;
      }
      ...
      if (shouldPrintValue) {
        ...
        maskedObject[key] = maskObject(source[key], maskKey as Mask);
