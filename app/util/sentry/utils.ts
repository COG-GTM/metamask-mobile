...
type MaskValue = boolean | Mask | unknown[] | symbol;

interface Mask {
  [key: PropertyKey]: MaskValue;
}
...
export function maskObject(
  objectToMask?: Record<string, unknown> | null,
  mask: Mask = {},
    ...
    if (mask[key] !== undefined && mask[key] !== AllProperties) {
      ...
      if (shouldPrintValue) {
        ...
        maskedObject[key] = maskObject(
          objectToMask[key] as Record<string, unknown> | null,
          maskKey as Mask,
        );
 ...
 * @returns {(event|null)}
 */
export function excludeEvents<T extends Partial<TransactionEvent>>(
  event: T | null,
): T | null {
