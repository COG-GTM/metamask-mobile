...
interface SentryReport {
  message?: string;
  exception?: {
    values: { value?: string; stacktrace?: { frames: { filename: string }[] } }[];
  };
  contexts?: {
...
type MaskRule = boolean | MaskObject | MaskRule[];
...
export function maskObject(
  ...
  return Object.keys(objectToMask).reduce<Record<string, unknown>>(
    (maskedObject, key) => {
      ...
      let maskKey: MaskRule | undefined = allPropertiesMask;
...
export function excludeEvents(
  ...
  if (event?.transaction === TraceName.UIStartup) {
    event.tags = getTraceTags(store.getState()) as Record<string, unknown>;
...
export function setupSentry(): void {
  ...
  const init = async () => {
    ...
    Sentry.init({
      ...
      beforeSend: (report) => rewriteReport(report as SentryReport),
      beforeBreadcrumb: (breadcrumb) =>
        rewriteBreadcrumb(breadcrumb as SentryBreadcrumb),
      beforeSendTransaction: (event) =>
        excludeEvents(event as SentryTransactionEvent),
