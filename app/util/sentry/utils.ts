...
function removeSES(report: SentryReport): void {
  ...
  if (stacktrace && stacktraceFrames) {
    stacktrace.frames = stacktraceFrames.filter(
...
function rewriteReport<T extends SentryEvent>(reportEvent: T): T {
  const report = reportEvent as unknown as SentryReport;
  try {
...
export function excludeEvents<T extends SentryEvent>(
  ...
  if (event?.transaction === TraceName.UIStartup) {
    const mutableEvent = event as SentryEvent;
    mutableEvent.tags = getTraceTags(store.getState());

    if (Device.isAndroid()) {
      const appLaunchTime = Performance.appLaunchTime;
      const formattedAppLaunchTime = (mutableEvent.start_timestamp = Number(
        `${appLaunchTime.toString().slice(0, 10)}.${appLaunchTime
          .toString()
          .slice(10)}`,
      ));
      if (mutableEvent.start_timestamp !== formattedAppLaunchTime) {
        mutableEvent.start_timestamp = formattedAppLaunchTime;
      }
    }
  }
