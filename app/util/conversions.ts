...
export interface GetEthConversionFromWeiHexOptions {
  value: string;
  fromCurrency?: string;
...
export interface GetWeiHexFromDecimalValueOptions {
  value: string | number;
  fromCurrency?: string;
  conversionRate?: number;
...
export function sumHexWEIs(hexWEIs: (string | undefined | null | false)[]): string {
  return (hexWEIs.filter(Boolean) as string[]).reduce(addHexes);
}

export function sumHexWEIsToUnformattedFiat(
  hexWEIs: (string | undefined | null | false)[],
  convertedCurrency: string | undefined,
  conversionRate: number | undefined,
): string {
