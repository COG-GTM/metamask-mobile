...
const toDate = (timestamp: DateInput): Date =>
  // `new Date(undefined)` returns "Invalid Date" while `new Date(null)`
  // returns the unix epoch. Both are matched by the original JavaScript
  // behavior where the input was untyped.
