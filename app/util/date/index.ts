...
type Timestamp = number | string | Date | undefined;
type DateInput = Exclude<Timestamp, undefined>;

export function toLocaleDateTime(timestamp: Timestamp): string {
  const dateObj = new Date(timestamp as DateInput);
  const date = dateObj.toLocaleDateString();
  const time = dateObj.toLocaleTimeString();
  return `${date} ${time}`;
}

export function toDateFormat(timestamp: Timestamp): string {
  const date = new Date(timestamp as DateInput);
...
export function toLocaleDate(timestamp: Timestamp): string {
  return new Date(timestamp as DateInput).toLocaleDateString();
}

export function toLocaleTime(timestamp: Timestamp): string {
  return new Date(timestamp as DateInput).toLocaleTimeString();
}
 ...
 */
export const formatTimestampToYYYYMMDD = (timestamp: Timestamp): string => {
  const date = new Date(timestamp as DateInput);
