...
export function toLocaleDateTime(timestamp: number | string | Date) {
...
export function toDateFormat(timestamp?: number | string | Date) {
  // `new Date(undefined)` yields an invalid date, which is the pre-existing
  // behaviour for callers that pass no timestamp.
...
export function toLocaleDate(timestamp?: number | string | Date) {
  return new Date(timestamp as number | string | Date).toLocaleDateString();
}

export function toLocaleTime(timestamp: number | string | Date) {
 ...
 */
export function msBetweenDates(date: Date) {
 ...
 */
export const formatTimestampToYYYYMMDD = (
 ...
 */
export const getTimeDifferenceFromNow = (timestamp: number) => {
