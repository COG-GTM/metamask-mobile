...
export const getAnalyticsParams = (
  ...
  const pageInfo = {
    ...
    ...meta,
  } as { analytics?: JsonMap; url?: string };
  ...
  const analyticsParams: JsonMap = {
    ...
    ...(pageInfo.analytics as JsonMap | undefined),
  };

  try {
    const chainId = selectEvmChainId(store.getState());
    analyticsParams.chain_id = getDecimalChainId(chainId as string);
...
export const showWalletConnectNotification = (
  ...
  InteractionManager.runAfterInteractions(() => {
     ...
     */
    const origin = (
      (messageParams.origin as string).toLowerCase() as string & {
        replaceAll(searchValue: string, replaceValue: string): string;
      }
    ).replaceAll(':', '');
    const isWCOrigin = origin.startsWith(
      (
        WALLET_CONNECT_ORIGIN as string & {
          replaceAll(searchValue: string, replaceValue: string): string;
        }
      )
        .replaceAll(':', '')
    ...
    const isSDKOrigin = origin.startsWith(
      (
        AppConstants.MM_SDK.SDK_REMOTE_ORIGIN as string & {
          replaceAll(searchValue: string, replaceValue: string): string;
        }
      )
        .replaceAll(':', '')
