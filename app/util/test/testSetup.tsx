...
jest.mock('react-native-quick-crypto', () => ({
  ...
  subtle: {
    ...
    exportKey: jest.fn((_format: unknown, _key: unknown) => Promise.resolve(new Uint8Array([1, 2, 3, 4]))),
    encrypt: jest.fn(
      (_algorithm: unknown, _key: unknown, _data: unknown) => Promise.resolve(
          new Uint8Array([
...
jest.mock('react-native-keychain', () => ({
  ...
  setInternetCredentials: jest
    .fn(((..._args: unknown[]) => undefined) as () => unknown)
    .mockResolvedValue({ service: 'metamask', storage: 'storage' } as never),
...
afterEach(() => {
  ...
  (global as unknown as { gc?: (full: boolean) => void }).gc?.(true);
});

(global as unknown as { crypto: { getRandomValues: (arr: Uint8Array) => Uint8Array } }).crypto = {
