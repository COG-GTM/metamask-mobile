...
/* eslint-disable import/no-namespace, @typescript-eslint/no-require-imports, @typescript-eslint/no-var-requires */
...
declare global {
  ...
  var segmentMockClient: Record<string, jest.Mock> | null;
  // eslint-disable-next-line no-var
...
jest.mock('react-native-quick-crypto', () => ({
  ...
  subtle: {
    ...
    deriveBits: jest.fn((_algorithm, _baseKey, length) => {
    ...
    exportKey: jest.fn((_format, _key) => Promise.resolve(new Uint8Array([1, 2, 3, 4]))),
    encrypt: jest.fn((_algorithm, _key, _data) => Promise.resolve(new Uint8Array([
        123,  34, 116, 101, 115,
        116,  34,  58,  34, 100,
         97, 116,  97,  34, 125
      ]))),
    decrypt: jest.fn((_algorithm, _key, _data) => Promise.resolve(new Uint8Array([
        123,  34, 116, 101, 115,
        116,  34,  58,  34, 100,
         97, 116,  97,  34, 125
      ]))),
...
jest.mock('redux-persist', () => ({
  ...
  persistReducer: (_: unknown, reducer: unknown) => reducer || ((state: unknown) => state),
...
afterEach(() => {
  ...
  const maybeGc = global.gc as ((options?: boolean) => void) | undefined;
  maybeGc?.(true);
});
...
jest.mock('@react-native-firebase/messaging', () => {
  const messagingModule = () => ({
      ...
      hasPermission: jest.fn(() =>
        Promise.resolve(messagingModule.AuthorizationStatus.AUTHORIZED),
      ),
