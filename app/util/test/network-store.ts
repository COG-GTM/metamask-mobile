...
const fetchWithTimeout = (
  url: string,
): Promise<AxiosResponse<FixtureResponseData>> =>
  new Promise<AxiosResponse<FixtureResponseData>>((resolve, reject) => {
    axios
      .get<FixtureResponseData>(url)
...
class ReadOnlyNetworkStore {
  ...
  // Async Storage
  // Use a permissive return type so that downstream consumers in
  // `storage-wrapper.ts` retain the untyped/`any`-style flexibility they had
  // before this file was migrated from JavaScript.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
