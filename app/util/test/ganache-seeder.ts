...
class GanacheSeeder {
  ...
  constructor(ganacheProvider: EthereumProvider) {
  ...
  async deploySmartContract(contractName: SmartContractName) {
    const ethersProvider = new Web3Provider(
      this.ganacheProvider as unknown as ExternalProvider,
      'any',
    );
