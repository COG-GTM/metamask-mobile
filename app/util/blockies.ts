...
class PNG {
  ...
  color(
    ...
    const colorVal = (((((alpha << 8) | red) << 8) | green) << 8) | blue;
...
interface BlockiesOptsResolved {
  seed: string;
  size: number;
  scale: number;
  color: [number, number, number];
...
function buildOpts(opts: BlockiesOpts): BlockiesOptsResolved {
  if (!opts.seed) {
