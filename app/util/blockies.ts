...
/**
 * HSL color representation as [hue, saturation, lightness]
 * All values are in the range [0, 1]
 */
type HSLColor = [number, number, number];

/**
 * RGBA color representation as [red, green, blue, alpha]
 * RGB values are in the range [0, 255], alpha is in the range [0, 255]
 */
type RGBAColor = [number, number, number, number];

/**
 * Options for generating a blockie
 */
...
/**
 * Resolved blockie options with all required fields
 */
interface ResolvedBlockieOptions {
  seed: string;
  size: number;
  scale: number;
  color: HSLColor;
...
/**
 * Color palette mapping color values to their palette index characters
 */
...
/**
 * Cache for storing generated blockie data URLs
 */
...
class PNG {
  ...
  color(red: number, green: number, blue: number, alpha?: number): string {
    const alphaValue = alpha !== undefined && alpha >= 0 ? alpha : 255;
    const colorValue =
      (((((alphaValue << 8) | red) << 8) | green) << 8) | blue;

    if (typeof this.palette[colorValue] === 'undefined') {
      ...
      this.buffer[this.trns_offs + 8 + this.pindex] =
        String.fromCharCode(alphaValue);

      this.palette[colorValue] = String.fromCharCode(this.pindex++);
    }
    return this.palette[colorValue];
  }
...
const randseed: number[] = [0, 0, 0, 0]; // Xorshift: [x, y, z, w] 32 bit values
...
function buildOpts(opts: BlockieOptions): ResolvedBlockieOptions {
  if (!opts.seed) {
 ...
 */
class Blockies {
  static cache: BlockieCache = {};
}

/**
 * Generates a blockie identicon data URL for the given address
 * @param address - The address to generate a blockie for
...
export function toDataUrl(address: string): string {
  ...
  // Register background color in palette (index 0) - the return value is not used
  // but the side effect of registering the color is needed
