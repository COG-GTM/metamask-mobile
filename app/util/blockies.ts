/**
 * Blockies - Ethereum-style identicon generator
 *
...
const PNG = function (this: PNGInstance, width: number, height: number, depth: number): void {
  this.width = width;
  this.height = height;
  this.depth = depth;
  ...
  const _crc32: number[] = new Array<number>();
  ...
  this.color = (red: number, green: number, blue: number, alpha?: number): string => {
    ...
    const colorValue = (((((alphaValue << 8) | red) << 8) | green) << 8) | blue;
  ...
  this.fillRect = (x: number, y: number, w: number, h: number, color: string): void => {
    for (let i = 0; i < w; i++) {
      for (let j = 0; j < h; j++) {
...
} as unknown as new (width: number, height: number, depth: number) => PNGInstance;
...
const randseed: number[] = new Array<number>(4); // Xorshift: [x, y, z, w] 32 bit values
...
function buildOpts(opts: BlockiesOptions): BuildOptsResult {
  if (!opts.seed) {
