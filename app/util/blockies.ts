...
const PNG = function (
  ...
  this.index = function (x: number, y: number): number {
  ...
  this.color = function (
    ...
    const _alpha = alpha !== undefined && alpha >= 0 ? alpha : 255;
    const color = ((((_alpha << 8) | red) << 8) | green) << 8 | blue;
...
function buildOpts(opts: BlockiesOpts): BuiltOpts {
  if (!opts.seed) {
