...
function buildOpts(opts: BlockiesOpts): Required<BlockiesOpts> {
  if (!opts.seed) {
...
function toDataUrl(address: string | undefined): string {
  const cache = address ? Blockies.cache[address] : undefined;
  if (address && cache) {
    return cache;
  }
  ...
  // The background color must be registered in the palette first, even though
  // only the foreground and spot colors are referenced below.
