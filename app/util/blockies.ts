/* eslint-disable no-var, eqeqeq, no-new-object, @typescript-eslint/prefer-for-of, @typescript-eslint/no-shadow, @typescript-eslint/no-array-constructor, @typescript-eslint/no-use-before-define, @typescript-eslint/no-unused-vars -- vendored blockies implementation, converted verbatim */
/**
...
interface Png {
  ...
  pindex: number;
  index(this: Png, x: number, y: number): number;
  color(
    this: Png,
    red: number,
    green: number,
  ...
  getBase64(this: Png): string;
  getDump(this: Png): string;
  fillRect(
    this: Png,
    x: number,
    y: number,
...
const PNG = function (this: Png, width: number, height: number, depth: number) {
  this.width = width;
  this.height = height;
  this.depth = depth;
  ...
  this.color = function (
    ...
    alpha = (alpha as number) >= 0 ? (alpha as number) : 255;
