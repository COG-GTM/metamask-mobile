...
function buildOpts(opts: { seed: string }): BlockiesOpts {
  if (!opts.seed) {
...
function toDataUrl(address: string): string {
  ...
  // Registers the background color as the first palette entry (index 0), which
  // corresponds to the default/unfilled pixels.
