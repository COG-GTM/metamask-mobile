/**
 * A handy module to generate blockies identicons as base64-encoded PNG data
 * URIs.
 *
 * @version 1.0
...
class PNG {
  ...
  color(red: number, green: number, blue: number, alpha?: number): string {
    ...
    const color = (((((a << 8) | red) << 8) | green) << 8) | blue;
    ...
    if (typeof this.palette[color] === 'undefined') {
      ...
      this.buffer[this.trns_offs + 8 + this.pindex] = String.fromCharCode(a);
...
function buildOpts(opts: { seed: string } & Partial<BlockiesOptions>): BlockiesOptions {
  if (!opts.seed) {
...
export function toDataUrl(address: string | undefined): string {
