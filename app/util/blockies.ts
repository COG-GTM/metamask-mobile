/* eslint-disable no-bitwise, prefer-const, no-var, no-redeclare, eqeqeq, @typescript-eslint/no-this-alias */
/**
...
const randseed = new Array(4) as number[]; // Xorshift: [x, y, z, w] 32 bit values
...
function buildOpts(opts: BlockiesOpts): BuiltBlockiesOpts {
  if (!opts.seed) {
...
export function toDataUrl(address: string): string {
  ...
  // bgcolor is computed for side effects (palette setup) but not used directly
  void bgcolor;
