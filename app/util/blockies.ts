/* Vendored legacy implementation kept as-is; only types were added. */
/* eslint-disable no-var, eqeqeq, @typescript-eslint/prefer-for-of, @typescript-eslint/no-use-before-define */
/**
...
/**
 * An HSL colour, with every component in the set [0, 1].
 */
type HslColor = [number, number, number];

/**
 * An RGBA colour, with every component in the set [0, 255].
 */
...
class PNG {
  ...
  color(red: number, green: number, blue: number, alpha?: number) {
    alpha = (alpha as number) >= 0 ? alpha : 255;
    const color =
      ((((((alpha as number) << 8) | red) << 8) | green) << 8) | blue;
  ...
  getDump() {
    ...
    function crc32(png: string[], offs: number, size: number) {
      ...
      for (let i = 4; i < size - 4; i += 1) {
        crc =
          crc32Table[(crc ^ png[offs + i].charCodeAt(0)) & 0xff] ^
          ((crc >> 8) & 0x00ffffff);
      }
...
export function toDataUrl(address?: string) {
  const cache = Blockies.cache[address as string];
  if (address && cache) {
    return cache;
  }

  const opts = buildOpts({ seed: (address as string).toLowerCase() });
  ...
  Blockies.cache[address as string] = ret;
  return ret;
}
