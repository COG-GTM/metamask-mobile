...
interface ResolvedBlockiesOptions {
  seed: string;
  size: number;
  scale: number;
  color: number[];
...
function toDataUrl(address: string): string {
  ...
  const [bgH, bgS, bgL] = opts.bgcolor;
  p.color(...hsl2rgb(bgH, bgS, bgL));
  const [cH, cS, cL] = opts.color;
  const color = p.color(...hsl2rgb(cH, cS, cL));
  const [sH, sS, sL] = opts.spotcolor;
  const spotcolor = p.color(...hsl2rgb(sH, sS, sL));
