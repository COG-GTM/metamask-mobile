...
function write(buffer: string[], offs: number, ...data: string[]): void {
  for (const part of data) {
    for (let j = 0; j < part.length; j++) {
      buffer[offs++] = part.charAt(j);
    }
  }
}
...
class PNG {
  ...
  private crc32(png: string[], offs: number, size: number): void {
    let crc = -1;
