...
import {
  ...
  SUPPORTED_NOTIFICATION_BLOCK_EXPLORERS } from
'@metamask/notification-services-controller/notification-services/ui';
 ...
 */
const isSameDay = (currentDate, dateToCheck) =>
 ...
 */
export function formatMenuItemDate(date, locale = 'en') {
  if (!date) {
...
export const formatAmount = (numericAmount, opts) => {
  ...
  const handleShouldEllipse = (decimalPlaces) =>
...
export const getNetworkFees = async (notification) => {
  ...
  try {
    ...
    const calculateUsdAmount = (value, decimalPlaces) =>
    formatAmount(
      parseFloat(value) *
...
export const getNotificationBadge = (trigger_type) => {
  switch (trigger_type) {
 ...
 */
export function getNativeTokenDetailsByChainId(chainId) {
 ...
 */
export function getBlockExplorerByChainId(chainId) {
 ...
 */
export const getUsdAmount = (amount, decimals, usd) => {
