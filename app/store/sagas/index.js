...
import {
  ...
  restoreXMLHttpRequest } from
'./xmlHttpRequestOverride';
import EngineService from '../../core/EngineService';
import { AppStateEventProcessor } from '../../core/AppStateEventListener';

export function* appLockStateMachine() {
  let biometricsListenerTask;
  while (true) {
...
export function* authStateMachine() {
  ...
  while (true) {
    ...
    const appLockStateMachineTask = yield fork(appLockStateMachine);
 ...
 */
export function* biometricsStateMachine(originalBioStateMachineId) {
  ...
  } else if (action?.type === UserActionType.AUTH_SUCCESS) {// Authentication successful. Navigate to wallet.
