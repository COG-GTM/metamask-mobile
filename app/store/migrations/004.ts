...
type ByAddressAndNetwork = Record<string, Record<string, unknown>>;

interface Migration004State {
  engine: {
    backgroundState: {
      TokensController: {
        allTokens: ByAddressAndNetwork;
      };
      CollectiblesController: {
        allCollectibleContracts: ByAddressAndNetwork;
        allCollectibles: ByAddressAndNetwork;
      };
...
export default function migrate(incomingState: unknown) {
  const state = incomingState as Migration004State;
  ...
  const newAllCollectibleContracts: ByAddressAndNetwork = {};
  const newAllCollectibles: ByAddressAndNetwork = {};
  const newAllTokens: ByAddressAndNetwork = {};
