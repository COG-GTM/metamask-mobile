...
const BUILT_IN_NETWORKS = new Map([
['sepolia', '0xaa36a7'],
['mainnet', '0x1'],
 ...
 */
export default function migrate(oldState) {
  const version = 76;
  ...
  for (const [origin, subject] of Object.entries(subjects)) {
    ...
    if (!isNonEmptyArrayOfStrings(accountsCaveatValue)) {
      captureException(
        new Error(
          `Migration ${version}: Invalid state.PermissionController.subjects[${origin}].permissions[${PermissionNames.eth_accounts}].caveats[0].value of type ${
          typeof ethAccountsPermission.
          caveats?.[0]?.value}`
        )
      );
      return oldState;
    ...
    if (permittedChainsPermission) {
      ...
      if (!isNonEmptyArrayOfStrings(chainsCaveatValue)) {
        captureException(
          new Error(
            `Migration ${version}: Invalid state.PermissionController.subjects[${origin}].permissions[${PermissionNames.permittedChains}].caveats[0].value of type ${
            typeof permittedChainsPermission.
            caveats?.[0]?.value}`
          )
        );
        return oldState;
