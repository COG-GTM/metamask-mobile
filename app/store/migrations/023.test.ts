...
describe('Migration #23', () => {
  ...
  it('should migrate ambiguous network IDs based on available networks configured locally', () => {
    ...
    expect((newState as Record<string, any>).user).toStrictEqual(userInitialState);
    expect(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (newState as Record<string, any>).engine.backgroundState,
    ).toStrictEqual(
  ...
  it('should duplicate entries where multiple configured networks match network ID', () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    expect((newState as Record<string, any>).user).toStrictEqual(
