...
describe('migration #67', () => {
  ...
  const MOCK_STATE_WITH_ACCOUNTS = {
    engine: {
      backgroundState: {
        AccountsController: {
          internalAccounts: {
  ...
  const MOCK_STATE_WITH_EXISTING_SCOPES = {
    engine: {
      backgroundState: {
        AccountsController: {
          internalAccounts: {
  ...
  it('preserves accounts that have valid scopes', () => {
    ...
    const result = migration(stateCopy);
  ...
  it('handles invalid scopes property gracefully', () => {
    ...
    const result = migration(stateWithInvalidScopes);
    const accounts =
    result.engine.backgroundState.AccountsController.internalAccounts.
    accounts;
  ...
  it('logs unknown account types to Sentry', () => {
    ...
    const result = migration(stateWithUnknownType);
    const accounts =
    result.engine.backgroundState.AccountsController.internalAccounts.
    accounts;
  ...
  it('updates accounts that were previously migrated with old scope values', () => {
    ...
    const OLD_ETH_NAMESPACE_SCOPE = 'eip155';
    const OLD_SOL_NAMESPACE_SCOPE = 'solana';
    const OLD_BTC_NAMESPACE_SCOPE = 'bip122';

    const stateWithOldScopes = {
      engine: {
        backgroundState: {
          AccountsController: {
            internalAccounts: {
