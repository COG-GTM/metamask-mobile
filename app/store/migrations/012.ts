...
export default function migrate(state: unknown) {
  ...
  if (!isObject(collectiblesControllerState)) {
    captureException(
      new Error(
        `Migration 12: Invalid CollectiblesController state: '${typeof collectiblesControllerState}'`,
      ),
    );
    return state;
  }
  ...
  if (!isObject(preferencesControllerState)) {
    captureException(
      new Error(
        `Migration 12: Invalid PreferencesController state: '${typeof preferencesControllerState}'`,
      ),
    );
    return state;
  }
