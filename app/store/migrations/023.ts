...
export default function migrate(state: unknown) {
  if (!isObject(state)) {
    captureException(
      new Error(`Migration 23: Invalid root state: root state is not an object`),
    );
    return state;
  }
  ...
  if (!isObject(networkControllerState)) {
    ...
    ).some((addressEntries) => !isObject(addressEntries))
  ) {
  ...
  const localChainIds = Object.values(
    ...
    customChainIds.add(
      (networkConfiguration as Record<string, string>).chainId,
    );
  ...
  for (const [networkId, addressEntries] of Object.entries(
    addressBookControllerState.addressBook as Record<string, Record<string, unknown>>,
  )) {
