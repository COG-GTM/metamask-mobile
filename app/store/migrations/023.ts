...
export default function migrate(state: unknown): unknown {
  ...
  if (!isObject(networkControllerState)) {
    ...
    const invalidConfiguration = Object.entries(
      networkControllerState.networkConfigurations as Record<
        string,
        { chainId: string }
      >,
    ...
    const [invalidConfigurationId, invalidConfigurationValue] =
      invalidConfiguration;
    captureException(
      new Error(
        `Migration 23: Network configuration missing chain ID, id '${invalidConfigurationId}', keys '${Object.keys(
          invalidConfigurationValue as Record<string, unknown>,
        )}'`,
      ),
    );
    return typedState;
    ...
    const invalidAddressBookEntry = Object.entries(
      addressBookControllerState.addressBook as Record<
        string,
        Record<string, Record<string, unknown>>
      >,
    ...
    const [networkId, invalidEntries] = invalidAddressBookEntry;
    captureException(
      new Error(
  ...
  const localChainIds = Object.values(
    ...
    customChainIds.add(
      (networkConfiguration as { chainId: string }).chainId,
    );
