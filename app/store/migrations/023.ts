import { isObject, hasProperty, RuntimeObject } from '@metamask/utils';
...
export default function migrate(rawState: unknown) {
  const state = rawState as Migration23State;
  ...
  if (!isObject(networkControllerState)) {
    ...
    Object.values(addressBookControllerState.addressBook).some(
      (addressEntries) =>
        Object.values(addressEntries as RuntimeObject).some(
          (addressEntry) =>
  ...
  const localChainIds = Object.values(
    ...
    customChainIds.add((networkConfiguration as RuntimeObject).chainId);
      ...
      for (const chainId of recognizedChainIdCandidates) {
        if (recognizedChainIdCandidates.length > 1) {
          ambiguousAddressEntries[chainId] = Object.keys(
            addressEntries as RuntimeObject,
          );
        }
        migratedAddressBook[chainId] = mapValues(
          addressEntries as RuntimeObject,
          (entry) => ({
            ...(entry as RuntimeObject),
            chainId,
          }),
        );
      }
