...
const ambiguousNetworkData = ambiguousNetworks as Record<
  string,
  { chainIds: string[] }
>;
...
export default function migrate(state: unknown): Record<string, unknown> {
  ...
  if (!isObject(networkControllerState)) {
    ...
    captureException(
      new Error(`Migration 23: Invalid user state: '${typeof user}'`),
    );
    return typedState;
  }
  ...
  for (const [networkId, addressEntries] of Object.entries(
    ...
    if (ambiguousNetworkData[networkId]) {
      const chainIdCandidates = ambiguousNetworkData[networkId].chainIds;
