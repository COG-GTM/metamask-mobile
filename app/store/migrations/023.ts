...
const ambiguousNetworkMap = ambiguousNetworks as Record<
  string,
  { chainIds: string[] }
>;
...
export default function migrate(state: unknown): Record<string, unknown> {
  ...
  if (!isObject(networkControllerState)) {
    ...
    const invalidConfigEntry = Object.entries(
      networkControllerState.networkConfigurations,
    ).find(
      ([_networkConfigId, networkConfiguration]) =>
        !hasProperty(networkConfiguration as object, 'chainId'),
    );
    if (invalidConfigEntry) {
      const [invalidConfigurationId, invalidConfiguration] = invalidConfigEntry;
      captureException(
        new Error(
    ...
    if (ambiguousNetworkMap[networkId]) {
      ...
      for (const chainId of recognizedChainIdCandidates) {
        ...
        migratedAddressBook[chainId] = mapValues(
          addressEntries as Record<string, object>,
          (entry) => ({
            ...entry,
            chainId,
          }),
