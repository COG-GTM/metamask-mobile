...
const ambiguousNetworksById: Record<string, { chainIds: string[] }> =
  ambiguousNetworks;
...
export default function migrate(stateUnknown: unknown) {
  ...
  const invalidNetworkEntries = Object.entries(addressBook).find(
    ([_networkId, addressEntries]) => !isObject(addressEntries),
  );
  if (invalidNetworkEntries) {
    const [networkId, invalidEntries] = invalidNetworkEntries;
    captureException(
      new Error(
  ...
  const addressBookEntries = Object.entries(addressBook).filter(
    (entry): entry is [string, Record<PropertyKey, unknown>] =>
      isObject(entry[1]),
  );
  const isEntryMissingChainId = (addressEntry: unknown) =>
    !isObject(addressEntry) || !hasProperty(addressEntry, 'chainId');
  const networkWithInvalidEntry = addressBookEntries.find(
    ([_networkId, addressEntries]) =>
      Object.values(addressEntries).some(isEntryMissingChainId),
  );
  if (networkWithInvalidEntry) {
    const [networkId, invalidEntries] = networkWithInvalidEntry;
    const invalidEntry = Object.values(invalidEntries).find(
      isEntryMissingChainId,
    );
    captureException(
      new Error(
  ...
  const migratedAddressBook: Record<string, Record<PropertyKey, unknown>> = {};
  const ambiguousAddressEntries: Record<string, string[]> = {};
  for (const [networkId, addressEntries] of addressBookEntries) {
    if (ambiguousNetworksById[networkId]) {
      const chainIdCandidates = ambiguousNetworksById[networkId].chainIds;
