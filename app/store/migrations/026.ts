...
interface PhishingControllerLegacyState {
  listState?: unknown;
...
interface Migration026State {
  engine: {
    backgroundState: {
      ...
      PhishingController?: PhishingControllerLegacyState;
    };
  };
}
...
export default function migrate(incomingState: unknown) {
  const state = incomingState as Migration026State;
  ...
  if (phishingControllerState?.listState) {
    delete (
      state.engine.backgroundState
        .PhishingController as PhishingControllerLegacyState
    ).listState;
  } else {
    captureException(
      new Error(
