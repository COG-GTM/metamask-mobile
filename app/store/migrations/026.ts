...
export default function migrate(state: unknown) {
  ...
  const phishingControllerState =
    migrationState.engine.backgroundState.PhishingController;
  if (phishingControllerState?.listState) {
    delete migrationState.engine.backgroundState.PhishingController.listState;
  } else {
    captureException(
      new Error(
  ...
  if (
    ...
    migrationState.engine.backgroundState.PhishingController.hotlistLastFetched = 0;
    migrationState.engine.backgroundState.PhishingController.stalelistLastFetched = 0;
  } else {
    captureException(
      new Error(
