...
export default function migrate(state: unknown): unknown {
  if (!isObject(state)) {
    captureException(
      new Error(`Migration 009: Invalid root state: '${typeof state}'`),
    );
    return state;
  }
  ...
  if (
    ...
    captureException(
      new Error(`Migration 009: Invalid engine or backgroundState`),
    );
    return state;
  }
  ...
  if (!isObject(backgroundState.PreferencesController)) {
    captureException(
      new Error(`Migration 009: Invalid PreferencesController state`),
    );
    return state;
  }
