import Clipboard from '@react-native-clipboard/clipboard';
import Device from '../util/device';

const EXPIRE_TIME_MS = 60000;

// iOS-only methods that are not present in the published Clipboard typings.
interface ClipboardWithExpire {
  setStringExpire(content: string): Promise<void>;
  clearString(): Promise<void>;
}

const ClipboardWithExpire = Clipboard as unknown as typeof Clipboard &
  ClipboardWithExpire;

interface ClipboardManagerType {
  getString(): Promise<string>;
  setString(string: string | null): Promise<void>;
  expireTime: ReturnType<typeof setTimeout> | null;
  setStringExpire(string: string): Promise<void>;
}

const ClipboardManager: ClipboardManagerType = {
  async getString() {
    return await Clipboard.getString();
  },
  async setString(string) {
    await Clipboard.setString(string as string);
  },
  expireTime: null,
  async setStringExpire(string) {
    if (Device.isIos()) {
      await ClipboardWithExpire.setStringExpire(string);
    } else {
      await this.setString(string);
      if (this.expireTime) {
        clearTimeout(this.expireTime);
      }
      this.expireTime = setTimeout(async () => {
        const currentString = await this.getString();

        if (!currentString) return;

        await ClipboardWithExpire.clearString();
      }, EXPIRE_TIME_MS);
    }
  },
};

export default ClipboardManager;
