...
/**
 * A transaction being watched for notification purposes.
 */
...
type ShowTransactionNotificationFn = (opts: {
  autodismiss?: number;
  transaction?: NotificationTransactionData;
  status?: string;
}) => void;
...
type ShowSimpleNotificationFn = (opts: {
  id: number;
  autodismiss?: number;
  title?: string;
  description?: string;
...
type RemoveNotificationByIdFn = (id: string | number) => void;
...
type TransactionEventHandler = (transactionMeta: TransactionMeta) => void;
...
class NotificationManager {
  ...
  _transactionConfirmedListener?: TransactionEventHandler;

  _transactionSpeedupListener?: TransactionEventHandler;
   ...
   */
  showSimpleNotification = (data: SimpleNotificationInput) => {
    const id = Date.now();
  ...
  watchSubmittedTransaction(
    ...
    this._transactionFailedListener =
      Engine.controllerMessenger.subscribeOnceIf(
        'TransactionController:transactionFailed',
        (failedMeta) => {
          this._failedCallback(failedMeta as unknown as TransactionMeta);
        },
        (failedMeta) =>
          (failedMeta as unknown as TransactionMeta).id === transaction.id,
      );
