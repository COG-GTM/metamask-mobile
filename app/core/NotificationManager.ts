...
class NotificationManager {
  ...
  _showNotification = async (data: TransactionNotification): Promise<void> => {
  ...
  watchSubmittedTransaction(
    ...
    const nonce = String(transactionMeta.txParams.nonce);
  ...
  gotIncomingTransaction = async (
    ...
    try {
      ...
      const nonce = hexToBN(
        String(filteredTransactions[0].txParams.nonce),
      ).toString();
      const amount = renderFromWei(
        hexToBN(String(filteredTransactions[0].txParams.value)),
      );
