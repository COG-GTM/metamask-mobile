...
class NotificationManager {
  ...
  constructor(
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    _navigation: any,
    _showTransactionNotification: AnyFn,
    _hideTransactionNotification: AnyFn,
    _showSimpleNotification: AnyFn,
    _removeNotificationById: AnyFn,
  ) {
  ...
  watchSubmittedTransaction(transaction: any, speedUp = false) {
    ...
    this._transactionConfirmedListener =
      Engine.controllerMessenger.subscribeOnceIf(
        'TransactionController:transactionConfirmed',
        (meta) => {
          this._confirmedCallback(meta, transaction);
        },
        (meta) => meta.id === transaction.id,
      );
    ...
    this._transactionFailedListener =
      Engine.controllerMessenger.subscribeOnceIf(
        'TransactionController:transactionFailed',
        (meta) => {
          this._failedCallback(meta);
        },
        (meta) => (meta as { id?: string }).id === transaction.id,
      );
...
export default {
  init({
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    navigation: any;
    showTransactionNotification: AnyFn;
    hideCurrentNotification: AnyFn;
    showSimpleNotification: AnyFn;
    removeNotificationById: AnyFn;
  }) {
