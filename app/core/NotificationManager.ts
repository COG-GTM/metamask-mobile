...
/**
 * Id of a transaction that should be displayed when interacting with a notification
 */
type TransactionToViewId = string | number;

interface NotificationTransaction {
  id?: TransactionToViewId;
  nonce?: string;
  amount?: string;
  assetType?: string;
}
...
type ShowTransactionNotification = (notification: {
  autodismiss?: number;
  transaction?: NotificationTransaction;
  status?: string;
}) => void;
...
type ShowSimpleNotification = (notification: {
  id: number;
  autodismiss?: number;
  title?: string;
  description?: string;
...
type RemoveNotificationById = (id?: TransactionToViewId) => void;

/**
 * Transaction being watched, as received from the callers of the manager
 */
...
interface TransactionFailedEventPayload {
  actionId?: string;
  error: string;
  ...
  switch (notification.type) {
    ...
    case NotificationTransactionTypes.received:
      title = strings('notifications.received_title', {
        amount: (notification.transaction as NotificationTransaction).amount,
        assetType: (notification.transaction as NotificationTransaction)
          .assetType,
      });
...
class NotificationManager {
   ...
   */
  _transactionToView!: TransactionToViewId[];
  /**
   ...
   */
  _transactionsWatchTable: Record<string, TransactionToViewId[]> = {};

  _transactionFailedListener?: (payload: TransactionFailedEventPayload) => void;
  ...
  _viewTransaction = (id: TransactionToViewId) => {
    this._transactionToView.push(id);
    this.goTo('TransactionsHome');
  };

  _removeListeners = (_id?: TransactionToViewId) => {
  ...
  _showNotification = async (data: NotificationData) => {
    if (this._backgroundMode) {
      ...
      const pushData: {
        ...
        tag?: string;
        userInfo?: { action: string; id: TransactionToViewId };
      } = {
   ...
   */
  setTransactionToView = (id: TransactionToViewId) => {
    this._transactionToView.push(id);
  };
  ...
  watchSubmittedTransaction(transaction: WatchedTransaction, speedUp = false) {
    ...
    const smartTransactionListener = async (
      ...
      } as unknown as Parameters<typeof TransactionController.getTransactions>[0]);
