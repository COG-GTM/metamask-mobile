...
import {
  ...
  unrestrictedMethods,
  type CaveatSpecificationOptions,
} from './specifications';

// The validators under test only read `value`
const caveatWithValue = (value: unknown) =>
  ({ value } as Caveat<string, Json>);
      ...
      describe('validator', () => {
        it('rejects invalid array values', () => {
          ...
          [null, 'foo', {}, []].forEach((invalidValue) => {
            expect(() => validator(caveatWithValue(invalidValue))).toThrow(
        ...
        it('rejects addresses that have no corresponding identity', () => {
          ...
          expect(() => validator(caveatWithValue(caveatValues))).toThrow(
            `endowment:caip25 error: Received invalid value for caveat of type "${Caip25CaveatType}".`,
          );
        });
      });
