...
type CaveatSpecificationsOptions = Parameters<
  typeof getCaveatSpecifications
>[0];
type Caip25Caveat = Parameters<
  ReturnType<
    typeof getCaveatSpecifications
      ...
      describe('validator', () => {
        it('rejects invalid array values', () => {
          ...
          [null, 'foo', {}, []].forEach((invalidValue) => {
            expect(() =>
              validator({ value: invalidValue } as unknown as Caip25Caveat),
            ).toThrow(
        ...
        it('rejects addresses that have no corresponding identity', () => {
          ...
          expect(() =>
            validator({ value: caveatValues } as unknown as Caip25Caveat),
          ).toThrow(
            `endowment:caip25 error: Received invalid value for caveat of type "${Caip25CaveatType}".`,
          );
        });
