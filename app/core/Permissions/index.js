...
import { Caip25CaveatType, Caip25EndowmentPermissionName, getEthAccounts, getPermittedEthChainIds, setEthAccounts, setPermittedEthChainIds } from '@metamask/chain-agnostic-permission';
 ...
 */
export const sortAccountsByLastSelected = (accounts) => {
...
function getAccountsFromSubject(subject) {
  ...
  if (caveat) {
    ...
    const lowercasedEthAccounts = ethAccounts.map((address) =>
...
// TODO: Replace "any" with type
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function getPermittedChainIdsFromSubject(subject) {
  const caveats =
  subject.permissions?.[Caip25EndowmentPermissionName]?.caveats;
  if (!caveats) {
    return [];
 ...
 */
export const getDefaultCaip25CaveatValue = () => ({
  requiredScopes: {},
  optionalScopes: {
    'wallet:eip155': {
      accounts: []
...
export const getCaip25Caveat = (origin) => {
  let caip25Caveat;
  try {
...
export const removePermittedAccounts = (origin, accounts) => {
...
export const removeAccountsFromPermissions = async (addresses) => {
  const { PermissionController } = Engine.context;
 ...
 */
export const removePermittedChain = (hostname, chainId) => {
  ...
  const newPermittedChains = permittedChainIds.filter((chain) => chain !== chainId);
