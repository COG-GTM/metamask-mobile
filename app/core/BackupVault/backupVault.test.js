import {
  ...
  VAULT_BACKUP_KEY,
  VAULT_BACKUP_TEMP_KEY } from
'./constants';
import {
  backupVault,
  getVaultFromBackup,
  clearAllVaultBackups } from
'./backupVault';
...
import {
  ...
  setInternetCredentials } from
'react-native-keychain';

let mockKeychainState =
{};
...
jest.mock('react-native-keychain', () => ({
  ...
  getInternetCredentials: jest.fn(
    async (server) => mockKeychainState[server]
  ),
  resetInternetCredentials: jest.fn(
    async (server, _) => delete mockKeychainState[server]
  )
}));
...
describe('backupVault file', () => {
  ...
  describe('backupVault', () => {
    it('should throw error and skip primary backup if failed to backup temporary vault', async () => {
      ...
      setInternetCredentials.mockImplementationOnce(() => false);
