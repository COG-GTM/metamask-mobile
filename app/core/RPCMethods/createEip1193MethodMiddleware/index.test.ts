...
const getHandler = () => ({
  implementation: (req: { params?: unknown[] }, res: { result?: unknown }, _next: () => void, end: (err?: Error) => void, hooks: Record<string, () => unknown>) => {
...
describe('createEip1193MethodMiddleware', () => {
  ...
  it('should handle errors returned by the implementation', async () => {
    ...
    expect((response.error.data as Record<string, Record<string, string>>)?.cause?.message).toBe('test error');
  });
