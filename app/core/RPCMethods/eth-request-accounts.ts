...
> = async (
  ...
  const { origin } = req as JsonRpcRequest<JsonRpcParams> & {
    origin: string;
  };
  if (locks.has(origin)) {
  ...
  (
    trackDappViewedEvent as typeof trackDappViewedEvent &
      ((hostname: string, numberOfConnectedAccounts: number) => void)
