...
export function validateAddEthereumChainParams(params: any) {
  ...
  const extraKeys = Object.keys(params[0]).filter((key) => !(allowedKeys as any)[key]);
  if (extraKeys.length) {
    throw rpcErrors.invalidParams(
...
function validateRpcUrls(rpcUrls: string[] | null) {
...
export function findExistingNetwork(chainId: string, networkConfigurations: any) {
  const existingEntry = Object.entries(networkConfigurations).find(
    ([, networkConfiguration]) => (networkConfiguration as any).chainId === chainId,
  );
  if (existingEntry) {
    const [, networkConfiguration] = existingEntry as [string, any];
