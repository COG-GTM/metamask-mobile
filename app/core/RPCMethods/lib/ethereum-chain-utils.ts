...
/**
 * A network configuration as consumed by these helpers. Networks coming from
 * the `NetworkController` are augmented with legacy fields still set by some
 * callers.
 */
export type ChainSwitchNetworkConfiguration = NetworkConfiguration &
  Partial<{
    chainName: string;
    nickname: string;
    shortName: string;
...
export function findExistingNetwork(
  chainId: Hex,
  networkConfigurations: Record<string, ChainSwitchNetworkConfiguration>,
): [string, ChainSwitchNetworkConfiguration] | undefined {
...
/**
 * Hooks used by {@link switchToNetwork}.
 */
...
/**
 * Options for {@link switchToNetwork}.
 */
export interface SwitchToNetworkOptions {
  network: [string, ChainSwitchNetworkConfiguration];
  chainId: Hex;
  ...
  // Accepted for backwards compatibility with existing call sites; the
  // controllers and approval mode are read from `Engine.context` instead.
...
export async function switchToNetwork({
  ...
  hooks,
}: SwitchToNetworkOptions): Promise<void> {
  const {
    getCaveat,
