...
/**
 * A network configuration as used by the switch/add chain flows. It extends the
 * canonical {@link NetworkConfiguration} with the legacy, optional fields that
 * older network shapes may still carry.
 */
export type SwitchNetworkConfiguration = NetworkConfiguration &
  Partial<{
    chainName: string;
    nickname: string;
    shortName: string;
...
/**
 * The result of validating the params of a `wallet_addEthereumChain` request.
 */
...
/**
 * Options passed to the user approval callback used by the chain flows.
 */
...
/**
 * Method hooks required by {@link switchToNetwork}.
 */
...
function validateNativeCurrency(nativeCurrency: unknown): string {
  ...
  const ticker = nativeCurrencyObj?.symbol || 'ETH';
 ...
 * @param options.network - Network configuration of the chain being switched to.
 * @param options.chainId - The network client being switched to.
