...
/**
 * Network configuration enriched with legacy/optional fields that older
 * network state shapes may still carry.
 */
type ExtendedNetworkConfiguration = NetworkConfiguration & {
  chainName?: string;
  nickname?: string;
  shortName?: string;
  ticker?: string;
...
/**
 * A tuple of `[networkClientId, networkConfiguration]` describing an existing
 * network entry.
 */
export type NetworkTuple = [string, ExtendedNetworkConfiguration];
...
/**
 * Hooks used by `switchToNetwork`.
 */
export interface SwitchToNetworkHooks {
  getCaveat: (args: {
    target: string;
    caveatType: string;
  }) => CaveatResult | undefined;
...
export function validateAddEthereumChainParams(params: unknown) {
  ...
  const {
    ...
    nativeCurrency = null,
    rpcUrls,
  } = firstParam as AddEthereumChainRawParams;
  ...
  const extraKeys = Object.keys(firstParam).filter((key) => !allowedKeys[key]);
  if (extraKeys.length) {
    throw rpcErrors.invalidParams(
...
export function findExistingNetwork(
  chainId: Hex,
  networkConfigurations: Record<string, ExtendedNetworkConfiguration>,
...
export async function switchToNetwork({
  ...
  // Accepted for backwards compatibility with existing callers; unused here.
  controllers?: unknown;
  autoApprove?: unknown;
}) {
  const {
