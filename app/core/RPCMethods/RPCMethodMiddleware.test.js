...
import {
  ...
  getPermittedAccounts } from
'../Permissions';
import {
  getRpcMethodMiddleware,
  getRpcMethodMiddlewareHooks } from
'./RPCMethodMiddleware';
import {
...
'./spam';
import {
  NUMBER_OF_REJECTIONS_THRESHOLD } from

'../redux/slices/originThrottling';

import {
  Caip25CaveatType,
  Caip25EndowmentPermissionName } from
...
const mockGetPermittedAccounts = getPermittedAccounts;
const mockAddTransaction = addTransaction;

const mockProcessOriginThrottlingRejection =
processOriginThrottlingRejection;
const mockValidateOriginThrottling = validateOriginThrottling;
...
describe('getRpcMethodMiddleware', () => {
  ...
  describe('wallet_requestPermissions', () => {
    it('can requestPermissions for eth_accounts', async () => {
      const accounts = [`wallet:eip155:${addressMock}`];
      ...
      getCaip25PermissionFromLegacyPermissions.mockReturnValue(
        mockPermission
      );
  ...
  describe('eth_sendTransaction', () => {
    describe('browser', () => {
      it('sends the transaction and returns the resulting hash', async () => {
        ...
        expect(response.error).toBeUndefined();
        expect(response.result).toBe('fake-hash');
      });
  ...
  describe('personal_ecRecover', () => {
    ...
    it('returns the wrong address if the wrong data is provided', async () => {
      ...
      expect(response.result).not.toBe(
        helloWorldAddress
      );
    });
  ...
  describe('personal_sign', () => {
    async function sendRequest() {
      ...
      // TODO: Replace "any" with type
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      return await callMiddleware({ middleware, request });
    }
...
describe('getRpcMethodMiddlewareHooks', () => {
  ...
  describe('requestPermittedChainsPermissionIncrementalForOrigin', () => {
    it('should call requestPermittedChainsPermissionIncremental with correct params', () => {
      ...
      const mockRequestPermittedChainsPermissionIncremental = requestPermittedChainsPermissionIncremental;
