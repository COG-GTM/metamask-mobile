...
/**
 * The options every test passes to the handler besides the request itself.
 */
type OtherOptions = Omit<
  Parameters<typeof wallet_addEthereumChain>[0],
  'req'
> & {
        ...
        backgroundState: {
          NetworkController: {
            ...mockNetworkState(
              ...
              } as unknown as Parameters<typeof mockNetworkState>[1],
            ),
          },
        },
      },
...
describe('RPC Method - wallet_addEthereumChain', () => {
  ...
  afterEach(() => {
    jest.mocked(InteractionManager.runAfterInteractions).mockClear();
  ...
  describe('Approval Flow', () => {
    it('clears existing approval requests', async () => {
      jest.mocked(Engine.context.ApprovalController.clear).mockClear();
