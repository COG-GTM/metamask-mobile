...
import { wallet_addEthereumChain as wallet_addEthereumChainImpl } from './wallet_addEthereumChain';
import Engine from '../Engine';
import { mockNetworkState } from '../../util/test/network';
import MetaMetrics from '../Analytics/MetaMetrics';
import { flushPromises } from '../../util/test/utils';

// The production handler is strongly typed, but these tests intentionally pass
// partial mocks. Route calls through a thin wrapper that accepts loose input.
const wallet_addEthereumChain = (args: Record<string, unknown>) =>
  wallet_addEthereumChainImpl(
    args as unknown as Parameters<typeof wallet_addEthereumChainImpl>[0],
  );
...
describe('RPC Method - wallet_addEthereumChain', () => {
  ...
  it('should update the networkConfiguration that has a chainId that already exists in wallet state, and should switch to the existing network', async () => {
    const spyOnUpdateNetwork = jest
      ...
      .mockReturnValue(
        networkConfigurationResult as unknown as Promise<NetworkConfiguration>,
      );
