...
type NetworkStateOptions = Parameters<typeof mockNetworkState>[0];

// The tests intentionally exercise the handler with partial/invalid options.
const callAddEthereumChain = (options: object) =>
  wallet_addEthereumChain(options as unknown as AddEthereumChainOptions);
        ...
        backgroundState: {
          NetworkController: {
            ...mockNetworkState(
              ...
              } as unknown as NetworkStateOptions,
            ),
          },
        },
      },
...
describe('RPC Method - wallet_addEthereumChain', () => {
  ...
  beforeEach(() => {
    ...
    jest
      ...
      .mockImplementation(
        (callback) =>
          (callback as () => void)() as unknown as ReturnType<
