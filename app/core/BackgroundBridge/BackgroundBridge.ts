...
import type { RPCMethodsMiddleParameters } from '../RPCMethods/RPCMethodMiddleware';

type GenericJsonRpcMiddleware = JsonRpcMiddleware<JsonRpcParams, Json>;

interface BackgroundBridgeWcRequestActions {
  approveRequest: (params: { id: number; result: unknown }) => void;
  rejectRequest: (params: { id: number; error: unknown }) => void;
...
export interface BackgroundBridgeParams {
  ...
  ) => GenericJsonRpcMiddleware;
  isMainFrame: boolean;
  isRemoteConn?: boolean;
  sendMessage?: (msg: unknown) => void;
  isWalletConnect: boolean;
  wcRequestActions?: BackgroundBridgeWcRequestActions;
...
interface BridgePortLike {
  emit: (event: string, payload: unknown) => void;
  name?: string;
}

interface BridgeBackgroundState {
...
export class BackgroundBridge extends EventEmitter {
  ...
  isRemoteConn: boolean | undefined;
  ...
  async getProviderNetworkState(
  ...
  ): Promise<{ chainId: string; networkVersion: string }> {
    const networkClientId = Engine.controllerMessenger.call(
      'SelectedNetworkController:getNetworkClientIdForDomain',
      origin,
    );
    ...
    let networkVersion: string | null | undefined =
  ...
  async notifySelectedAddressChanged(
    ...
    if (!selectedAddress) {
      selectedAddress = this.getState().selectedAddress;
    }
    try {
  ...
  async onStateUpdate(memState?: BridgeBackgroundState): Promise<void> {
    if (!memState) {
  ...
  setupProviderEngine(): JsonRpcEngine {
    const origin: string = this.isMMSDK
      ? this.channelId ?? ''
      : this.hostname;
    ...
    // metadata
    engine.push(createOriginMiddleware({ origin }) as GenericJsonRpcMiddleware);
    engine.push(
      createSelectedNetworkMiddleware(
        Engine.controllerMessenger as unknown as Parameters<
          typeof createSelectedNetworkMiddleware
        >[0],
      ) as GenericJsonRpcMiddleware,
    );
    engine.push(createLoggerMiddleware({ origin }) as GenericJsonRpcMiddleware);
    // filter and subscription polyfills
    engine.push(filterMiddleware as GenericJsonRpcMiddleware);
    engine.push(subscriptionManager.middleware as GenericJsonRpcMiddleware);

    // Handle unsupported RPC Methods
    engine.push(
      createUnsupportedMethodMiddleware() as GenericJsonRpcMiddleware,
    );
        ...
        revokePermissionsForOrigin: (permissionKeys: string[]) => {
          try {
            PermissionController.revokePermissions({
              [origin]: permissionKeys as Parameters<
    ...
    engine.push(createTracingMiddleware() as GenericJsonRpcMiddleware);
    ...
    engine.push(createSanitizationMiddleware() as GenericJsonRpcMiddleware);

    // forward to metamask primary provider
    engine.push(
      providerAsMiddleware(proxyClient.provider) as GenericJsonRpcMiddleware,
    );
    return engine;
  }
   ...
   * @returns status
   */
  getState(): BridgeBackgroundState {
