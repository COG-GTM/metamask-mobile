...
interface BackgroundBridgeParams {
  webview?: { current?: unknown };
  url: string;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  getRpcMethodMiddleware: (args: { hostname: string; getProviderState: (origin?: string) => Promise<{ isUnlocked: boolean; chainId: string; networkVersion: string }> }) => any;
  isMainFrame: boolean;
...
export class BackgroundBridge extends EventEmitter {
  ...
  constructor({
    ...
    this.port = isRemoteConn
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ? new RemotePort(sendMessage as any)
  ...
  async getProviderState(origin?: string): Promise<{ isUnlocked: boolean; chainId: string; networkVersion: string }> {
    return {
        ...
        revokePermissionsForOrigin: (permissionKeys: any) => {
          try {
            PermissionController.revokePermissions({
              [origin as string]: permissionKeys,
            });
          } catch (e) {
