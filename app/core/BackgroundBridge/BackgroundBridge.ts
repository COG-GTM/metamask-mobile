...
import { Hex, Json, JsonRpcParams, NonEmptyArray } from '@metamask/utils';
import MobilePortStream, { MobilePort } from '../MobilePortStream';
...
type LegacyRequestedPermissions = Parameters<
  typeof getCaip25PermissionFromLegacyPermissions
>[1];

type RequestedPermissions = Parameters<
  typeof Engine.context.PermissionController.requestPermissions
>[1];
...
export interface BackgroundBridgeParams {
  ...
  getRpcMethodMiddleware: (
    params: RPCMethodsMiddleParameters,
...
/**
 * The flattened controller state exposed to the dapp provider.
 */
...
export class BackgroundBridge extends EventEmitter {
  ...
  getApprovedHosts?: (host: string) => Record<string, boolean | number>;
  channelId?: string;
  deprecatedNetworkVersions: Record<string, string | null | undefined>;
  ...
  addressSent: string;
  private readonly _webviewRef: unknown;

  constructor(params?: BackgroundBridgeParams) {
    super();
    // The parameters are required; the argument is only optional so that
    // constructing a mocked bridge without them still type checks.
    const {
      webview,
      url,
      getRpcMethodMiddleware,
      ...
      isMMSDK,
      channelId,
    } = params as BackgroundBridgeParams;
    ...
    this.hostname = new UrlParser(url).hostname;
    ...
    this.port = isRemoteConn
      ...
      : new Port(this._webviewRef, Boolean(isMainFrame));
    ...
    // The port classes extend an untyped `EventEmitter`, so their declared
    // types do not expose the emitter methods the stream listens on.
    const portStream = new MobilePortStream(
      this.port as unknown as MobilePort,
      url,
    );
    // setup multiplexing
  ...
  async getProviderNetworkState(
    ...
    if (!networkVersion) {
      ...
      networkVersion = await new Promise<string | null | undefined>(
        (resolve) => {
  ...
  async notifySelectedAddressChanged(selectedAddress?: string) {
    try {
      // Some callers do not know the selected address; the resulting error is
      // caught and logged below.
      const address = selectedAddress as string;
      let approvedAccounts: string[] = [];
      DevLogger.log(
        `notifySelectedAddressChanged: ${address} channelId=${this.channelId} wc=${this.isWalletConnect} url=${this.url}`,
      );
      ...
      if (found) {
        ...
        DevLogger.log(
          `notifySelectedAddressChanged url: ${this.url} hostname: ${this.hostname}: ${address}`,
          approvedAccounts,
        );
        this.sendNotification({
        ...
        DevLogger.log(
          `notifySelectedAddressChanged: selectedAddress ${address} not found in approvedAccounts`,
          approvedAccounts,
        );
      }
    } catch (err) {
  ...
  async onStateUpdate(memState?: BackgroundBridgeState) {
    if (!memState) {
    ...
    if (this.isWalletConnect || this.isRemoteConn) {
      if (
        ...
        this.addressSent = memState.selectedAddress as string;
   ...
   */
  setupProviderConnection(outStream: NodeJS.ReadWriteStream) {
   ...
   **/
  setupProviderEngine() {
    const origin = this.isMMSDK ? (this.channelId as string) : this.hostname;
    ...
    engine.push(
      createEip1193MethodMiddleware({
        // Permission-related
        getAccounts: (...args: [options?: { ignoreLock?: boolean }]) =>
          getPermittedAccounts(
            this.isMMSDK ? (this.channelId as string) : origin,
            ...args,
          ),
        getCaip25PermissionFromLegacyPermissionsForOrigin: (
          requestedPermissions?: LegacyRequestedPermissions,
        ) =>
    ...
    engine.push(
      Engine.context.PermissionController.createPermissionMiddleware({
        ...
        origin: this.isMMSDK ? (this.channelId as string) : origin,
      }),
    );
    ...
    engine.push(
      // The other middleware parameters are provided by the caller that built
      // this factory.
      this.createMiddleware({
        hostname: this.hostname,
        getProviderState: this.getProviderState.bind(this),
      } as RPCMethodsMiddleParameters),
    );
