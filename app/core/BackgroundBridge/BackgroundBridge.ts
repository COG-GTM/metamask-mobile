...
// eslint-disable-next-line import/no-nodejs-modules, @typescript-eslint/no-var-requires, @typescript-eslint/no-require-imports
...
type RpcMethodMiddlewareFactory = (params: {
  hostname: string;
  getProviderState: (origin?: string) => Promise<unknown>;
...
export interface BackgroundBridgeParams {
  webview?: WebviewRefLike | null;
  url: string;
  getRpcMethodMiddleware: RpcMethodMiddlewareFactory;
  isMainFrame: boolean;
  isRemoteConn?: boolean;
  sendMessage?: (msg: unknown) => void;
  isWalletConnect?: boolean;
  wcRequestActions?: unknown;
  getApprovedHosts?: (host: string) => Record<string, boolean> | string[];
...
export class BackgroundBridge extends EventEmitter {
  ...
  isMMSDK: boolean;
  isRemoteConn: boolean;
  _webviewRef: WebviewRefLike['current'] | null;
  disconnected: boolean;
  getApprovedHosts:
    | ((host: string) => Record<string, boolean> | string[])
    | undefined;
  channelId: string | undefined;
  deprecatedNetworkVersions: Record<string, string | null>;
  createMiddleware: RpcMethodMiddlewareFactory;
  port: (RemotePort | WalletConnectPort | Port) & PortLike;
  ...
  constructor({
    ...
    ) as (RemotePort | WalletConnectPort | Port) & PortLike;
    ...
    const portStream = new MobilePortStream(this.port as any, url);
    // setup multiplexing
  ...
  async getProviderNetworkState(
    ...
    const networkClientIdKey = networkClientId as string;
    let networkVersion: string | null =
      this.deprecatedNetworkVersions[networkClientIdKey] ?? null;
    if (!networkVersion) {
      const ethQuery = new EthQuery(networkClient.provider);
      networkVersion = await new Promise<string | null>((resolve) => {
        ethQuery.sendAsync<JsonRpcParams, string>(
          { method: 'net_version' },
          (...args: unknown[]) => {
            const [error, result] = args as [unknown, unknown];
            if (error) {
              console.error(error);
              resolve(null);
            } else {
      ...
      this.deprecatedNetworkVersions[networkClientIdKey] = networkVersion;
    }
  ...
  async onStateUpdate(memState?: { selectedAddress?: string }): Promise<void> {
    if (!memState) {
    ...
    if (this.isWalletConnect || this.isRemoteConn) {
      ...
      if (
        selectedAddress &&
        this.addressSent?.toLowerCase() !== selectedAddress.toLowerCase()
      ) {
        this.addressSent = selectedAddress;
        this.notifySelectedAddressChanged(selectedAddress);
      }
    }
  }
  ...
  onMessage = (msg: { name: string; data: unknown }): void => {
    (this.port as unknown as PortLike).emit('message', {
      name: msg.name,
      data: msg.data,
    });
  };
  ...
  onDisconnect = (): void => {
    ...
    const portWithName = this.port as unknown as PortLike;
    portWithName.emit('disconnect', {
      name: portWithName.name,
      data: null,
    });
  };
  ...
  setupProviderEngine(): JsonRpcEngine {
    ...
    type AnyMiddleware = JsonRpcMiddleware<JsonRpcParams, Json>;

    // metadata
    engine.push(createOriginMiddleware({ origin }) as AnyMiddleware);
    engine.push(
      createSelectedNetworkMiddleware(
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    engine.push(createLoggerMiddleware({ origin }) as AnyMiddleware);
  ...
  getState(): BackgroundBridgeState {
    ...
    } = Engine.datamodel.state as unknown as {
      PreferencesController: { selectedAddress: string };
    };
    return {
      isInitialized: !!vault,
      isUnlocked: true,
      network: legacyNetworkId() as string,
      selectedAddress,
    };
  }
}
