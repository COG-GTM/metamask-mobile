/* eslint-disable import/no-commonjs, @typescript-eslint/no-shadow */
...
export interface BackgroundBridgeOptions {
  ...
  url?: string;
  getRpcMethodMiddleware?: AnyFunction;
  isMainFrame?: boolean;
...
export class BackgroundBridge extends EventEmitter {
  ...
  constructor({
    ...
    isMMSDK,
    channelId,
  }: BackgroundBridgeOptions = {} as BackgroundBridgeOptions) {
    super();
    this.url = url ?? '';
    ...
    this.port = isRemoteConn
      ? new RemotePort(sendMessage as unknown as () => void)
    ...
    if (!networkVersion) {
      ...
      networkVersion = await new Promise<string | null>((resolve) => {
        ethQuery.sendAsync(
          ...
          }) as Parameters<typeof ethQuery.sendAsync>[1],
        );
      });
  ...
  async notifyChainChanged(params?: unknown): Promise<void> {
  ...
  async onStateUpdate(memState?: MemState): Promise<void> {
    if (!memState) {
  ...
  setupProviderEngine(): JsonRpcEngine {
    ...
    // metadata
    engine.push(
      createOriginMiddleware({ origin }) as unknown as Parameters<
        typeof engine.push
      >[0],
    );
    engine.push(
    ...
    engine.push(
      createLoggerMiddleware({ origin }) as unknown as Parameters<
        typeof engine.push
      >[0],
    );
    // filter and subscription polyfills
    engine.push(
      filterMiddleware as unknown as Parameters<typeof engine.push>[0],
    );
    engine.push(
      subscriptionManager.middleware as unknown as Parameters<
        typeof engine.push
      >[0],
    );
    ...
    engine.push(
      createEip1193MethodMiddleware({
        ...
        getCaip25PermissionFromLegacyPermissionsForOrigin: ((
          ...
          typeof createEip1193MethodMiddleware
        >[0]['getCaip25PermissionFromLegacyPermissionsForOrigin'],
