...
export const SecureKeychainTypes = {
...
type SecureKeychainType =
  (typeof SecureKeychainTypes)[keyof typeof SecureKeychainTypes];
...
class SecureKeychain {
  ...
  encryptPassword(password: string): Promise<string> {
    ...
    return encryptor.encrypt(data.code, { password });
  }
  ...
  decryptPassword(str: string): Promise<unknown> {
    ...
    return encryptor.decrypt(data.code, str);
  }
}
...
interface SecureKeychainModule {
  ...
  getSupportedBiometryType(): ReturnType<typeof Keychain.getSupportedBiometryType>;
  resetGenericPassword(): Promise<boolean>;
  getGenericPassword(): Promise<UserCredentials | null>;
  setGenericPassword(password: string, type?: SecureKeychainType): Promise<void>;
  ...
  TYPES: typeof SecureKeychainTypes;
}
...
const SecureKeychainModule: SecureKeychainModule = {
  ...
  async getGenericPassword(): Promise<UserCredentials | null> {
    if (instance) {
      try {
  ...
  async setGenericPassword(
    ...
    if (type === SecureKeychainTypes.BIOMETRICS) {
    ...
    } else if (type === SecureKeychainTypes.PASSCODE) {
    ...
    } else if (type === SecureKeychainTypes.REMEMBER_ME) {
      await metrics.addTraitsToUser({
        [UserProfileProperty.AUTHENTICATION_TYPE]:
      ...
      if (Platform.OS === 'ios') {
        try {
          ...
          if ((error as Error).message === 'User canceled the operation.') {
            ...
            const encryptedPwd = await instance.encryptPassword(password);
