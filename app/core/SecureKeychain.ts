...
export default {
  init(salt: string): SecureKeychain {
    ...
    // @ts-expect-error Original code uses function reference as truthy check
  ...
  async setGenericPassword(
  ...
  ): Promise<false | Keychain.Result | void> {
    const authOptions: {
    ...
    if (type === this.TYPES.BIOMETRICS) {
      ...
      return (await this.resetGenericPassword()) as false | void;
    }
