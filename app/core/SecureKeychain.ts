...
interface SecureKeychainModule {
  ...
  setGenericPassword(password: string, type: SecureKeychainType): Promise<void | boolean>;
...
const SecureKeychainModule: SecureKeychainModule = {
    ...
    if (instance) {
      try {
        ...
        if (keychainObject && keychainObject.password) {
          ...
          (keychainObject as Keychain.UserCredentials).password = decrypted.password;
  ...
  async setGenericPassword(password: string, type: SecureKeychainType): Promise<void | boolean> {
    const authOptions: {
