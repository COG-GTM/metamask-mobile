...
class SecureKeychain {
  ...
  encryptPassword(password: string): Promise<string> {
    return encryptor.encrypt(privates.get(this)?.code as string, { password });
  }

  decryptPassword(str: string): Promise<unknown> {
    return encryptor.decrypt(privates.get(this)?.code as string, str);
  }
}
let instance: SecureKeychain | undefined;

interface KeychainTypes {
...
interface SecureKeychainModule {
  init(salt: string): SecureKeychain | undefined;
    ...
    if (type === this.TYPES.BIOMETRICS) {
      ...
      if (Platform.OS === 'ios') {
        try {
          ...
          if (
            (error as Error).message === 'User canceled the operation.'
          ) {
