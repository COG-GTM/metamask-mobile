...
interface SecureKeychainDefaultOptions extends Keychain.Options {
  service: string;
...
interface SecureKeychainAuthOptions {
  accessible: Keychain.ACCESSIBLE;
  accessControl?: Keychain.ACCESS_CONTROL;
}

type SecureKeychainType = 'BIOMETRICS' | 'PASSCODE' | 'REMEMBER_ME';
...
const defaultOptions: SecureKeychainDefaultOptions = {
...
class SecureKeychain {
  ...
  encryptPassword(password: string): Promise<string> {
    ...
    return encryptor.encrypt(state?.code ?? '', { password });
  }

  decryptPassword(str: string): Promise<{ password: string }> {
    const state = privates.get(this);
    return encryptor.decrypt(state?.code ?? '', str) as Promise<{
      password: string;
    }>;
  }
}
...
const SecureKeychainModule: SecureKeychainModule = {
  init(salt: string): SecureKeychain {
    ...
    if (
      Device.isAndroid() &&
      (Keychain.SECURITY_LEVEL as unknown as { SECURE_HARDWARE?: unknown })
        ?.SECURE_HARDWARE
    )
  ...
  async setGenericPassword(
    ...
    const authOptions: SecureKeychainAuthOptions = {
      ...
      if (Platform.OS === 'ios') {
        try {
          ...
          if ((error as Error).message === 'User canceled the operation.') {
            ...
            const encryptedPasswordRetry = await instance.encryptPassword(
              password,
            );
