...
type SecureKeychainAuthType = 'BIOMETRICS' | 'PASSCODE' | 'REMEMBER_ME';

const privates = new WeakMap<SecureKeychainInner, SecureKeychainPrivates>();
 ...
 */
class SecureKeychainInner {
  static instance: SecureKeychainInner | undefined;
  isAuthenticating = false;

  constructor(code: string) {
    if (!SecureKeychainInner.instance) {
      privates.set(this, { code });
      SecureKeychainInner.instance = this;
    }

    return SecureKeychainInner.instance;
  }
...
let instance: SecureKeychainInner | undefined;

const SecureKeychainManager = {
  init(salt: string): SecureKeychainInner {
    instance = new SecureKeychainInner(salt);
  ...
  getInstance(): SecureKeychainInner | undefined {
    return instance;
  },
  ...
  TYPES: {
    ...
    REMEMBER_ME: 'REMEMBER_ME' as const,
  },
};

export default SecureKeychainManager;
