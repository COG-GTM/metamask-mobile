...
/**
 * The decrypted payload stored in the keychain.
 */
...
export default {
  init(salt: string) {
    ...
    if (Keychain.SECURITY_LEVEL?.SECURE_HARDWARE)
  ...
  async setGenericPassword(password: string, type?: SecureKeychainAuthType) {
      ...
      if (Platform.OS === 'ios') {
        try {
          ...
          if ((error as Error).message === 'User canceled the operation.') {
            ...
            const reEncryptedPassword = await instance?.encryptPassword(
              password,
            );
