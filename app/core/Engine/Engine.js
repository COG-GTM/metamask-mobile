...
import {
  ...
  TokenSearchDiscoveryDataController } from
...
import {
  ...
  buildSnapRestrictedMethodSpecifications } from
...
import {
  LedgerKeyring,
  LedgerMobileBridge,
  LedgerTransportMiddleware } from
'@metamask/eth-ledger-bridge-keyring';
import { Encryptor, LEGACY_DERIVATION_OPTIONS, pbkdf2 } from '../Encryptor';
import { getDecimalChainId, isTestNet } from '../../util/networks';
import {
  fetchEstimatedMultiLayerL1Fee,
  deprecatedGetNetworkId } from
'../../util/networks/engineNetworkUtils';
...
import {
  ...
  unrestrictedMethods } from
'../Permissions/specifications.js';
...
import {
  networkIdUpdated,
  networkIdWillUpdate } from
'../../core/redux/slices/inpageProvider';
...
import {
  ...
  SnapControllerUpdateSnapStateAction } from
'./controllers/snaps';
...
import {
  ...
  swapsSupportedChainIds } from
'./constants';
import {
  getGlobalChainId,
  getGlobalNetworkClientId } from
'../../util/networks/global-network';
...
export class Engine {
  ...
  metaMetricsId)
  {
    ...
    const snapRestrictedMethods = {
      ...
      handleSnapRpcRequest: async (args) =>
    ...
    const keyringSnapMethods = {
      getAllowedKeyringMethods: (origin) =>
...

  {
    ...
    if (selectedInternalAccount) {
      ...
      if (tokens.length > 0) {
        ...
        const tokenBalances =
        allTokenBalances?.[selectedInternalAccount.address]?.[
        chainId] ??
        {};
        tokens.forEach(
          (item) => {
            const exchangeRate =
            tokenExchangeRates?.[item.address]?.price;
            ...
            tokenExchangeRates?.[item.address]?.pricePercentChange1d;
  ...
  id,
  reason = providerErrors.userRejectedRequest(),
  opts = {})
  {
...
export default {
  ...
  getTotalEvmFiatAccountBalance(account) {
    assertEngineExists(instance);
