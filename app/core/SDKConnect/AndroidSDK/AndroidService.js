...
export default class AndroidService extends EventEmitter2 {
  communicationClient = NativeModules.CommunicationClient;
  connections = {};
  rpcQueueManager = new RPCQueueManager();
  bridgeByClientId = {};

  batchRPCManager = new BatchRPCManager('android');
  ...
  setupOnClientsConnectedListener() {
    this.eventHandler.onClientsConnected(async (sClientInfo) => {
      const clientInfo = JSON.parse(sClientInfo);
  ...
  setupOnMessageReceivedListener() {
    this.eventHandler.onMessageReceived((jsonMessage) => {
      const handleEventAsync = async () => {
        let parsedMsg;
  ...
  async removeConnection(channelId) {
    try {
