...
jest.mock('./wait.util', () => {
  ...
  return {
    ...originalModule,
    wait: (_ms) => new Promise((resolve) => setTimeout(resolve, 10)),
...
describe('wait.util', () => {
  ...
  test('waitForConnectionReadiness resolves when connection is ready', async () => {
    const connection = { isReady: false };
  ...
  test('waitForAndroidServiceBinding resolves when Android service is bound', async () => {
    ...
    SDKConnect.getInstance.mockReturnValue(mockSDKConnect);
