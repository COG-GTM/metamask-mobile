...
import {
  ...
  handleClientsReady,
  handleReceivedMessage } from
'./EventListenersHandlers';
...
export class Connection extends EventEmitter2 {
  ...
  constructor({
    ...
    DevLogger.log(
      ...
      this.channelId} typeof(protocolVersion)=${
      typeof protocolVersion}  protocolVersion=${protocolVersion} relayPersistence=${relayPersistence} initialConnection=${
      this.initialConnection} lastAuthorized=${
      this.lastAuthorized} trigger=${this.trigger}`,
      socketServerUrl,
      originatorInfo
    );
    ...
    this.remote = new RemoteCommunication({
      platformType: AppConstants.MM_SDK.PLATFORM,
      relayPersistence,
