...
export default class DeeplinkProtocolService {
  ...
  setupBridge(clientInfo) {
    DevLogger.log(
      `DeeplinkProtocolService::setupBridge for id=${
      clientInfo.clientId} exists=${
      !!this.bridgeByClientId[
      clientInfo.clientId]}} originatorInfo=${
  ...
  // TODO: Replace "any" with type
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  async sendMessage(message, forceRedirect) {
  ...
  async handleConnectionEventAsync({
    clientInfo,
    params
  ...
  async processDappRpcRequest(params)
...

  {
    ...
    const requestObject = JSON.parse(params.request);
...

  {
    ...
    const handleEventAsync = async () => {
      ...
      const sessionId = params.channelId;
      ...
      try {
        ...
        const checkForRpcMethodToSkip = () => {
          ...
          if (isBatchRequest) {
            const batchRpcMethods = data.params.map(
              (rpc) => rpc.method
            );
        ...
        if (isAccountChanged || !isRpcMethodToSkip && isChainChanged) {
          ...
          isAccountChanged ? 'account' : 'chain'} has changed. Please try again.`;
