...
class WalletConnect2Session {
  ...
  constructor({
    ...
    backgroundBridgeFactory = {
      ...
      create: (options) => new BackgroundBridge(options)
    }
  ...
  redirect = (context) => {
    DevLogger.log(
      `WC2::redirect context=${context} isDeeplink=${this.deeplink} navigation=${
    ...
    setTimeout(() => {
      if (Device.isIos() && parseInt(Platform.Version) >= 17) {
  ...
  async handleChainChange(chainIdDecimal) {
