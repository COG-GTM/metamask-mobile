...
interface WCPayload {
  id: number;
  method: string;
  params: WCPayloadParam[];
}

interface WCConnector {
  connected: boolean;
  session: WCSession;
  on(
    event: string,
    handler: (error: Error | null, payload: WCPayload) => void,
  ): void;
  ...
  approveSession(args: { chainId: number; accounts: string[] }): Promise<void>;
  rejectSession(): void;
  killSession(): void;
}
...
class WalletConnect {
  ...
  constructor(options: WalletConnectOptions, existing?: boolean) {
    ...
    this.walletConnector = new RNWalletConnect({
      ...
      typeof RNWalletConnect
    >[0]) as unknown as WCConnector;
          ...
          if (payload.method === 'eth_sendTransaction') {
            try {
              ...
              const trx = await addTransaction(
                payload.params[0] as unknown as TransactionParams,
                {
  ...
  startSession = async (sessionData: WCSession, existing?: boolean) => {
    const chainId = selectEvmChainId(store.getState());
    const selectedAddress =
      Engine.context.AccountsController.getSelectedAccount().address?.toLowerCase();
    const approveData = {
      chainId: parseInt(chainId, 10),
      accounts: [selectedAddress as string],
    };
    if (existing) {
  ...
  killSession = () => {
    ...
    this.walletConnector = null as unknown as WCConnector;
  };

  sessionRequest = async (peerInfo: WCSession) => {
...
const instance = {
  ...
  async newSession(
    ...
    const data: WalletConnectOptions = { uri, session: {} as WCSession };
    if (redirectUrl) {
