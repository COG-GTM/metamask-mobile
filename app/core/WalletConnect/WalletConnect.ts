...
class WalletConnect {
  ...
  constructor(options: WalletConnectOptions, existing?: boolean) {
    ...
    this.walletConnector?.on('call_request', async (error, payload) => {
      ...
      if (payload.method) {
        ...
        const payloadHostname = new URLParse(payloadUrl as string).hostname;
  ...
  startSession = async (
    ...
    this.backgroundBridge = new BackgroundBridge({
      ...
      getRpcMethodMiddleware: ({ hostname: _hostname, getProviderState }) =>
  ...
  sessionRequest = async (peerInfo: WalletConnectSession) => {
    ...
    try {
      const { host } = new URLParse(peerInfo.peerMeta?.url as string);
