...
interface WalletConnectConstructorOptions {
...
class WalletConnect {
  ...
  walletConnector: WalletConnector | null = null;

  constructor(options: WalletConnectConstructorOptions, existing?: boolean) {
        ...
        if (payloadHostname === this.backgroundBridge?.hostname) {
          ...
          if (payload.method === 'eth_sendTransaction') {
            try {
              ...
              checkActiveAccountAndChainId(accountChainCheck);
  ...
  rejectRequest = ({ id, error }: { id: number; error: unknown }) => {
    this.walletConnector?.rejectRequest({
      id,
      error,
    } as Parameters<WalletConnector['rejectRequest']>[0]);
    this.needsRedirect(id);
  };
...
const instance = {
  async init() {
    ...
    if (sessionData) {
      const sessions: WalletConnectSession[] = JSON.parse(sessionData);
  ...
  async newSession(
    ...
    const data: { uri: string; session: WalletConnectSession } = {
      uri,
      session: {},
    };
    if (redirectUrl) {
