...
type WalletConnectSession = RNWalletConnect['session'];
type WalletConnectPeerMeta = NonNullable<WalletConnectSession['peerMeta']> & {
  dappScheme?: string;
};
type WalletConnectRpcError = NonNullable<
  Parameters<RNWalletConnect['rejectRequest']>[0]['error']
>;

interface WalletConnectSessionData
  extends Partial<Omit<WalletConnectSession, 'peerMeta'>> {
...
class WalletConnect {
  ...
  constructor(options: WalletConnectOptions, existing?: boolean) {
    ...
    this.walletConnector = new RNWalletConnect({
      ...options,
      ...CLIENT_OPTIONS,
    } as ConstructorParameters<typeof RNWalletConnect>[0]);
    /**
          ...
          if (payload.method === 'eth_sendTransaction') {
            try {
              ...
              const reqObject = {
                ...
                origin: this.url.current as string,
                params: [
                  {
              ...
              this.rejectRequest({
                id: payload.id,
                error: err as WalletConnectRpcError,
              });
            }
            return;
          }
  ...
  startSession = async (
    ...
    this.backgroundBridge = new BackgroundBridge({
      ...
      isMainFrame: true,
    } as ConstructorParameters<typeof BackgroundBridge>[0]);
  };
