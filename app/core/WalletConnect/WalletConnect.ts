...
export interface WalletConnectOptions {
  uri?: string;
  session: Partial<WalletConnectSessionData>;
}

type WalletConnectRequestParam = TransactionParams & {
  chainId?: Hex;
  peerMeta?: WalletConnectPeerMeta;
};
...
interface WalletConnectRequestPayload {
  id: number;
  method: string;
  params: WalletConnectRequestParam[];
}

type RejectRequestParams = Parameters<RNWalletConnect['rejectRequest']>[0];
...
class WalletConnect {
  ...
  url: { current: string | null | undefined } = { current: null };
  ...
  constructor(options: WalletConnectOptions, existing?: boolean) {
    ...
    this.walletConnector.on(
      ...
      async (error: Error | null, payload: WalletConnectRequestPayload) => {
        ...
        if (payload.method) {
          const payloadUrl = (
            this.walletConnector.session.peerMeta as WalletConnectPeerMeta
          ).url;
          const payloadHostname = new UrlParser(payloadUrl).hostname;
          if (
            payloadHostname ===
            ...
            if (payload.method === 'eth_sendTransaction') {
              try {
                ...
                checkActiveAccountAndChainId({
                  ...
                  // The legacy account check is typed for numeric chain ids,
                  // while dapps send whatever their transaction contained.
                ...
                ppomUtil.validateRequest(
                  reqObject as unknown as PPOMRequest,
                  id,
                );
            ...
            (this.backgroundBridge as BackgroundBridge).onMessage({
              ...
              data: payload,
              origin: this.hostname as string,
            });
          }
        }
    ...
    this.walletConnector.on(
      'session_update',
      (error: Error | null, payload: WalletConnectRequestPayload) => {
    ...
    if (existing) {
      this.startSession(options.session as WalletConnectSessionData, existing);
    }
  }
  ...
  rejectRequest = ({ id, error }: { id: number; error: unknown }) => {
    this.walletConnector.rejectRequest({
      id,
      error: error as RejectRequestParams['error'],
    });
    this.needsRedirect(id);
  };
  ...
  startSession = async (
    ...
    this.hostname = new UrlParser(this.url.current as string).hostname;
    ...
    this.backgroundBridge = new BackgroundBridge({
      ...
      // The legacy WalletConnect v1 integration only supplies a subset of the
      ...
      getRpcMethodMiddleware: ({ getProviderState }) =>
        getRpcMethodMiddleware({
          ...
          tabId: false,
          isWalletConnect: true,
        } as unknown as RPCMethodsMiddleParameters),
      isMainFrame: true,
    });
  };
  ...
  killSession = () => {
    ...
    // The connector is discarded once its session has been killed.
  ...
  sessionRequest = async (peerInfo: WalletConnectSessionData) => {
    ...
    try {
      const { host } = new UrlParser(peerInfo.peerMeta.url);
...
const instance = {
  async init() {
    ...
    if (sessionData) {
      ...
      sessions.forEach((session: WalletConnectSessionData) => {
