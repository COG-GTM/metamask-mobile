...
interface WalletConnectOptions {
  uri?: string;
  session: Partial<WalletConnectSession>;
}
...
class WalletConnect {
  ...
  sessionRequest = async (peerInfo: WalletConnectSession): Promise<unknown> => {
...
const instance = {
  async init(): Promise<void> {
    ...
    if (sessionData) {
      ...
      sessions.forEach((session: WalletConnectSession & { lastTimeConnected?: string | Date }) => {
