/* eslint-disable @typescript-eslint/no-shadow, jsdoc/check-indentation */
...
class WalletConnect {
  ...
  constructor(options: WCOptions, existing?: boolean) {
     ...
     */
    this.walletConnector.on('session_request', async (error: Error | null, payload: { params: WCSession[] }) => {
      ...
      try {
        ...
        await this.sessionRequest(sessionData as WCSession);

        await this.startSession(sessionData as WCSession, existing);
     ...
     */
    this.walletConnector.on('call_request', async (error: Error | null, payload: WCPayload) => {
        ...
        if (payloadHostname === this.backgroundBridge?.hostname) {
          ...
          if (payload.method === 'eth_sendTransaction') {
            try {
              ...
              ppomUtil.validateRequest(reqObject as Parameters<typeof ppomUtil.validateRequest>[0], id);
          ...
          this.backgroundBridge?.onMessage({
            ...
            data: payload,
          } as Parameters<BackgroundBridge['onMessage']>[0]);
        }
      }
    ...
    this.walletConnector.on('session_update', (error: Error | null, payload: unknown) => {
  ...
  rejectRequest = ({ id, error }: { id: number; error: unknown }): void => {
    this.walletConnector?.rejectRequest({
      id,
      error: error as Parameters<RNWalletConnect['rejectRequest']>[0]['error'],
    });
    this.needsRedirect(id);
  };
  ...
  startSession = async (sessionData: WCSession, existing?: boolean): Promise<void> => {
    ...
    this.backgroundBridge = new BackgroundBridge({
      ...
      getRpcMethodMiddleware: ({ hostname: _hostname, getProviderState }: { hostname: string; getProviderState: Parameters<typeof getRpcMethodMiddleware>[0]['getProviderState'] }) =>
  ...
  sessionRequest = async (peerInfo: WCSession): Promise<unknown> => {
    ...
    try {
      ...
      return await ApprovalController.add({
        id: random(),
        origin: host,
        requestData: peerInfo as Parameters<typeof ApprovalController.add>[0]['requestData'],
...
const instance = {
  async init(): Promise<void> {
    ...
    if (sessionData) {
      const sessions: WCSession[] = JSON.parse(sessionData);
  ...
  async newSession(uri: string, redirectUrl: string | undefined, autosign: boolean, requestOriginatedFrom?: string): Promise<void> {
    ...
    const data: WCOptions = { uri, session: {} };
    if (redirectUrl) {
  ...
  getSessions: async (): Promise<WCSession[]> => {
