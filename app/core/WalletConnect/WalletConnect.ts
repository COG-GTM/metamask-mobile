...
const METHODS_TO_REDIRECT: Record<string, true> = {
...
class WalletConnect {
  ...
  constructor(options: WalletConnectOptions, existing?: boolean) {
    ...
    this.walletConnector.on(
      'session_request',
      async (error: Error | null, payload: { params: WalletConnectSession[] }) => {
      ...
      async (
        ...
        if (payload.method) {
          ...
          if (
            ...
            payloadHostname === this.backgroundBridge.hostname
          ) {
            ...
            if (payload.method === 'eth_sendTransaction') {
              try {
                ...
                checkActiveAccountAndChainId({
                  ...
                  typeof checkActiveAccountAndChainId
                >[0]);
  ...
  redirect = (): void => {
    ...
    setTimeout(() => {
      if (this.dappScheme.current || this.redirectUrl) {
        const target = this.dappScheme.current
  ...
  rejectRequest = ({ id, error }: { id: number; error: unknown }): void => {
    this.walletConnector.rejectRequest({
      id,
      error: error as Parameters<
        typeof this.walletConnector.rejectRequest
      >[0]['error'],
    });
    this.needsRedirect(id);
  };
...
interface WalletConnectInstanceInterface {
  init(): Promise<void>;
...
const instance: WalletConnectInstanceInterface = {
