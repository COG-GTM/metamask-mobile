...
/**
 * Metadata of the dapp peer connected through WalletConnect.
 */
interface PeerMeta {
  url: string;
  name?: string;
...
/**
 * A persisted or in-flight WalletConnect session.
 */
...
class WalletConnect {
  ...
  constructor(options: WalletConnectOptions, existing?: boolean) {
    ...
    this.walletConnector.on(
      ...
      async (
        error: Error | null,
        payload: { params: WalletConnectSession[] },
      ) => {
    ...
    this.backgroundBridge = new BackgroundBridge({
      ...
      getRpcMethodMiddleware: ({
        ...
        getRpcMethodMiddleware({
          ...
          tabId: false,
          isWalletConnect: true,
          // The original implementation passes browser-specific values that do
          // not match the middleware parameter types.
