...
describe('WalletConnect2Session', () => {
  ...
  it('initializes correctly in constructor', () => {
    ...
    expect(session.topicByRequestId).toEqual({
      '1': mockSession.topic
    });
  });
  ...
  it('sets deeplink correctly', () => {
    ...
    expect(session.deeplink).toBe(false);
  });
  ...
  it('updates session', async () => {
    ...
    session.updateSession = jest.fn().mockImplementation(async (_) => {
  ...
  it('subscribes to chain changes', async () => {
    ...
    selectEvmChainId.mockReturnValue('0x1');
    ...
    // Change the chain ID
    selectEvmChainId.mockReturnValue('0x2');
  ...
  it('does not trigger handleChainChange when handler is already running', async () => {
    ...
    session.isHandlingChainChange = true;
  ...
  it('logs warning on handleChainChange error', async () => {
    ...
    jest.
    spyOn(session, 'handleChainChange').
  ...
  describe('redirect', () => {
    ...
    describe('iOS specific behavior', () => {
      ...
      it('shows "return to dapp" modal if opening peerLink fails', async () => {
        ...
        Linking.openURL.mockReturnValue(
  ...
  describe('needsRedirect', () => {
    ...
    it('calls redirect when requestId exists in requestsToRedirect', () => {
      ...
      expect(session.requestsToRedirect[requestId]).toBeUndefined();
    });
    ...
    it('handles multiple requests correctly', () => {
      ...
      expect(session.requestsToRedirect['123']).toBeUndefined();

      // Other requests should still be there
      expect(session.requestsToRedirect['456']).toBe(true);
      expect(session.requestsToRedirect['789']).toBe(true);
      ...
      expect(session.requestsToRedirect['456']).toBeUndefined();
  ...
  it('handles wallet_switchEthereumChain correctly', async () => {
    ...
    session.topicByRequestId[switchChainRequest.id] =
    switchChainRequest.topic;

    // Call handleRequest with the switchChainRequest
    await session.handleRequest(switchChainRequest);
