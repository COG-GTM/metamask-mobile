...
import {
  findExistingNetwork,
  switchToNetwork } from
'../RPCMethods/lib/ethereum-chain-utils';
...
describe('WalletConnect Utils', () => {
  ...
  const mockStore = StoreModule.store;
  ...
  describe('checkWCPermissions', () => {
    beforeEach(() => {
      findExistingNetwork.mockReturnValue({ chainId: '0x1' });
    });
    ...
    it('throws error for non-existent network', async () => {
      findExistingNetwork.mockReturnValue(null);
      await expect(
        checkWCPermissions({
          origin: 'test',
  ...
  describe('getHostname', () => {
    ...
    it('handles errors gracefully and returns original URI', () => {
      ...
      expect(getHostname(maliciousUri)).toBe(maliciousUri);
