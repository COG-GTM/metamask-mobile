...
export class WC2Manager {
  ...
  static async initCore(projectId) {
    try {
  ...
  async onSessionProposal(proposal) {
    ...
    try {
      ...
      try {
        const hexChainId = `0x${walletChainIdDecimal.toString(16)}`;
    ...
    try {
      ...
      const caipChainId = `eip155:${walletChainIdDecimal}`;
