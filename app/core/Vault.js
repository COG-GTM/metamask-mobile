...
export const restoreLedgerKeyring = async (
  ...
  try {
    await withLedgerKeyring(async (keyring: any) => {
 ...
 */
export const getSeedPhrase = async (password = ''): Promise<string> => {
  const { KeyringController } = Engine.context;
  return (await (KeyringController.exportSeedPhrase as any)(password)) as string;
};
...
export const recreateVaultWithNewPassword = async (
  ...
  try {
    ...
    const simpleKeyrings = KeyringController.state.keyrings.filter(
      (keyring: any) => keyring.type === KeyringTypes.simple,
    );
  ...
  await (KeyringController.createNewVaultAndRestore as any)(
    newPassword,
    seedPhrase,
  );
  ...
  for (let i = 0; i < importedAccounts.length; i++) {
      await (KeyringController.importAccountWithStrategy as any)('privateKey', [
        importedAccounts[i],
      ]);
    }
  } catch (e) {
  ...
  for (const keyring of recreatedKeyrings as any[]) {
...
function hasKeyringType(state: any, type: KeyringTypes): boolean {
  return state?.keyrings?.some((keyring: any) => keyring.type === type);
}
...
async function getSerializedKeyring(type: KeyringTypes): Promise<unknown> {
  const { KeyringController } = Engine.context;
  return await (KeyringController.withKeyring as any)({ type }, ({ keyring }: any) =>
    keyring.serialize(),
  );
}
