...
export const restoreLedgerKeyring = async (
  ...
  try {
    await withLedgerKeyring(async (keyringWrapper) => {
      // Tests mock withLedgerKeyring to pass the keyring directly (see @ts-expect-error in test).
      // The cast accommodates both the actual {keyring, metadata} shape and the test mock shape.
      const ledgerKeyring = keyringWrapper as unknown as { deserialize: (opts: unknown) => Promise<void> };
      await ledgerKeyring.deserialize(serializedLedgerKeyring);
    });
  } catch (e) {
    Logger.error(
      e as Error,
