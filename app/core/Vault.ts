...
export const restoreLedgerKeyring = async (
  ...
  try {
    await withLedgerKeyring(async (selectedKeyring) => {
      const keyring = selectedKeyring.keyring as unknown as {
 ...
 */
export const getSeedPhrase = async (password = ''): Promise<unknown> => {
  const { KeyringController } = Engine.context;
...
async function getSerializedKeyring(type: KeyringTypes): Promise<unknown> {
  ...
  return await KeyringController.withKeyring(
    ...
    async ({ keyring }: { keyring: unknown }) => {
      const serializable = keyring as { serialize: () => Promise<unknown> };
