...
export const restoreLedgerKeyring = async (serializedLedgerKeyring: unknown) => {
  try {
    await withLedgerKeyring(async (keyring) => {
      // Preserves the historical runtime behavior: `deserialize` is invoked on
      // the value returned by `withLedgerKeyring` as it was in the original JS.
      await (keyring as unknown as LedgerKeyring).deserialize(
        serializedLedgerKeyring as Parameters<LedgerKeyring['deserialize']>[0],
      );
    });
  } catch (e) {
    Logger.error(
