...
/**
 * The subset of a keyring used when restoring serialized state.
 */
interface DeserializableKeyring {
  deserialize(serializedState: unknown): Promise<void>;
}
...
export const restoreQRKeyring = async (serializedQrKeyring: unknown) => {
  ...
  try {
    await KeyringController.restoreQRKeyring(
      serializedQrKeyring as Parameters<
        typeof KeyringController.restoreQRKeyring
      >[0],
    );
  } catch (e) {
    Logger.error(
...
export const restoreLedgerKeyring = async (
  ...
  try {
    // The callback argument is treated as the keyring itself here, matching the
    // existing runtime behavior and the expectations of the unit tests.
    await withLedgerKeyring(async (keyring) => {
      await (keyring as unknown as DeserializableKeyring).deserialize(
...
function hasKeyringType(
  ...
  return Boolean(state?.keyrings?.some((keyring) => keyring.type === type));
}
