...
export const restoreLedgerKeyring = async (
  ...
  try {
    await withLedgerKeyring(async (keyring) => {
      await (
        keyring as unknown as { deserialize: (s: unknown) => Promise<void> }
...
function hasKeyringType(
  state: KeyringControllerState | undefined,
  type: KeyringTypes,
): boolean {
  return Boolean(
    state?.keyrings?.some((keyring: KeyringObject) => keyring.type === type),
  );
}
...
async function getSerializedKeyring(type: KeyringTypes): Promise<unknown> {
  ...
  return await KeyringController.withKeyring({ type }, ({ keyring }) =>
    (keyring as unknown as { serialize: () => Promise<unknown> }).serialize(),
  );
}
