...
interface LedgerKeyringWithDeserialize {
  deserialize(serializedKeyring: string): Promise<void> | void;
}
...
export const restoreLedgerKeyring = async (
  serializedLedgerKeyring: string,
): Promise<void> => {
  try {
    await withLedgerKeyring(async (keyring) => {
      await (
        keyring as unknown as LedgerKeyringWithDeserialize
      ).deserialize(serializedLedgerKeyring);
    });
  } catch (e) {
    Logger.error(
      e as Error,
...
export const recreateVaultWithNewPassword = async (
  ...
  for (const keyring of recreatedKeyrings) {
    if (keyring.accounts.includes((selectedAddress as string).toLowerCase())) {
      Engine.setSelectedAddress(selectedAddress as string);
      return;
    }
  }
};
 ...
 */
async function getSerializedKeyring(type: string): Promise<string> {
  const { KeyringController } = Engine.context;
  return await KeyringController.withKeyring(
    { type } as Parameters<typeof KeyringController.withKeyring>[0],
    ({ keyring }) =>
      keyring.serialize() as unknown as Promise<string>,
  );
}
