...
import {
  ...
  AUTHENTICATION_STORE_PASSWORD_FAILED } from
'../../constants/error';
...
class AuthenticationService {
  authData = { currentAuthType: AUTHENTICATION_TYPE.UNKNOWN };
   ...
   */
  loginVaultCreation = async (password) => {
   ...
   */
  newWalletVaultAndRestore = async (
  password,
  parsedSeed,
  clearEngine) =>
  {
   ...
   * @param password - password provided by user, biometric, pincode
   */
  createWalletVaultAndKeychain = async (
  password) =>
  {
    // TODO: Replace "any" with type
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
   ...
   */
  lockApp = async ({ reset = true, locked = false } = {}) => {
    const { KeyringController } = Engine.context;
