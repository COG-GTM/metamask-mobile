...
interface RemoteImageProps extends Omit<ImageProps, 'source' | 'style' | 'onError'> {
  /**
...
const RemoteImage = (props: RemoteImageProps) => {
  ...
  const source = resolveAssetSource(props.source as ImageSourcePropType);
  const propSourceUri = (props.source as ImageURISource | undefined)?.uri;
  const isImageUrl = isUrl(propSourceUri ?? '');
  ...
  if (error && props.address) {
    return (
      <Identicon
        ...
        customStyle={props.style as unknown as ImageStyle}
      />
    );
  }

  if (isSVG) {
    const style = (props.style || {}) as unknown as ImageStyle;
    ...
    return (
      <>
        {props.isTokenImage ? (
          ...
          <FadeIn placeholderStyle={props.placeholderStyle}>
            <Image {...restProps} source={{ uri }} onError={onError} />
          </FadeIn>
        )}
      </>
    );
  ...
  const { style: _style, ...imageSpreadProps } = props;
  return <Image {...imageSpreadProps} source={{ uri }} onError={onError} />;
};
