...
const RemoteImage = (props: RemoteImageProps) => {
  ...
  const sourceUri = (props.source as ImageURISource)?.uri;
  ...
  useEffect(() => {
    ...
    async function resolveIpfsUrl() {
      try {
        const url = new URL(String(sourceUri));
  ...
  if (isSVG) {
    ...
    return (
      <ComponentErrorBoundary
        onError={props.onError as unknown as () => void}
        componentLabel="RemoteImage-SVG"
      >
        <View style={{ ...style, ...styles.svgContainer }}>
          <SvgUri
            {...(props as unknown as UriProps)}
            uri={uri}
            width={'100%'}
            height={'100%'}
            viewBox={viewbox}
                ...
                >
                  <View style={style}>
                    <Image
                      ...
                      {...(restProps as unknown as ImageProps)}
                      source={{ uri }}
                      onError={onError}
                      resizeMode={'cover'}
                    />
