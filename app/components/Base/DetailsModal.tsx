...
type DetailsModalComponent = React.FC<DetailsModalProps> & {
  ...
  Title: React.FC<React.ComponentPropsWithoutRef<typeof Text>>;
  CloseIcon: React.FC<TouchableOpacityProps>;
  Body: React.FC<ViewProps>;
  Section: React.FC<DetailsModalSectionProps>;
  SectionTitle: React.FC<React.ComponentPropsWithoutRef<typeof Text>>;
...
const DetailsModalHeader: React.FC<ViewProps> = ({ style, ...props }) => {
  const { colors } = useTheme();
  const styles = createStyles(colors);

  return <View style={[styles.header, style]} {...props} />;
};
const DetailsModalTitle: React.FC<
...
const DetailsModalCloseIcon: React.FC<TouchableOpacityProps> = ({
  style,
  ...props
}) => {
  const { colors } = useTheme();
...
const DetailsModalBody: React.FC<ViewProps> = ({ style, ...props }) => {
  const { colors } = useTheme();
  const styles = createStyles(colors);
...
const DetailsModalSectionTitle: React.FC<
  React.ComponentPropsWithoutRef<typeof Text>
> = ({ style, ...props }) => {
  const { colors } = useTheme();
  const styles = createStyles(colors);
