...
interface StatusTextProps extends Omit<TextProps, 'status'> {
  status: string;
  context?: string;
  testID?: string;
}

export const ConfirmedText = ({ testID, ...props }: TextProps) => (
  <Text testID={testID} bold green style={styles.status} {...props} />
);

export const PendingText = ({ testID, ...props }: TextProps) => {
...
export const FailedText = ({ testID, ...props }: TextProps) => {
  const { colors } = useTheme();
  return (
    <Text
      testID={testID}
