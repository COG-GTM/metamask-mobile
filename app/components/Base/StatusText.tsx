...
type StatusTextVariantProps = React.ComponentProps<typeof Text>;

interface StatusTextProps extends StatusTextVariantProps {
  status: string;
  context?: string;
}

export const ConfirmedText = ({ testID, ...props }: StatusTextVariantProps) => (
  <Text testID={testID} bold green style={styles.status} {...props} />
);

export const PendingText = ({ testID, ...props }: StatusTextVariantProps) => {
...
export const FailedText = ({ testID, ...props }: StatusTextVariantProps) => {
  const { colors } = useTheme();
  return (
    <Text
      testID={testID}
