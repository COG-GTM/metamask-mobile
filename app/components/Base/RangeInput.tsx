...
export interface RangeInputProps {
   ...
   */
  inputInsideLabel?: string | null | false;
  /**
...
const RangeInput = ({
  ...
  const increaseNumber = useCallback(() => {
    const newValue = new BigNumber(value as BigNumber.Value).plus(
      new BigNumber(increment),
    );
    if (!new BigNumber(max as BigNumber.Value).isNaN() && newValue.gt(max as BigNumber.Value))
      return;
    changeValue(newValue.toString());
  }, [changeValue, increment, max, value]);

  const decreaseNumber = useCallback(() => {
    const newValue = new BigNumber(value as BigNumber.Value).minus(
      new BigNumber(increment),
    );
    if (!new BigNumber(min as BigNumber.Value).isNaN() && newValue.lt(min as BigNumber.Value))
      return;
    changeValue(newValue.toString());
