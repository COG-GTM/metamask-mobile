...
const RangeInput = ({
  ...
  const decreaseNumber = useCallback(() => {
    ...
    if (min !== undefined && !new BigNumber(min).isNaN() && newValue.lt(min))
      return;
    changeValue(newValue.toString());
  ...
  const checkLimits = useCallback(() => {
    if (min !== undefined && new BigNumber(value || 0).lt(min)) {
  ...
  return (
    <View>
      ...
      <View style={(styles as any).rangeInputContainer(Boolean(error))}>
        ...
        <View style={styles.inputContainer}>
          <TextInput
            style={(styles as any).input(Boolean(error))}
