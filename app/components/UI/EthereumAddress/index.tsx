...
// WALLET_ACCOUNT_ADDRESS_LABEL is not exported by WalletView.testIds; the
// namespace import preserves the original (undefined) runtime lookup.
...
const WALLET_ACCOUNT_ADDRESS_LABEL = (
  WalletViewTestIds as unknown as Record<string, string | undefined>
...
> {
  ...
  constructor(props: EthereumAddressProps) {
    ...
    this.state = {
      ensName: null,
      address: formatAddress(address as string, type ?? 'full'),
    };
  }
  ...
  formatAndResolveIfNeeded() {
    ...
    const formattedAddress = formatAddress(address as string, type ?? 'full');
