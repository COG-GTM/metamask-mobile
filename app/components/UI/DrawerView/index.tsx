...
import {
  toggleInfoNetworkModal as toggleInfoNetworkModalAction,
  toggleNetworkModal,
} from '../../../actions/modals';
...
import {
  ...
  networkSwitched as networkSwitchedAction,
...
interface DrawerViewProps {
   ...
   */
  newAssetTransaction: (...args: unknown[]) => void;
  /**
   ...
   */
  wizard: { step: number; [key: string]: unknown };
  /**
...
class DrawerView extends PureComponent<DrawerViewProps> {
  state = {
  ...
  copyAccountToClipboard = async () => {
    await ClipboardManager.setString(this.selectedChecksummedAddress as string);
  ...
  onInfoNetworksModalClose = () => {
    ...
    onboardNetworkAction(chainId as string);
    ...
    safePromiseHandler(
      toggleInfoNetworkModal() as unknown as () => void,
      100,
    );
  };
  ...
  render() {
    ...
    const account: Record<string, unknown> = {
      ...
      ...selectedInternalAccount,
      ...accounts[this.selectedChecksummedAddress as string],
    };
    const { name, ens } = account as { name?: string; ens?: string };
    account.balance =
      (accounts[this.selectedChecksummedAddress as string] &&
        renderFromWei(
          accounts[this.selectedChecksummedAddress as string].balance as string,
        )) ||
      0;
        ...
        <ScrollView>
          ...
          <View style={styles.account}>
            <View style={styles.accountBgOverlay}>
              ...
              <TouchableOpacity
                style={(styles as Record<string, ViewStyle>).accountInfo}
                onPress={this.openAccountSelector}
                        ...
                        <TouchableOpacity
                          ...
                          ref={
                            (item.name === strings('drawer.browser') &&
                              this.browserSectionRef) as unknown as React.LegacyRef<View>
                          }
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
  ...
  onboardNetworkAction: (chainId: string) => dispatch(onboardNetworkActionCreator(chainId)),
  networkSwitched: ({
    networkUrl,
    networkStatus,
  }: {
    ...
    dispatch(networkSwitchedAction({ networkUrl, networkStatus })),
  toggleInfoNetworkModal: () => dispatch(toggleInfoNetworkModalAction(false)),
});
