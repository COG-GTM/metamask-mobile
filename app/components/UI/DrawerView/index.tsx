...
type Navigation = NavigationProp<ParamListBase> & {
  replace: StackNavigationProp<ParamListBase>['replace'];
  dangerouslyGetState: () => { routes: Parameters<
...
interface DrawerViewProps extends IWithMetricsAwarenessProps {
  /**
   ...
   */
  newAssetTransaction: (selectedAsset: ReturnType<typeof getEther>) => void;
  /**
   ...
   */
  tokenBalances: ReturnType<typeof selectContractBalances>;
  /**
   ...
   */
  onboardNetworkAction: (chainId: Hex) => void;
  /**
   * returns switched network state
   */
  switchedNetwork?: { networkUrl: string; networkStatus: boolean };
  /**
...
interface DrawerViewState {
  showProtectWalletModal?: boolean;
  account: {
    ens?: string;
    name?: string;
    address?: string;
...
class DrawerView extends PureComponent<DrawerViewProps, DrawerViewState> {
  ...
  async componentDidUpdate() {
    ...
    if (!this.props.passwordSet || !this.props.seedphraseBackedUp) {
      ...
      this.props.tokens.forEach((token: { address: string }) => {
        if (
          this.props.tokenBalances[token.address as Hex] &&
          !isZero(this.props.tokenBalances[token.address as Hex])
        ) {
          tokenFound = true;
        }
      });
      if (
        ...
        (this.currentBalance as number) > 0 ||
        tokenFound ||
  ...
  onSend = async () => {
    this.props.newAssetTransaction(getEther(this.props.ticker as string));
  ...
  copyAccountToClipboard = async () => {
    ...
    // TODO: `toggleReceiveModal` is not defined on this component
    // @ts-expect-error preserving existing behaviour
  ...
  onInfoNetworksModalClose = () => {
    const {
      ...
      toggleInfoNetworkModal: toggleInfoNetwork,
    } = this.props;

    onboardNetwork(chainId);
    onNetworkSwitched({ networkUrl: '', networkStatus: false });

    // Wrap the toggle call in a setTimeout to avoid awaiting a non-promise function.
    // TODO: `safePromiseHandler` takes a function, not the result of calling one
    // @ts-expect-error preserving existing behaviour
    safePromiseHandler(toggleInfoNetwork(), 100);
  };
  ...
  render() {
    ...
    const account = {
      // TODO: overwritten by the `selectedInternalAccount` spread below
      // @ts-expect-error preserving existing behaviour
      address: this.selectedChecksummedAddress,
      name: nameFromState,
      ens: ensFromState,
    ...
    account.balance = ((accounts[this.selectedChecksummedAddress] &&
    ...
    const fiatBalanceStr = renderFiat(
      this.currentBalance,
      currentCurrency as string,
    );
    const accountName = isDefaultAccountName(name as string) && ens ? ens : name;
        ...
        <ScrollView>
          ...
          <View style={styles.account}>
            <View style={styles.accountBgOverlay}>
              ...
              <TouchableOpacity
                // TODO: `createStyles` defines no `accountInfo`
                // @ts-expect-error preserving existing behaviour
                    ...
                    {section
                      ...
                      .map((item, j) => (
                        <TouchableOpacity
                          ...
                          // TODO: resolves to `false` for every item but the browser one
                          // @ts-expect-error preserving existing behaviour
                          ref={
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
  ...
  newAssetTransaction: (selectedAsset: ReturnType<typeof getEther>) =>
    dispatch(newAssetTransaction(selectedAsset)),
  protectWalletModalVisible: () => dispatch(protectWalletModalVisible()),
  onboardNetworkAction: (chainId: Hex) =>
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
  // `withMetricsAwareness` only declares the `metrics` prop; the remaining props
  // are supplied by `connect` and by the drawer navigator.
)(
  withMetricsAwareness(
    DrawerView as unknown as ComponentType<IWithMetricsAwarenessProps>,
  ),
);
