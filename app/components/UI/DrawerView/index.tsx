...
type DrawerNavigation = StackNavigationProp<ParamListBase> & {
  dangerouslyGetState: () => { routes: unknown };
};

interface DrawerKeyring {
  accounts: string[];
  type: string;
}

interface DrawerAlertConfig {
  isVisible: boolean;
  autodismiss: number;
  content: string;
  data: { msg: string };
...
interface DrawerViewProps extends IWithMetricsAwarenessProps {
  ...
  keyrings: DrawerKeyring[];
  toggleNetworkModal: () => void;
  showAlert: (config: DrawerAlertConfig) => void;
  ...
  switchedNetwork?: unknown;
  networkSwitched: (args: {
    networkUrl: string;
...
class DrawerView extends PureComponent<DrawerViewProps, DrawerViewState> {
  ...
  browserSectionRef = React.createRef<React.ElementRef<typeof TouchableOpacity>>();
  ...
  getImageIcon(name: string) {
    ...
    return (
      <Image
        source={ICON_IMAGES[name as keyof typeof ICON_IMAGES]}
  ...
  getSelectedImageIcon(name: string) {
    ...
    return (
      <Image
        source={ICON_IMAGES[`selected-${name}` as keyof typeof ICON_IMAGES]}
  ...
  copyAccountToClipboard = async () => {
    ...
    (this as unknown as { toggleReceiveModal: () => void }).toggleReceiveModal();
  ...
  onInfoNetworksModalClose = () => {
    ...
    this.props.onboardNetworkAction(chainId);
    this.props.networkSwitched({ networkUrl: '', networkStatus: false });

    // Wrap the toggle call in a setTimeout to avoid awaiting a non-promise function.
    safePromiseHandler(
      this.props.toggleInfoNetworkModal() as unknown as () => void,
      100,
    );
  };
  ...
  renderProtectModal = () => {
    ...
    return (
      <Modal
        isVisible={this.state.showProtectWalletModal as boolean}
  ...
  render() {
    ...
    const accountsMap = accounts as Record<
      string,
    ...
    const account: {
      ...
      ens: ensFromState,
      ...selectedInternalAccount,
      ...accountsMap[this.selectedChecksummedAddress],
    };
    const { name, ens } = account;
    account.balance =
      (accountsMap[this.selectedChecksummedAddress] &&
        renderFromWei(
          accountsMap[this.selectedChecksummedAddress]?.balance as string,
        )) ||
      0;
        ...
        <ScrollView>
          ...
          <View style={styles.account}>
            <View style={styles.accountBgOverlay}>
              ...
              <TouchableOpacity
                style={
                  (styles as { accountInfo?: StyleProp<ViewStyle> })
                    .accountInfo
                }
                onPress={this.openAccountSelector}
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
  ...
  showAlert: (config: DrawerAlertConfig) => dispatch(showAlert(config)),
  newAssetTransaction: (selectedAsset: object) =>
    dispatch(newAssetTransaction(selectedAsset)),
  protectWalletModalVisible: () => dispatch(protectWalletModalVisible()),
  onboardNetworkAction: (chainId: string) =>
