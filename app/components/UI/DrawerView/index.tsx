...
class DrawerView extends PureComponent<DrawerViewProps, DrawerViewState> {
  ...
  async componentDidUpdate() {
    ...
    if (!this.props.passwordSet || !this.props.seedphraseBackedUp) {
      ...
      this.props.tokens.forEach((token) => {
        const balances = this.props.tokenBalances as Record<string, string>;
        if (balances[token.address] && !isZero(balances[token.address])) {
          tokenFound = true;
        }
      });
      if (
  ...
  getSelectedImageIcon(name: string) {
    ...
    return (
      <Image
        source={
          ICON_IMAGES[`selected-${name}` as keyof typeof ICON_IMAGES]
        }
  ...
  render() {
    ...
    const accountsByAddress = accounts as Record<string, { balance: string }>;
    const account: DrawerAccount = {
      address: this.selectedChecksummedAddress,
      name: nameFromState,
      ens: ensFromState,
      ...selectedInternalAccount,
      ...accountsByAddress[this.selectedChecksummedAddress],
    };
    const { name, ens } = account;
    account.balance =
      (accountsByAddress[this.selectedChecksummedAddress] &&
        renderFromWei(
          accountsByAddress[this.selectedChecksummedAddress].balance,
        )) ||
      0;
        ...
        <ScrollView>
          ...
          <View style={styles.account}>
            <View style={styles.accountBgOverlay}>
              ...
              <TouchableOpacity
                style={(styles as { accountInfo?: object }).accountInfo}
                onPress={this.openAccountSelector}
