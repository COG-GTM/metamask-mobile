...
interface DrawerAccountBalance {
  balance: string;
}
...
interface DrawerProviderConfig {
  type: string;
  rpcUrl?: string;
}

interface DrawerRenderAccount {
  address: string;
  name?: string;
  ens?: string;
...
interface DrawerViewProps extends IWithMetricsAwarenessProps {
  ...
  */
   ...
   */
  providerConfig: DrawerProviderConfig;
  /**
   * List of accounts from the AccountTrackerController
   */
  accounts: Record<string, DrawerAccountBalance>;
  /**
  ...
  */
   ...
   * List of keyrings
   */
  keyrings: { accounts: string[]; type: string }[];
  /**
...
class DrawerView extends PureComponent<DrawerViewProps, DrawerViewState> {
  ...
  browserSectionRef =
    React.createRef<React.ComponentRef<typeof TouchableOpacity>>();
  ...
  get themeColors(): Theme['colors'] {
    return (this.context as Theme)?.colors || mockTheme.colors;
  }
  ...
  getImageIcon(name: keyof typeof ICON_IMAGES) {
    const colors = this.themeColors;
    const styles = createStyles(colors);
  ...
  getSections = (): DrawerMenuItem[][] => {
    ...
    if (type === RPC) {
      ...
      blockExplorerName = getBlockExplorerName(blockExplorer as string);
    }
    return [
      [
        {
  ...
  copyAccountToClipboard = async () => {
    ...
    (
      this as unknown as { toggleReceiveModal: () => void }
    ).toggleReceiveModal();
  ...
  renderProtectModal = () => {
    ...
    return (
      <Modal
        isVisible={Boolean(this.state.showProtectWalletModal)}
  ...
  render() {
    ...
    ) as unknown as DrawerRenderAccount;
        ...
        <ScrollView>
          ...
          <View style={styles.account}>
            <View style={styles.accountBgOverlay}>
              ...
              <TouchableOpacity
                style={
                  (styles as { accountInfo?: StyleProp<ViewStyle> }).accountInfo
                }
                onPress={this.openAccountSelector}
                        ...
                        <TouchableOpacity
                          ...
                          ref={
                            (item.name === strings('drawer.browser') &&
                              this.browserSectionRef) as React.Ref<
