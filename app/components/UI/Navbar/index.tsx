...
import {
  ...
  TouchableOpacity,
  View,
 ImageSourcePropType, StyleProp, ViewStyle } from 'react-native';
...
type ParentWithPop = { pop?: (count?: number) => void } | undefined;
...
interface RequestedTrade {
  ...
  token_to?: string;
  request_type?: string;
  custom_slippage?: string;
  chain_id?: string;
  token_from_amount?: string;
}

type TrackEventArg = Parameters<
  ReturnType<typeof MetaMetrics.getInstance>['trackEvent']
>[0];
...
export function getTransactionOptionsTitle(
  ...
  const transactionMode = (route.params?.mode as string) ?? '';
  const { name } = route;
  const leftText =
    transactionMode === 'edit'
      ? strings('transaction.cancel')
      : strings('transaction.edit');
  const disableModeChange = route.params?.disableModeChange as
    | boolean
    | undefined;
  const modeChange = route.params?.dispatch as
    | ((arg: string) => void)
    | undefined;
...
export function getSendFlowTitle(
  ...
  const rightAction = () => {
    ...
    resetTransaction();
    (navigation.dangerouslyGetParent?.() as ParentWithPop)?.pop?.();
  };
...
export function getWalletNavbarOptions(
  accountActionsRef: React.Ref<React.ElementRef<typeof PickerAccount>>,
  selectedInternalAccount: InternalAccountLike,
  accountName: string,
  accountAvatarType: React.ComponentProps<typeof PickerAccount>['accountAvatarType'],
  networkName: string,
  ...
  formattedAddress = getFormattedAddressFromInternalAccount(
    selectedInternalAccount as unknown as Parameters<
  ...
  const onScanSuccess = (
    data: { private_key?: string; seed?: string; [key: string]: unknown },
    content: string,
  ) => {
    if (data.private_key) {
      Alert.alert(
                ...
                style={[
                  styles.notificationsBadge,
                  {
                    backgroundColor: unreadNotificationCount
                      ...
                      : (themeColors.background as unknown as {
                          transparent?: string;
                        }).transparent,
                  },
                ]}
...
export function getImportTokenNavbarOptions(
  ...
  const innerStyles = StyleSheet.create({
    ...
    headerShadow: {
      elevation: 2,
      shadowColor: (themeColors.background as unknown as { primary?: string })
        .primary,
    ...
    headerLeft: () => (
      ...
      >
        <Icon
          ...
          size={IconSize.Lg}
          style={innerStyles.headerBackIcon as unknown as StyleProp<ViewStyle>}
        />
      </TouchableOpacity>
    ),
...
export function getWebviewNavbar(
  ...
  const innerStyles = StyleSheet.create({
    ...
    headerIcon: {
      color: (themeColors as unknown as { default?: string }).default,
    },
  });
...
export function getSwapsAmountNavbar(
  ...
  const title = (route.params?.title as string) ?? 'Swap';
  return {
    headerTitle: () => (
    ...
    headerRight: () => (
      // eslint-disable-next-line react/jsx-no-bind
      <TouchableOpacity
        onPress={() => (navigation.dangerouslyGetParent?.() as ParentWithPop)?.pop?.()}
        style={styles.closeButton}
      >
        <Text style={innerStyles.headerButtonText}>
          {strings('navigation.cancel')}
...
export function getSwapsQuotesNavbar(
  ...
  const leftActionText =
    (route.params?.leftAction as string) ?? strings('navigation.back');

  const leftAction = () => {
    const trade = route.params?.requestedTrade as RequestedTrade | undefined;
    const selectedQuote = route.params?.selectedQuote;
    const quoteBegin = route.params?.quoteBegin as number | undefined;
    if (!selectedQuote) {
      trackEvent(
        MetricsEventBuilder.createEventBuilder(
...
export function getBridgeTransactionDetailsNavbar(navigation: NavigationLike) {
  const leftAction = () => navigation.pop?.();
