...
/**
 * Navigation methods used by the navbar helpers that are not part of the
 * base `NavigationProp` type but are available on the navigators in use.
 */
...
export function getEditableOptions(
  ...
  function navigationPop() {
    (navigation as unknown as NavigationExtras).pop();
  }
...
export function getPaymentRequestOptionsTitle(
  ...
  return {
    ...
    headerRight: () => (
      // eslint-disable-next-line react/jsx-no-bind
      <TouchableOpacity
        onPress={() => (navigation as unknown as NavigationExtras).pop()}
...
export function getTransactionOptionsTitle(
  ...
  const transactionMode = (route.params?.mode as string | undefined) ?? '';
  const { name } = route;
  const leftText =
  ...
  const rightAction = () => (navigation as unknown as NavigationExtras).pop();
...
export function getSendFlowTitle(
  ...
  const rightAction = () => {
    const providerType =
      (route?.params?.providerType as string | undefined) ?? '';
    ...
    resetTransaction();
    (navigation as unknown as NavigationExtras).dangerouslyGetParent()?.pop();
  };
  const leftAction = () => (navigation as unknown as NavigationExtras).pop();
...
export function getOnboardingNavbarOptions(
  ...
  const headerLeftHide =
    headerLeft ||
    (route.params?.headerLeft as (() => React.ReactNode) | undefined);
  const innerStyles = StyleSheet.create({
    headerStyle: {
      backgroundColor: themeColors.background.default,
      shadowColor: importedColors.transparent,
      ...
      Device.isIos() ? (
        ...
        >
          <IonicIcon
            ...
            size={24}
            style={
              (innerStyles as unknown as Record<string, ViewStyle>).headerIcon
            }
          />
        </TouchableOpacity>
      ),
...
export function getWalletNavbarOptions(
  ...
  >['accountAvatarType'],
  networkName: string,
  networkImageSource: React.ComponentProps<typeof PickerNetwork>['imageSource'],
  onPressTitle: () => void,
  navigation: NavbarNavigation,
  themeColors: ThemeColors,
      ...
      <View style={innerStyles.headerTitle}>
        <PickerAccount
          ...
          cellAccountContainerStyle={
            (styles as unknown as Record<string, ViewStyle>).account
          }
        ...
        {isNotificationsFeatureEnabled() && (
          <View>
            ...
            <ButtonIcon
              ...
              style={
                (styles as unknown as Record<string, ViewStyle>)
                  .notificationButton
              }
            />
...
export function getImportTokenNavbarOptions(
  ...
  const innerStyles = StyleSheet.create({
    ...
    headerShadow: {
      elevation: 2,
      shadowColor: (themeColors.background as unknown as Record<string, string>)
        .primary,
...
export function getPaymentSelectorMethodNavbar(
  ...
  return {
    ...
    headerRight: () => (
      ...
      <TouchableOpacity
        onPress={() => {
          (navigation as unknown as NavigationExtras)
...
export function getSwapsAmountNavbar(
  ...
  const title = (route.params?.title as string | undefined) ?? 'Swap';
  return {
    headerTitle: () => (
...
export function getSwapsQuotesNavbar(
  ...
  const leftActionText =
    (route.params?.leftAction as string | undefined) ??
    strings('navigation.back');

  const leftAction = () => {
    const trade = route.params?.requestedTrade as SwapsTrade;
