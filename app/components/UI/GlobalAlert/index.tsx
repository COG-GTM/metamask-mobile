...
interface GlobalAlertStyles {
  modal: ViewStyle;
  copyAlert: (
    width?: number,
  ) => React.ComponentProps<typeof ElevatedView>['style'];
  copyAlertIcon: ViewStyle;
  copyAlertText: TextStyle;
}

// `copyAlert` is a style factory rather than a plain style object, which
// `StyleSheet.create` cannot describe, hence the cast.
const createStyles = (colors: Theme['colors']): GlobalAlertStyles =>
  StyleSheet.create({
    modal: {
      margin: 0,
      width: '100%',
  ...
  }) as unknown as GlobalAlertStyles;
