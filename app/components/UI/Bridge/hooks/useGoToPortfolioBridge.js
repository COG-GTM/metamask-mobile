 ...
 */
export default function useGoToPortfolioBridge(location) {
  const chainId = useSelector(selectEvmChainId);
  // TODO: Replace "any" with type
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const browserTabs = useSelector((state) => state.browser.tabs);
  const { navigate } = useNavigation();
  const { trackEvent, createEventBuilder } = useMetrics();
  return (address) => {
    const existingBridgeTab = browserTabs.find((tab) =>
    isBridgeUrl(tab.url)
    );
    ...
    if (existingBridgeTab) {
      ...
      params.newTabUrl = `${
      AppConstants.BRIDGE.URL}/?metamaskEntry=mobile&srcChain=${
