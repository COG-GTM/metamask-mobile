...
export const useTopTokens = ({ chainId }) => {
  const swapsChainCache = useSelector(selectChainCache);
  ...
  const topAssetsFromFeatureFlags = useSelector(
    (state) => selectTopAssetsFromFeatureFlags(state, chainId)
  );
  ...
  const topTokens = useMemo(() => {
    ...
    const addTokenIfNotExists = (token) => {
