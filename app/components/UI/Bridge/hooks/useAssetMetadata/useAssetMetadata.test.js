...
jest.mock('./utils', () => ({
  fetchAssetMetadata: (...args) => mockFetchAssetMetadata(...args),
  getAssetImageUrl: (...args) => mockGetAssetImageUrl(...args)
}));
...
describe('useAssetMetadata', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    useSelector.mockReturnValue(true); // allowExternalServices = true
    mockGetAssetImageUrl.mockReturnValue('mock-image-url');
  });

  it('should return undefined when external services are disabled', async () => {
    useSelector.mockReturnValue(false); // allowExternalServices = false
