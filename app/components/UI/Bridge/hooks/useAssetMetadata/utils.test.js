...
describe('asset-utils', () => {
  ...
  describe('toAssetId', () => {
    ...
    it('should create Solana token asset ID correctly', () => {
      ...
      const chainId = MultichainNetwork.Solana;
  ...
  describe('getAssetImageUrl', () => {
    ...
    it('should handle asset IDs with multiple colons', () => {
      const assetId = 'test:chain:1/token:0x123';
  ...
  describe('fetchAssetMetadata', () => {
    ...
    beforeEach(() => {
      jest.clearAllMocks();
      toEvmCaipChainId.mockReturnValue(mockChainId);
      toHex.mockImplementation((val) => val);
    });
    ...
    it('should fetch EVM token metadata successfully', async () => {
      ...
      handleFetch.mockResolvedValueOnce([mockMetadata]);
    ...
    it('should return undefined when metadata processing fails', async () => {
      handleFetch.mockResolvedValueOnce([null]);
