...
export const BridgeSourceNetworkSelector = () => {
  ...
  const [candidateSourceChainIds, setCandidateSourceChainIds] = useState(selectedSourceChainIds);
  ...
  const handleApply = useCallback(async () => {
    ...
    if (candidateSourceChainIds.length === 1) {
      const evmNetworkConfiguration = evmNetworkConfigurations[candidateSourceChainIds[0]];
      ...
      ///: BEGIN:ONLY_INCLUDE_IF(keyring-snaps)
      if (!evmNetworkConfiguration) {
        await onNonEvmNetworkChange(candidateSourceChainIds[0]);
      }
      ///: END:ONLY_INCLUDE_IF
  ...
  const formatFiatValue = useCallback((value) =>
