...
export const BridgeSourceTokenSelector = () => {
  ...
  const renderItem = useCallback(
    ({ item }) => {
      ...
      const handleTokenPress = async (token) => {
        ...
        evmNetworkConfigurations[token.chainId];
        ...
        ///: BEGIN:ONLY_INCLUDE_IF(keyring-snaps)
        if (!evmNetworkConfiguration) {
          await onNonEvmNetworkChange(token.chainId);
        }
        ///: END:ONLY_INCLUDE_IF
  ...
  return (
    <BridgeTokenSelectorBase
      ...
      <BridgeSourceNetworksBar
        ...
        selectedSourceChainIds={selectedSourceChainIds}
