...
const createStyles = (colors: any, brandColors: any): any =>
  StyleSheet.create({
    itemWrapper: {
...
const splitIntoSubArrays = (array: any, count: any) => {
  const newArray = [];
...
function CollectibleContractElement({
  ...
  const longPressedCollectible = useRef<any>(null);
  const { colors, themeAppearance, brandColors } = useTheme();
  const styles: any = createStyles(colors, brandColors);
  ...
  const handleMenuAction = (index: any) => {
    if (index === 1) {
      removeNft();
    } else if (index === 0) {
      refreshMetadata();
