...
import { Theme as AppTheme } from '../../../util/theme/models';
...
/** Shape of the asset objects rendered and selected by this view. */
...
interface PaymentRequestState {
  ...
  /**
   * Not set by this component; read by `renderEnterAmount` for parity with the
...
> {
  ...
  updateNavBar = () => {
    ...
    navigation.setOptions(
      getPaymentRequestOptionsTitle(
        ...
        navigation,
        route as Parameters<typeof getPaymentRequestOptionsTitle>[2],
        colors,
      ),
    );
  };
  ...
  renderSelectAssets = () => {
    ...
    const themeAppearance =
      (this.context as unknown as AppTheme)?.themeAppearance || 'light';
  ...
  updateAmount = (amount?: string) => {
    ...
    if (
      ...
      res = this.handleFiatPrimaryCurrency(
        amount?.replace(',', '.') as string,
      );
    } else {
      res = this.handleETHPrimaryCurrency(amount?.replace(',', '.') as string);
    }
