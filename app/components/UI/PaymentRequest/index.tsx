...
import AssetListBase from './AssetList';
import {
  weiToFiat,
  toWei,
  balanceToFiat,
...
const AssetList = AssetListBase as unknown as React.ComponentType<{
...
interface PaymentRequestProps {
  ...
  ticker?: string;
  tokenList?: AssetItem[];
  route?: { params?: { receiveAsset?: AssetItem } };
}
...
> {
  ...
  updateNavBar = () => {
    ...
    navigation.setOptions(
      getPaymentRequestOptionsTitle(
        ...
        navigation as unknown as Parameters<
          typeof getPaymentRequestOptionsTitle
        >[1],
        route as unknown as Parameters<
          typeof getPaymentRequestOptionsTitle
        >[2],
        colors,
      ),
  ...
  componentDidMount = () => {
    ...
    fuse.setCollection(tokenList as AssetItem[]);
  };
   ...
   */
  goToAmountInput = async (selectedAsset: AssetItem) => {
    const { navigation } = this.props;
    navigation &&
   ...
   */
  handleSearch = (searchInputValue: unknown) => {
    const { tokenList } = this.props;
    if (typeof searchInputValue !== 'string') {
      searchInputValue = this.state.searchInputValue;
    }

    const fuseSearchResult = fuse.search(searchInputValue as string);
    const addressSearchResult = (tokenList || []).filter((token) =>
      toLowerCaseEquals(token.address, searchInputValue),
    );
    const results = [...addressSearchResult, ...fuseSearchResult];
    this.setState({ searchInputValue: searchInputValue as string, results });
  };
            ...
            <View style={styles.ethContainer}>
              <View style={styles.amounts}>
                <View style={styles.split}>
                  ...
                  <Text
                    style={
                      isTestNet(chainId as string) ? styles.testNetEth : styles.eth
                    }
                    numberOfLines={1}
                  >
                    {symbol}
...
const ConnectedPaymentRequest = connector(
  PaymentRequest as unknown as Parameters<typeof connector>[0],
);

export default ConnectedPaymentRequest as unknown as React.ComponentType<{
