...
// AssetList is an untyped JavaScript component whose inferred prop types are
// incomplete.
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const AssetList = AssetListBase as any;
...
const fuse = new Fuse<PaymentAsset>([], {
  shouldSort: true,
  threshold: 0.45,
  location: 0,
  distance: 100,
...
> {
   ...
   */
  goToAmountInput = async (selectedAsset: PaymentAsset) => {
    const { navigation } = this.props;
  ...
  handleSearch = (searchInputValue: string) => {
    ...
    const addressSearchResult = tokenList.filter((token: PaymentAsset) =>
  ...
  renderSelectAssets = () => {
    ...
    const userTokens = tokens.map((token: PaymentAsset) => {
      const contract = tokenList.find(
        (contractToken: PaymentAsset) =>
          contractToken.address === token.address,
      );
        ...
        {isTDSupportedForNetwork && (
          <View style={styles.searchWrapper}>
            ...
            <TextInput
              ...
              onSubmitEditing={
                this.handleSearch as unknown as () => void
              }
  ...
  handleETHPrimaryCurrency = (amount?: string) => {
    const { conversionRate, currentCurrency, contractExchangeRates } =
      this.props;
    const selectedAsset = this.state.selectedAsset as PaymentAsset;
    let secondaryAmount;
    const symbol = selectedAsset.symbol;
    const undefAmount = (isDecimal(amount as string) &&
    !ethersUtils.isHexString(amount as string)
      ? amount
      : 0) as string | number;
  ...
  handleFiatPrimaryCurrency = (amount?: string) => {
    ...
    const exchangeRate =
      selectedAsset?.address &&
      contractExchangeRates?.[selectedAsset.address]?.price;
    const undefAmount = ((isDecimal(amount as string) && amount) || 0) as
      | string
      | number;
...
) as unknown as React.ComponentType<PaymentRequestOwnProps>;
