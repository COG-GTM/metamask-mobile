...
type ExchangeRates = Record<string, { price?: number } | undefined>;
...
interface PaymentRequestProps {
  ...
  contractExchangeRates?: ExchangeRates;
  searchEngine?: string;
...
> {
  ...
  amountInput = React.createRef<RNTextInput>();
  searchInput = React.createRef<RNTextInput>();
  ...
  updateNavBar = () => {
    ...
    navigation?.setOptions(
      getPaymentRequestOptionsTitle(
        ...
        navigation as Parameters<typeof getPaymentRequestOptionsTitle>[1],
        route,
        colors,
      ),
    );
  ...
  componentDidMount = () => {
    ...
    fuse.setCollection((tokenList ?? []) as Asset[]);
  };
  ...
  renderSelectAssets = () => {
    ...
    const userTokens = tokens.map((token: Asset) => {
      const contract = tokenList.find(
        (contractToken: Asset) => contractToken.address === token.address,
      );
  ...
  handleETHPrimaryCurrency = (amount?: string): AmountResult => {
    ...
    if (selectedAsset.symbol !== 'ETH') {
      ...
      secondaryAmount = weiToFiat(
        toWei(undefAmount as string | number),
        conversionRate,
        currentCurrency as string,
      );
    }
  ...
  onNext = () => {
    ...
    const cryptoAmount = this.state.cryptoAmount;
    const selectedAsset = this.state.selectedAsset as Asset;
