...
interface PaymentRequestProps {
   ...
   */
  tokenList: ReturnType<typeof selectTokenListArray>;
  /**
   * Object that represents the current route info like params passed to it
   */
  route: RouteProp<{ params: PaymentRequestRouteParams }, 'params'>;
}
...
const fuse = new Fuse<ReturnType<typeof selectTokenListArray>[number]>([], {
  shouldSort: true,
  threshold: 0.45,
  location: 0,
  distance: 100,
...
> {
  ...
  handleSearch = (
    ...
    const addressSearchResult = tokenList.filter((token) =>
      toLowerCaseEquals(token.address, query),
    );
    const results = [
      ...addressSearchResult,
      ...fuseSearchResult,
    ] as PaymentRequestAsset[];
    this.setState({ searchInputValue: query, results });
  };
  ...
  handleFiatPrimaryCurrency = (amount?: string) => {
    ...
    if (selectedAsset.symbol !== 'ETH' && exchangeRate && exchangeRate !== 0) {
      ...
      secondaryAmount =
        renderFromTokenMinimalUnit(
          secondaryMinimalUnit as string,
          selectedAsset.decimals as number,
        ) +
        ' ' +
      ...
      secondaryAmount =
        renderFromWei(
          fiatNumberToWei(undefAmount, conversionRate as number) as string,
        ) +
        ' ' +
        strings('unit.eth');
  ...
  switchPrimaryCurrency = async () => {
    ...
    await this.setState({
      internalPrimaryCurrency:
        primarycurrencies[
          internalPrimaryCurrency as keyof typeof primarycurrencies
        ],
    });
