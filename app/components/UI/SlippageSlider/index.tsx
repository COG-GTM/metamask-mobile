...
/**
 * `Animated.Value` keeps its current numeric value on a private `_value` field,
 * which is not part of the public React Native types.
 */
type AnimatedValueWithValue = Animated.Value & { _value: number };
...
const SlippageSlider = ({
  ...
  const pan = useRef(new Animated.Value(0) as AnimatedValueWithValue).current;
