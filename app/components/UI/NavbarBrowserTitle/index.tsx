...
interface NavbarBrowserTitleRoute {
  params?: {
    showUrlModal?: () => void;
  };
}
...
const NetworkEntries = Networks as Record<string, NetworkEntry | undefined>;
...
class NavbarBrowserTitle extends PureComponent<Props> {
  ...
  getNetworkName(providerConfig: ProviderConfig) {
    let name: string = { ...Networks.rpc, color: null }.name;

    if (providerConfig) {
      if (providerConfig.nickname) {
        name = providerConfig.nickname;
      } else if (providerConfig.type) {
        const currentNetwork = NetworkEntries[providerConfig.type];
  ...
  render = () => {
    ...
    const color = NetworkEntries[providerConfig.type]?.color || null;
