...
describe('deriveBalanceFromAssetMarketDetails', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    safeToChecksumAddress.mockReturnValue('0x123');
  });

  it('returns balanceFiat and balanceValueFormatted when token data is available', () => {
    renderFromTokenMinimalUnit.mockReturnValue('2');
    balanceToFiatNumber.mockReturnValue(4);
    addCurrencySymbol.mockReturnValue('$4.00');
  ...
  it('returns TOKEN_BALANCE_LOADING when token balance is missing', () => {
    ...
    renderFromTokenMinimalUnit.mockReturnValue('');
  ...
  it('returns "< 0.01 USD" if token asset fiat balance is less than 0.01', () => {
    renderFromTokenMinimalUnit.mockReturnValue('2');
    balanceToFiatNumber.mockReturnValue(0.005);
    addCurrencySymbol.mockReturnValue('$0.01');
