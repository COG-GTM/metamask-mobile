...
import networkListRaw from '../../../util/networks';
...
const TransactionHeader = (props: TransactionHeaderProps) => {
  ...
  const networkList = networkListRaw as unknown as Record<
    string,
  ...
  const renderNetworkStatusIndicator = () => {
    ...
    const networkStatusIndicatorColor =
      (networkType && networkList[networkType]?.color) || colors.error.default;
  ...
  const renderTitle = () => {
    ...
    if (originIsDeeplink) title = renderShortAddress(spenderAddress as string);
      ...
      title = getHost((origin as string).split(WALLET_CONNECT_ORIGIN)[1]);
    else if (originIsMMSDKRemoteConn) {
      title = getHost(
        (origin as string).split(AppConstants.MM_SDK.SDK_REMOTE_ORIGIN)[1],
      );
  ...
  const renderNetworkContainer = () => (
    <View style={styles.networkContainer}>
      ...
      <Text style={styles.network}>
        {props.nickname ||
          (props.networkType && networkList[props.networkType]?.shortName)}
      </Text>
    </View>
  );
