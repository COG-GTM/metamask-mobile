...
const TransactionHeader: React.FC<TransactionHeaderProps> = (props) => {
  ...
  const originIsWalletConnect =
    props.currentPageInformation.origin?.startsWith(WALLET_CONNECT_ORIGIN);

  const originIsMMSDKRemoteConn = props.currentPageInformation.origin?.startsWith(
   ...
   */
  const renderNetworkStatusIndicator = (): React.ReactElement => {
   ...
   */
  const renderSecureIcon = (): React.ReactElement | null => {
  ...
  const renderTopIcon = (): React.ReactElement => {
  ...
  const renderDomainUrlContainer = (): React.ReactElement => (
    <View
  ...
  const renderNetworkContainer = (): React.ReactElement => (
    <View style={styles.networkContainer}>
      {renderNetworkStatusIndicator()}
      <Text style={styles.network}>
        {props.nickname ||
          networkList[props.networkType as keyof typeof networkList]?.shortName}
      </Text>
    </View>
  );
