...
const TransactionHeader = (props: TransactionHeaderProps) => {
  ...
  const networks = networkList as NetworkList;
  ...
  const renderNetworkStatusIndicator = () => {
    ...
    const networkStatusIndicatorColor =
      (networkType && networks[networkType]?.color) || colors.error.default;
  ...
  const renderNetworkContainer = () => (
    <View style={styles.networkContainer}>
      ...
      <Text style={styles.network}>
        {props.nickname ||
          (props.networkType && networks[props.networkType]?.shortName)}
      </Text>
    </View>
  );
  const currentOrigin = props.currentPageInformation.origin || '';
  const showOrigin = !INTERNAL_ORIGINS.includes(currentOrigin);
