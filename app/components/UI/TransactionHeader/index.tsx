...
const networks = networkList as Record<
  string,
...
const TransactionHeader = (props: TransactionHeaderProps) => {
  ...
  const renderNetworkStatusIndicator = () => {
    ...
    const networkStatusIndicatorColor = network?.color || colors.error.default;
  ...
  const renderTitle = () => {
    ...
    if (originIsDeeplink) title = renderShortAddress(spenderAddress as string);
      ...
      title = getHost(origin?.split(WALLET_CONNECT_ORIGIN)[1] as string);
    else if (originIsMMSDKRemoteConn) {
      title = getHost(
        origin?.split(AppConstants.MM_SDK.SDK_REMOTE_ORIGIN)[1] as string,
      );
    }
  ...
  const renderNetworkContainer = () => (
    <View style={styles.networkContainer}>
      ...
      <Text style={styles.network}>
        {props.nickname ||
          (props.networkType ? networks[props.networkType]?.shortName : null)}
      </Text>
    </View>
  );
  const showOrigin = !INTERNAL_ORIGINS.includes(
    props.currentPageInformation.origin as string,
  );
