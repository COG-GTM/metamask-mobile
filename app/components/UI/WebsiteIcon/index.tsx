...
/**
 * `react-native-svg` sizes accept numbers and strings only, while style
 * dimensions can also hold values such as `null` or animated nodes.
 */
const toNumberProp = (
  dimension: DimensionValue | undefined,
): NumberProp | undefined =>
  typeof dimension === 'number' || typeof dimension === 'string'
    ? dimension
    : undefined;
    ...
    return (
      <View style={viewStyle}>
        {imageSVG ? (
          <SvgUri
            uri={imageSVG}
            width={toNumberProp(style?.width)}
            height={toNumberProp(style?.height)}
            style={style}
