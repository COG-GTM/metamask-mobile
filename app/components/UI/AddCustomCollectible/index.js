...
const AddCustomCollectible = ({
  ...
  // TODO: Replace "any" with type
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const assetTokenIdInput = React.createRef();
  ...
  const getAnalyticsParams = useCallback(() => {
    try {
      ...
      Logger.error(error, 'AddCustomCollectible.getAnalyticsParams');
      return undefined;
    }
  }, [chainId]);

  const validateCustomCollectibleAddress = async () => {
    let validated = true;
  ...
  const validateCustomCollectible = async () => {
  ...
  const onTokenIdChange = (newTokenId) => {
