 ...
 */
interface OptinMetricsProps extends IWithMetricsAwarenessProps {
...
class OptinMetrics extends PureComponent<OptinMetricsProps, OptinMetricsState> {
  ...
  getStyles = () => {
    const { colors } = this.context as unknown as Theme;
    return createStyles({ colors });
  };

  actionsList: OptinAction[] = isPastPrivacyPolicyDate
    ? [1, 2, 3].map((value) => ({
      action: value,
  ...
  componentDidMount() {
    ...
    BackHandler.addEventListener(
      'hardwareBackPress',
      this.handleBackPress as () => boolean,
    );
  }

  componentDidUpdate(_prevProps: OptinMetricsProps, prevState: OptinMetricsState) {
    // Update the navbar
    this.updateNavBar();
    ...
    if (
      ...
      if (scrollViewHeight !== undefined && scrollViewHeight >= scrollViewContentHeight) {
   ...
   */
  renderLegacyAction = (
    { action, description, prefix }: OptinAction,
    i: number,
  ) => {
  ...
  renderAction = ({ description, prefix }: OptinAction, i: number) => {
    const styles = this.getStyles();
