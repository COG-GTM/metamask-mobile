...
const createStyles = ({ colors }: Props): any =>
  StyleSheet.create({
    root: {
      ...baseStyles.flexGrow,
 ...
 */
class OptinMetrics extends PureComponent<Props> {
  ...
  getStyles = () => {
    const { colors, typography } = (this as any).context;
...
// @ts-expect-error -- legacy JavaScript UI type boundary
   ...
   */
  renderLegacyAction = ({ action, description, prefix }: any, i: any) => {
  ...
  renderAction = ({ description, prefix }: any, i: any) => {
    const styles = this.getStyles();
   ...
   * @param {number} height
   */
  onContentSizeChange = (_: any, height: any) => (this.setState({ scrollViewContentHeight: height }));
      ...
      >
        ...
        >
          <View style={styles.wrapper}>
            ...
            {this.actionsList.map((action: any, i: any) =>
              isPastPrivacyPolicyDate
