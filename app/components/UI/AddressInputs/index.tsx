...
const FlaggedText = Text as React.FC<
  React.ComponentProps<typeof Text> & { red?: boolean; black?: boolean }
>;

const createStyles = (colors: Theme['colors'], layout = 'horizontal') => {
  ...
  if (confusableCollection.length) {
    const texts = toAddressName?.split('').map((char, index) => {
      ...
      if (confusableCollection.includes(char)) {
        ...
        return (
          <FlaggedText red key={index}>
            {replacement}
          </FlaggedText>
        );
      }
      return (
        <FlaggedText black key={index}>
          {char}
        </FlaggedText>
      );
    });
        ...
        >
          <View style={styles.input}>
            <TextInput
              ...
              autoFocus
              onBlur={onInputBlur}
              onSubmitEditing={onSubmit as (() => void) | undefined}
