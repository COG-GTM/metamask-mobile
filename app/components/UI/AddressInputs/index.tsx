...
type AddressInputsLayout = 'horizontal' | 'vertical';

// `red` and `black` are legacy props kept for behavioural parity
const LegacyText = Text as unknown as ComponentType<
  React.ComponentProps<typeof Text> & { red?: boolean; black?: boolean }
>;

const AddressBookWrapper = AddToAddressBookWrapper as unknown as ComponentType<{
  address: string;
  children: ReactNode;
}>;
  ...
  if (confusableCollection.length) {
    const texts = toAddressName?.split('').map((char, index) => {
      ...
      if (confusableCollection.includes(char)) {
        ...
        return (
          <LegacyText red key={index}>
            {replacement}
          </LegacyText>
        );
      }
      return (
        <LegacyText black key={index}>
          {char}
        </LegacyText>
      );
    });
