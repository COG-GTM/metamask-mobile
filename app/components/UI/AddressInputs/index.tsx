...
type AddressInputsLayout = 'horizontal' | 'vertical' | string;

/**
 * `red` and `black` are not part of the component library `Text` props. They
 * are kept to preserve the existing runtime behaviour of this component.
 */
const redTextProps = { red: true } as unknown as TextProps;
const blackTextProps = { black: true } as unknown as TextProps;

const createStyles = (
  colors: Theme['colors'],
  layout: AddressInputsLayout = 'horizontal',
) => {
    ...
    const texts = toAddressName
      ...
      .map((char: string, index: number) => {
        ...
        if (confusableCollection.includes(char)) {
          ...
          return (
            <Text {...redTextProps} key={index}>
              {replacement}
            </Text>
          );
        }
        return (
          <Text {...blackTextProps} key={index}>
            {char}
          </Text>
        );
      });
...
interface AddressToProps {
   ...
   */
  onSubmit?: (() => void) | ((address: string) => void);
  /**
        ...
        >
          <View style={styles.input}>
            <TextInput
              ...
              autoFocus
              onBlur={onInputBlur}
              onSubmitEditing={onSubmit as () => void}
