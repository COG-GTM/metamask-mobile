...
const ScreenViewAny = ScreenView as any;
...
function SwapsQuotesView({
  ...
  const destinationToken = [...swapsTokens, ...tokens].find((token: any) =>
  ...
  const [firstLoadTime, setFirstLoadTime] = useState<any>(Date.now());
  const [isFirstLoad, setIsFirstLoad] = useState<any>(true);
  const [shouldFinishFirstLoad, setShouldFinishFirstLoad] = useState<any>(false);
  const [remainingTime, setRemainingTime] = useState<any>(POLLING_INTERVAL);

  const [allQuotesFetchTime, setAllQuotesFetchTime] = useState<any>(null);
  const [trackedRequestedQuotes, setTrackedRequestedQuotes] = useState<any>(false);
  const [trackedReceivedQuotes, setTrackedReceivedQuotes] = useState<any>(false);
  const [trackedError, setTrackedError] = useState<any>(false);
  const [animateOnGasChange, setAnimateOnGasChange] = useState<any>(false);
  const [isAnimating, setIsAnimating] = useState<any>(false);
  const [isHandlingSwap, setIsHandlingSwap] = useState<any>(false);
  const [multiLayerL1ApprovalFeeTotal, setMultiLayerL1ApprovalFeeTotal] =
    useState<any>(null);

  /* Selected quote, initially topAggId (see effects) */
  const [selectedQuoteId, setSelectedQuoteId] = useState<any>(null);
  ...
  const [isSaving] = useState<any>(false);
  const [isInFetch, setIsInFetch] = useState<any>(false);
...
// @ts-expect-error -- legacy JavaScript UI type boundary
  ...
  const allQuotes = useMemo(() => {
    ...
    const orderedAggregators = hasConversionRate
      ...
      : Object.values(quotes).sort((a: any, b: any) => {
    ...
    return orderedAggregators.map(
      (quoteValue: any) => quotes[quoteValue.aggregator],
    );
  }, [hasConversionRate, quoteValues, quotes]);

  /* Get the selected quote, by default is topAggId */
  const selectedQuote = useMemo(
    () => allQuotes.find((quote: any) => quote?.aggregator === selectedQuoteId),
    ...
    async (isHardwareAddress: any) => {
      try {
        ...
        if (
          ...
          const waitPromise = new Promise((resolve: any) =>
...
// @ts-expect-error -- legacy JavaScript UI type boundary
  ...
  const [pollToken, setPollToken] = useState<any>(null);
...
// @ts-expect-error -- legacy JavaScript UI type boundary
  ...
  if (isFirstLoad || (!error?.key && !selectedQuote)) {
    return (
      <ScreenViewAny contentContainerStyle={styles.screen} scrollEnabled={false}>
        <LoadingAnimation
  ...
  if (!isInPolling && error?.key) {
    ...
    return (
      <ScreenViewAny contentContainerStyle={styles.screen}>
