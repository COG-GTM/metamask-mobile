...
import type {
  ...
  QuoteValues as SwapsQuoteValues,
...
interface StateProps {
  ...
  quoteValues: Record<string, SwapsQuoteValues>;
  approvalTransaction: ReturnType<typeof selectSwapsApprovalTransaction>;
  error: ReturnType<typeof selectSwapsError>;
  ...
  usedGasEstimate: ReturnType<typeof selectSwapsUsedGasEstimate>;
  usedCustomGas: ReturnType<typeof selectSwapsUsedCustomGas>;
...
interface SwapsQuotesViewProps extends StateProps, DispatchProps {}

// eslint-disable-next-line @typescript-eslint/consistent-type-definitions
type SwapsQuotesViewRouteParams = {
  SwapsQuotesView: {
...
function SwapsQuotesView({
  ...
  const route =
    useRoute<
      RouteProp<SwapsQuotesViewRouteParams, 'SwapsQuotesView'>
    >();
  ...
  const allQuotes = useMemo(() => {
    ...
    const orderedAggregators = hasConversionRate
      ...
      : Object.values(quotes).sort((a, b) => {
          const comparison = new BigNumber(b.destinationAmount ?? 0).comparedTo(
            a.destinationAmount ?? 0,
          );
          if (comparison === 0) {
  ...
  const selectedQuoteValue = useMemo(() => {
    const currentQuoteValue = selectedQuoteId
    ...
    const fees = {
      ethFee: calculateEthFeeForMultiLayer({
        ...
        ethFee: currentQuoteValue.ethFee as unknown as number,
      }),
      maxEthFee: calculateEthFeeForMultiLayer({
        multiLayerL1FeeTotal: multiLayerL1ApprovalFeeTotal,
        ethFee: currentQuoteValue.maxEthFee as unknown as number,
      }),
    };
    return {
      ...currentQuoteValue,
    ...
    selectedQuoteId ? quoteValues[selectedQuoteId] : undefined,
  ...
  const [
    ...
    const tokenBalanceEnough = tokenBalanceBN.gte(sourceBN);
    const tokenBalanceMissing = tokenBalanceEnough
      ? null
    ...
    const ethAmountBN = isSwapsNativeAsset(sourceToken)
      ...
      : new BigNumber(0);
    const ethBalanceBN = new BigNumber(accounts[selectedAddress].balance);
    const gasBN = toWei(selectedQuoteValue?.maxEthFee || '0').toString();
    const ethBalanceEnough = canUseGasIncludedSwap
      ? true
      : ethBalanceBN.gte(ethAmountBN.plus(gasBN));
    const ethBalanceMissing = ethBalanceEnough
      ? null
  ...
  const approvalMinimumSpendLimit = useMemo(() => {
    ...
    return fromTokenMinimalUnit(sourceAmount ?? '0', sourceToken.decimals ?? 0);
  ...
  const updateSwapsTransactions = useCallback(
    async (transactionMetaId: string, approvalTransactionMetaId?: string) => {
      ...
      addSwapsTransaction(transactionMetaId, {
        ...
        sourceAmount,
        destinationAmount: selectedQuote.destinationAmount ?? '0',
        ...
        analytics: {
          ...
          token_to_amount: fromTokenMinimalUnitString(
            selectedQuote.destinationAmount ?? '0',
            destinationToken.decimals ?? 0,
          ),
      ...
      try {
        ...
        if (isHardwareAddress) {
          ...
          Engine.controllerMessenger.subscribeOnceIf(
            ...
            (confirmedMeta) => {
              if (confirmedMeta.status === TransactionStatus.confirmed) {
                handleSwapTransaction(approvalTransactionMetaId);
              }
            },
            (confirmedMeta) => confirmedMeta.id === transactionId,
          );
        }
  ...
  const handleQuotesErrorMetric = useCallback(
    (quoteError?: { key?: string; description?: string }) => {
      const data = {
        token_from: sourceToken.symbol,
        token_to: destinationToken.symbol,
        request_type: hasEnoughTokenBalance ? 'Order' : 'Quote',
  ...
  const buyEth = useCallback(() => {
    try {
      ...
      Logger.error(e as Error, 'Navigation: Error when navigating to buy ETH.');
    }
    ...
    () => {
      if (selectedQuote) {
        ...
        if (gasEstimateType === GAS_ESTIMATE_TYPES.ETH_GASPRICE) {
          ...
          gasEstimate = {
            gasPrice: (gasFeeEstimates as { gasPrice?: string }).gasPrice,
          ...
          gasEstimate = {
            gasPrice: (gasFeeEstimates as Record<string, string>)[selected],
            selected,
          };
          ...
          const feeMarketEstimates = gasFeeEstimates as Record<
            string,
            GasFeeEstimateLevel
          >;
          gasEstimate = {
            maxFeePerGas: feeMarketEstimates[selected].suggestedMaxFeePerGas,
            maxPriorityFeePerGas:
              feeMarketEstimates[selected].suggestedMaxPriorityFeePerGas,
            estimatedBaseFee: (gasFeeEstimates as { estimatedBaseFee?: string })
              .estimatedBaseFee,
            selected,
          };
        }
                ...
                >
                  ...
                  {usedGasEstimate.gasPrice ? (
                    <View style={styles.quotesFiatColumn}>
                      <Text primary bold>
                        {renderFromWei(toWei(selectedQuoteValue?.ethFee ?? '0'))}{' '}
                        {getTicker(ticker)}
                      </Text>
...
const mapStateToProps = (state: RootState): StateProps => ({
  ...
  quotes: selectSwapsQuotes(state) as unknown as Record<string, Quote>,
