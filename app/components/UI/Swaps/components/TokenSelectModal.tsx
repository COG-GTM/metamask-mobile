...
import useFetchTokenMetadata, {
  type TokenMetadata,
} from '../utils/useFetchTokenMetadata';
...
/**
 * A token from the token list, which carries balance information once it has
 * been sorted by fiat value.
 */
type SelectableToken = Token & Partial<TokenWithFiatValue>;

/**
 * The list ref also accepts the legacy `y` offset parameter.
 */
type TokenListRef = React.ElementRef<typeof FlatList> & {
  scrollToOffset(params: { animated?: boolean; y?: number }): void;
};
...
interface OwnProps {
  ...
  initialTokens: Token[];
  onItemPress: (token: SelectableToken | TokenMetadata) => void;
...
function TokenSelectModal({
  ...
  const list = useRef<TokenListRef>(null);
  ...
  const filteredInitialTokens = useMemo<SelectableToken[]>(
    () =>
  ...
  const tokenMetadataDetails = tokenMetadata.metadata;

  const renderItem = useCallback(
    ({ item }: { item: SelectableToken }) => {
      ...
      <SafeAreaView style={styles.modalView}>
        ...
        {shouldFetchToken ? (
          <View style={styles.resultsView}>
            {loadingTokenMetadata ? (
            ...
            ) : tokenMetadata.valid && tokenMetadataDetails ? (
              <View style={styles.resultRow}>
                <ListItem>
                  <ListItem.Content>
                    <ListItem.Icon>
                      <TokenIcon
                        medium
                        icon={tokenMetadataDetails.iconUrl}
                        symbol={tokenMetadataDetails.symbol}
                      />
                    </ListItem.Icon>
                    <ListItem.Body>
                      <ListItem.Title>
                        {tokenMetadataDetails.symbol}
                      </ListItem.Title>
                      {tokenMetadataDetails.name && (
                        <Text>{tokenMetadataDetails.name}</Text>
                      )}
                    </ListItem.Body>
                    <ListItem.Amounts>
                      <TouchableOpacity
                ...
                <TokenImportModal
                  ...
                  token={tokenMetadataDetails}
                  onPressImport={() =>
                    handlePressImportToken(tokenMetadataDetails)
                  }
                />
              </View>
            ) : (
