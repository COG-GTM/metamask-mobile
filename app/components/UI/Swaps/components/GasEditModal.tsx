...
/**
 * Shape of the gas fee controller estimates, which differs per estimate type:
 * fee-market keys the options by name, legacy keys gas prices by name and
 * eth_gasPrice exposes a single `gasPrice`.
 */
interface GasFeeEstimates {
  [option: string]: GasFeeEstimateOption | string | undefined;
...
/**
 * Hex amounts are returned either as strings or as BN instances depending on
 * the estimate type, and are only ever read through `toString(16)`.
 */
interface HexAmount {
...
interface EIP1559GasFeeSelection {
  suggestedMaxFeePerGas?: string;
  suggestedMaxPriorityFeePerGas?: string;
  suggestedGasLimit?: string;
  estimatedBaseFee?: string;
...
/**
 * `parseTransaction*` are untyped JS helpers whose destructured parameters are
 * all inferred as required; the swaps flow only ever passes the gas-related
 * subset (`{ onlyGas: true }`).
 */
type ParseTransactionEIP1559Params = Parameters<
  typeof parseTransactionEIP1559
>[0];
type ParseTransactionLegacyParams = Parameters<
  typeof parseTransactionLegacy
>[0];
...
interface GasEditModalProps {
   ...
   */
  onGasUpdate: (customGas: UpdatedGasFee, suggestedGasLimit?: string) => void;
  /**
    ...
    if (gasEstimateType === GAS_ESTIMATE_TYPES.FEE_MARKET) {
        ...
        parseTransactionEIP1559(
          {
            ...
            selectedGasFee: {
              suggestedMaxFeePerGas: (
                gasFeeEstimates[gasSelected] as GasFeeEstimateOption
              ).suggestedMaxFeePerGas,
            ...
            gasFeeEstimates,
          } as ParseTransactionEIP1559Params,
          { onlyGas: true },
        ),
      );
    } else {
      setLegacyTransactionDataTemp(
        parseTransactionLegacy(
          ...
          } as ParseTransactionLegacyParams,
          { onlyGas: true },
        ),
      );
    }
      ...
      >
        {gasEstimateType === GAS_ESTIMATE_TYPES.FEE_MARKET ? (
                  ...
                  <TouchableOpacity onPress={showGasFeeRecommendation}>
                    <Text noMargin link bold small centered>
                      ...
                      <MaterialCommunityIcon
                        name="information"
                        size={14}
                        // `styles` declares no `labelInfo`, so this has always
                        // resolved to `undefined`.
                        style={
                          (styles as Record<string, StyleProp<TextStyle>>)
                            .labelInfo
                        }
                      />
                    </Text>
          ...
          <EditGasFeeLegacy
            ...
            // `EditGasFeeLegacy` declares no `gasPriceConversion` prop, so this
            // has always been ignored; it is spread to keep the element as-is.
            {...({
