...
interface GasEditModalProps {
  ...
  gasFeeEstimates: Record<string, Record<string, string> & { estimatedBaseFee?: string; gasPrice?: string }>;
  defaultGasFeeOptionLegacy?: string;
  defaultGasFeeOptionFeeMarket?: string;
  isVisible: boolean;
  onGasUpdate: (gasParams: GasUpdateParams, gasLimit: string) => void;
...
function GasEditModal({
  ...
  const [EIP1559TransactionDataTemp, setEIP1559TransactionDataTemp] = useState<Record<string, string | boolean | undefined>>(
    {},
  );
  const [LegacyTransactionDataTemp, setLegacyTransactionDataTemp] = useState<Record<string, string | boolean | undefined>>(
    {},
  );
  const [
