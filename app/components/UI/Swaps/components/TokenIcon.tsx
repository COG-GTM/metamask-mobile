...
type TokenIconStyles = ReturnType<typeof createStyles> & {
  tokenSymbolBiggest?: StyleProp<TextStyle>;
};
...
function TokenIcon({
  ...
  const { colors } = useTheme();
  const styles = createStyles(colors) as TokenIconStyles;
  ...
  const getSource = useCallback((): ImageSourcePropType | null => {
    ...
    if (symbol && Object.keys(imageIcons).includes(symbol)) {
      return (imageIcons as Record<string, ImageSourcePropType>)[symbol];
    }
