...
interface AggregatorMetadataMap {
  [key: string]: AggregatorMetadataItem;
}

interface MetadataItem extends AggregatorMetadataItem {
  key: string;
}
...
interface LoadingAnimationProps {
  finish?: boolean;
  onAnimationEnd?: () => void;
  aggregatorMetadata?: AggregatorMetadataMap;
  headPan?: boolean;
}
...
function LoadingAnimation({
  ...
  const positions: PositionsMap = useMemo(
    () =>
      headPan
        ? metadata.reduce((acc: PositionsMap, curr, index) => {
  ...
  const opacities: OpacitiesMap = useMemo(
    () =>
      headPan
        ? metadata.reduce(
            (acc: OpacitiesMap, curr) => ({
              ...acc,
  ...
  const animationSequence = useMemo(
    () =>
      headPan
        ? [
            ...metadata.reduce(
              (acc: Animated.CompositeAnimation[], cur, index, array) => [
                ...acc,
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!aggregatorMetadata) {
        ...
        const metadataArray: MetadataItem[] = Object.entries(aggregatorMetadata).map(
          ([key, value]) => ({
            key,
            ...value,
          }),
    ...
    <View style={styles.screen}>
      ...
      <View style={styles.foxContainer} pointerEvents="none">
        <View style={styles.foxWrapper}>
          <Image
            source={foxImage as ImageSourcePropType}
