...
interface AggregatorMetadataValue {
  color: string;
  iconPng: string;
  title?: string;
}

interface AggregatorMetadataEntry extends AggregatorMetadataValue {
  key: string;
}
...
interface LoadingAnimationProps {
  finish?: boolean;
  onAnimationEnd?: () => void;
  aggregatorMetadata?: Record<string, AggregatorMetadataValue> | null;
  headPan?: boolean;
}
...
function LoadingAnimation({
  ...
  /* References */
  const foxRef = useRef<FoxRef | null>(null);
  const foxHeadPan = useRef(
    new (Animated.ValueXY as unknown as new (
      x: number,
      y: number,
    ) => Animated.ValueXY)(0, 0),
  ).current;
   ...
   */
  const positions = useMemo<Record<string, [number, number, number, number]>>(
    () =>
      headPan
  ...
  const opacities = useMemo<Record<string, Animated.Value>>(
    () =>
      headPan
        ? metadata.reduce(
            (acc, curr) => ({
      ...
      headPan
        ? [
            ...
            Animated.parallel([
              ...
              Animated.timing(
                opacities[[...metadata].pop()?.key as string],
                {
                  toValue: 0,
                  duration: PAN_DURATION,
                  useNativeDriver: true,
