...
type AggregatorMetadataItem = AggregatorInfo & { key: string };

type Positions = Record<string, [number, number, number, number]>;
type Opacities = Record<string, Animated.Value>;
...
interface LoadingAnimationProps {
   ...
   */
  aggregatorMetadata?: Record<string, AggregatorInfo>;
  /**
...
function LoadingAnimation({
  ...
  /* References */
  const foxRef = useRef<FoxRef | undefined>(undefined);
  ...
  const positions: Positions = useMemo(
    () =>
      headPan
        ? metadata.reduce<Positions>((acc, curr, index) => {
  ...
  const opacities: Opacities = useMemo(
    () =>
      headPan
        ? metadata.reduce<Opacities>(
            (acc, curr) => ({
              ...acc,
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!aggregatorMetadata) {
        ...
        const newMetadata = Object.entries(aggregatorMetadata).map(
          ([key, value]) => ({
            key,
            ...value,
          }),
