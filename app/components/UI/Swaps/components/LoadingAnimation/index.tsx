...
function LoadingAnimation({
  ...
  const [metadata, setMetadata] = useState<AggregatorMetadata[]>([]);
  const [shouldStart, setShouldStart] = useState<boolean>(false);
  const [hasStarted, setHasStarted] = useState<boolean>(false);
  const [hasFinished, setHasFinished] = useState<boolean>(false);
  const [hasStartedFinishing, setHasStartedFinishing] = useState<boolean>(false);
  ...
  /* References */
  const foxRef = useRef<{ injectJavaScript?: (script: string) => void; reload?: () => void }>();
  ...
  const opacities = useMemo(
    () =>
      headPan
        ? metadata.reduce(
            (acc: Record<string, Animated.Value>, curr) => ({
              ...acc,
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!aggregatorMetadata) {
        ...
        const aggregatorData = Object.entries(aggregatorMetadata).map(
          ([key, value]: [string, unknown]) => ({
            key,
            ...(value as Record<string, unknown>),
          }),
        );
        setMetadata(aggregatorData as AggregatorMetadata[]);
        setShouldStart(true);
      }
    })();
  ...
  useEffect(() => {
    const listener = currentQuoteIndexValue.addListener(({ value }: { value: number }) => {
  ...
  useEffect(() => {
    const listener = foxHeadPan.addListener(({ x, y }: { x: number; y: number }) => {
  ...
  return (
    <View style={styles.screen}>
      ...
      <View style={styles.foxContainer} pointerEvents="none">
        ...
        {renderLogos &&
          headPan &&
          metadata?.map((agg: AggregatorMetadata) => (
            <Animated.View
              key={agg.key}
              style={[
                styles.aggContainer,
