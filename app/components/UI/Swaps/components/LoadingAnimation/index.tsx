...
function LoadingAnimation({
  ...
  const [hasFinished, setHasFinished] = useState<any>(false);
  const [hasStartedFinishing, setHasStartedFinishing] = useState<any>(false);
  const [renderLogos, setRenderLogos] = useState<any>(false);
  const [currentQuoteIndex, setCurrentQuoteIndex] = useState<any>(0);
...
// @ts-expect-error -- legacy JavaScript UI type boundary
  const foxHeadPan = useRef<any>(new Animated.ValueXY(0, 0)).current;
  const currentQuoteIndexValue = useRef<any>(new Animated.Value(0)).current;
  const progressValue = useRef<any>(new Animated.Value(0)).current;
  ...
  const styles: any = createStyles(colors, shadows);
  ...
  const positions = useMemo(
    () =>
      headPan
        ? metadata.reduce((acc: any, curr: any, index: any) => {
    ...
    () =>
      headPan
        ? [
            ...
            ...metadata.reduce(
              (acc: any, cur: any, index: any, array: any) => [
                ...acc,
...
// @ts-expect-error -- legacy JavaScript UI type boundary
  ...
  useEffect(() => {
    const listener = currentQuoteIndexValue.addListener(({ value }: any) => {
  ...
  useEffect(() => {
    const listener = foxHeadPan.addListener(({ x, y }: any) => {
