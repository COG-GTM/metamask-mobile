...
import { Quote, QuoteValues } from '@metamask/swaps-controller/dist/types';
...
const calculateEthFee = calculateEthFeeForMultiLayer as (params: {
...
type SwapsQuote = Omit<Quote, 'destinationAmount'> & {
  ...
  priceSlippage?: {
    ...
    destinationAmountInETH?: string | number | null;
  };
};
...
function QuotesModal({
  ...
  useEffect(() => {
    setSelectedDetailsQuoteIndex(null);
  }, [quotes]);
  ...
  if (multiLayerL1ApprovalFeeTotal) {
    selectedDetailsQuoteValuesEthFee = calculateEthFee({
