...
import { Token } from './utils/token-list-utils';
import { TokenMetadata } from './utils/useFetchTokenMetadata';
...
const AnimatableViewWithRef = AnimatableView as React.ComponentType<
  React.ComponentProps<typeof AnimatableView> & {
    ref?: React.Ref<KeypadAnimatableView>;
  }
>;
...
type SwapsAmountRoute = RouteProp<
  Record<string, SwapsAmountRouteParams | undefined>,
  string
>;

interface KeypadAnimatableView {
  shake?: () => void;
}
...
interface SwapsAmountViewProps {
   ...
   */
  tokenExchangeRates: Record<string, { price?: number } | undefined>;
  /**
...
function SwapsAmountView({
  ...
  const [sourceToken, setSourceToken] = useState<Token | null | undefined>(() =>
    swapsTokens?.find((token) =>
      toLowerCaseEquals(token.address, initialSource),
    ),
  );
  ...
  const keypadViewRef = useRef<KeypadAnimatableView>(null);
  ...
  const canSetAnInitialSourceToken =
    ...
    initialSource &&
    swapsControllerTokens &&
    (swapsTokens?.length as number) > 0 &&
    !sourceToken;
  ...
  useEffect(() => {
    if (canSetAnInitialSourceToken) {
      setIsSourceSet(true);
      setSourceToken(
        (swapsTokens as Token[]).find((token) =>
          toLowerCaseEquals(token.address, initialSource),
        ),
      );
    }
  ...
  const currencyAmount = useMemo(() => {
    ...
    if (isSwapsNativeAsset(sourceToken)) {
      ...
      const exchangeRate =
        tokenExchangeRates && (sourceAddress as string) in tokenExchangeRates
          ? tokenExchangeRates[sourceAddress as string]?.price
          : undefined;
  ...
  const handleGetQuotesPress = useCallback(async () => {
    ...
    return navigation.navigate(
      ...
      setQuotesNavigationsParams(
        ...
        slippage,
        [sourceToken, destinationToken] as Parameters<
          typeof setQuotesNavigationsParams
        >[4],
      ),
    );
  }, [
  ...
  const handleSourceTokenPress = useCallback(
    (item: Token | TokenMetadata) => {
      toggleSourceModal();
      setSourceToken(item as Token);
  ...
  const handleDestinationTokenPress = useCallback(
    (item: Token | TokenMetadata) => {
      ...
      setDestinationToken(item as Token);
  ...
  const handleUseMax = useCallback(() => {
    ...
    setAmount(
      fromTokenMinimalUnitString(
        (balanceAsUnits as { toString: (radix?: number) => string }).toString(
          10,
        ),
        sourceToken.decimals,
      ),
  ...
  return (
    ...
    >
      <View style={styles.content}>
        ...
        >
          ...
          <TokenSelectModal
            ...
            tokens={swapsTokens as Token[]}
            initialTokens={tokensWithBalance as Token[]}
        ...
        >
          ...
          <TokenSelectModal
            ...
            tokens={swapsTokens as Token[]}
            initialTokens={[
              swapsUtils.getNativeSwapsToken(chainId) as unknown as Token,
              ...(tokensTopAssets as Token[])
        ...
        <View>
          ...
          !isSwapsNativeAsset(destinationToken) ? (
            destinationTokenHasEnoughOcurrances ? (
                ...
                <Text small centered>
                  <Text reset bold>
                    {strings('swaps.verified_on_sources', {
                      sources: (destinationToken as Token).occurrences,
                    })}
                  </Text>
              ...
              <ActionAlert
                type={
                  !(destinationToken as Token & { occurances?: number })
                ...
                {(textStyle) => (
                  ...
                  >
                    <Text style={textStyle} bold centered>
                      {!(destinationToken as Token).occurrences ||
                      isDynamicToken(destinationToken)
                        ? strings('swaps.added_manually', {
                            symbol: (destinationToken as Token).symbol,
                            // eslint-disable-next-line no-mixed-spaces-and-tabs
                          })
                        : strings('swaps.only_verified_on', {
                            symbol: (destinationToken as Token).symbol,
                            occurrences: (destinationToken as Token)
                              .occurrences,
                            // eslint-disable-next-line no-mixed-spaces-and-tabs
                          })}
                    </Text>
      ...
      <View
        ...
        pointerEvents={disabledView ? 'none' : 'auto'}
      >
        <AnimatableViewWithRef ref={keypadViewRef}>
          <Keypad
            onChange={handleKeypadChange}
            value={amount}
            currency="native"
          />
        </AnimatableViewWithRef>
