...
function SwapsAmountView({
  ...
  const previousSelectedAddress = useRef<any>();
...
// @ts-expect-error -- legacy JavaScript UI type boundary
  ...
  const [slippage, setSlippage] = useState<any>(AppConstants.SWAPS.DEFAULT_SLIPPAGE);
  const [isInitialLoadingTokens, setInitialLoadingTokens] = useState<any>(false);
  const [, setLoadingTokens] = useState<any>(false);
  const [isSourceSet, setIsSourceSet] = useState<any>(() =>
    Boolean(
  ...
  const [isDestinationSet, setIsDestinationSet] = useState<any>(false);
  ...
  const [hasDismissedTokenAlert, setHasDismissedTokenAlert] = useState<any>(true);
  ...
  const [isDirectWrapping, setIsDirectWrapping] = useState<any>(false);
...
// @ts-expect-error -- legacy JavaScript UI type boundary
              ...
              activeCurrency: swapsTokens?.find((token: any) =>
                toLowerCaseEquals(token.address, initialSource),
              )?.symbol,
...
// @ts-expect-error -- legacy JavaScript UI type boundary
  ...
  const handleSlippageChange = useCallback((value: any) => {
    setSlippage(value);
  }, []);
...
const mapDispatchToProps = (dispatch: any) => ({
  setLiveness: (chainId: any, featureFlags: any) =>
