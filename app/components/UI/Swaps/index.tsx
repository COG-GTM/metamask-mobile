...
type Token = SwapsToken & { occurances?: number };
...
function SwapsAmountView({
  ...
  const route =
    useRoute<RouteProp<{ params: SwapsAmountViewRouteParams }, 'params'>>();
  ...
  const keypadViewRef = useRef<
    (RNView & { shake?: () => Promise<unknown> }) | null
  >(null);
  ...
  const showMaxBalanceLink = shouldShowMaxBalanceLink({
    sourceToken: sourceToken as Token,
    shouldUseSmartTransaction,
    hasBalance,
  });
