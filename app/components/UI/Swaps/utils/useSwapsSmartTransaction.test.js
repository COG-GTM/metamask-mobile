...
import {
  getGasIncludedTransactionFees } from
...
describe('useSwapsSmartTransaction', () => {
  ...
  beforeEach(() => {
    ...
    selectEvmChainId.mockReturnValue(mockChainId);
    selectIsEIP1559Network.mockReturnValue(mockIsEIP1559Network);
    selectSwapsApprovalTransaction.mockReturnValue(null);

    // Setup controller mocks
    Engine.context.SmartTransactionsController.getFees.mockResolvedValue(mockSmartTransactionFees);
    Engine.context.TransactionController.approveTransactionsWithSameNonce.mockResolvedValue(['0xsignedtx1', '0xsignedtx2']);
  });
  ...
  it('should successfully submit a trade transaction without approval', async () => {
    ...
    Engine.context.SmartTransactionsController.submitSignedTransactions.mockResolvedValueOnce(mockTradeResponse);

    const { result } = renderHookWithProvider(() =>
    useSwapsSmartTransaction({
      quote: { trade: mockTradeTransaction },
  ...
  it('should successfully submit both approval and trade transactions', async () => {
    ...
    selectSwapsApprovalTransaction.mockReturnValue(mockApprovalTransaction);

    // Mock different UUIDs for approval and trade transactions
    Engine.context.SmartTransactionsController.submitSignedTransactions.mockResolvedValueOnce(mockApprovalResponse).
  ...
  it('should use gas-included transaction fees when quote has isGasIncludedTrade flag', async () => {
    ...
    getGasIncludedTransactionFees.mockReturnValue(mockGasIncludedFees);
    Engine.context.SmartTransactionsController.submitSignedTransactions.mockResolvedValueOnce(mockTradeResponse);
