...
export function isSwapsNativeAsset(token: SwapTokenBase | null | undefined) {
  return (
    Boolean(token) && token?.address === swapsUtils.NATIVE_SWAPS_TOKEN_ADDRESS
  );
}

export function isDynamicToken(token: SwapTokenBase | null | undefined) {
  return (
    Boolean(token) &&
    token.occurrences === 1 &&
 ...
 * @return {object} Object containing sourceTokenAddress, destinationTokenAddress, sourceAmount and slippage
 */
export function getQuotesNavigationsParams(route: { params?: Record<string, unknown> }) {
...
interface FetchParamsOptions {
  slippage?: number;
  sourceToken: SwapTokenBase;
  destinationToken: SwapTokenBase;
  sourceAmount: string;
...
interface MaxBalanceLinkParams {
  sourceToken: SwapTokenBase | null | undefined;
