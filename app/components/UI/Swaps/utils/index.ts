...
export function isSwapsAllowed(chainId: any) {
...
export function isSwapsNativeAsset(token: any) {
  return (
    Boolean(token) && token?.address === swapsUtils.NATIVE_SWAPS_TOKEN_ADDRESS
  );
}

export function isDynamicToken(token: any) {
  return (
    Boolean(token) &&
    token.occurrences === 1 &&
 ...
 */
export function setQuotesNavigationsParams(sourceTokenAddress: any, destinationTokenAddress: any, sourceAmount: any, slippage: any, tokens: any = [], ) {
  return {
    sourceTokenAddress,
...
export function useRatio(numeratorAmount: any, numeratorDecimals: any, denominatorAmount: any, denominatorDecimals: any, ) {
  const ratio = useMemo(
    () =>
...
export function getErrorMessage(errorKey: any) {
...
export function getQuotesSourceMessage(type: any) {
  switch (type) {
    case 'DEX': {
      return [
