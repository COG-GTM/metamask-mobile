...
fees) =>
{
  ...
  ); // fees is an array, so we will get an array back
...
export const useSwapsSmartTransaction = ({ quote, gasEstimates


}) => {
  const chainId = useSelector(selectEvmChainId);
  const isEIP1559Network = useSelector(selectIsEIP1559Network);
  const approvalTransaction = useSelector(selectSwapsApprovalTransaction);
  ...
  const submitSwapsSmartTransaction = async () => {
    ...
    if (quote?.isGasIncludedTrade) {
      smartTransactionFees = getGasIncludedTransactionFees(quote);
    }
