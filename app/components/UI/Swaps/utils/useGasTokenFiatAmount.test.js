...
describe('useGasTokenFiatAmount', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    hexToDecimal.mockReturnValue('123');
    toChecksumHexAddress.mockReturnValue(mockChecksumAddress);
    swapsUtils.calcTokenAmount.mockReturnValue(
      new BigNumber('0.000123')
    );
    toWei.mockReturnValue('123000000000000');
    weiToFiat.mockReturnValue('0.18');
  });
  ...
  it('should return undefined when checksumAddress is not available', () => {
    toChecksumHexAddress.mockReturnValue(null);
    ...
    expect(
      String(swapsUtils.calcTokenAmount.mock.calls[0][0])
    ).toEqual('123');
    expect(swapsUtils.calcTokenAmount.mock.calls[0][1]).toBe(18);
    expect(toWei).not.toHaveBeenCalled();
    expect(weiToFiat).not.toHaveBeenCalled();
  });
  ...
  it('should return empty string when weiToFiat returns falsy value', () => {
    weiToFiat.mockReturnValue(null);
  ...
  it('should handle invalid token address in fiatConversionRates', () => {
    toChecksumHexAddress.mockReturnValue('0xInvalidAddress');
    weiToFiat.mockReturnValue(null);
