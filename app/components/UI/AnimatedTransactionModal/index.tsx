...
class AnimatedTransactionModal extends PureComponent<AnimatedTransactionModalProps> {
  ...
  state = {
    ...
    rootHeight: null as number | null,
    customGasHeight: customGasHeightPlaceHolder,
    transactionReviewDataHeight: null as number | null,
  ...
  generateTransform = (
    valueType: XTranslationName | 'modal' | 'saveButton',
    outRange: number[],
  ) => {
    const { modalValue } = this.state;
    ...
    let value = this.reviewToEditValue;
    if (valueType === 'editToAdvanced') value = this.editToAdvancedValue;
  ...
  getAnimatedModalValueForAdvancedCG = () => {
    ...
    return 70 / ((rootHeight as number) - customGasHeight);
  };
  ...
  getTransformValue = () => {
    ...
    return (rootHeight as number) - customGasHeight;
  };
