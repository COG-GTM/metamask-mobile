...
export type TransactionModalMode = 'review' | 'edit';

export type XTranslationName =
  | 'reviewToEdit'
  | 'editToAdvanced'
  | 'reviewToData';
...
export interface AnimatedTransactionModalProps {
   ...
   */
  onModeChange?: (mode: TransactionModalMode) => void;
  /**
...
interface AnimatedTransactionModalState {
  ...
  toAdvancedFrom: TransactionModalMode;
...
> {
  ...
  onModeChange = (mode: TransactionModalMode) => {
    if (mode === 'edit') {
  ...
  generateTransform = (valueType: TransformValueType, outRange: number[]) => {
    const { modalValue } = this.state;
