 ...
 */
interface AccountApprovalPageInformation {
  url?: string;
  title?: string;
  icon?: string;
  origin?: string;
...
/**
 * The SDK API is declared with an options object, but the legacy call sites in
 * this screen pass positional arguments.
 */
const removeSdkChannel = (channelId?: string, sendTerminate?: boolean) => {
  const instance = SDKConnect.getInstance();
  return (
    instance.removeChannel as unknown as (
      channelId?: string,
      sendTerminate?: boolean,
    ) => void
  ).call(instance, channelId, sendTerminate);
};
...
interface AccountApprovalProps {
   ...
   */
  currentPageInformation: AccountApprovalPageInformation;
  /**
...
class AccountApproval extends PureComponent<AccountApprovalProps> {
  _isMounted?: boolean;
  ...
  componentDidMount = () => {
    ...
    const prefixedUrl = prefixUrlWithProtocol(currentPageInformation?.url as string);
    const { hostname } = new URLParse(prefixedUrl as string);
  ...
  onConfirm = () => {
    if (
      ...
      removeSdkChannel(this.props.currentPageInformation.channelId, true);
