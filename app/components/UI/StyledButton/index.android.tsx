...
type FontStyle = ReturnType<typeof getStyles>['fontStyle'];
...
export default class StyledButton extends PureComponent<StyledButtonProps> {
  static defaultProps = {
    // `React.PureComponent` declares no `defaultProps`; the spread is a no-op
    // kept from the JS original.
    ...(PureComponent as { defaultProps?: Partial<StyledButtonProps> })
      .defaultProps,
    styleDisabled: { opacity: 0.6 },
    disabledContainerStyle: { opacity: 0.6 },
  };

  renderGroupedChildren = (fontStyle: FontStyle) => {
