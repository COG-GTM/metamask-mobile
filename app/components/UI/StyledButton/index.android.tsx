...
interface StyledButtonAndroidProps extends AccessibilityProps {
  /**
...
interface AndroidTouchableProps {
  onPress?: () => void;
  onPressOut?: () => void;
}
 ...
 */
export default class StyledButton extends PureComponent<StyledButtonAndroidProps> {
  ...
  renderGroupedChildren = (fontStyle: StyleProp<TextStyle>[]) => {
    ...
    const children = coalesceNonElementChildren(
      this.props.children,
      (groupedChildren: unknown, index: number) => (
        <Text
          key={index}
          style={style}
          allowFontScaling={this.props.allowFontScaling}
        >
          {groupedChildren as React.ReactNode}
        </Text>
      ),
    ) as React.ReactNode[];
  ...
  render = () => {
    ...
    const touchableProps: AndroidTouchableProps = {};
