...
const createStyles = (colors: Theme['colors']) =>
  StyleSheet.create({
    ...
    warningContainer: {
      // `left` is not a valid React Native `alignItems` value, it is kept
      // as-is to preserve the existing runtime behavior.
...
interface PhishingModalProps {
  ...
  goBackToSafety?: () => void;
  /**
   * Called to the user decides to visit the ETH phishing detector
   */
  goToETHPhishingDetector?: () => void;
  /**
   * Called to the user decides to visit EtherScamDB
   */
  goToEtherscam?: () => void;
}
...
export default class PhishingModal extends PureComponent<PhishingModalProps> {
  ...
  render() {
    ...
    const urlObj = new URLParse(this.props.fullUrl as string);
