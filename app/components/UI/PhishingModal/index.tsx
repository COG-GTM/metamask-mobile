...
// ETHEREUM_DETECTION_TITLE is not declared by the test id module, it resolves
// to undefined at runtime as it did in the original JavaScript file.
...
const { ETHEREUM_DETECTION_TITLE } = ExternalWebsitesTestIds as Record<
  string,
  string
>;
...
const createStyles = (colors: Theme['colors']) =>
  StyleSheet.create({
    ...
    // `left` is not a valid `alignItems` value, kept as is to preserve rendering
    warningContainer: {
      alignItems: 'left',
...
interface PhishingModalProps {
  ...
  goBackToSafety?: () => void;
  /**
   * Called when the user wants to see the ETH phishing detector
   */
  goToETHPhishingDetector?: () => void;
  /**
   * Called when the user wants to see EtherScamDB
   */
  goToEtherscam?: () => void;
}
