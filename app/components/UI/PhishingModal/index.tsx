...
// @ts-expect-error - ETHEREUM_DETECTION_TITLE is not exported from the untyped testIds file; import preserved for behavior parity
...
const createStyles = (colors: Colors) =>
  StyleSheet.create({
    ...
    warningContainer: {
      // @ts-expect-error - 'left' is not a valid FlexAlignType, preserved for snapshot parity
      alignItems: 'left',
    },
    buttonWrapper: {
      marginTop: 32,
...
interface Props {
  ...
  /**
   * Called when the user decides to inspect the ETH phishing detector
   */
  goToETHPhishingDetector?: () => void;
  /**
   * Called when the user decides to view the Etherscam database
   */
  goToEtherscam?: () => void;
  /**
  ...
  return (
    <View style={styles.phishingModalWrapper}>
      ...
      <Button
        ...
        // @ts-expect-error - Button requires onPress, but it has always been optional here; passed through unchanged for behavior parity
        onPress={goBackToSafety}
