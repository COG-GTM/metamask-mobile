...
/** The thumbnail is a function component, but the list keeps refs to each one. */
const Thumbnail = TabThumbnail as unknown as React.FC<
  TabThumbnailProps & { ref?: React.Ref<View>; key?: React.Key }
>;
...
const createStyles = (colors: ThemeColors, shadows: Theme['shadows']) =>
  StyleSheet.create({
    noTabs: {
      flex: 1,
      alignItems: 'center',
...
interface TabsProps {
   ...
   * Opens a new tab
   */
  newTab: (...args: unknown[]) => void;
  /**
   * Closes a tab
   */
  closeTab: (...args: unknown[]) => void;
  /**
   * Closes all tabs
   */
  closeAllTabs: (...args: unknown[]) => void;
  /**
   * Dismiss the entire view
   */
  closeTabsView: (...args: unknown[]) => void;
  /**
   * Switches to a specific tab
   */
  switchToTab: (...args: unknown[]) => void;
  /**
   * Sets the current tab used for the animation
   */
  animateCurrentTab: (...args: unknown[]) => void;
...
class Tabs extends PureComponent<TabsProps, TabsState> {
  thumbnails: Record<string, React.RefObject<View>> = {};
  ...
  getStyles = () => {
    ...
    const shadows =
      (this.context as unknown as Theme)?.shadows || mockTheme.shadows;
  ...
  renderTabs(tabs: TabItem[], activeTab?: string) {
    const styles = this.getStyles();
      ...
      >
        {tabs.map((tab: TabItem) => (
          ...
          <Thumbnail
            ...
            key={tab.id}
            tab={tab as unknown as TabThumbnailProps['tab']}
            isActiveTab={activeTab === tab.id}
            onClose={
              this.props.closeTab as unknown as TabThumbnailProps['onClose']
            }
            onSwitch={
              this.onSwitch as unknown as TabThumbnailProps['onSwitch']
            }
          />
        ))}
      </ScrollView>
...
export default withMetricsAwareness(
  Tabs as unknown as ComponentType<IWithMetricsAwarenessProps>,
);
