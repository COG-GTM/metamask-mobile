...
type MetricsLike = IUseMetricsHook;

type TabsWithMetrics = React.ComponentType<
  Omit<Props, keyof IWithMetricsAwarenessProps>
>;
...
class Tabs extends PureComponent<Props, State> {
  ...
  createTabsRef(tabs: Tab[]) {
    tabs.forEach((tab) => {
      this.thumbnails[tab.id] = React.createRef<unknown>();
    });
  }
  ...
  renderTabs(tabs: Tab[], activeTab?: number | string) {
    const styles = this.getStyles();
