...
// TabThumbnail is a function component; the existing code attaches refs to it,
// which requires widening its props type to accept `ref`.
const TabThumbnailWithRef = TabThumbnail as React.ComponentType<
...
class Tabs extends PureComponent<TabsProps, TabsState> {
  ...
  componentDidMount() {
    if (this.props.tabs.length > TABS_VISIBLE) {
      ...
      let index = 0;
      this.props.tabs.forEach((tab, i: number) => {
