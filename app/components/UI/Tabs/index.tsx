...
// TabThumbnail is a memoized component whose typings do not expose a ref.
import TabThumbnailBase from './TabThumbnail';
...
const TabThumbnail = TabThumbnailBase as any;
...
interface TabsProps extends IWithMetricsAwarenessProps {
   ...
   */
  animateCurrentTab?: (tab: Tab) => void; // eslint-disable-line react/no-unused-prop-types
}
...
class Tabs extends PureComponent<TabsProps, TabsState> {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  thumbnails: Record<number, React.RefObject<any>> = {};
  ...
  componentDidMount() {
    if (this.props.tabs.length > TABS_VISIBLE) {
      ...
      let index = 0;
      this.props.tabs.forEach((tab: Tab, i: number) => {
...
export default // eslint-disable-next-line @typescript-eslint/no-explicit-any
  withMetricsAwareness(Tabs as any);
