...
/**
 * `createStyles` does not define a `checkIcon` entry, so the icons below are
 * rendered without a style. The optional member keeps that behaviour explicit.
 */
...
const getTitle = (
  status: string | undefined,
  {
    ...
    amount,
    assetType,
  }: Pick<BaseNotificationData, 'nonce' | 'amount' | 'assetType'>,
) => {
  switch (status) {
    case 'pending':
...
export const getDescription = (
  status: string | undefined,
  {
    amount = null,
    type = null,
  }: { amount?: string | null; type?: string | null },
) => {
