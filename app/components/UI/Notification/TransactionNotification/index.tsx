...
interface TransactionElementType {
  actionKey?: string;
  notificationKey?: string;
  [key: string]: unknown;
}

type TransactionDetailsType = Record<string, unknown>;
...
type TransactionNotificationProps = OwnProps & StateProps;
...
function TransactionNotification(props: TransactionNotificationProps) {
  ...
  const [transactionDetails, setTransactionDetails] = useState<
    TransactionDetailsType | undefined
  >(undefined);
  const [transactionElement, setTransactionElement] = useState<
    TransactionElementType | undefined
  >(undefined);
  ...
  const validateActionBalance = useMemo(
    () =>
  ...
  const onSpeedUpPress = useCallback(() => {
    const disabled = validateActionBalance(tx, SPEED_UP_RATE, accounts);
    setTransactionAction(ACTION_SPEEDUP);
    setTransactionActionDisabled(disabled);
  ...
  const onCancelPress = useCallback(() => {
    const disabled = validateActionBalance(tx, CANCEL_RATE, accounts);
  ...
  useEffect(() => {
    async function getTransactionInfo() {
      ...
      const [decodedElement, decodedDetails] = (await decodeTransaction({
        ...
        swapsTransactions,
        swapsTokens,
      } as unknown as Parameters<typeof decodeTransaction>[0])) as unknown as [
    ...
    <>
      <Animated.View
        style={[
          ...
          {
            transform: [
              { translateY: notificationAnimated as unknown as number },
            ],
          },
        ]}
      >
        <ElevatedView style={styles.elevatedView} elevation={100}>
          <BaseNotification
            status={currentNotification.status as string}
            data={{
              ...tx?.txParams,
...
export default connect<StateProps, object, OwnProps, RootState>(
  mapStateToProps as unknown as (state: RootState) => StateProps,
