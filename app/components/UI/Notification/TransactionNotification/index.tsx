 ...
 * flow but is not part of the controller's published state type.
 */
...
/**
 * The JSDoc of `validateTransactionActionBalance` does not match its
 * implementation: it takes a numeric rate and returns a boolean.
 */
...
function TransactionNotification(props: TransactionNotificationProps) {
  ...
  const onSpeedUpPress = useCallback(() => {
    const isActionDisabled = isTransactionActionDisabled(
      tx,
      SPEED_UP_RATE,
      accounts,
    );
  ...
  useEffect(() => {
    async function getTransactionInfo() {
      const currentTx = transactions.find(
        ({ id }: TransactionMeta) => id === currentNotification.transaction?.id,
      );
      if (!currentTx) return;
      const {
        selectedAddress,
      ...
      const [currentTransactionElement, currentTransactionDetails] =
        (await decodeTransaction({
          ...
          swapsTransactions,
          swapsTokens,
        })) as unknown as [TransactionElementData, TransactionDetailsData];
      const existingGasPrice = new BigNumber(
        currentTx?.txParams?.gasPrice || '0x0',
      );
      ...
      setTx(currentTx);
      setTransactionElement(currentTransactionElement);
...
const mapStateToProps = (state: RootState) => {
  ...
  const transactionControllerState =
    TransactionController as TransactionControllerStateWithSwaps;
  ...
  return {
    ...
    swapsTransactions: transactionControllerState.swapsTransactions || {},
