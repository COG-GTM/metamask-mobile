...
// The notification renders transaction data aggregated from several controllers,
// which is not strictly typed in the store yet.
...
interface TransactionNotificationProps {
  ...
  onClose: () => void;
  animatedTimingStart: (animated: SharedValue<number>, value: number) => void;
...
function TransactionNotification(props: TransactionNotificationProps) {
  ...
  const [transactionElement, setTransactionElement] = useState<
    AnyRecord | undefined
  >(undefined);
  const [tx, setTx] = useState<AnyRecord>({});
  ...
  useEffect(() => {
    async function getTransactionInfo() {
      const foundTx = transactions.find(
        ({ id }: AnyRecord) => id === currentNotification.transaction.id,
      );
      if (!foundTx) return;
      const {
        selectedAddress,
      ...
      const [newTransactionElement, newTransactionDetails] =
      ...
      setTx(foundTx);
      setTransactionElement(newTransactionElement);
  ...
  if (tx.status === 'submitted') {
    const smartTx = smartTransactions.find(
      (stx: AnyRecord) => stx.txHash === tx.hash,
    );
    if (smartTx) {
      return null;
    }
...
export default connect(mapStateToProps)(
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  TransactionNotification as any,
