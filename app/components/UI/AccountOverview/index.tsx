...
const createStyles = (colors: Colors) =>
  StyleSheet.create({
    ...
    netWorthContainer: {
      ...({ justifyItems: 'center' } as unknown as object),
      alignItems: 'center',
      flexDirection: 'row',
    },
...
class AccountOverview extends PureComponent<Props, AccountOverviewState> {
  ...
  editableLabelRef: RefObject<View> = React.createRef();
  scrollViewContainer: RefObject<View> = React.createRef();
  mainView: RefObject<View> = React.createRef();
  ...
  input: RefObject<TextInput> = React.createRef();
  ...
  setAccountLabel = () => {
    ...
    Engine.setAccountLabel(
      ...
      this.isAccountLabelDefined(accountLabel)
        ? accountLabel
        : (accountWithMatchingToAddress as InternalAccounts[number]).metadata
            .name,
    );
