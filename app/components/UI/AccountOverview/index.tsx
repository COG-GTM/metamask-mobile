...
type AccountOverviewInternalAccounts = Parameters<
  typeof renderAccountName
>[1];

interface AccountOverviewBrowserTab {
  id?: number;
  url?: string;
  [key: string]: unknown;
}
...
interface AccountOverviewProps {
   ...
   */
  internalAccounts: AccountOverviewInternalAccounts;
  /**
   ...
   */
  onRef: (...args: unknown[]) => void;
  /**
   * Prompts protect wallet modal
   */
  protectWalletModalVisible: (...args: unknown[]) => void;
  /**
   ...
   */
  browserTabs: AccountOverviewBrowserTab[];
  /**
...
class AccountOverview extends PureComponent<AccountOverviewProps> {
  state = {
  ...
  setAccountLabel = () => {
    ...
    const accountWithMatchingToAddress = internalAccounts.find(
      (account) => toLowerCaseEquals(account.address, selectedAddress),
    );
  ...
  onOpenPortfolio = () => {
    ...
    const existingPortfolioTab = browserTabs.find(
      (tab: AccountOverviewBrowserTab) => isPortfolioUrl(tab.url as string),
    );
    let existingTabId;
    let newTabUrl;
