...
const createStyles = (colors: Colors): any =>
  StyleSheet.create({
    scrollView: {
    ...
    netWorthContainer: {
      // @ts-expect-error justifyItems is not valid in React Native but exists in original code
      justifyItems: 'center',
      alignItems: 'center',
      flexDirection: 'row',
    },
...
class AccountOverview extends PureComponent<Props, ComponentState> {
  ...
  editableLabelRef = React.createRef<TextInput>();
  ...
  setAccountLabel = () => {
    ...
    const accountWithMatchingToAddress = internalAccounts.find((account: { address: string }) =>
  ...
  render() {
    ...
    const themeCtx = (this.context as { colors?: typeof mockTheme.colors; themeAppearance?: string }) || {};
    const colors = themeCtx.colors || mockTheme.colors;
    const themeAppearance = themeCtx.themeAppearance || 'light';
            ...
            >
              {accountLabelEditable ? (
                <TextInput
                  ...
                  keyboardAppearance={themeAppearance as 'default' | 'light' | 'dark'}
                />
              ) : (
...
const mapStateToProps = (state: RootState): StateProps => ({
  ...
  internalAccounts: selectInternalAccounts(state) as unknown as StateProps['internalAccounts'],
...
const mapDispatchToProps = (dispatch: (action: unknown) => void): DispatchProps => ({
  ...
  newAssetTransaction: (selectedAsset) =>
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    dispatch(newAssetTransaction(selectedAsset as any)),
});
...
export default (connect as any)(
  mapStateToProps,
  mapDispatchToProps,
)(withMetricsAwareness(AccountOverview as any));
