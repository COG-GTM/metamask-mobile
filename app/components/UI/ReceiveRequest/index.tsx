...
interface ReceiveRequestAlertConfig {
  isVisible: boolean;
  autodismiss: number;
  content: string;
  data: { msg: string };
...
interface ReceiveRequestProps extends IWithMetricsAwarenessProps {
   ...
   */
  showAlert: (config: ReceiveRequestAlertConfig) => void;
  /**
...
> {
  ...
  onBuy = async () => {
    ...
    if (!isNetworkBuySupported) {
      ...
      navigation.navigate(
        ...(createBuyNavigationDetails() as unknown as Parameters<
          NavigationProp<ParamListBase>['navigate']
        >),
      );
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
  showAlert: (config: ReceiveRequestAlertConfig) => dispatch(showAlert(config)),
