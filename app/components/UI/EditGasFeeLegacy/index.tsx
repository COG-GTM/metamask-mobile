...
export interface LegacyGasOptions {
  gasPrice?: string;
  low?: string;
  medium?: string;
  high?: string;
...
export interface EditGasFeeLegacyRecommended {
  name: string;
  render: React.ReactNode | (() => React.ReactNode);
}

type RangeInfoModalType = 'gas_limit' | 'gas_price' | false | null;

export interface EditGasFeeLegacyProps {
  selected?: string;
  gasFee: LegacyGasFee;
  gasOptions?: LegacyGasOptions;
  ...
  recommended?: EditGasFeeLegacyRecommended;
...
const EditGasFeeLegacy = ({
  ...
  const [showRangeInfoModal, setShowRangeInfoModal] =
    useState<RangeInfoModalType>(false);
              ...
              {!onlyAdvanced && (
                <View>
                  <HorizontalSelector
                    ...
                    disabled={false}
                    // Original JS passed the render function itself (not its
                    // result); kept identical to avoid a behaviour change.
                    options={
                      renderOptions as unknown as React.ComponentProps<
