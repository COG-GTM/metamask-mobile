...
interface LegacyGasFeeValues {
  suggestedGasLimit?: string;
  suggestedGasPrice?: string;
  [key: string]: unknown;
}

/** The shared selector requires `disabled`, which this screen never sets. */
const OptionSelector = HorizontalSelector as unknown as React.FC<
...
const EditGasFeeLegacy = ({
  ...
  const [selectedOption, setSelectedOption] = useState<string | null | undefined>(
    selected,
  );
  ...
  const toggleAdvancedOptions = () => {
    ...
    setShowAdvancedOptions((value) => !value);
  };
  ...
  const changeGas = (gas: Record<string, unknown>, option: string | null) => {
  ...
  const renderLabel = (isSelected: boolean, disabled: boolean, label: string) => (
  ...
  const renderOptions = () =>
    [
      ...
      .map(({ name, label, ...option }) => ({
        ...
        ...option,
        ...(extendOptions[name] as Record<string, unknown>),
      }));
    ...
    if (typeof warning === 'string')
      return (
        <Alert
          ...
          style={(styles as Record<string, ViewStyle>).warningContainer}
        >
          {() => (
