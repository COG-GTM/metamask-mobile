...
import {
  toTokenMinimalUnit,
  fromTokenMinimalUnitString } from
'../../../../../util/number';
...
const SelectorButton = BaseSelectorButton;
...
createNavigationDetails(Routes.RAMP.BUILD_QUOTE);
...
const BuildQuote = () => {
  ...
  const [amountBNMinimalUnit, setAmountBNMinimalUnit] = useState();
  ...
  const handleKeypadChange = useCallback(
    ({ value, valueAsNumber }) => {
      ...
      if (isSell) {
        setAmountBNMinimalUnit(
          toTokenMinimalUnit(`${value}`, selectedAsset?.decimals ?? 0)
        );
      }
    },
      ...
      <ScreenLayout>
        <ScreenLayout.Body>
          <ErrorView
            ...
            ctaOnPress={showPaymentMethodsModal}
            location={screenLocation} />
