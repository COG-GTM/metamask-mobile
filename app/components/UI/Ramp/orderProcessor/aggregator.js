...
export const aggregatorOrderToFiatOrder = (aggregatorOrder) => ({
...
options)
{
  ...
  if (
  options?.forced !== true &&
  (order.data?.pollingSecondsMinimum ?? 0) > 0 &&
  order.lastTimeFetched &&
  order.lastTimeFetched +
  (order.data?.pollingSecondsMinimum ?? 0) * 1000 >
  now)
  {
    return order;
  }
