...
import {
  ...
  setFiatOrdersGetStartedSell } from
'../../../../reducers/fiatOrders';
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        Logger.error(error, `RampSDKProvider SDK.regions() failed`);
        setSdkError(error);
      }
    })();
  }, []);

  const sdk = useMemo(() => sdkModule, [sdkModule]);

  const dispatch = useDispatch();

  const INITIAL_SELECTED_REGION = useSelector(
    fiatOrdersRegionSelectorAgg
  );
...
export const withRampSDK = (Component) => (props) =>
