...
interface CollectibleDetails {
  ...
  creator?: CollectibleCreator | string | null;
...
interface CollectibleOverviewProps {
  ...
  /**
   * Navigation object (passed through by consumers; unused internally)
   */
  navigation?: unknown;
  /**
   ...
   */
  onTranslation?: (isTranslating: boolean) => void;
}
...
const CollectibleOverview = ({
  ...
  const renderCollectibleInfoRow = useCallback(
    ({ key, value, onPress, type }: CollectibleInfoRowParams) => {
  ...
  const gestureHandlerWrapper = useCallback(
    (child: React.ReactNode) => (
      // @ts-expect-error - PanGestureHandler is not correctly typed and react-native-gesture-handler is outdated
      <PanGestureHandler
          ...
          <View style={styles.generalContainer}>
            {collectible?.creator && (
              <View style={styles.userContainer}>
                ...
                >
                  {(collectible.creator as CollectibleCreator).user
                    ?.username && (
            ...
            <Text primary noMargin big>
              ...
              {renderShortText(collectible.tokenId as unknown as string, 8)}
            </Text>
          </View>
