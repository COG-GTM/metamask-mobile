...
/** The gesture handler types omit `children`, which this wrapper requires. */
const GestureHandler = PanGestureHandler as unknown as React.FC<
 ...
 */
interface CollectibleOverviewCollectible {
  name?: string | null;
...
const CollectibleOverview = ({
  ...
  const renderCollectibleInfo = () => [
    ...
    renderCollectibleInfoRow({
      ...
      value:
        ...
        toLocaleDate(
          new Date(collectible?.lastSale?.event_timestamp as string),
        ).toString(),
      type: FieldType.Text,
    }),
    renderCollectibleInfoRow({
  ...
  const collectibleCreator =
    collectible?.creator && typeof collectible.creator === 'object'
      ? collectible.creator
      : undefined;
  ...
  const isCollectionIconRenderable = Boolean(
    displayNftMedia ||
      (!displayNftMedia &&
        ...
        isIPFSUri(collectible.logo as string)),
  );
          ...
          <View style={styles.generalContainer}>
            {collectible?.creator && (
              <View style={styles.userContainer}>
                ...
                <View
                  {...({ numberOfLines: 1 } as unknown as ViewProps)}
                  style={styles.userInfoContainer}
                >
                  {collectibleCreator?.user?.username && (
...
const mapStateToProps = (
  state: RootState,
  props: { collectible: CollectibleOverviewCollectible },
) => ({
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
  addFavoriteCollectible: (
    ...
    chainId: string | undefined,
    collectible: CollectibleOverviewCollectible,
  ) => dispatch(addFavoriteCollectibleAction(selectedAddress, chainId, collectible)),
  removeFavoriteCollectible: (
    selectedAddress: string | undefined,
    chainId: string | undefined,
    collectible: CollectibleOverviewCollectible,
