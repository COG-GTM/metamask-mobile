...
import { LastSale } from '@metamask/assets-controllers';
...
interface Collectible {
  ...
  logo?: string | null;
  contractName?: string;
  creator?: string | { user?: { username?: string } };
  lastSale?: LastSale & { event_timestamp?: string; total_price?: string };
}
...
/**
 * `View` accepts a `numberOfLines` prop in this legacy markup; it is not part
 * of the typed `View` props, so a typed alias is used to keep the markup as-is.
 */
const LegacyView = View as unknown as React.ComponentType<
  React.ComponentProps<typeof View> & { numberOfLines?: number }
>;

/**
 * `PanGestureHandler` does not declare `children` in its prop types with the
 * installed React types, so a typed alias is used to keep the markup as-is.
 */
const PanGestureHandlerCompat =
  PanGestureHandler as unknown as React.ComponentType<
    React.ComponentProps<typeof PanGestureHandler> & {
      children?: React.ReactNode;
      waitFor?: React.Ref<unknown>;
    }
  >;
  ...
  const renderCollectibleInfoRow = useCallback(
    ({ key, value, onPress, type }: CollectibleInfoRow) => {
      ...
      if (type === FieldType.Link) {
        if (!isLinkSafe(value as string)) return null;
      }
      return (
              ...
              <View style={styles.userContainer}>
                {isCollectionIconRenderable && (
                  <RemoteImage
                    ...
                    source={{ uri: collectible.logo as string }}
                    style={styles.userImage}
                  />
                )}
                <LegacyView numberOfLines={1} style={styles.userInfoContainer}>
                  {(collectible.creator as { user?: { username?: string } })
                    .user?.username && (
                    <Text black bold noMargin big={!IS_SMALL_DEVICE}>
                      {
                        (
                          collectible.creator as {
