...
const LegacyPanGestureHandler = PanGestureHandler as unknown as ComponentType<
  React.ComponentProps<typeof PanGestureHandler> & { children?: ReactNode }
>;
...
interface Collectible {
  ...
  lastSale?: CollectibleLastSale | null;
  creator?: string | { user?: { username?: string } } | null;
  [key: string]: unknown;
}
...
interface CollectibleOverviewOwnProps {
  /**
   * Navigation object, unused by this component but passed by callers
   */
  navigation?: unknown;
  /**
...
interface CollectibleOverviewProps extends CollectibleOverviewOwnProps {
  /**
   * Chain id
   */
  chainId?: string;
...
const CollectibleOverview = ({
  ...
  const creatorUsername =
    typeof collectible?.creator === 'object' && collectible?.creator !== null
  ...
  const gestureHandlerWrapper = useCallback(
    (child: ReactElement) => (
      <LegacyPanGestureHandler
        waitFor={scrollViewRef as unknown as React.RefObject<React.Component>}
        activeOffsetY={[0, 0]}
        activeOffsetX={[0, 0]}
...
) as unknown as ComponentType<CollectibleOverviewOwnProps>;
