...
interface CollectibleOverviewProps {
  ...
  /**
   * Navigation object passed by parent screens
   */
  navigation?: unknown;
}
...
const CollectibleOverview = ({
  ...
  const collectibleToFavorites = useCallback(() => {
    const action = isInFavorites
      ? removeFavoriteCollectible
      : addFavoriteCollectible;
    action?.(selectedAddress as string, chainId as string, collectible);
  }, [
    selectedAddress,
    chainId,
    collectible,
    ...
    (child: React.ReactElement) => (
      <PanGestureHandler
        {...({
          ...
          children: child,
        } as React.ComponentProps<typeof PanGestureHandler>)}
      />
    ),
