...
// Transaction, gas and controller shapes are highly dynamic, so they are
...
class Transactions extends PureComponent<TransactionsProps, TransactionsState> {
  ...
  existingGas: AnyRecord | null = null;
  existingTx: AnyRecord | null = null;
  ...
  getItemLayout = (
    _data: ArrayLike<AnyRecord> | null | undefined,
    index: number,
  ) => ({
    length: ROW_HEIGHT,
  ...
  keyExtractor = (item: AnyRecord) => item.id.toString();

  onSpeedUpAction = (
    speedUpAction: boolean,
    existingGas?: AnyRecord,
    tx?: AnyRecord,
  ) => {
    this.existingGas = existingGas as AnyRecord;
    this.speedUpTxId = tx?.id;
    this.existingTx = tx as AnyRecord;
  ...
  speedUpTransaction = async (transactionObject?: AnyRecord) => {
    try {
  ...
  signQRTransaction = async (tx: AnyRecord) => {
    const { KeyringController, ApprovalController } = Engine.context;
    await KeyringController.resetQRKeyringState();
    await ApprovalController.accept(tx.id, undefined, { waitForResult: true });
  };

  signLedgerTransaction = async (transaction: AnyRecord) => {
  ...
  cancelUnsignedQRTransaction = async (tx: AnyRecord) => {
  ...
  cancelTransaction = async (transactionObject?: AnyRecord) => {
    try {
      if (transactionObject?.error) {
  ...
  renderItem = ({ item, index }: { item: AnyRecord; index: number }) => (
    <TransactionElement
      tx={item}
      i={index}
    ...
    if (this.speedUpTxId) {
      InteractionManager.runAfterInteractions(() => {
        this.onSpeedUpAction(
          true,
          this.existingGas as AnyRecord,
          this.existingTx as AnyRecord,
        );
      });
    }
    if (this.cancelTxId) {
  ...
  renderList = () => {
    ...
    const transactions =
      submittedTransactions?.length
        ? submittedTransactions
            .sort((a: AnyRecord, b: AnyRecord) => b.time - a.time)
      ...
      <View style={styles.wrapper}>
        ...
        {!isSigningQRObject && this.state.cancelIsOpen && (
          <TransactionActionModal
            ...
            feeText={renderCancelGas() as string | undefined}
        ...
        {!isSigningQRObject && this.state.speedUpIsOpen && (
          <TransactionActionModal
            ...
            confirmButtonMode={'confirm'}
            cancelText={strings('transaction.nevermind')}
            feeText={renderSpeedUpGas() as string | undefined}
  ...
  getCancelOrSpeedupValues(transactionObject?: AnyRecord) {
  ...
  getGasPriceEstimate() {
    ...
    return addHexPrefix(
      decGWEIToHexWEI(estimateGweiDecimal as unknown as string) as string,
    );
  }
}
...
export default connect(
  ...
  mapDispatchToProps,
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
)(withQRHardwareAwareness(Transactions as any));
