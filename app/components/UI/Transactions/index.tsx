...
/**
 * `validateTransactionActionBalance` is a JS module whose JSDoc types do not
 * match its runtime signature, so it is re-typed locally.
 */
const validateTransactionActionBalance = (
  transaction: TransactionMetaLike,
  rate: number,
...
/** Values returned by the update-gas modals when confirming a cancel/speed up. */
interface CancelOrSpeedupObject {
  error?: unknown;
...
interface GasFeeEstimatesLike {
  medium?: { suggestedMaxFeePerGas?: string; [key: string]: unknown } | string;
  gasPrice?: string;
  [key: string]: unknown;
}
...
interface TransactionsProps {
   ...
   */
  submittedTransactions: TransactionMetaLike[];
  /**
   * An array of transactions objects that have been confirmed
   */
  confirmedTransactions: TransactionMetaLike[];
  /**
   ...
   */
  onScrollThroughContent: (...args: unknown[]) => void;
...
> {
  ...
  state: TransactionsState = {
    selectedTx: new Map<string, boolean>(),
    ready: false,
    refreshing: false,
    cancelIsOpen: false,
  ...
  mounted = false;
  scrolling = false;
  existingGas: ExistingGasLike | null = null;
  existingTx: TransactionMetaLike | null = null;
  cancelTxId: string | null = null;
  speedUpTxId: string | null = null;
  selectedTx: { id: string; index: number } | null = null;

  flatList = React.createRef<FlatList<TransactionMetaLike>>();
  ...
  updateBlockExplorer = () => {
    ...
    } else if (chainId && isNonEvmChainId(chainId)) {
  ...
  toggleDetailsView = (id: string, index: number) => {
    const selected = this.selectedTx;
    ...
    if (
      ...
      this.toggleDetailsView(selected.id, selected.index);
  ...
  getItemLayout = (
    _data: ArrayLike<TransactionMetaLike> | null | undefined,
    index: number,
  ) => ({
    length: ROW_HEIGHT,
  ...
  keyExtractor = (item: TransactionMetaLike) => item.id.toString();

  onSpeedUpAction = (speedUpAction: boolean, existingGas: ExistingGasLike, tx: TransactionMetaLike) => {
  ...
  onCancelAction = (cancelAction: boolean, existingGas: ExistingGasLike, tx: TransactionMetaLike) => {
    this.existingGas = existingGas;
  ...
  speedUpTransaction = async (transactionObject?: CancelOrSpeedupObject) => {
    try {
      if (transactionObject?.error) {
        throw new SpeedupTransactionError(transactionObject.error as string);
      }
  ...
  signQRTransaction = async (tx: TransactionMetaLike) => {
    const { KeyringController, ApprovalController } = Engine.context;
    await KeyringController.resetQRKeyringState();
    await ApprovalController.accept(tx.id, undefined, { waitForResult: true });
  };

  signLedgerTransaction = async (transaction: TransactionMetaLike) => {
    ...
    this.props.navigation.navigate(
      ...createLedgerTransactionModalNavDetails({
        ...
        replacementParams:
          transaction?.replacementParams as ReplacementTxParams | undefined,
      }),
    );
  };

  cancelUnsignedQRTransaction = async (tx: TransactionMetaLike) => {
  ...
  cancelTransaction = async (transactionObject?: CancelOrSpeedupObject) => {
    try {
      if (transactionObject?.error) {
        throw new CancelTransactionError(transactionObject.error as string);
      }
  ...
  renderList = () => {
    ...
    const renderRetryGas = (rate: number) => {
      ...
      const gasPrice = Number(this.existingGas.gasPrice);
  ...
  getCancelOrSpeedupValues(transactionObject?: CancelOrSpeedupObject) {
    ...
    if (Number(this.existingGas?.gasPrice) !== 0) {
