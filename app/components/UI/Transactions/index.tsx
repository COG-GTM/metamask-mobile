...
interface Tx {
  ...
  chainId?: string;
  time?: number;
  txParams?: { gas?: string; [key: string]: unknown };
  error?: unknown;
  [key: string]: unknown;
}
...
class Transactions extends PureComponent<TransactionsProps, TransactionsState> {
  ...
  toggleDetailsView = (id: string, index?: number) => {
    ...
    if (this.selectedTx && oldId !== id && oldIndex !== index) {
      ...
      this.toggleDetailsView(oldId as string, oldIndex as number | undefined);
  ...
  getItemLayout = (_data: ArrayLike<Tx> | null | undefined, index: number) => ({
    length: ROW_HEIGHT,
    offset: (this.props.headerHeight ?? 0) + ROW_HEIGHT * index,
    index,
  });

  keyExtractor = (item: Tx) => item.id.toString();
  ...
  onSpeedUpAction = (
    ...
    if (existingGas.isEIP1559Transaction) {
      ...
      const speedUpConfirmDisabled = (
        validateTransactionActionBalance as unknown as (
          transaction: unknown,
          rate: unknown,
          accounts: unknown,
        ) => boolean
      )(tx, SPEED_UP_RATE, this.props.accounts);
  ...
  onCancelAction = (
    ...
    if (existingGas.isEIP1559Transaction) {
      ...
      )(tx, CANCEL_RATE, this.props.accounts);
  ...
  cancelUnsignedQRTransaction = async (tx: Tx) => {
  ...
  renderItem = ({ item, index }: { item: Tx; index: number }) => {
    const elementProps = {
      tx: item,
      i: index,
    ...
    const renderRetryGas = (rate: number) => {
      ...
      const increasedGasPrice =
        gasPrice === 0
          ...
          : Math.floor((gasPrice as number) * rate);
  ...
  getGasPriceEstimate() {
    const gasFeeEstimates = this.props.gasFeeEstimates as {
