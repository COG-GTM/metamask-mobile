...
export interface TransactionsOwnProps {
  assetSymbol?: string;
  /**
   ...
   */
  submittedTransactions?: TransactionObject[];
  /**
   * An array of transactions objects that have been confirmed
   */
  confirmedTransactions: TransactionObject[];
  /**
   ...
   */
  onRefSet?: (ref: React.RefObject<FlatList<TransactionObject>>) => void;
  /**
...
type TransactionsStateProps = ReturnType<typeof mapStateToProps>;
type TransactionsDispatchProps = ReturnType<typeof mapDispatchToProps>;

type TransactionsProps = TransactionsOwnProps &
  TransactionsStateProps &
  TransactionsDispatchProps;
...
export interface CancelOrSpeedUpTransactionObject {
  error?: string;
  suggestedMaxFeePerGasHex?: string;
  suggestedMaxPriorityFeePerGasHex?: string;
}

interface LedgerSignableTransaction {
  id?: string | null;
...
class Transactions extends PureComponent<TransactionsProps, TransactionsState> {
  ...
  existingTx: TransactionObject | null = null;
  cancelTxId: string | null = null;
  speedUpTxId: string | null = null;
  selectedTx: SelectedTx | null = null;
  mounted = false;
  scrolling = false;

  flatList = React.createRef<FlatList<TransactionObject>>();

  getTheme(): Theme {
    return (this.context as Theme | undefined) || mockTheme;
  }
  ...
  init() {
    ...
    if (txToView) {
      setTimeout(() => {
        ...
        if (index >= 0) {
          this.toggleDetailsView(String(txToView), index);
        }
      }, 1000);
    }
  }
  ...
  toggleDetailsView = (id: string | undefined, index: number | undefined) => {
    const oldId = this.selectedTx?.id;
    const oldIndex = this.selectedTx?.index;

    if (this.selectedTx && oldId !== id && oldIndex !== index) {
      this.selectedTx = null;
      this.toggleDetailsView(oldId ?? undefined, oldIndex ?? undefined);
      ...
      this.setState((state) => {
        ...
        if (show && (this.props.headerHeight || index) && index !== undefined) {
          InteractionManager.runAfterInteractions(() => {
  ...
  renderLoader = () => {
    const { colors, typography } = this.getTheme();
  ...
  getItemLayout = (
    _data: ArrayLike<TransactionObject> | null | undefined,
    index: number,
  ) => ({
    length: ROW_HEIGHT,
  ...
  keyExtractor = (item: TransactionObject) => String(item.id);
  ...
  signQRTransaction = async (tx: TransactionObject) => {
    const { KeyringController, ApprovalController } = Engine.context;
    await KeyringController.resetQRKeyringState();
    await ApprovalController.accept(tx.id as string, undefined, {
      waitForResult: true,
    });
  };

  signLedgerTransaction = async (transaction: LedgerSignableTransaction) => {
  ...
  cancelUnsignedQRTransaction = async (tx: TransactionObject) => {
  ...
  renderItem = ({ item, index }: ListRenderItemInfo<TransactionObject>) => (
    <TransactionElement
      tx={item}
      i={index}
      ...
      conversionRate={this.props.conversionRate ?? undefined}
      currentCurrency={this.props.currentCurrency}
      navigation={
        this.props.navigation as unknown as NavigationProp<ParamListBase>
      }
      txChainId={item.chainId}
    />
  );

  toggleRetry = (errorMsg: string | undefined) =>
  ...
  getGasPriceEstimate() {
    ...
    const medium =
      gasFeeEstimates && 'medium' in gasFeeEstimates
        ? gasFeeEstimates.medium
        : undefined;
    const gasPrice =
      gasFeeEstimates && 'gasPrice' in gasFeeEstimates
        ? gasFeeEstimates.gasPrice
        : undefined;
    const estimateGweiDecimal =
      (typeof medium === 'object' ? medium.suggestedMaxFeePerGas : undefined) ??
      medium ??
      gasPrice ??
      '0';

    const hexWei = decGWEIToHexWEI(String(estimateGweiDecimal));
    return typeof hexWei === 'string' ? addHexPrefix(hexWei) : String(hexWei);
  }
}
...
type QRHardwareAwareProps = Parameters<
  typeof withQRHardwareAwareness
>[0] extends ComponentClass<infer P>
  ? P
  : never;
...
export default connect(
  ...
  withQRHardwareAwareness(
    Transactions as unknown as ComponentClass<QRHardwareAwareProps>,
  ),
);
