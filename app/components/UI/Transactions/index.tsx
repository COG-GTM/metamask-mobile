...
const createStyles = (colors: any, typography: any): any =>
  StyleSheet.create({
    wrapper: {
...
// @ts-expect-error -- legacy JavaScript UI type boundary
  ...
  toggleDetailsView = (id: any, index: any) => {
...
// @ts-expect-error -- legacy JavaScript UI type boundary
  ...
  signLedgerTransaction = async (transaction: any) => {
    ...
    const onConfirmation = (isComplete: any) => {
      if (isComplete) {
...
// @ts-expect-error -- legacy JavaScript UI type boundary
  ...
  toggleRetry = (errorMsg: any) =>
    this.setState((state: any) => ({ retryIsOpen: !state.retryIsOpen, errorMsg }));
