...
interface TransactionsProps {
  ...
  submittedTransactions?: TransactionLike[];
  confirmedTransactions?: TransactionLike[];
...
> {
  ...
  existingGas: GasObject | null = null;
  existingTx: TransactionLike | null = null;
  cancelTxId: string | null = null;
  speedUpTxId: string | null = null;
  selectedTx: SelectedTxRef | null = null;
  ...
  componentDidUpdate() {
    ...
    if (
      this.props.confirmedTransactions?.some(
        (transaction: TransactionLike) => transaction.id === this.existingTx?.id,
      )
    ) {
  ...
  init() {
    ...
    if (txToView) {
      setTimeout(() => {
        const index =
          this.props.transactions?.findIndex(
            (tx: TransactionLike) => txToView === tx.id,
          ) ?? -1;
        if (index >= 0) {
  ...
  toggleDetailsView = (id: string, index: number) => {
    ...
    if (this.selectedTx && oldId !== id && oldIndex !== index) {
      ...
      this.setState((state: TransactionsState) => {
  ...
  keyExtractor = (item: TransactionLike) => String(item.id);

  onSpeedUpAction = (
    speedUpAction: boolean,
    existingGas: GasObject,
    tx: TransactionLike,
  ) => {
  ...
  speedUpTransaction = async (transactionObject?: TransactionLike) => {
    try {
  ...
  signQRTransaction = async (tx: TransactionLike) => {
    const { KeyringController, ApprovalController } = Engine.context;
    await KeyringController.resetQRKeyringState();
    await ApprovalController.accept(tx.id, undefined, { waitForResult: true });
  };

  signLedgerTransaction = async (transaction: TransactionLike) => {
  ...
  cancelUnsignedQRTransaction = async (tx: TransactionLike) => {
  ...
  cancelTransaction = async (transactionObject?: TransactionLike) => {
    try {
      if (transactionObject?.error) {
  ...
  renderItem = ({ item, index }: ListRenderItemInfo<TransactionLike>) => (
    <TransactionElement
      tx={item}
      i={index}
    ...
    if (this.speedUpTxId) {
      InteractionManager.runAfterInteractions(() => {
        this.onSpeedUpAction(
          true,
          this.existingGas as GasObject,
          this.existingTx as TransactionLike,
        );
      });
    }
    if (this.cancelTxId) {
  ...
  renderList = () => {
    ...
    const transactions: TransactionLike[] =
      submittedTransactions?.length
        ? submittedTransactions
            .sort((a: TransactionLike, b: TransactionLike) => b.time - a.time)
  ...
  getCancelOrSpeedupValues(transactionObject?: TransactionLike) {
