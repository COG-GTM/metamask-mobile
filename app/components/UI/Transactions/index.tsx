...
import {
  ...
  type LedgerTransactionModalParams,
...
interface TransactionsProps {
  ...
  transactions?: TxItem[];
  submittedTransactions?: TxItem[];
  confirmedTransactions?: TxItem[];
  ...
  gasFeeEstimates?: {
    medium?: { suggestedMaxFeePerGas?: string };
    gasPrice?: string;
    [key: string]: unknown;
  };
  tokenChainId?: string;
...
> {
  ...
  updateBlockExplorer = () => {
    ...
    if (type === RPC) {
      blockExplorer =
        findBlockExplorerForRpc(rpcUrl, networkConfigurations as object) ||
  ...
  keyExtractor = (item: TxItem) => String(item.id);
  ...
  onSpeedUpAction = (
    ...
    if (existingGas.isEIP1559Transaction) {
      ...
      const speedUpConfirmDisabled = Boolean(
  ...
  onCancelAction = (
    ...
    if (existingGas.isEIP1559Transaction) {
      ...
      const cancelConfirmDisabled = Boolean(
        validateTransactionActionBalance(
          tx,
  ...
  handleSpeedUpTransactionFailure = (e: unknown) => {
    ...
    InteractionManager.runAfterInteractions(
      this.toggleRetry(message) as unknown as undefined,
    );
    this.setState({
      speedUp1559IsOpen: false,
      speedUpIsOpen: false,
  ...
  signQRTransaction = async (tx: TxItem) => {
  ...
  signLedgerTransaction = async (transaction: SignLedgerTx) => {
  ...
  cancelUnsignedQRTransaction = async (tx: TxItem) => {
  ...
  renderItem = ({ item, index }: { item: TxItem; index: number }) => {
    const elementProps = {
      tx: item,
      i: index,
    ...
    } as unknown as React.ComponentProps<typeof TransactionElement>;
    return <TransactionElement {...elementProps} />;
  };
    ...
    if (this.speedUpTxId) {
      InteractionManager.runAfterInteractions(() => {
        this.onSpeedUpAction(
          true,
          this.existingGas as ExistingGasLike,
          this.existingTx as TxItem,
        );
      });
    }
    if (this.cancelTxId) {
...
export default connect(
  ...
  withQRHardwareAwareness(
    Transactions as unknown as ComponentClass<{
      QRState?: IQRState;
      isSigningQRObject?: boolean;
