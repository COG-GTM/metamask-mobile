...
import { TransactionMetaLike } from '../../Transactions/types';
import Avatar, {
  AvatarSize,
  AvatarVariant,
...
const DetailsModal = DetailsModalBase as unknown as {
  ...
  Section: React.FC<{ children?: React.ReactNode; borderBottom?: boolean }>;
  Column: React.FC<{ children?: React.ReactNode; end?: boolean }>;
  SectionTitle: React.FC<{ children?: React.ReactNode; upper?: boolean }>;
};
...
const Text = TextBase as unknown as React.FC<
  Partial<TextProps> & { small?: boolean; primary?: boolean }
>;

/** Decoded transaction details rendered by this view. */
...
interface TransactionDetailsProps {
   ...
   */
  transactionObject: Partial<TransactionMetaLike> & { networkID?: string };
  /**
   ...
   */
  showSpeedUpModal: (...args: unknown[]) => void;
  showCancelModal: (...args: unknown[]) => void;
...
> {
  ...
  fetchTxReceipt = async (
    ...
    await query(getGlobalEthQuery(), 'getTransactionReceipt', [
      transactionHash,
    ]);
  ...
  getBlockExplorerForChain = (
    ...
    networkConfigurations: Record<string, NetworkConfigurationLike> | undefined,
  ) => {
    ...
    } else if (isLineaMainnetChainId(txChainId as `0x${string}`)) {
          ...
          <DetailsModal.Column>
            ...
            {!!txParams?.nonce && (
              <Text small primary>{`#${parseInt(
                (txParams.nonce as string).replace(
                  regex.transactionNonce,
                  '',
                ),
                16,
              )}`}</Text>
...
const mapStateToProps = (
  state: RootState,
  ownProps: { transactionObject: Partial<TransactionMetaLike> },
) => ({
...
type TransactionDetailsOwnProps = Partial<TransactionDetailsProps>;
