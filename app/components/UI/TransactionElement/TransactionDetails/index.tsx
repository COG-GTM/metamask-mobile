...
interface DetailsModalViewProps {
  children?: ReactNode;
  style?: StyleProp<ViewStyle>;
}

interface DetailsModalTextProps {
  children?: ReactNode;
...
const DetailsModal = DetailsModalBase as unknown as React.FC<{
  ...
  CloseIcon: React.FC<DetailsModalViewProps & { onPress?: () => void }>;
  Body: React.FC<DetailsModalViewProps>;
  Section: React.FC<DetailsModalViewProps & { borderBottom?: boolean }>;
  SectionTitle: React.FC<DetailsModalTextProps & { upper?: boolean }>;
  Column: React.FC<DetailsModalViewProps & { end?: boolean }>;
};
...
interface TransactionDetailsTransactionObject extends DecodedTransaction {
  time?: number;
  networkID?: string;
}
...
interface TransactionDetailsProps
  extends Omit<DecodeTransactionArgs, 'tx' | 'chainId'> {
  /**
  ...
  */
   ...
   */
  transactionObject: TransactionDetailsTransactionObject;
  /**
   ...
   */
  networkConfigurations?: Record<string, NetworkConfiguration | undefined>;
  /**
...
interface TransactionDetailsState {
  ...
  updatedTransactionDetails: TransactionDetailsType | undefined;
}
...
> {
  ...
  getBlockExplorerForChain = (
    ...
    networkConfigurations: TransactionDetailsProps['networkConfigurations'],
  ) => {
    // First check for network configuration block explorer
    let blockExplorer =
      networkConfigurations?.[txChainId as string]?.blockExplorerUrls?.[
        networkConfigurations[txChainId as string]
              ...
              <View style={styles.accountNameLabel}>
                <View style={styles.accountNameAvatar}>
                  <Avatar
                    ...
                    type={
                      (
                        AvatarAccountType as unknown as Record<
                          string,
                          AvatarAccountType
                        >
                      ).Jazzicon
...
const mapStateToProps = (
  state: RootState,
  ownProps: { transactionObject: TransactionDetailsTransactionObject },
) => ({
  ...
  shouldUseSmartTransaction: selectShouldUseSmartTransaction(
    state,
    ownProps.transactionObject.chainId as Hex,
  ),
});

TransactionDetails.contextType = ThemeContext;

export interface TransactionDetailsOwnProps {
  transactionObject: TransactionDetailsTransactionObject;
  transactionDetails: TransactionDetailsType;
  navigation?: TransactionDetailsProps['navigation'];
  chainId?: string;
  close?: () => void;
  showSpeedUpModal?: () => void;
  showCancelModal?: () => void;
...
export default connect(mapStateToProps)(
  withNavigation(
    TransactionDetails as unknown as React.FC<{
      navigation: CompatNavigationProp<NavigationProp<ParamListBase>>;
    }>,
  ),
) as unknown as React.ComponentType<TransactionDetailsOwnProps>;
