...
/**
 * These helpers consume and produce loosely shaped bags of values that come
 * from untyped controller state and are re-assembled across many branches, so
 * they are typed as open records rather than exhaustive shapes.
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type DecodeArgs = Record<string, any>;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type DecodeResult = Record<string, any>;


function calculateTotalGas(transaction: DecodeArgs) {
  const {
    gas,
    gasPrice,
    gasUsed,
...
function renderGwei(transaction: DecodeArgs) {
  const {
    gasPrice,
    estimatedBaseFee,
    maxFeePerGas,
...
function getTokenTransfer(args: DecodeArgs) {
  const {
    tx: {
  ...
  let transactionDetails: DecodeResult = {
...
function getCollectibleTransfer(args: DecodeArgs) {
  const {
    tx: {
  ...
  const ticker = networkConfigurationsByChainId?.[txChainId]?.nativeCurrency;
  const collectible = collectibleContracts.find((contract: DecodeArgs) =>
  ...
  let transactionDetails: DecodeResult = { renderValue: renderCollectible };
...
export function decodeIncomingTransfer(args: DecodeArgs) {
  const {
    tx: {
...
async function decodeTransferTx(args: DecodeArgs) {
  const {
    tx: {
      txParams,
...
function decodeTransferFromTx(args: DecodeArgs) {
  const {
    tx: {
      txParams,
...
function decodeDeploymentTx(args: DecodeArgs) {
  const {
    tx: {
      txParams,
...
function decodeConfirmTx(args: DecodeArgs) {
  const {
    tx: {
      txParams,
...
function decodeSwapsTx(args: DecodeArgs) {
  const {
    swapsTransactions,
    swapsTokens,
    conversionRate,
  ...
  const smartTransaction =
    Engine.context.SmartTransactionsController.state.smartTransactionsState.smartTransactions[
      chainId as `0x${string}`
    ]?.find((stx: DecodeArgs) => stx.txHash === hash);
  ...
  const sourceToken = swapsTokens?.find(
    ({ address }: DecodeArgs) => address === swapTransaction?.sourceToken?.address,
  );
  const destinationToken =
    swapTransaction?.destinationToken?.swaps ||
    swapsTokens?.find(
      ({ address }: DecodeArgs) => address === swapTransaction?.destinationToken?.address,
    );
 ...
 */
export default async function decodeTransaction(args: DecodeArgs) {
  const {
    tx,
    selectedAddress,
    chainId,
