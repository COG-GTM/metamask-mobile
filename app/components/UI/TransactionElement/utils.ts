...
function getTokenTransfer(args: any) {
  const {
    tx: {
...
function getCollectibleTransfer(args: any) {
  const {
    tx: {
  ...
  const ticker = networkConfigurationsByChainId?.[txChainId]?.nativeCurrency;
  const collectible = collectibleContracts.find((collectible: any) =>
  ...
  let transactionDetails: any = { renderValue: renderCollectible };
...
export function decodeIncomingTransfer(args: any) {
  const {
    tx: {
...
async function decodeTransferTx(args: any) {
  const {
    tx: {
      txParams,
...
function decodeTransferFromTx(args: any) {
  const {
    tx: {
      txParams,
...
function decodeDeploymentTx(args: any) {
  const {
    tx: {
      txParams,
...
function decodeConfirmTx(args: any) {
  const {
    tx: {
      txParams,
...
function decodeSwapsTx(args: any) {
  const {
    swapsTransactions,
    swapsTokens,
    conversionRate,
...
// @ts-expect-error -- legacy JavaScript UI type boundary
  ...
  const sourceToken = swapsTokens?.find(
    ({ address }: any) => address === swapTransaction?.sourceToken?.address,
  );
  const destinationToken =
    swapTransaction?.destinationToken?.swaps ||
    swapsTokens?.find(
      ({ address }: any) => address === swapTransaction?.destinationToken?.address,
    );
