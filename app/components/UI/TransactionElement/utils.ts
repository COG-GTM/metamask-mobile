...
type TransactionTxParams = TransactionParams & {
  gasUsed?: string;
...
type DecodeArgsWithGas = DecodeTransactionArgs & { totalGas: BN };

/**
 * Shape the decoding helpers assume at runtime, where the caller-provided
 * values are present.
 */
type ResolvedTxParams = TransactionTxParams &
  Required<
    Pick<
      TransactionTxParams,
      'from' | 'to' | 'data' | 'nonce' | 'gas' | 'gasPrice' | 'value'
    >
  >;

type ResolvedDecodeArgs = Required<
  Omit<
    DecodeTransactionArgs,
    'tx' | 'exchangeRate' | 'assetSymbol' | 'actionKey' | 'transactions'
  >
> &
  Pick<
    DecodeTransactionArgs,
    'exchangeRate' | 'assetSymbol' | 'actionKey' | 'transactions'
  > & {
    tx: DecodedTransaction & {
      txParams: ResolvedTxParams;
      transferInformation: Required<
        NonNullable<DecodedTransaction['transferInformation']>
      >;
    };
    totalGas: BN;
  };

function calculateTotalGas(transaction: TransactionTxParams) {
  const {
    gas,
    gasPrice,
    gasUsed,
  ...
  if (isEIP1559Transaction(transaction)) {
    ...
    return hexToBN(eip1559GasHex.gasFeeMinHex as string);
  }
  const gasBN = hexToBN(gas as string);
  const gasPriceBN = hexToBN(gasPrice as string);
  ...
  if (multiLayerL1FeeTotal) {
    totalGas = hexToBN(
      sumHexWEIs([BNToHex(totalGas), multiLayerL1FeeTotal]) as string,
    );
  }
  return totalGas;
}

function renderGwei(transaction: TransactionTxParams) {
  const {
    gasPrice,
    estimatedBaseFee,
    maxFeePerGas,
...
export function decodeIncomingTransfer(
  args: DecodeTransactionArgs & { totalGas?: BN | string },
): [TransactionElementType, TransactionDetailsType] {
  const {
    tx: {
...
function decodeConfirmTx(
  ...
  const tokenList: Record<string, { symbol?: string }> =
...
function decodeSwapsTx(
  ...
  const swapTransaction =
    swapsTransactions?.[id as string] ||
  ...
  const decimalDestinationAmount =
    ...
    renderFromTokenMinimalUnit(
      !!swapTransaction?.receivedDestinationAmount &&
        ...
        ? swapTransaction.receivedDestinationAmount
        : (swapTransaction.destinationAmount as unknown as number),
      swapTransaction.destinationToken.decimals,
    );
  ...
  const renderSourceTokenFiatNumber = balanceToFiatNumber(
    decimalSourceAmount as unknown as number,
  ...
  const renderDestinationTokenFiatNumber = balanceToFiatNumber(
    decimalDestinationAmount as unknown as number,
    conversionRate as number,
