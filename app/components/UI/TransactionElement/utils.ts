...
/**
 * The transaction decoding helpers operate on loosely-typed redux/controller
 * data whose shapes are not yet modelled in TypeScript. We use a permissive
 * index-signature type to preserve the existing runtime behavior while adding
 * explicit (if broad) parameter types.
 */
...
type TxElement = Record<string, unknown>;
...
function getTokenTransfer(args: DecodeArgs): [TxElement, TxDetails] {
  const {
    tx: {
  ...
  let transactionDetails: TxDetails = {
...
function getCollectibleTransfer(args: DecodeArgs): [TxElement, TxDetails] {
  const {
    tx: {
  ...
  let transactionDetails: TxDetails = { renderValue: renderCollectible };
...
async function decodeTransferTx(args: DecodeArgs) {
  ...
  let [transactionElement, transactionDetails]: [TxElement, TxDetails] =
    isCollectible
