...
/**
 * The decoding helpers below receive loosely shaped data coming from several
 * controllers and reducers, so their arguments are intentionally permissive.
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type AnyRecord = Record<string, any>;

export type DecodedTransaction = [AnyRecord, AnyRecord];

// eslint-disable-next-line @typescript-eslint/no-explicit-any
function calculateTotalGas(transaction: any) {
  const {
    gas,
    gasPrice,
    gasUsed,
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function renderGwei(transaction: any) {
  const {
    gasPrice,
    estimatedBaseFee,
    maxFeePerGas,
...
function getTokenTransfer(args: AnyRecord): DecodedTransaction {
  const {
    tx: {
  ...
  let transactionDetails: AnyRecord = {
...
function getCollectibleTransfer(args: AnyRecord): DecodedTransaction {
  const {
    tx: {
  ...
  const ticker = networkConfigurationsByChainId?.[txChainId]?.nativeCurrency;
  const collectible = collectibleContracts.find((item: AnyRecord) =>
  ...
  let transactionDetails: AnyRecord = { renderValue: renderCollectible };
...
export function decodeIncomingTransfer(args: AnyRecord): DecodedTransaction {
  const {
    tx: {
...
function decodeTransferFromTx(args: AnyRecord): DecodedTransaction {
  const {
    tx: {
      txParams,
...
function decodeDeploymentTx(args: AnyRecord): DecodedTransaction {
  const {
    tx: {
      txParams,
...
function decodeConfirmTx(args: AnyRecord): DecodedTransaction {
  const {
    tx: {
      txParams,
...
function decodeSwapsTx(
  ...
  const smartTransaction =
    Engine.context.SmartTransactionsController.state.smartTransactionsState.smartTransactions[
      chainId
    ]?.find((stx: AnyRecord) => stx.txHash === hash);
  ...
  const sourceToken = swapsTokens?.find(
    ({ address }: AnyRecord) =>
      address === swapTransaction?.sourceToken?.address,
  );
...
export default async function decodeTransaction(
  ...
  let transactionElement: AnyRecord | undefined;
  let transactionDetails: AnyRecord | undefined;
  ...
  return [transactionElement, transactionDetails] as DecodedTransaction;
}
