...
/**
 * `StatusText` and the `DetailsModal` parts are still JavaScript, so their
 * inferred props mark every destructured key (`context`, `style`) as required.
 * These aliases describe how they are actually called.
 */
const StatusTextItem = StatusText as unknown as React.ComponentType<{
  testID?: string;
  status: TransactionStatus;
  style?: StyleProp<TextStyle>;
}>;
const DetailsModalHeader =
  DetailsModal.Header as unknown as React.ComponentType<{
    children?: React.ReactNode;
  }>;
const DetailsModalTitle = DetailsModal.Title as unknown as React.ComponentType<{
  children?: React.ReactNode;
  onPress?: () => void;
}>;
const DetailsModalCloseIcon =
  DetailsModal.CloseIcon as unknown as React.ComponentType<{
    onPress?: () => void;
  }>;
...
/**
 * A transaction as consumed by this list item: `TransactionMeta` plus the flag
 * the transaction views set to mark where the "account imported" row goes.
 */
type TransactionElementTx = TransactionMeta &
  Record<string, unknown> & {
    insertImportTime?: boolean;
    isSmartTransaction?: boolean;
    txParams: TransactionMeta['txParams'] & Record<string, unknown>;
  };

/**
 * Props injected by `connect`, kept in sync with `mapStateToProps` below.
 */
...
/**
 * Shape produced by `decodeTransaction`, which is still JavaScript.
 */
...
interface TransactionElementProps extends StateProps {
  assetSymbol?: string;
  /**
        ...
        <>
          ...
          >
            <Text style={styles.importText}>
              ...
              <FAIcon
                name="info-circle"
                style={(styles as Record<string, TextStyle>).infoIcon}
              />
            </Text>
      ...
      <>
        ...
        {detailsModalVisible && (
          ...
          >
            <DetailsModal>
              <DetailsModalHeader>
                <DetailsModalTitle onPress={this.onCloseDetailsModal}>
                  {transactionElement?.actionKey}
                </DetailsModalTitle>
                <DetailsModalCloseIcon onPress={this.onCloseDetailsModal} />
              </DetailsModalHeader>
              <TransactionDetails
        ...
        >
          <DetailsModal>
            <DetailsModalHeader>
              <DetailsModalTitle onPress={this.onCloseImportWalletModal}>
                {strings('transactions.import_wallet_label')}
              </DetailsModalTitle>
              <DetailsModalCloseIcon onPress={this.onCloseImportWalletModal} />
            </DetailsModalHeader>
...
/**
 * Callers of this list item are still JavaScript and pass partially populated
 * transactions, so the boundary stays permissive while the class itself does not.
 */
type TransactionElementWithBridgeProps = Partial<
  Omit<TransactionElementProps, 'bridgeTxHistoryData' | 'tx'>
> & { tx: Partial<TransactionElementTx> };
