...
type DecodedElement = Record<string, unknown> & {
...
interface TransactionElementState {
  ...
  transactionElement?: DecodedElement;
...
> {
   ...
   */
  renderTxElement = (transactionElement: DecodedElement) => {
    const {
    ...
    if (isBridgeTransaction && bridgeTxHistoryItem) {
      title =
        getBridgeTxActivityTitle(bridgeTxHistoryItem as BridgeHistoryItem) ??
        title;
    }
    return (
      <>
            ...
            <ListItem.Body>
              ...
              !isBridgeComplete ? (
                <BridgeActivityItemTxSegments
                  bridgeTxHistoryItem={
                    bridgeTxHistoryItem as BridgeHistoryItem | undefined
                  }
...
const ConnectedTransactionElement = connect(mapStateToProps)(
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  TransactionElement as unknown as ComponentType<any>,
);

interface TransactionElementWrapperProps {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  tx: any;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  [key: string]: any;
...
export default TransactionElementWithBridge;
