...
// Some children below are (pre-existing) un-invoked render functions; React
// renders them as empty and the shallow snapshots capture them as-is.
const asRenderChild = (render: () => React.ReactNode) =>
  render as unknown as React.ReactNode;
...
export interface EIP1559GasOption {
  suggestedMaxFeePerGas?: string;
  suggestedMaxPriorityFeePerGas?: string;
  [key: string]: unknown;
}
...
export interface EditGasFee1559UpdateOption {
  ...
  maxFeeThreshold?: string | number;
  maxPriortyFeeThreshold?: string | number;
}
...
export interface EditGasFee1559Props {
   ...
   */
  gasOptions?: EIP1559GasOptions;
  /**
   * Function called when user selected or changed the gas
   */
  onChange?: (gas: EIP1559GasFee, selectedOption: string | null) => void;
  /**
   ...
   */
  extendOptions?: Record<string, { disabled?: boolean; error?: boolean }>;
  /**
...
const EditGasFee1559 = ({
  ...
  const [showInfoModal, setShowInfoModal] = useState<InfoModalType>(false);
  ...
  const changeGas = (gas: EIP1559GasFee, option: string | null) => {
  ...
  let gasFeePrimary: string | undefined,
    ...
    maxFeePerGasPrimary: string | undefined,
    maxPriorityFeePerGasPrimary: string | undefined,
    gasFeeMaxSecondary: string | undefined;
          ...
          <View>
            <View>
              <View style={styles.customGasHeader}>
                ...
                <Text bold black>
                  {asRenderChild(renderDisplayTitle)}
                </Text>
                <Icon
                  name={'arrow-back'}
                  size={24}
            ...
            {asRenderChild(renderWarning)}
            {asRenderChild(renderError)}
            <FadeAnimationView
