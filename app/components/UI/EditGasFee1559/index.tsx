...
// `disabled` is optional in practice for this legacy usage
const LegacyHorizontalSelector = HorizontalSelector as unknown as ComponentType<
  Partial<React.ComponentProps<typeof HorizontalSelector>>
>;

// the legacy markup passes a misspelled `disable` prop, kept for parity
const LegacyTouchableOpacity = TouchableOpacity as unknown as ComponentType<
...
interface EditGasFee1559Props {
   ...
   */
  recommended?: { name?: string; render?: ReactNode };
  /**
...
const EditGasFee1559 = ({
  ...
  const [showInfoModal, setShowInfoModal] = useState<string | null | boolean>(
    false,
  );
  ...
  const toggleAdvancedOptions = () => {
    ...
    setShowAdvancedOptions((show) => !show);
  };

  const toggleLearnMoreModal = () => {
    setShowLearnMoreModal((show) => !show);
  };
          ...
          <View>
            <View>
              <View style={styles.customGasHeader}>
                ...
                <Text bold black>
                  {renderDisplayTitle as unknown as ReactNode}
                </Text>
                <Icon
                  name={'arrow-back'}
                  size={24}
