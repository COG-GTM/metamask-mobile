...
const EditGasFee1559 = ({
  ...
  const [showInfoModal, setShowInfoModal] = useState<InfoModalName | boolean>(
    false,
  );
  ...
  // `disable` (rather than `disabled`) is kept from the JavaScript
  // implementation; `TouchableOpacity` ignores it, so renaming it would change
  // behaviour
  const advancedOptionsButtonProps = { disable: updateOption?.showAdvanced };
  ...
  const toggleLearnMoreModal = () => {
    setShowLearnMoreModal((currentValue) => !currentValue);
  };
