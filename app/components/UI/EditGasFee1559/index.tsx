...
interface GasFee {
  ...
  // callers pass the whole gas fee estimate, wait times included
  [key: string]: unknown;
}
...
const EditGasFee1559 = ({
  ...
  const changeGas = (gas: GasFee, option: string | null) => {
  ...
  const changedMaxFeePerGas = (value: string) => {
    const lowerValue = new BigNumber(
      gasOptions?.[warningMinimumEstimateOption]?.suggestedMaxFeePerGas as string,
    );
    const higherValue = new BigNumber(
    ...
    <View>
      ...
      >
        <View>
          ...
          {/* TODO: `disabled` is a required prop that has never been set here */}
        ...
        <View style={styles.advancedOptionsContainer}>
          <TouchableOpacity
            // TODO: `TouchableOpacity` has no `disable` prop; it is `disabled`
            // @ts-expect-error preserving existing behaviour
          ...
          <View>
            <View>
              <View style={styles.customGasHeader}>
                ...
                <Text bold black>
                  {/* TODO: this renders the function itself, not its result */}
                  {/* @ts-expect-error preserving existing behaviour */}
                  {renderDisplayTitle}
                </Text>
                <Icon
            ...
            {/* TODO: these render the functions themselves, not their result */}
            {renderWarning}
            {renderError}
            <FadeAnimationView
