...
interface CollectibleContractsProps {
  ...
  collectibles: Record<string, NftCollectible[]>;
  isNftFetchingProgress?: boolean;
  navigation?: CollectibleContractsNavigation;
  favoriteCollectibles: NftCollectible[];
...
const NftText = Text as React.ComponentType<
...
const debouncedNavigation = debounce(
  (navigation: CollectibleContractsNavigation, collectible: NftCollectible) => {
...
const CollectibleContracts = ({
  ...
  const collectibles = filteredCollectibles.filter(
    (singleCollectible: NftCollectible) =>
   ...
   */
  const shouldUpdateCollectibleMetadata = (collectible: NftCollectible) =>
  ...
  const updateAllCollectibleMetadata = useCallback(
    async (collectiblesToUpdate: NftCollectible[]) => {
      const { NftController } = Engine.context;
      // Filter out ignored collectibles
      const filteredcollectibles = collectiblesToUpdate.filter(
        (collectible: NftCollectible) => !isCollectibleIgnored(collectible),
      );

      // filter removable collectible
      const removable = filteredcollectibles.filter((single: NftCollectible) =>
        String(single.tokenId).includes('e+'),
      );
      const updatable = filteredcollectibles.filter(
        (single: NftCollectible) => !String(single.tokenId).includes('e+'),
      );

      removable.forEach((elm: NftCollectible) => {
        removeFavoriteCollectible(selectedAddress, chainId, elm);
      });

      filteredcollectibles.forEach((collectible: NftCollectible) => {
      ...
      if (updatable.length !== 0) {
        await NftController.updateNftMetadata({
          ...
          userAddress: selectedAddress as string,
        });
      }
    },
  ...
  const renderFavoriteCollectibles = useCallback(() => {
    const favoriteFilteredCollectibles = favoriteCollectibles.map(
      (collectible: NftCollectible) =>
        collectibles.find(
          ({ tokenId, address }: NftCollectible) =>
      ...
      Boolean(favoriteFilteredCollectibles.length) && (
        <CollectibleContractElement
          ...
          contractCollectibles={
            favoriteFilteredCollectibles as NftCollectible[]
          }
          collectiblesVisible
        />
      )
    ...
    () => (
      <View style={styles.emptyContainer}>
        <Image
          ...
          resizeMode={'contain'}
        />
        <NftText center style={styles.emptyTitleText} bold>
          {strings('wallet.no_nfts_yet')}
        </NftText>
        <NftText center big link onPress={goToLearnMore}>
