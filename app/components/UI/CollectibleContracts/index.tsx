...
const debouncedNavigation = debounce(
  (
    navigation: CollectibleContractsProps['navigation'],
    collectible: Nft,
  ): void => {
...
const CollectibleContracts: React.FC<CollectibleContractsProps> = ({
   ...
   */
  const shouldUpdateCollectibleMetadata = (collectible: Nft): boolean =>
  ...
  const updateAllCollectibleMetadata = useCallback(
    async (collectiblesToUpdate: Nft[]): Promise<void> => {
  ...
  const goToAddCollectible = useCallback((): void => {
  ...
  const renderCollectibleContract = useCallback(
    (item: NftContract, index: number): React.ReactElement => {
  ...
  const getNftDetectionAnalyticsParams = useCallback(
    (
      nft: Nft & { chainId?: string },
    ): { chain_id: number; source: string } | undefined => {
      try {
        return {
