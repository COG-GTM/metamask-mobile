...
/**
 * Collectibles rendered in the list are only guaranteed to carry an address and
 * a token id; the rest of the NFT metadata is filled in asynchronously.
 */
type CollectibleListItem = Partial<Nft> & { address: string; tokenId: string };
...
interface OwnProps {
  ...
  /**
   * Label of the tab this view is rendered in
   */
  tabLabel?: string;
}
...
interface StateProps {
  ...
  /**
   * Object of collectibleContract objects keyed by chain id
   */
  collectibleContracts: Record<string, CollectibleContract[]>;
  /**
   * Object of collectibles keyed by chain id
   */
...
type CollectibleContractsProps = OwnProps & StateProps & DispatchProps;

const debouncedNavigation = debounce(
  (navigation: Navigation | undefined, collectible: CollectibleListItem) => {
...
/**
 * The empty state passes `Base/Text` style props, which the React Native `Text`
 * ignores; they are kept to preserve the rendered output.
 */
const EmptyStateText = Text as unknown as React.FC<
  TextProps & {
    center?: boolean;
    big?: boolean;
    bold?: boolean;
...
const CollectibleContracts = ({
  ...
  const renderFavoriteCollectibles = useCallback(() => {
    const favorites = favoriteCollectibles.map((collectible) =>
      collectibles.find(
        ({ tokenId, address }) =>
    ...
    return (
      Boolean(favorites.length) && (
        <CollectibleContractElement
          ...
          key={'Favorites'}
          contractCollectibles={favorites}
          collectiblesVisible
        />
      )
    );
    ...
    () => (
      <View style={styles.emptyContainer}>
        <Image
          ...
          resizeMode={'contain'}
        />
        <EmptyStateText center style={styles.emptyTitleText} bold>
          {strings('wallet.no_nfts_yet')}
        </EmptyStateText>
        <EmptyStateText center big link onPress={goToLearnMore}>
