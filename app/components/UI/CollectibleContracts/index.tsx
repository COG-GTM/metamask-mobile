 ...
 */
/** Legacy call sites pass typography flags that React Native's Text ignores. */
const TextCompat = Text as unknown as React.FC<
...
/** The connected element is untyped for these legacy prop shapes. */
const ContractElement = CollectibleContractElement as unknown as React.FC<{
  onPress: (collectible: CollectibleLike) => void;
  asset: Record<string, unknown>;
  contractCollectibles?: (CollectibleLike | undefined)[];
...
interface CollectibleContractsProps {
   ...
   */
  collectibleContracts: Record<string, CollectibleContractLike[]>;
  /**
   * Array of collectibles objects
   */
  collectibles: Record<string, CollectibleLike[]>;
  /**
   ...
   */
  favoriteCollectibles: CollectibleLike[];
  /**
...
const CollectibleContracts = ({
   ...
   */
  const shouldUpdateCollectibleMetadata = (collectible: CollectibleLike) =>
  ...
  const updateAllCollectibleMetadata = useCallback(
    async (collectiblesToUpdate: CollectibleLike[]) => {
      const { NftController } = Engine.context;
      // Filter out ignored collectibles
      const filteredcollectibles = collectiblesToUpdate.filter(
        (collectible: CollectibleLike) => !isCollectibleIgnored(collectible),
      );

      // filter removable collectible
      const removable = filteredcollectibles.filter((single: CollectibleLike) =>
        String(single.tokenId).includes('e+'),
      );
      const updatable = filteredcollectibles.filter(
        (single: CollectibleLike) => !String(single.tokenId).includes('e+'),
      );

      removable.forEach((elm: CollectibleLike) => {
        removeFavoriteCollectible(selectedAddress, chainId, elm);
      });

      filteredcollectibles.forEach((collectible: CollectibleLike) => {
  ...
  const renderCollectibleContract = useCallback(
    (item: CollectibleContractLike, index: number) => {
  ...
  const renderFavoriteCollectibles = useCallback(() => {
    const mappedFavoriteCollectibles = favoriteCollectibles.map((collectible) =>
      collectibles.find(
        ({ tokenId, address }) =>
          compareTokenIds(
    ...
    return (
      Boolean(mappedFavoriteCollectibles.length) && (
        <ContractElement
          onPress={onItemPress}
          asset={{ name: 'Favorites', favorites: true }}
          key={'Favorites'}
          contractCollectibles={mappedFavoriteCollectibles}
          collectiblesVisible
        />
      )
    );
    ...
    () => (
      <View style={styles.emptyContainer}>
        <Image
          ...
          resizeMode={'contain'}
        />
        <TextCompat center style={styles.emptyTitleText} bold>
          {strings('wallet.no_nfts_yet')}
        </TextCompat>
        <TextCompat center big link onPress={goToLearnMore}>
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
  removeFavoriteCollectible: (selectedAddress: string, chainId: string, collectible: CollectibleLike) =>
...
export default connect(
  ...
  CollectibleContracts as unknown as React.FC<Record<string, unknown>>,
);
