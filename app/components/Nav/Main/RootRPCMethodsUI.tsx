...
const hstInterface = new ethers.utils.Interface(
  abi as ethers.utils.Fragment[],
);

/**
 * Loosely typed transaction params, as the runtime object carries extra
 * MetaMask specific fields that are not part of the controller type.
 */
...
interface LooseTransactionMeta {
  id: string;
  origin?: string;
  hash?: string;
  chainId?: string;
  ...
  txParams: LooseTxParams;
}

type SwapsTransactionsMap = Record<string, SwapsTransaction>;
...
const RootRPCMethodsUI = (props: RootRPCMethodsUIProps) => {
      ...
      try {
        ...
        trackEvent(
          createEventBuilder(event)
            ...
            .addSensitiveProperties({ ...sensitiveParameters } as JsonMap)
            .build(),
        );
      } catch (e) {
          ...
          (finishedTransactionMeta) => {
            if (finishedTransactionMeta.status === 'submitted') {
              NotificationManager.watchSubmittedTransaction({
                ...finishedTransactionMeta,
                assetType: (finishedTransactionMeta.txParams as LooseTxParams)
                  .assetType,
              });
            } else {
  ...
  const onUnapprovedTransaction = useCallback(
    async (transactionMetaOriginal: TransactionMeta) => {
      ...
      ) as unknown as LooseTransactionMeta;
      ...
      if (
        ...
        transactionMeta.txParams.gas = hexToBN(gas as string);
        ...
        if (
          ...
          transactionMeta.txParams.readableValue = fromWei(
            transactionMeta.txParams.value as BN,
          );
