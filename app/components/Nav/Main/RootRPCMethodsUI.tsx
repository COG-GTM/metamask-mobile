...
// @ts-expect-error ts(7016) human-standard-token-abi is not typed
...
/**
 * Transaction params are mutated in place (hex strings replaced with BN,
 * readable value added) before being dispatched to the transaction reducer.
 */
interface MutableTransactionMeta extends Omit<TransactionMeta, 'txParams'> {
  txParams: Omit<TransactionParams, 'gas' | 'gasPrice' | 'value'> & {
    gas?: string | BN;
    gasPrice?: string | BN;
    value?: string | BN;
...
const RootRPCMethodsUI = (props: RootRPCMethodsUIProps) => {
  ...
  const trackSwaps: TrackSwaps = useCallback(
    async (
      ...
      try {
        ...
        updateSwapsTransaction(
          transactionMeta.id,
          (swapsTransaction: SwapsTransaction) => {
  ...
  const autoSign = useCallback(
    async (transactionMeta: MutableTransactionMeta) => {
  ...
  const onUnapprovedTransaction = useCallback(
    async (transactionMetaOriginal: TransactionMeta) => {
      const transactionMeta: MutableTransactionMeta = cloneDeep(
