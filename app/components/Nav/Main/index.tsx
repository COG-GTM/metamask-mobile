...
interface MainProps {
  ...
  removeNotificationById: (id: Parameters<typeof removeNotificationById>[0]) => void;
  /**
...
interface PreviousNetworkConfig {
  chainId?: MainProps['chainId'];
  type?: ReturnType<typeof selectProviderConfig>['type'];
}
...
const Main: React.FC<MainProps> & { router?: unknown } = (props) => {
  ...
  const removeConnectionStatusListener =
    useRef<ReturnType<typeof NetInfo.addEventListener>>();
  ...
  const previousProviderConfig = useRef<PreviousNetworkConfig | undefined>(
    undefined,
  );
  const previousNetworkConfigurations = useRef<
    ReturnType<typeof selectNetworkConfigurations>
  >({});
  ...
  const hasNetworkChanged = useCallback(
    (
      // eslint-disable-next-line @typescript-eslint/no-shadow
      chainId: MainProps['chainId'],
      previousConfig: PreviousNetworkConfig | undefined,
      // eslint-disable-next-line @typescript-eslint/no-shadow
  ...
  const termsOfUse = useCallback(async () => {
    if (props.navigation) {
      await navigateTermsOfUse((key, params) =>
  ...
  const renderDeprecatedNetworkAlert = (
    // eslint-disable-next-line @typescript-eslint/no-shadow
    chainId: MainProps['chainId'],
    backUpSeedphraseVisible: boolean | undefined,
  ) => {
    if (
      DEPRECATED_NETWORKS.includes(
        chainId as (typeof DEPRECATED_NETWORKS)[number],
      ) &&
      showDeprecatedAlert
    ) {
...
Main.router = (MainNavigator as { router?: unknown }).router;
