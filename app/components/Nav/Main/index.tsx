...
interface MainProps {
  ...
  route: RouteProp<Record<string, { [key: string]: unknown }>, string>;
...
const Main = (props: MainProps) => {
  ...
  const removeConnectionStatusListener = useRef<NetInfoSubscription | undefined>();

  const { connectionChangeHandler } = useConnectionHandler(props.navigation);

  const removeNotVisibleNotificationsAction = props.removeNotVisibleNotifications;
  ...
  const checkInfuraAvailability = useCallback(async () => {
    if (props.providerType !== 'rpc') {
      try {
        ...
        if (error.message === AppConstants.ERRORS.INFURA_BLOCKED_MESSAGE) {
  ...
  const handleAppStateChange = useCallback(
    ...
    [backgroundMode, removeNotVisibleNotificationsAction],
  );
  ...
  const previousProviderConfig = useRef<{ chainId?: string; type?: string } | undefined>(undefined);
  const previousNetworkConfigurations = useRef<Record<string, NetworkConfiguration> | undefined>(undefined);
  ...
  const hasNetworkChanged = useCallback(
    (currentChainId: string, previousConfig: { chainId?: string; type?: string } | undefined, isEvmSelected: boolean) => {
...
const mapDispatchToProps = (dispatch: (action: unknown) => void) => ({
  ...
  showSimpleNotification: (args: SimpleNotificationArgs) => dispatch(showSimpleNotification(args)),
