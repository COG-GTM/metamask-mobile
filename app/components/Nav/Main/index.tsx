...
const NotificationComponent = Notification as any;
...
const Main = (props: MainProps) => {
  ...
  const hasNetworkChanged = useCallback(
    (
      ...
      isCurrentEvmSelected: boolean,
    ) => {
      if (!previousConfig) return false;

      return isCurrentEvmSelected
        ? currentChainId !== previousConfig.chainId ||
            providerConfig.type !== ('type' in previousConfig ? previousConfig.type : undefined)
        : currentChainId !== previousConfig.chainId;
    },
    [providerConfig.type],
  );
  ...
  const renderDeprecatedNetworkAlert = (
    deprecatedChainId: ReturnType<typeof selectChainId>,
    backUpSeedphraseVisible?: boolean,
  ) => {
    if (
      DEPRECATED_NETWORKS.includes(deprecatedChainId as `0x${string}`) &&
      showDeprecatedAlert
    ) {
  ...
  return (
    <React.Fragment>
      <View style={styles.flex}>
        ...
        <GlobalAlert />
        <FadeOutOverlay />
        <NotificationComponent navigation={props.navigation} />
        <RampOrders />
        <SwapsLiveness />
        <BackupAlert
...
(Main as typeof Main & { router?: unknown }).router = (
...
const mapDispatchToProps = (dispatch: any) => ({
  ...
  removeNotificationById: (id: unknown) =>
