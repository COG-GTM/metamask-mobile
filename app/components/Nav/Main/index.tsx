...
const MainNavigatorView = MainNavigator as unknown as React.ComponentType<{
  navigation: MainNavigation;
}>;
const RootRPCMethodsUIView =
  RootRPCMethodsUI as unknown as React.ComponentType<{
    navigation: MainNavigation;
  }>;

interface MainNavigation {
  navigate: (route: string, params?: NavigationParams) => void;
  [key: string]: unknown;
}
...
const Main = (props: MainProps) => {
  ...
  const handleRemoveNotVisibleNotifications =
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      removeConnectionStatusListener.current = NetInfo.addEventListener(
        connectionChangeHandler as unknown as Parameters<
  ...
  const termsOfUse = useCallback(async () => {
    if (props.navigation) {
      await navigateTermsOfUse(
        props.navigation.navigate as unknown as Parameters<
  ...
  const renderDeprecatedNetworkAlert = (
    ...
    if (
      DEPRECATED_NETWORKS.includes(networkChainId as `0x${string}`) &&
      showDeprecatedAlert
    ) {
  ...
  return (
    <React.Fragment>
      <View style={styles.flex}>
        {!forceReload ? (
          <MainNavigatorView navigation={props.navigation} />
        ) : (
          renderLoader()
        )}
        <GlobalAlert />
        ...
        <RootRPCMethodsUIView navigation={props.navigation} />
      </View>
    </React.Fragment>
  );
};
