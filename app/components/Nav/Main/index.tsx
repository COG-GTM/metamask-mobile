...
/**
 * Components that are not migrated to TypeScript yet but receive a
 * `navigation` prop.
 */
type NavigationAwareComponent = React.ComponentType<{
  navigation: NavigationProp<ParamListBase>;
}>;

const MainNavigatorWithNavigation =
  MainNavigator as unknown as NavigationAwareComponent;
const NotificationWithNavigation =
  Notification as unknown as NavigationAwareComponent;
...
const Main = (props: MainProps) => {
  ...
  useEffect(() => {
    if ((DEPRECATED_NETWORKS as string[]).includes(props.chainId)) {
  ...
  const previousProviderConfig = useRef<
    ProviderConfig | { chainId: string } | undefined
  >(undefined);
  const previousNetworkConfigurations =
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    useRef<Record<string, any> | undefined>(undefined);
  ...
  const hasNetworkChanged = useCallback(
    (
      ...
      previousConfig: ProviderConfig | { chainId: string } | undefined,
  ...
  useEffect(() => {
    if (
      ...
      if (isPortfolioViewEnabled()) {
        // `setTokenNetworkFilter` is not part of the controller type yet.
  ...
  const renderDeprecatedNetworkAlert = (
    ...
    if (
      (DEPRECATED_NETWORKS as string[]).includes(alertChainId) &&
      showDeprecatedAlert
    ) {
...
// Legacy static assignment kept for backwards compatibility with react-navigation v4 screens.
