...
interface NavigationOnlyProps {
  navigation: NavigationProp<ParamListBase>;
}

// MainNavigator and Notification are still untyped JS modules, so their props
// are inferred as empty; these aliases describe the props they receive here.
const MainNavigatorWithNavigation =
  MainNavigator as React.ComponentType<NavigationOnlyProps>;
const NotificationWithNavigation =
  Notification as React.ComponentType<NavigationOnlyProps>;
...
const Main = (props: MainProps) => {
  ...
  const removeConnectionStatusListener = useRef<() => void>();
  ...
  const previousProviderConfig = useRef<
    ProviderConfig | { chainId: SelectedChainId } | undefined
  >(undefined);
  ...
  const hasNetworkChanged = useCallback(
    (
      ...
      previousConfig: ProviderConfig | { chainId: SelectedChainId } | undefined,
      // eslint-disable-next-line @typescript-eslint/no-shadow
      ...
      return isEvmSelected
        ? chainId !== previousConfig.chainId ||
            providerConfig.type !== (previousConfig as ProviderConfig).type
        : chainId !== previousConfig.chainId;
    },
    [providerConfig.type],
  );
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      ...
      removeConnectionStatusListener.current = NetInfo.addEventListener(
        connectionChangeHandler as NetInfoChangeHandler,
      );
    }, 1000);
  ...
  const renderDeprecatedNetworkAlert = (
    ...
    if (DEPRECATED_NETWORKS.includes(chainId as Hex) && showDeprecatedAlert) {
...
Main.router = (MainNavigator as unknown as { router: unknown }).router;
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
  ...
  removeNotificationById: (id: number) => dispatch(removeNotificationById(id)),
