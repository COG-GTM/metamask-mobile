...
interface RouteProps<P = Record<string, unknown>> {
  route: { params?: P };
}

type NavOptions = React.ComponentProps<typeof Stack.Screen>['options'];

const navOpts = (comp: unknown): NavOptions =>
  (comp as { navigationOptions?: NavOptions } | undefined)?.navigationOptions;
...
const clearStackNavigatorOptions: StackNavigationOptions = {
  ...
  cardStyle: {
    ...
    // @ts-expect-error - cardStyleInterpolator is supplied via cardStyle here
    // for legacy compatibility with the existing JS implementation.
...
const AssetStackFlow = (props: RouteProps<{ address?: string }>) => (
  <Stack.Navigator>
    <Stack.Screen
      name={'Asset'}
      component={Asset}
...
const AssetModalFlow = (props: RouteProps) => (
  <Stack.Navigator
    mode={'modal'}
...
const BrowserFlow = (props: RouteProps) => (
  <Stack.Navigator
...
interface DrawerContextValue {
  drawerRef: RefObject<DrawerHandle> | null;
}
...
const NotificationsOptInStack = () => (
  <Stack.Navigator initialRouteName={Routes.NOTIFICATIONS.OPT_IN}>
    <Stack.Screen
      // @ts-expect-error legacy: `mode` is not a valid Stack.Screen prop and is
      // ignored at runtime; preserved here to avoid behaviour changes during
      // the JS → TS migration.
      mode={'modal'}
...
const HomeTabs = () => {
  ...
  type ExtendedTabOptions = BottomTabNavigationOptions & {
  ...
  const options: Record<string, ExtendedTabOptions> = {
    home: {
  ...
  return (
    <DrawerContext.Provider value={{ drawerRef }}>
      <Drawer ref={drawerRef as React.Ref<never>}>
        <Tab.Navigator
...
const Webview = () => (
  <Stack.Navigator>
    <Stack.Screen
      ...
      // @ts-expect-error legacy: see NotificationsOptInStack for context
      mode={'modal'}
...
const NftDetailsModeView = (
  props: RouteProps<{ collectible?: unknown }>,
) => (
  <Stack.Navigator>
    <Stack.Screen
...
const NotificationsModeView = (_props: RouteProps) => (
  <Stack.Navigator>
    <Stack.Screen
...
const MainNavigator = () => (
  ...
  >
    ...
    <Stack.Screen
      ...
      // @ts-expect-error legacy: `headerTitle`/`headerStyle` are not valid
      // direct props on Stack.Screen and are ignored at runtime; preserved
      // here to avoid behaviour changes during the JS → TS migration.
      headerTitle={() => (
        <Image
    ...
    <Stack.Screen
      ...
      options={{
        headerShown: true,
        ...(navOpts(GeneralSettings) as object | undefined),
      }}
    />
    <Stack.Screen
