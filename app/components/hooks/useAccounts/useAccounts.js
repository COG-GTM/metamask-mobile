...
const useAccounts = ({
  ...
  const checkBalanceError = useCallback(
    (balance) => checkBalanceErrorFn?.(balance),
  ...
  const fetchENSNames = useCallback(
    async ({
      ...
      const latestENSbyAccountAddress = {};
      ...
      const fetchENSName = async (accountIndex) => {
  ...
  const getAccounts = useCallback(() => {
    ...
    let yOffset = 0;
    let selectedIndex = 0;
    const flattenedAccounts = internalAccounts.map(
      (internalAccount, index) => {
        ...
        const mappedAccount = {
          ...
          type: internalAccount.metadata.keyring.type,
          yOffset,
          isSelected,
