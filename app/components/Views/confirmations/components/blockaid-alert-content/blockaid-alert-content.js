...
const getReportUrl = (encodedData) =>
`${FALSE_POSITIVE_REPORT_BASE_URL}?data=${encodeURIComponent(
  encodedData.toString()
)}&utm_source=${UTM_SOURCE}`;

const BlockaidAlertContent = ({ alertDetails, securityAlertResponse, onContactUsClicked }) => {
  ...
  return (
    <>
      <Text variant={DEFAULT_BANNERBASE_DESCRIPTION_TEXTVARIANT}>
        {strings(REASON_DESCRIPTION_I18N_KEY_MAP[securityAlertResponse.reason])}
      </Text>
      <Accordion
