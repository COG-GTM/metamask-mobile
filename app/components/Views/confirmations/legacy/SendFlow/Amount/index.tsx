...
// @ts-expect-error - no type declarations for this internal RN utility
...
type AmountAsset = AmountSelectedAsset;
...
interface AmountTransactionState {
  ...
  selectedAsset: AmountSelectedAsset;
...
type GasBN = ReturnType<typeof hexToBN>;
...
interface AmountState {
  ...
  estimatedTotalGas?: GasBN | string;
...
class Amount extends PureComponent<AmountProps, AmountState> {
  ...
  componentDidMount = async () => {
    ...
    this.tokens = [getEther(ticker), ...tokens] as AmountAsset[];
    ...
    if (internalPrimaryCurrencyIsCrypto) {
      ...
      if (maxFiatInput) {
        value = `${renderFromWei(
          ...
          ) as unknown as Parameters<typeof renderFromWei>[0],
          18,
        )}`;
      }
    }
  ...
  prepareTransaction = async (value?: string) => {
    ...
    if (isNativeToken(selectedAsset as unknown as TokenI)) {
      ...
      transaction.data = generateTransferData('transfer', {
        toAddress: transactionTo,
        amount: BNToHex(tokenAmount as Parameters<typeof BNToHex>[0]),
      });
    ...
    if (isDecimal(value as string)) {
      ...
      if (!amountError) {
        if (isNativeToken(selectedAsset as unknown as TokenI)) {
          ...
          weiInput = (weiValue as GasBN).add(estimatedTotalGas as GasBN);
        } else {
  ...
  useMax = () => {
    ...
    const conversionRate = this.props.conversionRate as number;
    ...
    if (isNativeToken(selectedAsset as unknown as TokenI)) {
      ...
      const realMaxValue = balanceBN.sub(estimatedTotalGas as GasBN);
      const maxValue =
  ...
  onInputChange = (
    ...
    const { contractExchangeRates, currentCurrency, ticker, setMaxValueMode } =
      this.props;
    const conversionRate = this.props.conversionRate as number;
      ...
      if (internalPrimaryCurrencyIsCrypto) {
        ...
        inputValueConversion = `${renderFromTokenMinimalUnit(
          fiatNumberToTokenMinimalUnit(
            ...
            exchangeRate,
            selectedAsset.decimals,
          ) as unknown as Parameters<typeof renderFromTokenMinimalUnit>[0],
          selectedAsset.decimals,
        )}`;
    ...
    this.setState({
      ...
      amountError: undefined,
      hasExchangeRate,
      maxFiatInput: (!useMax && undefined) as unknown as string | undefined,
    });
  };
  ...
  handleSelectedAssetBalance = (
    ...
    } else if (isNativeToken(selectedAsset as unknown as TokenI)) {
  ...
  renderToken = (token: AmountAsset, index: number) => {
    ...
    if (isNativeToken(token as unknown as TokenI)) {
      balance = renderFromWei(accounts[selectedAddress].balance as string);
      ...
      const exchangeRate = (
        contractExchangeRates ? contractExchangeRates[address]?.price : undefined
      ) as number;
    ...
    return (
      ...
      >
        <View style={styles.assetElement}>
          {isNativeToken(token as unknown as TokenI) ? (
  ...
  processCollectibles = () => {
    ...
    sortedCollectibles.forEach((collectible) => {
      ...
      const isTradable =
        !transferInfo[address] || transferInfo[address]?.tradable;
  ...
  renderTokenInput = () => {
    ...
    const colors = (this.context as unknown as Theme).colors || mockTheme.colors;
    const themeAppearance = (this.context as unknown as Theme).themeAppearance || 'light';
    const styles = createStyles(colors);
          ...
          <View style={styles.actionsWrapper}>
            <View style={styles.action}>
              ...
              >
                ...
                <View {...({ styles: styles.switchWrapper } as object)}>
      ...
      <View style={styles.collectibleInputWrapper}>
        <View style={styles.collectibleInputImageWrapper}>
          <CollectibleMedia
            small
            collectible={
              selectedAsset as unknown as React.ComponentProps<
                typeof CollectibleMedia
              >['collectible']
            }
          />
...
export default connect(
  ...
  withMetricsAwareness(
    Amount as unknown as Parameters<typeof withMetricsAwareness>[0],
  ),
);
