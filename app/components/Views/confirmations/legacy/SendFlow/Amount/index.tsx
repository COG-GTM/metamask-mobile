...
interface AmountProps extends OwnProps, StateProps, DispatchProps {}
...
interface AmountState {
  ...
  maxFiatInput?: string | boolean;
  currentBalance?: string;
}

const asTokenI = (asset: AmountAsset): TokenI => asset as unknown as TokenI;
...
class Amount extends PureComponent<AmountProps, AmountState> {
  ...
  validateAmount = (
    ...
    if (isDecimal(value as string)) {
      ...
      let weiValue: BN4 = 0 as unknown as BN4;
      try {
    ...
    if (isNativeToken(asTokenI(selectedAsset))) {
      ...
      if (internalPrimaryCurrencyIsCrypto) {
        ...
        inputValueConversion = `${renderFromWei(
          fiatNumberToWei(processedInputValue, conversionRate) as unknown as BN4,
        )}`;
  ...
  processCollectibles = () => {
    ...
    sortedCollectibles.forEach((collectible) => {
      ...
      const asset = collectible as unknown as AmountAsset;
      if (!asset.name) asset.name = collectibleContract?.name;
      if (!asset.image) asset.image = collectibleContract?.logo;
          ...
          <View style={styles.actionsWrapper}>
            <View style={styles.action}>
              ...
              >
                ...
                {/* @ts-expect-error legacy: invalid `styles` prop preserved for snapshot parity */}
            ...
            >
              {() => (
                <View style={styles.warningTextContainer}>
                  <Text
                    // @ts-expect-error legacy: invalid `red` prop preserved for snapshot parity
                    red
...
) as unknown as ComponentType<AmountExternalProps>;
