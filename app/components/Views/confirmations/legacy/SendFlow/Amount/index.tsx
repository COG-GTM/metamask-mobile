...
import BaseCollectibleMedia from '../../../../../UI/CollectibleMedia';
...
import { isNativeToken as isNativeTokenUtil } from '../../../utils/generic';
...
const isNativeToken = isNativeTokenUtil as (asset: AmountAsset) => boolean;

const CollectibleMedia = BaseCollectibleMedia as React.ComponentType<
  React.ComponentProps<typeof BaseCollectibleMedia> & {
    iconStyle?: ViewStyle;
    containerStyle?: ViewStyle;
  }
>;

const Text = RNText as React.ComponentType<TextProps & { red?: boolean }>;
const View = RNView as React.ComponentType<ViewProps & { styles?: ViewStyle }>;
...
interface AmountStateProps {
  ...
  collectibleContracts: { address: string; name?: string; logo?: string }[];
  conversionRate?: number;
  currentCurrency: string;
  gasEstimateType: string;
  gasFeeEstimates: Record<string, string> & {
...
interface AmountDispatchProps {
  prepareTransaction: (transaction: AmountTransaction) => void;
...
type AmountProps = AmountStateProps &
  AmountDispatchProps &
  AmountOwnProps & { metrics: IUseMetricsHook };
...
class Amount extends PureComponent<AmountProps, AmountState> {
  ...
  componentDidMount = async () => {
    ...
    this.tokens = [
      getEther(ticker as string) as unknown as AmountAsset,
      ...tokens,
    ];
  ...
  prepareTransaction = async (value: string) => {
    ...
    if (isNativeToken(selectedAsset)) {
      ...
      transaction.data = generateTransferData('transfer', {
        toAddress: transactionTo,
        amount: BNToHex(tokenAmount as never),
      });
  ...
  validateAmount = (
    ...
    if (isDecimal(value as string)) {
      ...
      let weiValue: BN4 = hexToBN('0x0');
      try {
        weiValue = toWei(value as string) as unknown as BN4;
      } catch (error) {
      ...
      if (!amountError) {
        ...
        amountError = gte(weiBalance as never, weiInput as never)
          ? undefined
  ...
  onInputChange = (
    ...
    if (isNativeToken(selectedAsset)) {
      ...
      try {
        weiValue = toWei(processedInputValue) as unknown as BN4;
      } catch (error) {
        // Do nothing
      }
    ...
    return (
      ...
      >
        <View style={styles.assetElement}>
          <CollectibleMedia
            small
            collectible={collectible as never}
            iconStyle={styles.tokenImage}
            containerStyle={styles.tokenImage}
          />
          <View style={styles.assetInformationWrapper}>
      ...
      <View style={styles.collectibleInputWrapper}>
        <View style={styles.collectibleInputImageWrapper}>
          <CollectibleMedia
            ...
            collectible={selectedAsset as never}
          />
        </View>
            ...
            <View style={styles.actionsWrapper}>
              ...
              <View style={styles.action}>
                ...
                >
                  ...
                  <View styles={(styles as { arrow?: ViewStyle }).arrow}>
                    <Ionicons
                      name="arrow-down"
                      size={16}
                      color={colors.primary.inverse}
...
export default connect(
  ...
  withMetricsAwareness(
    Amount as unknown as ComponentClass<IWithMetricsAwarenessProps>,
  ),
) as unknown as React.ComponentType<AmountOwnProps>;
