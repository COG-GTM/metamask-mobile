...
const AddressList = ({
  ...
  const parseAddressBook = useCallback(
    (networkAddressBookList: Contact[]) => {
      const contacts: Contact[] = networkAddressBookList.map((contact) => {
        const chainEntries = ambiguousAddressEntries?.[chainId];
        const isAmbiguousAddress = Boolean(
          chainId &&
            Array.isArray(chainEntries) &&
            chainEntries.includes(contact.address),
        );
        return {
          ...contact,
  ...
  const getNetworkAddressBookList = useCallback((): Contact[] => {
    if (inputSearch && fuse) {
