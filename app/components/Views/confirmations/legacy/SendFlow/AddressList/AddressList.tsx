...
interface Contact extends AddressBookEntry {
  isAmbiguousAddress?: boolean;
  isSmartContract?: boolean;
}

type AddressListElement = string | Contact;
...
const AddressList = ({
  ...
  const networkAddressBook = useMemo(
    () => addressBook[chainId as `0x${string}`] || {},
    [addressBook, chainId],
  );
    ...
    (networkAddressBookList: Contact[]) => {
      const contacts = networkAddressBookList.map((contact) => {
        const isAmbiguousAddress =
          chainId &&
          (ambiguousAddressEntries?.[chainId] as string[] | undefined)?.includes(
            contact.address,
          );
        return {
          ...contact,
      ...
      <View style={styles.yourContactcWrapper}>
        ...
        {internalAccounts.map((account) => (
          <AddressElement
            ...
            onIconPress={onIconPress as () => void}
