...
interface SendFlowProps {
  ...
  setSelectedAsset: (asset: unknown) => void;
  // TODO: Replace "any" with type
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
class SendFlow extends PureComponent<SendFlowProps, SendFlowState> {
  ...
  addressToInputRef = React.createRef<{ focus: () => void } | null>();
  ...
  updateParentState = (state: Partial<SendFlowState>) => {
    this.setState({ ...state } as Pick<SendFlowState, keyof SendFlowState>);
  };
    ...
    return (
      ...
      >
        ...
        {!toSelectedAddressReady ? (
          <AddressList
            chainId={globalChainId}
            inputSearch={toAccount as string}
...
const mapDispatchToProps = (dispatch: any) => ({
  ...
  setSelectedAsset: (selectedAsset: unknown) =>
    dispatch(
      setSelectedAsset(
        selectedAsset as Parameters<typeof setSelectedAsset>[0],
      ),
    ),
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export default connect(
  ...
  withMetricsAwareness(
    SendFlow as unknown as Parameters<typeof withMetricsAwareness>[0],
  ),
);
