...
interface SendToProps extends SendToStateProps, SendToDispatchProps {
  /**
...
class SendFlow extends PureComponent<SendToProps, SendToState> {
  ...
  validateAddressOrENSFromInput = async (toAccount: string) => {
    ...
    const {
      ...
      isOnlyWarning,
      confusableCollection,
      // `isOnlyWarning` is only produced by code that is currently commented
      // out in `validateAddressOrENS`, so it always resolves to undefined.
  ...
  onToSelectedAddressChange = (toAccount: string) => {
    const currentChain =
      ...
      (this.props.ambiguousAddressEntries[this.props.globalChainId] as
        | string[]
        | undefined);
