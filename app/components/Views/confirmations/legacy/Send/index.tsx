...
interface SendTransactionState {
  ...
  assetType?: string;
  providerType?: string;
  selectedAsset: SendSelectedAsset;
  ensRecipient?: string;
...
interface SendDispatchProps {
   ...
   */
  showAlert: (config: SendAlertConfig) => void;
  /**
...
interface SendOwnProps {
   ...
   */
  route: { params?: { txMeta?: SendTxMeta } };
}

type SendProps = SendOwnProps &
  SendStateProps &
  SendDispatchProps & {
    /**
 ...
 */
class Send extends PureComponent<SendProps, SendComponentState> {
  static propTypes = {
    /**
     * Object that represents the navigator
     */
  ...
  handleNewTxMeta = async ({
    ...
    let newTxMeta: Record<string, unknown> = {};
    let txRecipient;
    switch (action) {
      case 'send-eth':
      ...
      case 'send-token': {
        ...
        )) as { to?: string; ensRecipient?: string };
        if (!to) return;
        const tokenAmount =
          (parameters?.uint256 &&
   ...
   */
  handleTokenDeeplink = async (addressInput: string) => {
    const { tokens, tokenList } = this.props;
    const address = toChecksumAddress(addressInput);
    ...
    const token: { address: string; decimals?: number; symbol?: string } = {
      address,
    };
    try {
  ...
  onConfirm = async () => {
    ...
    try {
      ...
      const legacyTransactionMeta = transactionMeta as unknown as {
        id: string;
      ...
      if (assetType === 'ETH') {
        checksummedAddress = toChecksumAddress(
          legacyTransactionMeta.transaction.to as string,
        );
      ...
      if (legacyTransactionMeta.error) {
        throw legacyTransactionMeta.error;
      }
      this.setState({
        transactionConfirmed: false,
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
  ...
  showAlert: (config: SendAlertConfig) => dispatch(showAlert(config)),
  toggleDappTransactionModal: () =>
