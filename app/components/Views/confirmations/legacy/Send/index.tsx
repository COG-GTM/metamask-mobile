...
// The transaction and txMeta objects carry many optional fields from
// different sources, so they are typed permissively.
...
class Send extends PureComponent<SendProps, SendState> {
  ...
  handleNewTxMeta = async ({
    ...
    const fromAccount = internalAccounts.find(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (account: any) => toLowerCaseEquals(account.address, selectedAddress),
    );
  ...
  handleTokenDeeplink = async (address: string) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const stateToken = tokens.find((token: any) => token.address === address);
    if (stateToken) {
      return stateToken;
    }
    // Finally try to query the contract
    const { AssetsContractController } = Engine.context;
    const token: SendTransaction = { address };
    try {
  ...
  onConfirm = async () => {
    ...
    try {
      ...
      const { result, transactionMeta } = (await addTransaction(
        ...
        transactionMeta: SendTransaction;
      };
  ...
  renderModeComponent() {
    if (this.state.mode === EDIT) {
      return (
        <EditAmount
          transaction={this.props.transaction as never}
...
const mapDispatchToProps = (dispatch: any) => ({
  ...
  toggleDappTransactionModal: () => dispatch(toggleDappTransactionModalAction()),
});
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(withMetricsAwareness(Send as unknown as Parameters<typeof withMetricsAwareness>[0]));
