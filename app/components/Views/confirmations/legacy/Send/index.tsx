...
class Send extends PureComponent<Props, State> {
  ...
  handleNewTxMeta = async ({ target_address, action, parameters = null }: any) => {
    ...
    // @ts-expect-error Legacy JS migration - TS2339
    if (!newTxMeta.value) {
      // @ts-expect-error Legacy JS migration - TS2339, TS2345
  ...
  onConfirm = async () => {
    ...
    try {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const existingContact = globalChainId ? (addressBook as any)[globalChainId]?.[checksummedAddress as any] : undefined;
      if (!existingContact) {
