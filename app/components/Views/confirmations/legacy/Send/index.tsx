...
interface SendStateProps {
   ...
   * Transaction state
   */
  transaction: RootState['transaction'];
  /**
...
interface SendProps extends SendStateProps, SendDispatchProps {
  /**
...
class Send extends PureComponent<SendProps, SendState> {
  ...
  handleTokenDeeplink = async (address: string) => {
    ...
    const token: Record<string, unknown> = { address };
    try {
  ...
  onConfirm = async () => {
    ...
    try {
      ...
      // The legacy `transaction` and `data` fields are still populated at
      // runtime but are absent from the current `TransactionMeta` type.
      const legacyMeta = transactionMeta as unknown as {
      ...
      if (assetType === 'ETH') {
        checksummedAddress = toChecksumAddress(legacyMeta.transaction.to);
      } else if (assetType === 'ERC20') {
        try {
          const [addressTo] = decodeTransferData(
            'transfer',
            legacyMeta.transaction.data,
          );
          if (addressTo) {
            checksummedAddress = toChecksumAddress(addressTo);
          }
        } catch (e) {
          Logger.log('Error decoding transfer data', legacyMeta.data);
        }
...
const mapDispatchToProps = (dispatch: Dispatch) => ({
  ...
  showAlert: (config: AlertConfig) => dispatch(showAlertAction(config)),
  toggleDappTransactionModal: () =>
