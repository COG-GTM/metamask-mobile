...
import BaseAddNickname from '../../components/ApproveTransactionReview/AddNickname';
...
import ApproveTransactionReview, {
  TokenAllowanceState,
...
interface ApproveAnalyticsParams {
...
type ApproveProps = ApproveStateProps &
...
interface ApproveState {
  ...
  eip1559GasTransaction: GasTransactionState;
  legacyGasObject: LegacyGasObjectState;
  legacyGasTransaction: GasTransactionState;
 ...
 */
const AddNickname = BaseAddNickname as React.ComponentType<
  Partial<React.ComponentProps<typeof BaseAddNickname>> & {
    savedContactListToArray?: { address: string; name: string }[];
  }
>;

class Approve extends PureComponent<ApproveProps, ApproveState> {
  appStateListener: { remove: () => void } | undefined;
    ...
    if (
      ...
      if (
        ...
        (!shallowEqual(prevProps.gasFeeEstimates, this.props.gasFeeEstimates) ||
          !transaction.gas.eq(prevProps?.transaction?.gas as BN4) ||
          !this.state.ready)
      ) {
  ...
  componentWillUnmount = async () => {
    ...
    await (stopGasPolling as (pollToken?: string) => Promise<void>)(
  ...
  prepareTransaction = () => {
    ...
    return buildTransactionParams({
      ...
      showCustomNonce,
      transaction,
    }) as TransactionParams & { id?: string };
  };
  ...
  onConfirm = async () => {
    ...
    try {
      ...
      await (updateTransaction as (tx: TransactionMeta) => Promise<void>)(
        updatedTx as TransactionMeta,
      );
      ...
      if (
        !approveError?.message.startsWith(KEYSTONE_TX_CANCELED) &&
        !approveError?.message.startsWith(STX_NO_HASH_ERROR)
      ) {
        Alert.alert(
  ...
  updateTransactionState = (gas: GasTransactionState) => {
...
export default connect(
  ...
  withMetricsAwareness(
    Approve as unknown as ComponentClass<IWithMetricsAwarenessProps>,
  ),
) as unknown as React.ComponentType<ApproveOwnProps>;
