...
// Legacy connect/HOC-wrapped components retain permissive prop typing.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const TransactionReviewView = TransactionReview as unknown as React.ComponentType<any>;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const EditGasFee1559View = EditGasFee1559 as unknown as React.ComponentType<any>;
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const EditGasFeeLegacyView = EditGasFeeLegacy as unknown as React.ComponentType<any>;

// Gas data objects are dynamically shaped across legacy and EIP1559 flows.
interface GasData {
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  [key: string]: any;
}

// The transaction object carries many optional fields from different sources.
...
interface TransactionEditorProps {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  setTransactionObject: (transaction: any) => any;
  promptedFromApproval?: boolean;
  dappTransactionModalVisible?: boolean;
  ticker?: string;
  gasEstimateType?: string;
  gasFeeEstimates: GasData;
...
> {
  ...
  componentDidMount = async () => {
    ...
    if (!hasGasLimit)
      handleGetGasLimit(transaction as never, setTransactionObject);
  ...
  componentWillUnmount = () => {
    const { GasFeeController } = Engine.context;
    (GasFeeController.stopPolling as (token?: unknown) => void)(
      this.state.pollToken,
    );
  };
  ...
  handleUpdateAmount = async (amount: any, mounting = false) => {
    ...
    if (assetType === 'ETH') {
      const { gas } = mounting
        ? { gas: gasLimit }
        : await estimateGas({ amount, data, to }, transaction as never);
  ...
  handleUpdateData = async (data: string) => {
    const { transaction } = this.props;
    const { gas } = await estimateGas({ data }, transaction as never);
    this.setState({ data });
  ...
  handleDataGeneration = async (opts: any) => {
    ...
    const generateData = {
      ...
      ERC721: () => {
        ...
        const collectibleTransferInformation =
          ...
          ] as { tradable?: boolean; method?: string });
        if (!to) return;
    ...
    const data = generateData[assetType as 'ERC20' | 'ERC721']();
  ...
  calculateTempGasFee = (gas: GasData, selected: string) => {
    const { transaction } = this.props;
    if (selected && gas) {
  ...
  cancelGasEdition = () => {
    this.setState({
      ...
      stopUpdateGas: false,
      gasSelectedTemp: this.state.gasSelected as string,
    });
    this.props.onModeChange?.('review');
  };
