...
interface GasParams {
  ...
  EIP1559GasData?: any;
  gasSelected?: string | boolean | null;
}
...
class Approval extends PureComponent<ApprovalProps, ApprovalState> {
  ...
  #transactionFinishedListener?: unknown;
    ...
    try {
      ...
      if (!transactionHandled) {
        ...
        Engine.controllerMessenger.tryUnsubscribe(
          ...
          this.#transactionFinishedListener as never,
        );
  ...
  isTxStatusCancellable = (transaction?: ApprovalTransaction) => {
    if (
  ...
  getAnalyticsParams = ({ gasEstimateType, gasSelected }: GasParams = {}) => {
  ...
  onConfirm = async ({
    ...
    try {
      ...
      const fullTx = transactions.find(({ id }) => id === transaction.id) as
        | (ApprovalTransaction & {
            txParams?: { type?: string };
          })
        | undefined;
   ...
   */
  prepareTransaction = ({ EIP1559GasData, gasEstimateType }: GasParams) => {
...
(Approval as unknown as { contextType: typeof ThemeContext }).contextType =
  ThemeContext;
...
export default connect(
  ...
  withMetricsAwareness(
    Approval as unknown as Parameters<typeof withMetricsAwareness>[0],
  ),
);
