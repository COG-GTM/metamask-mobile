...
interface ApprovalSelectedAsset {
  address?: string;
  symbol?: string;
  contractName?: string;
  decimals?: number;
...
interface ApprovalTransactionState extends TransactionParams {
  id: string;
  origin: string;
  chainId?: Hex;
  assetType?: string;
  selectedAsset: ApprovalSelectedAsset;
  status?: string;
}
...
interface ApprovalStateProps {
   ...
   * Transaction state
   */
  transaction: ApprovalTransactionState;
  /**
...
interface ApprovalAnalyticsOptions {
...
class Approval extends PureComponent<ApprovalProps, ApprovalState> {
  ...
  isTxStatusCancellable = (transaction?: { status?: string }) => {
    if (
  ...
  trackEditScreen = async () => {
    ...
    const actionKey = await (
      getTransactionReviewActionKey as unknown as (tx: {
        transaction: ApprovalTransactionState;
      }) => Promise<string>
    )({ transaction });
    metrics.trackEvent(
  ...
  getAnalyticsParams = ({
    ...
    try {
      ...
      } as Parameters<typeof TransactionController.getTransactions>[0])?.[0];
  ...
  onConfirm = async ({
    ...
    try {
      ...
      if (
        (fullTx as TransactionMeta).txParams.type !==
      ...
      if (isLedgerAccount) {
        ...
        this.props.navigation.navigate(
          ...
          } as unknown as LedgerTransactionModalParams),
        );
        this.props.hideModal();
        return;
      }
