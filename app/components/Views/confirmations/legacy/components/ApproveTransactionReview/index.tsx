...
import {
  ...
  type TransactionType as BlockaidTransactionType,
} from '../../../../../../util/blockaid';
...
import TransactionReview, {
  type GasTransaction,
} from '../TransactionReview/TransactionReviewEIP1559Update';
...
import type { UseGasTransactionProps } from '../../../../../../core/GasPolling/types';
import type { NormalizedTransaction } from '../TransactionReview/TransactionReviewInformation';
import type { VerifyContractDetailsProps } from './VerifyContractDetails/VerifyContractDetails.types';
...
export interface ApproveTransactionReviewOwnProps {
  onCancel?: () => void;
  onConfirm?: () => void;
  ...
  legacyGasObject?: { legacyGasLimit?: string; suggestedGasPrice?: string };
  eip1559GasObject?: {
...
export type ApproveTransactionReviewProps = ApproveTransactionReviewOwnProps &
  ApproveTransactionReviewStateProps &
  ApproveTransactionReviewDispatchProps &
  ApproveTransactionReviewInjectedProps;

interface ApproveTransactionReviewToken {
  tokenSymbol?: string;
  tokenDecimals?: number;
  tokenName?: string;
  tokenValue?: string;
...
interface ApproveTransactionReviewState {
  ...
  transaction: NormalizedTransaction;
  token: ApproveTransactionReviewToken;
  isReadyToApprove?: boolean;
  tokenSpendValue: string;
  showGasTooltip: boolean;
  gasTransactionObject: GasTransaction;
...
> {
  ...
  componentDidMount = async () => {
    ...
    const token = Object.values(tokenList).filter(
      (listToken) => listToken.address === to,
    );
  ...
  getAnalyticsParams = () => {
    ...
    try {
      const isDapp = !Object.values<string | undefined>(
        AppConstants.DEEPLINKS,
      ).includes(transaction?.origin);

      const params: JsonMap = {
        ...baseParams,
        referral_type: isDapp ? 'dapp' : transaction?.origin ?? null,
      };
        ...
        body={
          <View>
            <Text>
              ...
              <Text>{strings('transaction.gas_education_3')}</Text>
            </Text>
            <Text>{strings('transaction.gas_education_4')}</Text>
            <TouchableOpacity onPress={this.openLinkAboutGas}>
              <Text>{strings('transaction.gas_education_learn_more')}</Text>
            </TouchableOpacity>
          </View>
        }
      />
                        ...
                        {((isERC2OToken && isReadyToApprove) ||
                          ...
                          <View style={styles.transactionWrapper}>
                            <TransactionReview
                              ...
                              gasObject={
                                !showFeeMarket
                                  ? (legacyGasObject as UseGasTransactionProps['gasObject'])
                                  : eip1559GasObject
                              }
    ...
    return (
      <VerifyContractDetails
        closeVerifyContractView={
          closeVerifyContractDetails ?? (() => undefined)
        }
...
const mapStateToProps = (
  ...
  return {
    ...
    securityAlertResponse: selectCurrentTransactionSecurityAlertResponse(
      state,
    ) as unknown as SecurityAlertResponse | undefined,
  };
};
...
// @react-navigation/compat's withNavigation types collapse the wrapped props to `never`
// (Exclude<P, InjectedProps> on a non-union), so the HOC output is re-typed explicitly.
const ApproveTransactionReviewWithNavigation = withNavigation(
...
export default connect(
  mapStateToProps,
  mapDispatchToProps,
)(ApproveTransactionReviewWithNavigation);
