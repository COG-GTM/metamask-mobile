...
/**
 * Subset of the parsed legacy transaction data returned by `useGasTransaction`
...
interface LegacyGasFeeEstimates {
  low?: string;
  high?: string;
  gasPrice?: string;
}
...
interface EditGasFeeLegacyProps {
  onCancel?: () => void;
  onSave?: (
    gasTransaction: LegacyGasTransaction,
    gasObject: LegacyGasObject,
  ) => void;
  error?: string;
...
const EditGasFeeLegacy = ({
  ...
  ) as unknown as LegacyGasFeeEstimates;
  ...
  const changeGas = useCallback((gas: Partial<LegacyGasTransaction>) => {
  ...
  const changedGasPrice = useCallback(
    (value: string) => {
      let newGas: Partial<LegacyGasTransaction>;
