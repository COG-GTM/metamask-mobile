...
type UpdateOptionType = RenderInputProps['updateOption'];

interface GasTransactionResult {
...
const EditGasFee1559Update = ({
  ...
  const [gasObject, updateGasObject] = useState<GasObjectState>({
  ...
  >[0]) as unknown as GasTransactionResult;
  ...
  const changedMaxPriorityFee = useCallback(
    (value: string) => {
      ...
      const higherValue = new BigNumber(
        gasOptions?.['high' as keyof typeof gasOptions]
          ?.suggestedMaxPriorityFeePerGas,
  ...
  const changedMaxFeePerGas = useCallback(
    (value: string) => {
      const lowerValue = new BigNumber(
        gasOptions?.[warningMinimumEstimateOption as keyof typeof gasOptions]
          ?.suggestedMaxFeePerGas,
      );
      const higherValue = new BigNumber(
        gasOptions?.['high' as keyof typeof gasOptions]?.suggestedMaxFeePerGas,
      ).multipliedBy(new BigNumber(1.5));
      const updateFloor = new BigNumber(
  ...
  const RightLabelComponent = ({ value }: { value: string }) => (
    <Text noMargin small grey>
      ...
      {
        gasOptions?.[suggestedEstimateOption as keyof typeof gasOptions]
          ?.suggestedMaxFeePerGas
      }{' '}
      GWEI
    </Text>
  ...
  const renderInputs = (option: UpdateOptionType) => (
    <View>
      <FadeAnimationView
