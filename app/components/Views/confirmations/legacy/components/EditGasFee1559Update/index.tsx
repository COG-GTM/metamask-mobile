...
/**
 * Gas object handled by this component and returned to its parent.
 */
...
/**
 * Subset of the parsed EIP1559 transaction data returned by `useGasTransaction`
 * that is used by this component.
 */
export interface EIP1559GasTransaction extends EIP1559GasObject {
...
export interface EIP1559UpdateOption {
  isCancel?: boolean;
  maxFeeThreshold?: BigNumber.Value;
  maxPriortyFeeThreshold?: BigNumber.Value;
...
interface EditGasFee1559UpdateProps {
  ...
  gasOptions?: Record<string, EIP1559GasObject>;
  primaryCurrency?: string;
  chainId?: string;
  onCancel?: () => void;
  onChange?: (selected: string | null) => void;
  onSave?: (
    gasTransaction: EIP1559GasTransaction,
    gasObject: EIP1559GasObject,
  ) => void;
  error?: React.ReactNode;
  dappSuggestedGas?: boolean;
  ignoreOptions?: string[];
  updateOption?: EIP1559UpdateOption;
  ...
  isAnimating?: boolean;
  analyticsParams?: Record<string, unknown> & { view?: string };
...
/**
 * `HorizontalSelector` declares `disabled` as required, and it is not passed
 * here.
 */
const GasOptionsSelector = HorizontalSelector as React.FC<
...
const EditGasFee1559Update = ({
  ...
  const save = useCallback(() => {
    ...
    )(gasTransaction, newGasPriceObject);
  }, [
    getAnalyticsParams,
    onSave,
    gasTransaction,
  ...
  const changeGas = useCallback(
    (gas: EIP1559GasObject, option: string | null) => {
      ...
      (onChange as (selected: string | null) => void)(option);
    },
    [onChange, gasObject],
  );
  ...
  const suggestedOption = suggestedEstimateOption as string;
  ...
  const RightLabelComponent = ({ value }: { value: string }) => (
    <Text noMargin small grey>
      ...
      {gasOptions?.[suggestedOption]?.suggestedMaxFeePerGas} GWEI
    </Text>
  );
  ...
  const renderInputs = (option?: EIP1559UpdateOption) => (
    <View>
      <FadeAnimationView
