...
interface TypedSignMessageParams extends Omit<MessageParams, 'data'> {
  data: string | TypedSignV1Data | TypedSignV1Data[];
}

type TypedSignVersion = keyof typeof typedSign;
...
class TypedSign extends PureComponent<Props, State> {
  ...
  rejectSignature = async () => {
    ...
    await handleSignatureAction(
      onReject,
      messageParams,
      typedSign[messageParams.version as TypedSignVersion],
    ...
    if (messageParams.version === 'V1') {
      return (
        <View style={styles.message}>
          {(messageParams.data as TypedSignV1Data[]).map((obj, i) => (
  ...
  render() {
    ...
    const rootView = showExpandedMessage ? (
      ...
      <SignatureRequest
        ...
        type={typedSign[messageParams.version as TypedSignVersion]}
        fromAddress={from}
