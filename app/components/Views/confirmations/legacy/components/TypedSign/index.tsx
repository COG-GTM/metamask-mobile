...
interface TypedSignProps {
   ...
   */
  messageParams: TypedAny;
  /**
   * Object containing current page title and url
   */
  currentPageInformation?: TypedAny;
  /**
   ...
   */
  securityAlertResponse?: TypedAny;
  /**
...
class TypedSign extends PureComponent<TypedSignProps, TypedSignState> {
  ...
  onSignatureError = ({ error }: TypedAny) => {
  ...
  updateShouldTruncateMessage = (e: TypedAny) => {
  ...
  renderTypedMessageV3 = (obj: TypedAny) => {
    ...
    if (messageParams.version === 'V1') {
      return (
        <View style={styles.message}>
          {messageParams.data.map((obj: TypedAny, i: number) => (
  ...
  render() {
    ...
    const messageWrapperStyles: TypedAny[] = [];
    let domain: TypedAny;
    const styles = this.getStyles();
...
const mapStateToProps = (state: RootState, ownProps: TypedAny) => {
  ...
  return {
    ...
    securityAlertResponse: (state as TypedAny).signatureRequest
      .securityAlertResponse,
  };
};
