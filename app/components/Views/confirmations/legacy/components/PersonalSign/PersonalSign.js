...
const PersonalSign = ({
  ...
  const { securityAlertResponse } = useSelector(
    ...
    (reduxState) => reduxState.signatureRequest
  );
  ...
  const networkType = useSelector((state) =>
  ...
  const getAnalyticsParams = useCallback(() => {
    ...
    if (pageInfo.url) {
      try {
        ...
        Logger.error(error, 'Error parsing URL in signature request');
      }
    }
