...
const WatchAssetRequest = ({
  ...
  // `styles.titleWrapper` and the module scoped `this` below are both undefined
  // at runtime, exactly as in the JavaScript implementation.
  const legacyStyles = styles as unknown as { titleWrapper?: ViewStyle };
  const legacyThis = undefined as unknown as { cancelSignature: () => void };
  ...
  return (
    <View style={styles.root} testID={AssetWatcherSelectorsIDs.CONTAINER}>
      ...
      <View style={legacyStyles.titleWrapper}>
        <Text style={styles.title} onPress={legacyThis.cancelSignature}>
