...
/**
 * Subset of the parsed transaction data returned by `useGasTransaction` that
 * is rendered by this component.
 */
export interface GasTransactionSummary {
...
interface TransactionReviewEIP1559UpdateProps {
  ...
  onlyGas?: boolean;
  updateTransactionState?: (gasTransaction: GasTransactionSummary) => void;
...
const TransactionReviewEIP1559Update = ({
  ...
  const gasTransaction = useGasTransaction({
    ...
    gasObjectLegacy,
    multiLayerL1FeeTotal,
  }) as unknown as GasTransactionSummary;
  ...
  useEffect(() => {
    if (gasEstimationReady) {
      (
        updateTransactionState as (
          gasTransaction: GasTransactionSummary,
        ) => void
      )(gasTransaction);
    }
