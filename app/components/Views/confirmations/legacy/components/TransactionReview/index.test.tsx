...
const TransactionReview =
  TransactionReviewComponent as unknown as ComponentType<TxAny>;
...
jest.mock('@react-navigation/compat', () => {
  ...
  return {
    actualNav,
    withNavigation: (obj: TxAny) => obj,
  };
});

jest.mock('react-native-gzip', () => ({
  deflate: (val: TxAny) => val,
}));
...
describe('TransactionReview', () => {
  ...
  it('should match snapshot', () => {
    const container = renderWithProvider(
      <TransactionReview
        ...
        generateTransform={generateTransform}
      />,
      { state: mockState as unknown as DeepPartial<RootState> },
    );
  ...
  it('should display blockaid banner', async () => {
    ...
    const blockaidMetricsParamsSpy = jest
      ...
      .mockImplementation(
        ({ result_type, reason, providerRequestsCount }: TxAny) => ({
  ...
  it('should have enabled confirm button if from account has balance', async () => {
    jest
      ...
      .mockReturnValue(Promise.resolve(undefined) as TxAny);
  ...
  it('should not have confirm button disabled if from account has no balance and also if there is no error', async () => {
    ...
    jest.mock('react-redux', () => ({
      ...jest.requireActual('react-redux'),
      useSelector: (fn: TxAny) => fn(mockNewState),
    }));
  ...
  it('should have confirm button disabled if error is defined', async () => {
    jest.mock('react-redux', () => ({
      ...jest.requireActual('react-redux'),
      useSelector: (fn: TxAny) => fn(mockState),
    }));
    const { getByRole } = renderWithProvider(
      <TransactionReview
        EIP1559GasData={{}}
