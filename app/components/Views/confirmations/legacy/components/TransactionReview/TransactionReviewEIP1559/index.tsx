...
interface TransactionReviewEIP1559Props {
  ...
  /**
   * Total values passed by TransactionReviewInformation (currently not rendered)
   */
  totalNative?: string;
  totalConversion?: string;
  totalMaxNative?: string;
  /**
   * True if transaction is over the available funds (currently not rendered)
   */
  over?: boolean;
}
...
const TransactionReviewEIP1559 = ({
  ...
  const toggleLearnMoreModal = useCallback(() => {
    setShowLearnMoreModal((visible) => !visible);
  }, []);
  const { colors } = useTheme();
  const styles = createStyles(colors);
  ...
  let gasFeePrimary: string | undefined,
    gasFeeSecondary: string | undefined,
