...
const TransactionReviewEIP1559: React.FC<TransactionReviewEIP1559Props> = ({
  ...
  const toggleLearnMoreModal = useCallback(() => {
    setShowLearnMoreModal((showLearnMoreModal: boolean) => !showLearnMoreModal);
  }, []);
  const { colors } = useTheme();
  const styles = createStyles(colors);
  ...
  const edit = useCallback(() => {
    if (!isAnimating && onEdit) onEdit();
  }, [isAnimating, onEdit]);

  const isMainnet = isMainnetByChainId(chainId || '');
  const nativeCurrencySelected = primaryCurrency === 'ETH' || !isMainnet;
  let gasFeePrimary: string | undefined, gasFeeSecondary: string | undefined, gasFeeMaxPrimary: string | undefined;
            ...
            >
              ...
              >
                <MaterialCommunityIcons
                  name="information"
                  size={13}
                  style={originWarning ? styles.gasInfoIconWarning : styles.gasInfoIcon}
                />
              </TouchableOpacity>
            </Text>
          </View>
