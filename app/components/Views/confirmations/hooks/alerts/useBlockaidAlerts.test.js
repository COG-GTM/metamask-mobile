...
import { Reason } from '../../legacy/components/BlockaidBanner/BlockaidBanner.types';
...
describe('useBlockaidAlerts', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    useSecurityAlertResponse.mockReturnValue({ securityAlertResponse: mockSecurityAlertResponse });
  });

  it('returns an empty array when there is no security alert response', () => {
    useSecurityAlertResponse.mockReturnValue({ securityAlertResponse: null });
  ...
  it('returns an empty array when the result type is ignored', () => {
    useSecurityAlertResponse.mockReturnValue({
