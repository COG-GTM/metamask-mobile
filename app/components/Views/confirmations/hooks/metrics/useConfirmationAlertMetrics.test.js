...
describe('useConfirmationAlertMetrics', () => {
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    useConfirmationMetricEvents.mockReturnValue({
      setConfirmationMetric: mockSetConfirmationMetric
    });
    useAlerts.mockReturnValue(mockUseAlerts);
  });
  ...
  it('handles confirmed alerts correctly', () => {
    mockUseAlerts.isAlertConfirmed.mockImplementation((key) => key === AlertKeys.Blockaid);
  ...
  it('uses alertKey when there is no match in ALERTS_NAME_METRICS', () => {
    ...
    useAlerts.mockReturnValue(mockUseAlertsWithUnknownKey);
    useSelector.mockReturnValue({
      properties: {
  ...
  it('handles undefined alert_visualized', () => {
    mockUseAlerts.isAlertConfirmed.mockReturnValue(false);
    useSelector.mockReturnValue({
      properties: {
        ...baseAlertProperties,
