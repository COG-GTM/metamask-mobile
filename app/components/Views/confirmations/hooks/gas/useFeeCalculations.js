...
export const useFeeCalculations = (transactionMeta) => {
  ...
  const estimatedFees = useMemo(() => {
    ...
    const minimumFeePerGasBN = hexToBN(minimumFeePerGas);

    // `minimumFeePerGas` should never be higher than the `maxFeePerGas`
    if (minimumFeePerGasBN.gt(hexToBN(maxFeePerGas))) {
    ...
    const estimatedFee = multiplyHexes(
      supportsEIP1559 ? minimumFeePerGas : gasPrice,
      gasLimitNoBuffer
    );
