 ...
 */
export default class WalletConnectSessions extends PureComponent<Props, State> {
  ...
  renderSessions = () => {
    ...
    return (
      <>
        {sessions.map((session: Session) => this.renderV1(session))}
        {sessionsV2.map((session: Session, index: number) =>
  ...
  renderV2 = (session: Session, index: number) => {
    const colors = this.context?.colors || mockTheme.colors;
    const styles = createStyles(colors);
    return (
      <TouchableOpacity
