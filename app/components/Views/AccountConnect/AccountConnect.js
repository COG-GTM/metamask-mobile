...
import {
  ...
  AccountConnectScreens } from
'./AccountConnect.types';
...
import {
  ...
  getRequestedCaip25CaveatValue } from
'./utils';
...
const AccountConnect = (props) => {
  ...
  const inappBrowserOrigin = useSelector(getActiveTabUrl, isEqual);
  const accountsLength = useSelector(selectAccountsLength);
  const { wc2Metadata } = useSelector((state) => state.sdk);
  ...
  const secureIcon = useMemo(
    () =>
    getUrlObj(hostname).protocol === 'https:' ?
    IconName.Lock :
    IconName.LockSlash,
    [hostname]
  );
  ...
  const renderSingleConnectScreen = useCallback(() => {
    ...
    const defaultSelectedAccount = selectedAccount ?
    {
      ...selectedAccount,
      name:
