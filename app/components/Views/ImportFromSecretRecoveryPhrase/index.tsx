...
interface ImportFromSecretRecoveryPhraseOwnProps {
  /**
...
type ImportFromSecretRecoveryPhraseProps =
  ImportFromSecretRecoveryPhraseOwnProps &
...
const ImportFromSecretRecoveryPhrase = ({
  ...
  const updateBiometryChoice = async (enabled: boolean) => {
    await updateAuthTypeStorageFlags(enabled);
  ...
  const handleRejectedOsBiometricPrompt = async (parsedSeed: string) => {
    ...
    setBiometryType(
      newAuthData.availableBiometryType as BIOMETRY_TYPE | string | null,
    );
  ...
  const clearSecretRecoveryPhrase = async (phrase: string) => {
    ...
    if (
      ...
      parseSeedPhrase(phrase) === parsedClipboardContents
    ) {
      await Clipboard.clearString();
    }
  };

  const onSeedWordsChange = useCallback(async (phrase: string) => {
    setSeed(phrase);
    ...
    if (Device.isAndroid()) {
      ...
      if (androidOSVersion >= MINIMUM_SUPPORTED_CLIPBOARD_VERSION) {
        await clearSecretRecoveryPhrase(phrase);
      }
    }
  }, []);
  ...
  const renderSwitch = () => {
    const handleUpdateRememberMe = (enabled: boolean) => {
  ...
  const hiddenSRPInput = useCallback(
    () => (
      <OutlinedTextField
        ...
        containerStyle={
          inputWidth as unknown as TextFieldProps['containerStyle']
        }
