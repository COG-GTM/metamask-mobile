...
type OutlinedTextFieldViewProps = Omit<
  React.ComponentProps<typeof OutlinedTextField>,
  'style' | 'containerStyle' | 'inputContainerStyle'
> & {
...
const OutlinedTextFieldView =
  OutlinedTextField as unknown as React.ComponentType<OutlinedTextFieldViewProps>;
...
const ImportFromSecretRecoveryPhrase = ({
  ...
  const handleRejectedOsBiometricPrompt = async (parsedSeed: string) => {
    ...
    try {
      ...
      (
        this as unknown as {
          setState: (state: { loading: boolean; error: string }) => void;
        }
      ).setState({ loading: false, error: (err as Error).toString() });
    }
    setBiometryType(newAuthData.availableBiometryType as string);
  ...
  const clearSecretRecoveryPhrase = async (seedToClear: string) => {
    ...
    if (
      ...
      parseSeedPhrase(seedToClear) === parsedClipboardContents
    ) {
  ...
  const onQrCodePress = useCallback(() => {
    ...
    navigation.navigate(Routes.QR_TAB_SWITCHER, {
      ...
      disableTabber: true,
      onScanSuccess: ({ seed: scannedSeed = undefined }) => {
        if (scannedSeed) {
          setSeed(scannedSeed);
        } else {
          Alert.alert(
