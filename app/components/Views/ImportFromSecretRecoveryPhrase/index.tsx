...
// @ts-expect-error - zxcvbn ships no type declarations
  ...
  useEffect(() => {
    ...
    const setBiometricsOption = async () => {
      ...
      if (authData.currentAuthType === AUTHENTICATION_TYPE.PASSCODE) {
        setBiometryType(passcodeType(authData.currentAuthType) as string);
        setBiometryChoice(
          !(passcodePreviouslyDisabled && passcodePreviouslyDisabled === TRUE),
        );
      } else if (authData.availableBiometryType) {
        setBiometryType(authData.availableBiometryType as string);
  ...
  const handleRejectedOsBiometricPrompt = async (parsedSeed: string) => {
    ...
    try {
      await Authentication.newWalletAndRestore(
        password,
        newAuthData as AuthData,
        parsedSeed,
        true,
      );
    } catch (err) {
      // @ts-expect-error - `this` is undefined in this function component; kept to preserve behavior
          ...
          <View style={styles.field}>
            ...
            {(password !== '' && (
              ...
              >
                ...
                <Text
                  style={
                    (styles as Record<string, TextStyle>)[
...
export default connect(
  ...
  Partial<ImportFromSecretRecoveryPhraseProps>
>;
