...
const ImportFromSecretRecoveryPhrase = ({
  ...
  const [inputWidth, setInputWidth] = useState<{ width: '99%' | '100%' }>({
    width: '99%',
  });
  ...
  const onPressImport = async () => {
    ...
    if (importError) {
      ...
      try {
        ...
        const errAsError = importErr as Error;
        // Should we force people to enable passcode / biometrics?
        if (errAsError.toString() === PASSCODE_NOT_SET_ERROR) {
          Alert.alert(
            'Security Alert',
          ...
          Logger.log('Error with seed phrase import', errAsError.message);
        }
        track(MetaMetricsEvents.WALLET_SETUP_FAILURE, {
          wallet_setup_type: 'import',
          error_type: errAsError.toString(),
        });
      }
    }
  };
