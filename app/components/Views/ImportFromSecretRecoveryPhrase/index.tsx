...
interface ImportFromSecretRecoveryPhraseProps {
  ...
  /**
   * The action to update the password set flag in the redux store
   */
  passwordSet: () => void;
  /**
   * The action to set the locktime in the redux store
   */
  setLockTime: (time: number) => void;
  /**
   * The action to update the seedphrase backed up flag in the redux store
   */
...
const ImportFromSecretRecoveryPhrase = ({
  ...
  type OutlinedTextFieldStyle = ComponentProps<
    typeof OutlinedTextField
  >['style'];
  type OutlinedTextFieldContainerStyle = ComponentProps<
    typeof OutlinedTextField
  >['containerStyle'];
  ...
  const updateBiometryChoice = async (biometryEnabled: boolean) => {
    await updateAuthTypeStorageFlags(biometryEnabled);
    setBiometryChoice(biometryEnabled);
  };
  ...
  const clearSecretRecoveryPhrase = async (seedPhrase: string) => {
    ...
    if (
      ...
      parseSeedPhrase(seedPhrase) === parsedClipboardContents
    ) {
      await (
  ...
  const onSeedWordsChange = useCallback(async (updatedSeed: string) => {
    setSeed(updatedSeed);
    ...
    if (Device.isAndroid()) {
      ...
      if (androidOSVersion >= MINIMUM_SUPPORTED_CLIPBOARD_VERSION) {
        await clearSecretRecoveryPhrase(updatedSeed);
      }
    }
  }, []);
  ...
  const renderSwitch = () => {
    const handleUpdateRememberMe = (remember: boolean) => {
  ...
  const hiddenSRPInput = useCallback(
    () => (
      <OutlinedTextField
        style={styles.input as unknown as OutlinedTextFieldStyle}
        containerStyle={
          inputWidth as unknown as OutlinedTextFieldContainerStyle
        }
