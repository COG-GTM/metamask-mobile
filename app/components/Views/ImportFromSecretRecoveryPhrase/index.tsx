...
import { OutlinedTextField as OutlinedTextFieldBase } from 'react-native-material-textfield';
...
const OutlinedTextField =
  OutlinedTextFieldBase as unknown as React.ComponentType<
...
const ImportFromSecretRecoveryPhrase = ({
  ...
  const updateBiometryChoice = async (nextBiometryChoice: boolean) => {
    await updateAuthTypeStorageFlags(nextBiometryChoice);
    setBiometryChoice(nextBiometryChoice);
  };
  ...
  const onPressImport = async () => {
    ...
    if (validationError) {
      ...
      try {
        ...
        if ((caughtError as Error).toString() === PASSCODE_NOT_SET_ERROR) {
          Alert.alert(
            'Security Alert',
        ...
        track(MetaMetricsEvents.WALLET_SETUP_FAILURE, {
          wallet_setup_type: 'import',
          error_type: (caughtError as Error).toString(),
        });
      }
    }
  };
  ...
  const onSeedWordsChange = useCallback(async (seedPhrase: string) => {
    setSeed(seedPhrase);
    ...
    if (Device.isAndroid()) {
      ...
      if (androidOSVersion >= MINIMUM_SUPPORTED_CLIPBOARD_VERSION) {
        await clearSecretRecoveryPhrase(seedPhrase);
      }
    }
  }, []);
  ...
  const renderSwitch = () => {
    const handleUpdateRememberMe = (nextRememberMe: boolean) => {
