...
const ImportFromSecretRecoveryPhrase = ({
  ...
  const track = (event: IMetaMetricsEvent, properties?: JsonMap) => {
    ...
    // @ts-expect-error - `track` has always been called without properties for some events, and `addProperties` is called with `undefined` then
  ...
  const updateNavBar = () => {
    navigation.setOptions(
      // @ts-expect-error - `getOnboardingNavbarOptions` is JS, so its second parameter reads as a required `{ headerLeft }`; this screen has always passed `{}`
  ...
  const handleRejectedOsBiometricPrompt = async (parsedSeed: string) => {
    ...
    try {
      ...
      // @ts-expect-error - `this` is not bound in this function component; pre-existing, and this branch has always thrown
      this.setState({ loading: false, error: (err as Error).toString() });
    }
    // @ts-expect-error - `availableBiometryType` is optional; storing `undefined` here is pre-existing behaviour
    ...
    if (error) {
      ...
      try {
        ...
        try {
          ...
          if (
            // @ts-expect-error - `Device.isIos` is a function and is not called here; pre-existing behaviour
            Device.isIos &&
        ...
        if ((error as Error).toString() === PASSCODE_NOT_SET_ERROR) {
          ...
          Logger.log('Error with seed phrase import', (error as Error).message);
        }
        track(MetaMetricsEvents.WALLET_SETUP_FAILURE, {
          wallet_setup_type: 'import',
  ...
  const onSeedWordsChange = useCallback(async (seed: string) => {
    ...
    if (Device.isAndroid()) {
      const androidOSVersion = parseInt(
        // @ts-expect-error - `Release` only exists in the Android platform constants, and this branch is Android-only
  ...
  // @ts-expect-error - `passwordStrength` is only set once the password changes, so this is `undefined` on first render
