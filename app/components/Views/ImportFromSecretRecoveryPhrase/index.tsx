...
const ImportFromSecretRecoveryPhrase = ({
  ...
  const passwordInput = React.createRef<OutlinedTextField>();
  const confirmPasswordInput = React.createRef<OutlinedTextField>();
  ...
  const onPressImport = async () => {
    ...
    if (validationError) {
      ...
      try {
        ...
        if ((err as Error).toString() === PASSCODE_NOT_SET_ERROR) {
          Alert.alert(
            'Security Alert',
          ...
          Logger.log('Error with seed phrase import', (err as Error).message);
        }
        track(MetaMetricsEvents.WALLET_SETUP_FAILURE, {
          wallet_setup_type: 'import',
          error_type: (err as Error).toString(),
        });
      }
    }
  };
  ...
  const onSeedWordsChange = useCallback(async (seedValue: string) => {
    ...
    if (Device.isAndroid()) {
      const androidOSVersion = parseInt(
        (Platform as Extract<typeof Platform, { OS: 'android' }>).constants
          .Release,
        10,
      );
