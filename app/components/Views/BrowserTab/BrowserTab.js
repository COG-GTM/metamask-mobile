...
import {
  getCaip25Caveat,
  getPermittedAccountsByHostname } from
'../../../core/Permissions';
import Routes from '../../../constants/navigation/Routes';
import {
  selectIpfsGateway,
  selectIsIpfsGatewayEnabled } from
...
import {
  ...
  WebViewNavigationEventName } from
'./types';
...
import {
  ...
  isProductSafetyDappScanningEnabled } from
...
export const BrowserTab = ({
  ...
  const blockListType = useRef(''); // TODO: Consider improving this type
  ...
  const onSubmitEditingRef = useRef(
    async () => {
  ...
  const sessionENSNamesRef = useRef({});
  const ensIgnoreListRef = useRef([]);
  ...
  const isTabActive = useSelector(
    (state) => state.browser.activeTab === tabId
  );

  /**
   * whitelisted url to bypass the phishing detection
   */
  const whitelist = useSelector((state) => state.browser.whitelist);
   ...
   */
  const isHomepage = useCallback((checkUrl) => {
  ...
  const handleIpfsContent = useCallback(
    ...
    }) =>
    {
      ...
      try {
        ...
        const handleIpfsContentError = err;
  ...
  const triggerDappViewedEvent = useCallback((urlToTrigger) => {
  ...
  const checkTabPermissions = useCallback(() => {
    ...
    if (isConnected) {
      ...
      try {
        ...
        const checkTabPermissionsError = e;
  ...
  const onChangeUrlBar = useCallback((text) => {
  ...
  const handleWebviewNavigationChange = useCallback(
...

    {
      ...
      switch (eventName) {
        case OnLoadProgress:
          return onLoadProgress(syntheticEvent);
        case OnLoadEnd:
          return onLoadEnd({
        ...
        case OnLoadStart:
          return onLoadStart(syntheticEvent);
        default:
          return;
      }
    },
