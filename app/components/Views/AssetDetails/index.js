...
import {
  balanceToFiat,
  renderFromTokenMinimalUnit } from
'../../../util/number';
...
import {
  ...
  selectConversionRateBySymbol } from
...
import {
  ...
  selectTokenMarketDataByChainId } from
'../../../selectors/tokenRatesController';
import {
  selectContractBalances,
  selectTokensBalances } from
...
const AssetDetails = (props) => {
  ...
  const networkConfigurationByChainId = useSelector((state) =>
  ...
  const tokenExchangeRatesByChainId = useSelector((state) =>
  ...
  const token = isPortfolioViewEnabled() ?
  portfolioToken :
  legacyToken;
  ...
  const renderSectionTitle = (title, isFirst) =>
  <Text
  ...
  const renderTokenBalance = () => {
    ...
    allTokenBalances[selectedAccountAddress]?.[chainId]?.[
    address] :
