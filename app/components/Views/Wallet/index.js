...
import {
  ...
  storePrivacyPolicyClickedOrClosed as storePrivacyPolicyClickedOrClosedAction } from
'../../../reducers/legalNotices';
...
import {
  ...
  showNftFetchingLoadingIndicator as showNftFetchingLoadingIndicatorAction } from
'../../../reducers/collectibles';
import { WalletViewSelectorsIDs } from '../../../../e2e/selectors/wallet/WalletView.selectors';
import {
  getMetamaskNotificationsUnreadCount,
  getMetamaskNotificationsReadCount,
  selectIsMetamaskNotificationsEnabled } from
...
const Wallet = ({
  ...
  const wizardStep = useSelector((state) => state.wizard.step);
  /**
  ...
  const networkOnboardingState = useSelector(
    (state) => state.networkOnboarded.networkOnboardedState
  );
        ...
        if (isPortfolioViewEnabled()) {
          ...
          const importPromises = Object.entries(tokensByChainId).map(
            async ([networkId, allTokens]) => {
              const chainConfig = evmNetworkConfigurations[networkId];
  ...
  const getNftDetectionAnalyticsParams = useCallback((nft) => {
    try {
      ...
      Logger.error(error, 'Wallet.getNftDetectionAnalyticsParams');
      return undefined;
    }
  }, []);
