...
const HeaderTitleText = Text as React.ComponentType<
  TextProps & { variant?: TextVariant }
>;

const MultichainTxView =
  MultichainTransactionsView as React.ComponentType<{ tabLabel?: string }>;
const TxView = TransactionsView as unknown as React.ComponentType<{
  tabLabel?: string;
}>;
const RampOrders = RampOrdersList as React.ComponentType<{ tabLabel?: string }>;
  ...
  return (
    <ErrorBoundary navigation={navigation} view="ActivityView">
      ...
      <View style={styles.wrapper}>
        ...
        <ScrollableTabView
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          ref={tabViewRef as unknown as React.RefObject<any>}
        ...
        >
          {selectedAddress && isNonEvmAddress(selectedAddress) ? (
            ...
            <TxView tabLabel={strings('transactions_view.title')} />
          )}
          {hasOrders && (
            <RampOrders tabLabel={strings('fiat_on_ramp_aggregator.orders')} />
          )}
        </ScrollableTabView>
      </View>
    </ErrorBoundary>
