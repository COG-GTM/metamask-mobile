...
const createStyles = (params: { theme: Theme; vars: unknown }) => {
  const { theme } = params;
  const { colors } = theme;
...
// Tab children of ScrollableTabView receive a `tabLabel` prop read by the tab bar.
...
const RampOrdersListTab = RampOrdersList as unknown as ComponentType<TabProps>;

const TitleText = Text as ComponentType<
  TextProps & { variant?: TextVariant; children?: ReactNode }
>;
      ...
      createEventBuilder(MetaMetricsEvents.BROWSER_OPEN_ACCOUNT_SWITCH)
        .addProperties({
          number_of_accounts: Object.keys(
            (selectedAddress ? accountsByChainId[selectedAddress] : undefined) ??
              {},
          ).length,
        })
        .build(),
