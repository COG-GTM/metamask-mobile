...
interface ChoosePasswordOwnProps {
   ...
   */
  route: RouteProp<{ params?: Record<string, string | undefined> }, 'params'>;
}
...
type ChoosePasswordProps = ChoosePasswordOwnProps & ChoosePasswordDispatchProps;
...
> {
  ...
  termsOfUse = async () => {
    if (this.props.navigation) {
      await navigateTermsOfUse(
        this.props.navigation.navigate as unknown as Parameters<
    ...
    try {
      ...
      if (previous_screen === ONBOARDING) {
        try {
          ...
          if (typeof Device.isIos === 'function')
  ...
  handleRejectedOsBiometricPrompt = async () => {
    ...
    this.setState({
      biometryType: newAuthData.availableBiometryType as
        | BIOMETRY_TYPE
        | string
        | null,
      biometryChoice: false,
  ...
  recreateVault = async (
    password: string,
    authType?: Awaited<
      ReturnType<typeof Authentication.componentAuthenticationType>
    >,
  ) => {
    ...
    await Authentication.newWalletAndRestore(
      ...
      seedPhrase as unknown as Parameters<
        typeof Authentication.newWalletAndRestore
      >[2],
      true,
    );
