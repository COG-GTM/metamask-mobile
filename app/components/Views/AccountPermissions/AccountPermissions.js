...
import {
  ...
  getPermittedChainIdsByHostname } from
'../../../core/Permissions';
...
import {
  ...
  AccountPermissionsScreens } from
'./AccountPermissions.types';
...
const AccountPermissions = (props) => {
  ...
  const origin = useSelector(getActiveTabUrl, isEqual);
  ...
  const previousPermittedAccounts = useRef();
  ...
  const accountsFilteredByPermissions = useMemo(() => {
    const accountsByPermittedStatus =
  ...
  const handleSelectChainIds = useCallback(async (chainIds) => {
  ...
  const handleSelectAccountAddresses = useCallback((selectedAccounts, newUserIntent) => {
    try {
      ...
      accountsToAdd = normalizedSelectedAddresses.reduce((result, account) => {
  ...
  const handleSelectAccountAddressesFromEditView = useCallback((selectedAccounts) => {
    handleSelectAccountAddresses(selectedAccounts, USER_INTENT.EditMultiple);
  }, [handleSelectAccountAddresses]);

  const handleSelectAccountAddressesFromConnectMoreView = useCallback((selectedAccounts) => {
    ...
    const permissionsSummaryProps = {
      ...
      onAddNetwork: () => {
        ...
        try {
          ...
          Logger.error(e, 'Error getting permitted chains caveat');
        }
