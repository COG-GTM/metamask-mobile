...
import {
 ...
 IUseMetricsHook } from '../../../components/hooks/useMetrics';
                ...
                >
                  <CLIcon
                    {...({
                      ...
                      onPress: toggleModal,
                    } as unknown as React.ComponentProps<typeof CLIcon>)}
                  />
                </TouchableOpacity>
              </View>
              <TextInput
...
> {
  ...
  generateErrorReport = (error: unknown, errorInfo = '') => {
    const {
      view,
      metrics: { trackEvent, createEventBuilder },
    } = this.props;
    const analyticsParams: ErrorReportParams = {
    ...
    trackEvent(
      createEventBuilder(MetaMetricsEvents.ERROR_SCREEN_VIEWED)
        .addProperties(analyticsParams as unknown as JsonMap)
        .build(),
    );
  };
