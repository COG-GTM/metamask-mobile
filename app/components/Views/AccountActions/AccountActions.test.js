...
const generateInitialStateWithSelectedAccount = (account) => (
{
  ...initialState,
  engine: {
    backgroundState: {
...
describe('AccountActions', () => {
  ...
  it('navigates to webview when View on Etherscan is clicked', () => {
    ...
    selectProviderConfig.mockReturnValue(
      mockProviderConfig
    );
  ...
  describe('clicks remove account', () => {
    it('clicks remove button after popup shows to trigger the remove account process', async () => {
      ...
      //Click remove button
      await act(async () => {
        const alertButtons = alertFnMock.mock.calls[0][2];
