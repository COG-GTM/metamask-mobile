...
// The installed react-native-gesture-handler typings predate React 18 and omit
// `children`, so the component is re-typed here to accept them.
const TapGestureHandlerWithChildren = TapGestureHandler as React.ComponentType<
...
function MediaPlayer({
  ...
  // react-native-video resolves bundled asset ids at runtime but its types only
  // describe the object form, so the number form is widened here.
  const source: ReactVideoSource =
    typeof uri === 'number' ? (uri as unknown as ReactVideoSource) : { uri };
