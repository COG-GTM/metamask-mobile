...
const IPFSGatewaySettings = () => {
  ...
  const [onlineIpfsGateways, setOnlineIpfsGateways] = useState([]);
  ...
  const handleAvailableIpfsGateways = useCallback(async () => {
    ...
    const ipfsGatewaysPromises = ipfsGateways.map(async (gateway) => {
      const testUrl =
  ...
  const setIsIpfsGatewayEnabled = (isIpfsGatewatEnabled) => {
    PreferencesController.setIsIpfsGatewayEnabled(isIpfsGatewatEnabled);
  };

  const setIpfsGateway = (gateway) => {
