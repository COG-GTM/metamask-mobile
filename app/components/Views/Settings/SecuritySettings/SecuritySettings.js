...
const Settings = () => {
  ...
  const detectNftComponentRef = useRef(null);
  const {
  ...
  const setPassword = async (enabled, passwordType) => {
    setLoading(true);
    let credentials;
    try {
  ...
  const onSignInWithPasscode = async (enabled) => {
  ...
  const toggleUseTransactionSimulations = (value) => {
  ...
  const handleChangeText = (text) => setHintText(text);
