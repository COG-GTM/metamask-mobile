...
const TabView = View as ComponentType<ViewProps & { tabLabel?: string }>;
const CellAnyProps = Cell as unknown as ComponentType<Record<string, unknown>>;
...
interface NetworkSettingsProps {
  ...
  route: RouteProp<
    { NetworkSettings: NetworkSettingsRouteParams },
    'NetworkSettings'
  >;
  ...
  matchedChainNetwork?: { safeChainsList?: any[] } | null;
...
> {
  ...
  componentDidMount = () => {
    ...
    if (networkTypeOrRpcUrl) {
      if (allNetworks.find((net) => networkTypeOrRpcUrl === net)) {
        const networkInformation = (Networks as Record<string, any>)[
          networkTypeOrRpcUrl
        ];
  ...
  checkIfNetworkExists = async (rpcUrl: string) => {
    ...
    const defaultNetworks = getAllNetworks().map(
      (item) => (Networks as Record<string, any>)[item],
    );
    const checkDefaultNetworks = defaultNetworks.filter(
      (item) => Number(item.rpcUrl) === (rpcUrl as unknown),
    );
  ...
  addRpcUrl = async () => {
    ...
    if (isPortfolioViewEnabled()) {
      const PreferencesController = Engine.context
        ...
        setTokenNetworkFilter: (filter: Record<string, unknown>) => void;
      };
      if (!isAllNetworks) {
  ...
  switchToMainnet = async () => {
    ...
    const networkConfigurations = this.props.networkConfigurations as any;
  ...
  showNetworkOnboardingAction: ({
    ...
    dispatch(
      showNetworkOnboardingAction({
        ...
        nativeToken,
        showNetworkOnboarding,
      } as Parameters<typeof showNetworkOnboardingAction>[0]),
    ),
});
