...
// This legacy view manages highly dynamic network-configuration form state.
// A permissive index signature is used to type-only migrate without refactor.
...
interface NetworkSettingsProps {
  ...
  navigation: any;
  route: any;
  showNetworkOnboardingAction: (args: any) => void;
...
> {
  ...
  templateInfuraRpc = (endpoint: any) =>
  ...
  checkIfNetworkExists = async (rpcUrl: any) => {
    ...
    const defaultNetworks = getAllNetworks().map(
      (item) => Networks[item as keyof typeof Networks] as any,
    );
  ...
  onRpcUrlDelete = async (url: any) => {
    const { addMode } = this.state;
    await this.setState((prevState) => ({
      rpcUrls: prevState.rpcUrls.filter((rpcUrl: any) => rpcUrl.url !== url),
    }));
    this.validateName();
    if (addMode) {
      this.validateChainId();
  ...
  onBlockExplorerUrlDelete = async (url: any) => {
    ...
    await this.setState((prevState) => ({
      blockExplorerUrls: prevState.blockExplorerUrls.filter(
        (explorerUrl: any) => explorerUrl !== url,
      ),
    }));
    this.validateName();
    if (addMode) {
