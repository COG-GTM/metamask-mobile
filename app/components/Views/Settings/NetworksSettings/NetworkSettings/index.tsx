...
type CellWithoutAvatarProps = Omit<CellSelectWithMenuProps, 'avatarProps'> & {
  variant: CellVariant.SelectWithMenu;
};

const CellWithoutAvatar =
  Cell as unknown as ComponentType<CellWithoutAvatarProps>;
...
interface TabViewChildProps extends ViewProps {
  tabLabel: string;
}

const TabView = View as unknown as ComponentType<TabViewChildProps>;
...
interface NetworkSettingsProps extends IWithMetricsAwarenessProps {
  /**
...
> {
  state: NetworkSettingsState = {
    ...
    popularNetwork: {} as ExtendedNetwork,
  ...
  componentDidMount = () => {
    ...
    if (networkTypeOrRpcUrl) {
      if (allNetworks.find((net) => networkTypeOrRpcUrl === net)) {
        const networkInformation = (Networks as Record<string, NetworkInfo>)[
          networkTypeOrRpcUrl
        ];
        chainId = (networkInformation.chainId as Hex).toString();
      ...
      this.setState({
        rpcUrl,
        rpcName,
        rpcUrls: rpcUrls as RpcEndpointForm[],
        blockExplorerUrls: blockExplorerUrls as string[],
        selectedRpcEndpointIndex: selectedRpcEndpointIndex as number,
        blockExplorerUrl,
        nickname,
        chainId,
        ticker,
  ...
  validateRpcAndChainId = () => {
    ...
    if (rpcUrl && chainId) {
      ...
      this.validateSymbol(chainToMatch as SafeChain);
    }
  };
  ...
  checkIfNetworkExists = async (rpcUrl?: string) => {
    ...
    const defaultNetworks = getAllNetworks().map(
      (item) => (Networks as Record<string, NetworkInfo>)[item],
    );
    const checkDefaultNetworks = defaultNetworks.filter(
      (item) => String(Number(item.rpcUrl)) === rpcUrl,
    );
  ...
  handleNetworkUpdate = async ({
    ...
    const networkConfig = {
      ...
      rpcEndpoints: rpcUrls as UpdateNetworkFields['rpcEndpoints'],
  ...
  validateSymbol = (chainToMatch: SafeChain | null = null) => {
    ...
    const symbol = chainToMatch
      ...
      : (networkList as SafeChain)?.nativeCurrency?.symbol ?? null;
  ...
  validateName = (chainToMatch: SafeChain | null = null) => {
    ...
    const name = chainToMatch?.name || (networkList as SafeChain)?.name || null;
  ...
  customNetwork = () => {
    ...
    return this.state.showNetworkDetailsModal ? (
      <CustomNetwork
        ...
        switchTab={this.tabView as TabBarProps}
        ...
        <View style={styles.informationWrapper}>
          ...
          networkTypeOrRpcUrl ? (
            ...
            <ScrollableTabView
              ...
              ref={(tabView: ScrollableTabView | null) => {
