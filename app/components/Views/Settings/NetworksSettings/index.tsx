...
interface NetworksSettingsProps {
  networkConfigurations: Record<string, {
    rpcEndpoints: { name?: string; url: string; networkClientId: string }[];
    name: string;
    chainId: string;
...
interface NetworksSettingsState {
  searchString: string;
  filteredNetworks: NetworkFilterEntry[];
}
...
class NetworksSettings extends PureComponent<NetworksSettingsProps, NetworksSettingsState> {
  ...
  networkElement(name: string, image: ImageSourcePropType | undefined, i: number, networkTypeOrRpcUrl: string, isCustomRPC: boolean, color?: string | null) {
    const colors = this.context.colors || mockTheme.colors;
