...
> {
    ...
    const entry = Object.entries(networkConfigurations).find(
      ([, networkConfiguration]) =>
        networkConfiguration.rpcEndpoints.some(
          (rpcEndpoint: { networkClientId: string }) =>
  ...
  handleSearchTextChange = (text: string) => {
    ...
    const defaultNetwork = getAllNetworks().map(
      (networkType: string): FilteredNetwork => {
        const { color, name, chainId } = Networks[
          networkType as keyof typeof Networks
        ] as { name: string; color: string; chainId: string };
        return {
          name,
          color,
    ...
    const customRPC = Object.values(this.props.networkConfigurations).map(
      (networkConfiguration, i: number): FilteredNetwork => {
      ...
      return this.state.filteredNetworks.map(
        (data: FilteredNetwork, i: number) => {
          ...
          return (
            ...
            this.networkElement(
              name,
              (image || color) as ImageSourcePropType,
              i,
              networkTypeOrRpcUrl,
              isCustomRPC,
            )
...
(NetworksSettings as any).contextType = ThemeContext;
