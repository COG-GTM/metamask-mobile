...
type NetworksSettingsProps = NetworksSettingsOwnProps &
...
> {
  ...
  createActionSheetRef = (ref: ActionSheetRef) => {
  ...
  renderRpcNetworksView = () => {
    ...
    const filteredChain = Object.keys(networkConfigurations).reduce(
      (filtered: typeof networkConfigurations, key) => {
        const network =
        ...
        if (!excludedChainIds.includes(network.chainId)) {
          filtered[key as keyof typeof networkConfigurations] = network;
        }
        return filtered;
      },
      {} as typeof networkConfigurations,
    );
  ...
  handleSearchTextChange = (text: string) => {
    ...
    const defaultNetwork = getAllNetworks().map(
      (networkType: string): NetworkListItem => {
        const { color, name, chainId } = Networks[
          networkType as keyof typeof Networks
        ] as { name: string; color?: string; chainId?: Hex };
        return {
          name,
          color,
    ...
    const customRPC = Object.values(this.props.networkConfigurations).map(
      (networkConfiguration, i): NetworkListItem => {
