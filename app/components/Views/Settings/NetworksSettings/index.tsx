...
const AvatarNetworkWithVariant = AvatarNetwork as ComponentType<
  AvatarNetworkProps & { variant: AvatarVariant }
>;

interface DefaultNetworkInfo {
  name: string;
  shortName: string;
  networkId?: number;
  chainId?: Hex;
...
type NetworksSettingsStyles = ReturnType<typeof createStyles> & {
  mainnetHeader?: StyleProp<ViewStyle>;
  networkInfo?: StyleProp<ViewStyle>;
  networksWrapper?: StyleProp<ViewStyle>;
  syncConfirm?: StyleProp<ViewStyle>;
};
...
> {
  ...
  removeNetwork = async () => {
    ...
    const [chainId] = entry as [Hex, NetworkConfiguration];
  ...
  networkElement(
    ...
    const colors = (this.context as Theme).colors || mockTheme.colors;
    const styles = createStyles(colors) as NetworksSettingsStyles;
    return (
  ...
  renderOtherNetworks() {
    return this.getOtherNetworks().map((networkType, i) => {
      const { name, imageSource, color } = (
        Networks as Record<string, DefaultNetworkInfo>
      )[networkType];
  ...
  renderRpcNetworksView = () => {
    ...
    const filteredChain = Object.keys(networkConfigurations).reduce(
      (filtered: Record<string, NetworkConfiguration>, key) => {
