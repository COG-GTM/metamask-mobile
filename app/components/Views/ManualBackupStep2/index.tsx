...
const ManualBackupStep2 = ({
  ...
  const words =
    process.env.JEST_WORKER_ID === undefined ? shuffle(routeWords) : routeWords;
  ...
  const selectWord = useCallback(
    (word: string, i: number) => {
      ...
      const existingPosition = wordsDict[`${word},${i}`].currentPosition;
      if (existingPosition !== undefined) {
        tempCurrentIndex = existingPosition;
