...
export const Browser: React.FC<BrowserProps> = (props) => {
  ...
  const updateTabInfo = useCallback(
    (tabID: number, info: TabInfo) => {
      updateTab(tabID, info);
    },
    [updateTab],
  );

  const hideTabsAndUpdateUrl = (url: string): void => {
  ...
  const switchToTab = (tab: BrowserTab): void => {
    trackEvent(
  ...
  const closeAllTabsHandler = (): void => {
    if (tabs.length) {
      triggerCloseAllTabs();
      navigation.setParams({
        ...route.params,
  ...
  const closeTabHandler = (tab: BrowserTab): void => {
      ...
      if (tabs.length > 1) {
        tabs.forEach((t, i) => {
          if (t.id === tab.id) {
            ...
            setActiveTab(newTabToSelect.id);
            navigation.setParams({
              ...route.params,
...
const mapDispatchToProps = (dispatch: (action: unknown) => void) => ({
  ...
  updateTab: (id: number, url: Partial<BrowserTab>) =>
