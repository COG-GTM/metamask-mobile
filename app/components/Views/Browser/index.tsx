...
/**
 * Represents a browser tab entry in the Redux store.
 */
interface BrowserStoreTab {
  id: number;
  url: string;
  image?: string;
  linkType?: string;
...
interface BrowserNavigation {
  ...
  setParams: (params: Partial<BrowserRouteParams>) => void;
  goBack: () => void;
}
...
export const Browser: React.FC<BrowserProps> = (props) => {
  ...
  const previousTabs = useRef<BrowserStoreTab[] | null>(null);
  ...
  const updateTabInfo = useCallback(
    (tabID: number, info: { url?: string; isArchived?: boolean; image?: string }) => {
  ...
  const switchToTab = (tab: BrowserStoreTab) => {
    trackEvent(
  ...
  const closeTab = (tab: BrowserStoreTab) => {
  ...
  const renderBrowserTabWindows = useCallback(
    () =>
      tabs
        ...
        .map((tab) => (
          // @ts-expect-error - BrowserTab is a connected component; Redux injects additional props (activeTab, searchEngine, etc.)
          <BrowserTab
            id={tab.id}
            key={`tab_${tab.id}`}
            initialUrl={tab.url}
