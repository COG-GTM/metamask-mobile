...
/**
 * Route params for the Browser component
 */
...
/**
 * Props for the Browser component from navigation
 */
...
/**
 * Props for the Browser component from Redux mapDispatchToProps
 */
...
/**
 * Props for the Browser component from Redux mapStateToProps
 */
...
/**
 * Combined props for the Browser component
 */
type BrowserProps = BrowserNavigationProps & BrowserDispatchProps & BrowserStateProps;
...
export const Browser: React.FC<BrowserProps> = (props) => {
  ...
  const previousTabs = useRef<Tab[] | null>(null);
  ...
  const closeTabHandler = (tab: Tab) => {
    ...
    if (tab.id === activeTabId) {
      if (tabs.length > 1) {
        tabs.forEach((t, i) => {
          if (t.id === tab.id) {
            let newTabToSelect = tabs[i - 1];
            if (tabs[i + 1]) {
              newTabToSelect = tabs[i + 1];
            }
            triggerSetActiveTab(newTabToSelect.id);
            navigation.setParams({
              ...route.params,
              url: newTabToSelect.url,
            });
          }
        });
      } else {
...
const mapDispatchToProps = (dispatch: Dispatch): BrowserDispatchProps => ({
  // @ts-expect-error The createNewTab action accepts optional linkType parameter
  ...
  updateTab: (id: number, data: Partial<Tab>) => dispatch(updateTab(id, data)),
});
