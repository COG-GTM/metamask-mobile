...
interface BrowserTabViewProps {
  ...
  initialUrl: string;
  linkType?: string;
  updateTabInfo: (tabID: number, info: Partial<BrowserTabInfo>) => void;
  showTabs: () => void;
...
const BrowserTabView =
  BrowserTab as unknown as ComponentType<BrowserTabViewProps>;
...
interface BrowserProps {
   ...
   */
  updateTab: (id: number, url: Partial<BrowserTabInfo>) => void;
  /**
   * Array of tabs
   */
  tabs: BrowserTabInfo[];
...
export const Browser = (props: BrowserProps) => {
  ...
  useFocusEffect(
    useCallback(() => {
      if (
        isSolanaAccount(
          currentSelectedAccount as Parameters<typeof isSolanaAccount>[0],
        )
      ) {
        toastRef?.current?.showToast({
          ...
          networkImageSource:
            ...
            require('../../../images/solana-logo.png') as ImageSourcePropType,
          labelOptions: [
            {
              label: `${strings(
  ...
  const newTab = useCallback(
    (url?: string, newTabLinkType?: string) => {
  ...
  const hideTabsAndUpdateUrl = (url: string | null) => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      ...
      setTabIdleTimes((prevIdleTimes) => {
        ...
        tabs.forEach((tab: BrowserTabInfo) => {
    ...
    () => {
      ...
      if (!currentUrl && !existingTabId) {
        ...
        const activeTab = tabs.find(
          (tab: BrowserTabInfo) => tab.id === activeTabId,
        );
        if (activeTab) {
    ...
    () => {
      ...
      if (newTabUrl && deeplinkTimestamp) {
        ...
        const existingTab = tabs.find(
          (tab: BrowserTabInfo) => tab.id === existingTabId,
        );
        if (existingTab) {
    ...
    (url: string, tabID: number) =>
      new Promise((resolve, reject) => {
        captureScreen({
          ...
          THUMB_WIDTH,
          THUMB_HEIGHT,
        } as unknown as Parameters<typeof captureScreen>[0]).then(
          (uri) => {
            updateTab(tabID, {
              url,
              image: uri,
