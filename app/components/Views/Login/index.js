...
import {
  passcodeType,
  updateAuthTypeStorageFlags } from
'../../../util/authentication';
...
import HelpText, {
  HelpTextSeverity } from
'../../../component-library/components/Form/HelpText';
...
import {
  ...
  WRONG_PASSWORD_ERROR_ANDROID_2 } from
'./constants';
import {
...
const Login = () => {
  ...
  const setOnboardingWizardStep = (step) =>
  dispatch(setOnboardingWizardStepUtil(step));
  const setAllowLoginWithRememberMe = (enabled) =>
  ...
  const updateBiometryChoice = async (newBiometryChoice) => {
  ...
  const onLogin = async () => {
    ...
    try {
      ...
      containsErrorMessage(loginError, JSON_PARSE_ERROR_UNEXPECTED_TOKEN))
      {
        ...
        } catch (vaultCorruptionErr) {
          const vaultCorruptionError = vaultCorruptionErr;
  ...
  const renderSwitch = () => {
    const handleUpdateRememberMe = (rememberMeChoice) => {
