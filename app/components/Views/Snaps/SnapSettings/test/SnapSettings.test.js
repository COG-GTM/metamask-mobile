...
import {
  ...
  KEYRING_SNAP_REMOVAL_WARNING_TEXT_INPUT } from
'../../KeyringSnapRemovalWarning/KeyringSnapRemovalWarning.constants';
...
describe('SnapSettings with keyring snap', () => {
  ...
  const mockKeyringSnapId = 'npm:@metamask/snap-simple-keyring-snap';
  const mockKeyringSnap = {
    blocked: false,
    enabled: true,
  ...
  const mockKeyringSnapPermissions = {
    'endowment:keyring': {
      id: 'Bjj3InYtb6U4ak-uja0f_',
  ...
  it('renders KeyringSnapRemovalWarning when removing a keyring snap', async () => {
    const { getByTestId } = renderWithProvider(<SnapSettings />, {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      state: initialStateWithKeyringSnap
    });
