...
const NetworkSelector = () => {
  ...
  const networkMenuSheetRef = useRef(null);

  const rpcMenuSheetRef = useRef(null);

  const deleteModalSheetRef = useRef(null);
  ...
  const showNetworkModal = (networkConfiguration) => {
  ...
  networkName) =>
  {
    const searchResult = networks.filter(({ name }) =>
  ...
  const isNoSearchResults = (networkIdenfier) => {
