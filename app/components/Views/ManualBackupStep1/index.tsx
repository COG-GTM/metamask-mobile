...
const ManualBackupStep1 = ({
  ...
  const tryExportSeedPhrase = async (exportPassword: string) => {
  ...
  const tryUnlockWithPassword = async (unlockPassword: string | undefined) => {
    setReady(false);
    try {
      const seedPhrase = await tryExportSeedPhrase(unlockPassword ?? '');
      setWords(seedPhrase);
      setView(SEED_PHRASE);
      setReady(true);
    } catch (e) {
