...
interface Props {
  ...
  */
   ...
   */
  route: {
    params?: ManualBackupStep1Params;
  };
  /**
...
const ManualBackupStep1 = ({ route, navigation, appTheme }: Props) => {
  ...
  const tryExportSeedPhrase = async (passwordValue: string) => {
  ...
  const onPasswordChange = (passwordValue: string) => {
  ...
  const tryUnlockWithPassword = async (passwordValue: string) => {
    setReady(false);
    try {
      const seedPhrase = await tryExportSeedPhrase(passwordValue);
      setWords(seedPhrase);
      setView(SEED_PHRASE);
      setReady(true);
    } catch (e) {
