...
// TODO: Once all reducers across every workstream are valid TypeScript Redux
// reducers, this type can be automatically generated using the
// `StateFromReducersMapObject` type from redux. Some reducers owned by other
// workstreams are not valid reducers yet (e.g. `legalNotices`,
// `experimentalSettings` infer to `never`), so the slices migrated in this
// workstream are typed explicitly while the remaining ones are left loose.
