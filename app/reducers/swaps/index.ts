...
type SwapsFeatureFlags = any;

// TODO: Replace "any" with type
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type SwapsRootState = any;
...
export interface SetSwapsLivenessAction {
  ...
  payload: { chainId: string; featureFlags?: SwapsFeatureFlags };
}
...
type SwapsUserToken = (SwapsToken | Token) &
  Partial<Pick<Token, 'hasBalanceError' | 'image'>>;
...
const swapsStateSelector = (state: SwapsRootState): SwapsState => state.swaps;
/**
...
export const swapsLivenessMultichainSelector = createSelector(
  [swapsStateSelector, (_state: SwapsRootState, chainId: Hex) => chainId],
  (swapsState, chainId) => swapsState[chainId]?.isLive || false,
);
...
export const selectSwapsChainFeatureFlags = createSelector(
  swapsStateSelector,
  (_state: SwapsRootState, transactionChainId?: Hex) =>
...
const swapsControllerAndUserTokens = createSelector(
  ...
  (swapsTokens, tokens) => {
    const values = [
      ...((swapsTokens || []) as SwapsUserToken[]),
      ...((tokens || []) as SwapsUserToken[]),
    ]
      .filter(Boolean)
...
const swapsControllerAndUserTokensMultichain = createSelector(
  ...
  (swapsTokens, allTokens, currentUserAddress) => {
    ...
    const values = [
      ...((swapsTokens || []) as SwapsUserToken[]),
      ...((allUserTokensCrossChains || []) as SwapsUserToken[]),
    ]
      .filter(Boolean)
      .reduce((map, { hasBalanceError, image, ...token }) => {
        const key = token.address.toLowerCase();
...
export const swapsTokensWithBalanceSelector = createSelector(
  ...
  (chainId, tokens, tokenList, balances) => {
    ...
    const tokensAddressesWithBalance = Object.entries<unknown>(balances)
