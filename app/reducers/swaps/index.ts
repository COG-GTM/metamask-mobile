...
interface SetSwapsLivenessPayload {
  chainId: Hex;
  featureFlags: SwapsFeatureFlags | null;
}

interface SetSwapsLivenessAction extends Action<typeof SWAPS_SET_LIVENESS> {
  payload: SetSwapsLivenessPayload;
}

interface SetSwapsHasOnboardedAction extends Action<typeof SWAPS_SET_HAS_ONBOARDED> {
  payload: boolean;
}

type SwapsAction = SetSwapsLivenessAction | SetSwapsHasOnboardedAction | Action<string>;

export const setSwapsLiveness = (chainId: Hex, featureFlags: SwapsFeatureFlags | null): SetSwapsLivenessAction => ({
...
function addMetadata(chainId: Hex, tokens: SwapsToken[], tokenList: Record<string, TokenMetadata>): SwapsToken[] {
...
const swapsControllerAndUserTokens = createSelector(
  ...
  selectTokens,
  (swapsTokens, tokens) => {
    const values = [...(swapsTokens || []), ...((tokens as TokenWithBalance[]) || [])]
      .filter(Boolean)
...
const swapsControllerAndUserTokensMultichain = createSelector(
  ...
  (swapsTokens, allTokens, currentUserAddress) => {
    ...
    const allUserTokensCrossChains = allTokensArr.reduce<TokenWithBalance[]>(
      (acc, tokensElement) => {
        const found = (tokensElement as Record<string, TokenWithBalance[]>)[currentUserAddress as string] || [];
...
export const swapsTokensSelector = createSelector(
  ...
  (chainId, tokens, tokenList) => {
    ...
    return addMetadata(chainId as Hex, tokens, tokenList as Record<string, TokenMetadata>);
  },
);
...
export const swapsTokensWithBalanceSelector = createSelector(
  ...
  (chainId, tokens, tokenList, balances) => {
    ...
    const tokensAddressesWithBalance = Object.entries(balances || {})
      ...
      .sort(([, balanceA], [, balanceB]) => (lte(balanceB as string, balanceA as string) ? -1 : 1))
    ...
    return addMetadata(chainId as Hex, result, tokenList as Record<string, TokenMetadata>);
  },
);
...
function swapsReducer(state: SwapsState = initialState, action: SwapsAction): SwapsState {
  switch (action.type) {
    case SWAPS_SET_LIVENESS: {
      const { chainId: rawChainId, featureFlags } = (action as SetSwapsLivenessAction).payload;
      ...
      const chain: SwapsChainState = {
        ...data,
        featureFlags: chainFeatureFlags as SwapsFeatureFlags,
    ...
    case SWAPS_SET_HAS_ONBOARDED: {
      return {
        ...state,
        hasOnboarded: Boolean((action as SetSwapsHasOnboardedAction).payload),
      };
    }
    default: {
      return state;
