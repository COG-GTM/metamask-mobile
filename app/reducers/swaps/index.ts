...
/**
 * Permissive token shape used while combining tokens that come from the Swaps
 * controller and the user's token list. The two sources have slightly
 * different shapes, so we model the union loosely here.
 */
interface SwapsTokenWithMetadata {
  address: string;
...
export const swapsLivenessMultichainSelector = createSelector(
  ...
  (swapsState, chainId) => swapsState[chainId as Hex]?.isLive || false,
);
...
const swapsControllerAndUserTokens = createSelector(
  ...
  (swapsTokens, tokens) => {
    const values = [...(swapsTokens || []), ...(tokens || [])]
      .filter(Boolean)
      .reduce((map, current) => {
        const { hasBalanceError, image, ...token } =
          current as SwapsTokenWithMetadata;
...
export const swapsTopAssetsSelector = createSelector(
  ...
  (chainId, tokens, tokenList, topAssetsList) => {
    ...
    const result = topAssetsList
      ...
      .filter(Boolean) as SwapsTokenWithMetadata[];
    return addMetadata(chainId, result, tokenList);
  },
);
...
function swapsReducer(
  ...
  switch (action.type) {
    case SWAPS_SET_LIVENESS: {
      ...
      const data = state[chainId as Hex] as SwapsChainState | undefined;
      ...
      if (!featureFlags) {
        return {
          ...state,
          [chainId as Hex]: chainNoFlags,
          [rawChainId as Hex]: chainNoFlags,
          featureFlags: undefined,
        };
      }
      ...
      const liveness = getSwapsLiveness(featureFlags, chainId as Hex);
