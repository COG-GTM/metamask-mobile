...
function addMetadata(chainId: string, tokens: SwapToken[], tokenList: Record<string, { name?: string }>) {
...
export const swapsTokensWithBalanceSelector = createSelector(
  ...
  (chainId, tokens, tokenList, balances) => {
    ...
    const tokensAddressesWithBalance = Object.entries(balances)
      ...
      .sort(([, balanceA], [, balanceB]) => (lte(balanceB as number, balanceA as number) ? -1 : 1))
...
type SwapsSetLivenessAction = {
  type: typeof SWAPS_SET_LIVENESS;
  payload: { chainId: string; featureFlags: FeatureFlagsGlobal | null };
};

type SwapsSetHasOnboardedAction = {
...
export interface SwapsState {
  ...
  featureFlags: FeatureFlagsGlobal | undefined;
