...
// TODO: Replace "[key: string]: unknown" with a fully-typed token
...
// TODO: Replace "[key: string]: unknown" with a fully-typed token list entry
type TokenListMap = Record<string, { name?: string } | undefined>;
...
interface SwapsFeatureFlags {
  ...
  smart_transactions?: SmartTransactionsFlags;
  [key: string]: unknown;
}
...
export interface SwapsState {
  ...
  featureFlags?: SwapsFeatureFlags;
  '0x1': SwapsChainState;
  [chainId: string]: SwapsChainState | SwapsFeatureFlags | boolean | undefined;
}
...
export const swapsSmartTxFlagEnabled = createSelector(
  // TODO: Type as RootState once all swaps selector consumers are migrated
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (state: any) => state.swaps,
  (swapsState) => {
...
const swapsControllerAndUserTokens = createSelector(
  ...
  (swapsTokens, tokens) => {
    const values = [...(swapsTokens || []), ...(tokens || [])]
      ...
      .reduce(
        (
          map: Map<string, SwapsTokenLike>,
          { hasBalanceError, image, ...token }: SwapsTokenLike,
        ) => {
