...
function addMetadata(chainId: string, tokens: Token[], tokenList: Record<string, TokenMetadata>): Token[] {
...
const swapsControllerAndUserTokensMultichain = createSelector(
  ...
  (swapsTokens, allTokens, currentUserAddress) => {
    ...
    const allUserTokensCrossChains = allTokensArr.reduce<Token[]>(
      (acc, tokensElement) => {
        const found = (tokensElement as Record<string, Token[]>)[currentUserAddress as string] || [];
...
export const swapsTokensSelector = createSelector(
  ...
  (chainId, tokens, tokenList) => {
    ...
    return addMetadata(chainId, tokens, tokenList as Record<string, TokenMetadata>);
  },
);
...
export const swapsTokensWithBalanceSelector = createSelector(
  ...
  (chainId, tokens, tokenList, balances) => {
    ...
    return addMetadata(chainId, result, tokenList as Record<string, TokenMetadata>);
  },
);
...
function swapsReducer(state: SwapsState = initialState, action: SwapsAction): SwapsState {
  switch (action.type) {
    case SWAPS_SET_LIVENESS: {
      const payload = action.payload as { chainId: string; featureFlags: Record<string, unknown> };
