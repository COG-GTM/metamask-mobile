...
function addMetadata(
  chainId: Hex,
  tokens: AggregatableToken[],
  tokenList: TokenListMap | TokenListToken[],
): AggregatableToken[] {
  if (!isMainnetByChainId(chainId) || Array.isArray(tokenList)) {
    return tokens;
  }
...
const swapsControllerAndUserTokens = createSelector(
  ...
  selectTokens,
  (swapsTokens, tokens) => {
    const merged: AggregatableToken[] = [
      ...(swapsTokens || []),
      ...(tokens || []),
    ].filter(Boolean);
    const values = merged
      .reduce<Map<string, AggregatableToken>>((map, { hasBalanceError, image, ...token }) => {
...
const swapsControllerAndUserTokensMultichain = createSelector(
  ...
  (swapsTokens, allTokens, currentUserAddress) => {
    ...
    const allUserTokensCrossChains = allTokensArr.reduce<AggregatableToken[]>(
      (acc, tokensElement) => {
        const found = (currentUserAddress
          ? tokensElement[currentUserAddress]
          : []) as AggregatableToken[];
...
export const swapsTokensWithBalanceSelector = createSelector(
  ...
  (chainId, tokens, tokenList, balances) => {
    ...
    const tokensWithBalance: AggregatableToken[] = [];
    const originalTokens: AggregatableToken[] = [];
...
export const swapsTopAssetsSelector = createSelector(
  ...
  (chainId, tokens, tokenList, topAssetsList) => {
    ...
    const result = topAssetsList
      ...
      .filter(Boolean) as AggregatableToken[];
    return addMetadata(chainId, result, tokenList);
  },
);
