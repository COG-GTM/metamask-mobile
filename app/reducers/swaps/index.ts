...
const swapsStateSelector = (state: Record<string, unknown>) => (state as { swaps: SwapsState }).swaps;
/**
...
const selectSwapsControllerState = (state: Record<string, Record<string, Record<string, unknown>>>) =>
  state.engine.backgroundState.SwapsController;

/**
 * Returns the swaps tokens from the state
 */
export const swapsControllerTokens = (state: Record<string, Record<string, Record<string, Record<string, unknown>>>>) =>
  (state.engine.backgroundState.SwapsController as { tokens: SwapToken[] }).tokens;
...
function swapsReducer(state: SwapsState = initialState, action: { type: string; payload?: Record<string, unknown> }): SwapsState {
  switch (action.type) {
