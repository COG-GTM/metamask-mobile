...
export type FavoriteCollectiblesByChain = Record<string, FavoriteCollectible[]>;
export type FavoriteCollectiblesByAddress = Record<
  string,
  FavoriteCollectiblesByChain
>;
...
export const collectiblesSelector = createDeepEqualSelector(
  ...
  selectChainId,
  selectAllNfts,
  (address, chainId, allNfts) => (address && allNfts[address]?.[chainId as Hex]) || [],
);
...
const collectiblesFavoritesReducer = (
  ...
  switch (action.type) {
    ...
    case REMOVE_FAVORITE_COLLECTIBLE: {
      const { chainId, collectible } = action;
