...
const emptyAction = { type: 'UNKNOWN' } as never;
...
describe('notifications reducer', () => {
  ...
  describe('actions', () => {
    let stateWithNotifications: ReturnType<typeof reducer> = reducer(
      undefined,
      { type: '__INIT__' as never },
    );
    ...
    beforeEach(() => {
      ...
      ].reduce<ReturnType<typeof reducer>>(
        (acc, current) => current(acc),
        reducer(undefined, { type: '__INIT__' as never }),
      );
    });
    ...
    it('should remove current notification', () => {
      ...
      const currentNotificationId =
        stateWithNotifications.notifications[0].id;
      const state = reducer(stateWithNotifications, {
