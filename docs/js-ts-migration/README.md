...
| **Claim a whole workstream, not individual files.** Workstreams are disjoint directory sets. | Sessions in different workstreams cannot touch the same folder, so their PRs cannot conflict. |
...
## 6. Known environment gotcha

`yarn install` currently fails on `react-native-tcp`: `package.json` pins it to the GitHub tarball
`aprock/react-native-tcp#98fbc801…`, and that repository no longer exists (HTTP 404). Until the
dependency is repointed at the published `react-native-tcp` npm package, every session has to work
around it locally before it can run `yarn lint:tsc`, `yarn lint`, or `yarn test:unit`.

Also note `app/util/termsOfUse/termsOfUseContent.ts` is generated by `yarn setup`; without it
`yarn lint:tsc` reports one unrelated error.
