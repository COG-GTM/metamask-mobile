...
When(/^On the onboarding wizard I tap on "([^"]*)" button$/, async (text: string) => {
  switch (text) {
    case 'Take a Tour':
...
When(/^I enter "([^"]*)" for account name$/, async (text: string) => {
  await WalletAccountModal.editAccountNameLabel(text);
});

Then(/^the tutorial modal heading should read "([^"]*)"$/, async (text: string) => {
...
Then(/^the account nickname should read "([^"]*)"$/, async (text: string) => {
