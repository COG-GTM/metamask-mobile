...
When(/^I input "([^"]*)" in the search field$/, async (text: string) => {
  await BrowserScreen.tapUrlBar();
  await AddressBarScreen.editUrlInput(text);
});

When(/^I tap on "([^"]*)" on account list$/, async (text: string) => {
...
Then(/^"([^"]*)" is the active wallet account$/, async (text: string) => {
...
When(/^I navigate to "([^"]*)"$/, async function (text: string) {
  await BrowserScreen.tapUrlBar();
...
Given(/^I have (\d+) browser tab displayed$/, async (number: number) => {
...
Then(/^"([^"]*)" view is displayed$/, async (text: string) => {
  switch (text) {
    case 'Add Favorites':
...
Then(/^Name field is pre populated with "([^"]*)"$/, async (text: string) => {
  await AddFavoriteScreen.titleEditTextContains(text);
});

Then(/^Url field is pre populated with "([^"]*)"$/, async (text: string) => {
  await AddFavoriteScreen.urlEditTextContains(text);
});

When(/^I tap on "([^"]*)" on the Add Favorite Screen$/, async (text: string) => {
  switch (text) {
    case 'Cancel':
...
Then(/^the "([^"]*)?" is displayed in the browser tab$/, async (text: string) => {
  await BrowserScreen.tapUrlBar();
  await AddressBarScreen.isUrlValueContains(text);
  await AddressBarScreen.tapUrlCancelButton();
});

Then(/^the favorite is not added on the home "([^"]*)" page$/, async (text: string) => {
  await BrowserScreen.tapUrlBar();
  await AddressBarScreen.editUrlInput(text);
...
When(/^I input "([^"]*)" in the favorite name field$/, async (text: string) => {
...
Then(/^I should see "([^"]*)" error title$/, async (text: string) => {
  await ExternalWebsitesScreen.isErrorPageTitle(text);
});

Then(/^I should see "([^"]*)" error message$/, async (text: string) => {
...
When(/^I input "([^"]*)" in address field$/, async (text: string) => {
  await AddressBarScreen.editUrlInput(text);
});
...
When(/^I tap on browser tab button with count (\d+)$/, async (number: number) => {
  await BrowserScreen.numberOfTapsEqualsTo(number);
...
Then(/^I tap on browser tab with text "([^"]*)?"/, async (text: string) => {
  const timeout = 1000;
...
Then(/^new browser tab is displayed on "([^"]*)"$/, async (text: string) => {
  await BrowserScreen.tapUrlBar();
  await AddressBarScreen.isUrlValueContains(text);
  await AddressBarScreen.tapUrlCancelButton();
});

Then(/^browser tab count is (\d+)$/, async (number: number) => {
  await driver.pause(2000);
  await BrowserScreen.numberOfTapsEqualsTo(number);
});

When(/^I input "([^"]*)" in my favorite url field$/, async (text: string) => {
  await AddFavoriteScreen.editUrlEditText(text);
});

When(/^I tap on "([^"]*)" button on multi browser tab view$/, async (text: string) => {
  switch (text) {
    case 'Close All':
...
When(/^I tap the "([^"]*)" option on the Option Menu$/, async (option: string) => {
  switch (option) {
...
When(/^I tap on "([^"]*)" in address field$/, async (button: string) => {
  switch (button) {
    case 'Cancel button':
      await AddressBarScreen.tapUrlCancelButton();
      break;
...
When(/^I select "([^"]*)" network option$/, async (network: string) => {
...
Then(/^the "([^"]*)" url is displayed in address field$/, async (text: string) => {
  await AddressBarScreen.isUrlValueContains(text);
});
...
Then(/^the browser view is on the "([^"]*)" website$/, async (url: string) => {
...
Then(/^I set "([^"]*)" as my primary account$/, async (text: string) => {
  await CommonScreen.tapOnText(text);
});
