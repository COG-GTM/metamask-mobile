...
Then(/^I enter in a contact name "([^"]*)?"/, async (text: string) => {
...
Given(/^I should see a warning message "([^"]*)?"/, async (message: string) => {
  await SendScreen.isSendWarningMessageVisible(message);
});

When(/^I see a button with text "([^"]*)?"/, async (text: string) => {
...
Then(/^the token (.*) being sent is visible/, async (token: string) => {
  await TransactionConfirmScreen.isCorrectTokenConfirm(token);
});

Then(/^the token amount (.*) to be sent is visible/, async (amount: string) => {
...
Then(/^I type amount "([^"]*)?" into amount input field/, async (amount: string) => {
