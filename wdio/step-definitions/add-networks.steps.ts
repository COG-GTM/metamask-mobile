...
When(/^"([^"]*)?" tab is displayed on networks screen/, async (netWorkTab: string) => {
  switch (netWorkTab) {
    case 'POPULAR':
...
When(/^I tap on network "([^"]*)?" to add it/, async (network: string) => {
...
Then(/^In settings I tap on "([^"]*)?"/, async (option: string) => {
...
Then(/^I tap on the "([^"]*)?" tab/, async (netWorkTab: string) => {
  switch (netWorkTab) {
    case 'POPULAR':
...
Then(/^I type "([^"]*)?" into Network name field/, async (data: string) => {
...
Then(/^I type "([^"]*)?" into the RPC url field/, async (data: string) => {
...
Then(/^I type "([^"]*)?" into the Chain ID field/, async (data: string) => {
...
Then(/^I type "([^"]*)?" into the Network symbol field/, async (data: string) => {
...
Then(/^I tap and hold network "([^"]*)?"/, async (network: string) => {
  await NetworksScreen.tapAndHoldNetwork(network);
});

Then(/^I should see an alert window with the text "([^"]*)?"/, async (text: string) => {
  await NetworksScreen.isButtonTextVisibleByXpath(text);
});

When(/^I click "([^"]*)?" on remove network modal/, async (text: string) => {
...
Then(/^I tap on network "([^"]*)?" on networks screen/, async (network: string) => {
  await NetworksScreen.tapOnNetwork(network);
});

Then(/^I switch to "([^"]*)?" in the network list modal /, async (text: string) => {
  const setTimeout = 1500;
  await driver.pause(setTimeout);
  await NetworkListModal.changeNetwork(text);
});

Then(/^a "([^"]*)?" button should be visible/, async (buttons: string) => {
  switch (buttons) {
    case 'Delete':
...
Then(/^I tap the "([^"]*)?" button/, async (buttons: string) => {
  switch (buttons) {
    case 'Delete':
