...
Then(/^"([^"]*)?" carousel item is displayed/, async (text: string) => {
  switch (text) {
...
When(/^I type (.*) in SRP field/, async (text: string) => {
  await ImportFromSeedScreen.typeSecretRecoveryPhrase(text);
});

When(/^I type (.*) in new password field/, async (text: string) => {
  await ImportFromSeedScreen.typeNewPassword(text);
});

When(/^I type (.*) in confirm password field/, async (text: string) => {
  await ImportFromSeedScreen.typeConfirmPassword(text);
});

Then(/^device alert (.*) is displayed/, async (text: string) => {
  await ImportFromSeedScreen.isAlertTextVisible(text);
});

Then(/^password strength (.*) is displayed/, async (text: string) => {
  await ImportFromSeedScreen.isPasswordStrengthTextCorrect(text);
});

When(/^On Wallet Setup Screen I tap "([^"]*)?"/, async (text: string) => {
  switch (text) {
...
When(/^I input a new password "([^"]*)?"/, async (password: string) => {
  await CreateNewWalletScreen.inputPasswordInFirstField(password);
});

When(/^I confirm the new password "([^"]*)?"/, async (password: string) => {
  await CreateNewWalletScreen.inputConfirmPasswordField(password);
});

When(/^Select "([^"]*)?" on remind secure modal/, async (button: string) => {
