...
let loginViewTime: number;
let walletViewToAppearTime: number;
let total: number;
Then(/^the app start time should not exceed "([^"]*)?" milliseconds$/, async (time: string) => {
...
Then(/^The Login screen should be visible in "([^"]*)?" seconds$/, async (time: string) => {
  ...
  await expect(loginViewTime).toBeLessThan(Number(time) * 1000);
...
Then(/^The wallet view appears in "([^"]*)?" seconds$/, async (time: string) => {
  ...
  await expect(walletViewToAppearTime).toBeLessThan(Number(time) * 1000);
...
When(/^I background the app for (\d+) seconds$/, async (time: number) => {
