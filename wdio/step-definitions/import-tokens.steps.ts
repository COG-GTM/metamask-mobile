...
When(/^I type (.*) into token Address field/, async (address: string) => {
...
When(/^I tap (.*) of the token Address field/, async (label: string) => {
...
Then(/^I should see "([^"]*)?" toast message/, async (toast: string) => {
