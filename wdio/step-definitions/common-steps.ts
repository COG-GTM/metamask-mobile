...
import OnboardingSucessScreen from '../screen-objects/OnboardingSucessScreen';
...
Given(/^I import wallet using seed phrase "([^"]*)?"/, async (phrase: string) => {
  const validAccount = Accounts.getValidAccount();
...
Then(/^"([^"]*)?" is visible/, async (text: string) => {
  const timeout = 2500;
  await driver.pause(timeout);
  await CommonScreen.isTextDisplayed(text);
});

Then(/^"([^"]*)?" is displayed on (.*) (.*) view/, async (text: string) => {
  const timeout = 1000;
  await driver.pause(timeout);
  await CommonScreen.isTextDisplayed(text);
});

Then(/^"([^"]*)?" is displayed/, async (text: string) => {
  const timeout = 1000;
  await driver.pause(timeout);
  await CommonScreen.isTextDisplayed(text);
});

Then(/^version "([^"]*)?" is displayed for app upgrade step/, async (text: string) => {
  const appUpgradeText = process.env[text] as string;
  const timeout = 1000;
  await driver.pause(timeout);
  await CommonScreen.isTextDisplayed(appUpgradeText);
});

Then(/^"([^"]*)?" is not displayed/, async (text: string) => {
  const timeout = 1000;
  await driver.pause(timeout);
...
Then(/^I tap on button with text "([^"]*)?"/, async (text: string) => {
  const timeout = 1000;
  await driver.pause(timeout);
...
When(/^I unlock wallet with (.*)$/, async (password: string) => {
  await LoginScreen.waitForScreenToDisplay();
...
Then(
  /^I tap (.*) "([^"]*)?" on (.*) (.*) view/,
  async (elementType: string, button: string) => {
...
Then(/^I tap (.*) containing text "([^"]*)?"/, async (elementType: string, button: string) => {
...
Then(
  ...
  async (elementType: string, text: string) => {
    await CommonScreen.isTextDisplayed(text);
  },
);
...
Then(
  ...
  async (elementType: string, textElement: string) => {
...
When(/^I install upgrade the app$/, async () => {
  await driver.installApp(process.env.BROWSERSTACK_ANDROID_APP_URL as string)
});
...
Then(/^I am on the "([^"]*)" account$/, async (accountName: string) => {
...
Then(/^tokens (.*) in account should be displayed$/, async (token: string) => {
