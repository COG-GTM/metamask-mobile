...
Then(/^I tap on the "([^"]*)?" button/, async (text: string) => {
...
Then(/^I input "([^"]*)?" into the contact name field/, async (name: string) => {
  await AddContact.waitForDisplay();
  await AddContact.fillContactNameField(name);
});

Then(/^I input "([^"]*)?" in the Address field/, async (name: string) => {
  await AddContact.fillAddressField(name);
});

Then(/^I tap on contact name "([^"]*)?"/, async (name: string) => {
  await driver.pause(2500);
  await Contacts.tapOnText(name); // duplicate of line # in the wallet-view.step file
});

Then(/^the saved contact "([^"]*)?" should appear/, async (contactName: string) => {
  await Contacts.waitForDisplayed();
...
Then(/I can edit the contact name to "([^"]*)?"/, async (name: string) => {
  await AddContact.waitForAddContactButton();
  await AddContact.fillContactNameField(name);
