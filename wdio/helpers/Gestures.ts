 ...
 * multiple times.
 */
let SCREEN_SIZE: ScreenSize | undefined;
...
type ElementLike = WebdriverIO.Element | Promise<WebdriverIO.Element>;
...
class Gestures {
  static async waitAndTap(element: ElementLike) {
    const elem = await element;
    await elem.waitForDisplayed({timeout: 25000});
    (await elem).touchAction(Actions.TAP);
  }

  static async tap(element: ElementLike, tapType = 'TAP') {
  ...
  static async tapTextByXpath(text: string, tapType = 'TAP') {
  ...
  static async tapByTextContaining(text: string, tapType = 'TAP') {
  ...
  static async longPress(element: ElementLike, waitTime: number) {
    const elem = await element;
  ...
  static async typeText(element: ElementLike, text: string) {
    const elem = await element;
    await elem.waitForDisplayed();
    (await elem).touchAction(Actions.TAP);
  ...
  static async setValueWithoutTap(element: ElementLike, text: string) {
  ...
  static async swipeOnPercentage(from: Coordinates, to: Coordinates) {
  ...
  static async swipe(from: Coordinates, to: Coordinates) {
    // TODO
  ...
  static calculateXY({ x, y }: Coordinates, percentage: number) {
    return {
      x: x * percentage,
      y: y * percentage,
    };
