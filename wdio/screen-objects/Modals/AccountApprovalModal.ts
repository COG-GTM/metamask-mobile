...
class AccountApprovalModal {
  ...
  async tapConnectMultipleAccountsButton() {
    ...
    const element = (await this.connectMultipleAccountsButton) as WebdriverIO.Element;
  ...
  async setTokenAmount(amount: string) {
  ...
  async isVisible() {
    const modalContainer = (await this.modalContainer) as WebdriverIO.Element;
    await modalContainer.waitForDisplayed();
  }

  async waitForDisappear() {
    const element = (await this.modalContainer) as WebdriverIO.Element;
    await element.waitForExist({ reverse: true });
  }

  async tapSelectAllButton() {
    const selectAllButton = (await this.selectAllButton) as WebdriverIO.Element;
