...
class TabBarModal {
  ...
  async tapWalletButton() {
    const walletButton = (await this.walletButton) as WebdriverIO.Element;
    await walletButton.waitForDisplayed();

    const browserScreen = (await BrowserScreen.container) as WebdriverIO.Element;
  ...
  async tapActionButton() {
    const actionButton = (await this.actionButton) as WebdriverIO.Element;
