...
class TermOfUseScreen {
  ...
  async isDisplayed() {
    const container = (await this.container) as WebdriverIO.Element;
  ...
  async acceptIsEnabled() {
    const element = (await this.acceptButton) as WebdriverIO.Element;
    return element.isEnabled();
  }

  async isCheckBoxChecked() {
    const element = (await this.checkbox) as WebdriverIO.Element;
    return element.isEnabled();
  }
