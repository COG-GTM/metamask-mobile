...
class WalletMainScreen {
  ...
  async tapImportTokensButton() {
    const importToken = (await this.ImportToken) as WebdriverIO.Element;
  ...
  async backupAlertModalIsVisible() {
    const element = (await this.backupAlertModal) as WebdriverIO.Element;
  ...
  async isNetworkNameCorrect(network: string) {
    const networkName = (await Selectors.getXpathElementByTextContains(network)) as WebdriverIO.Element;
    await networkName.waitForDisplayed();
  }

  async isTokenTextVisible(token: string) {
    const tokenText = (await Selectors.getXpathElementByTextContains(token)) as WebdriverIO.Element;
  ...
  async isSubmittedNotificationDisplayed() {
    const element = (await this.TokenNotificationTitle) as WebdriverIO.Element;
  ...
  async isNetworkNavbarTitle(text: string) {
  ...
  async waitForNetworkModalToDisappear() {
    const element = (await this.networkModal) as WebdriverIO.Element;
    await element.waitForExist({ reverse: true });
  }
}
