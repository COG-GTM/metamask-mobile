...
class WelcomeScreen {
  ...
  async waitForSplashAnimationToDisplay() {
    const elem = (await this.splashScreenMetamaskAnimationId) as WebdriverIO.Element;
    const getStartedElem = (await this.getStartedButton) as WebdriverIO.Element;
    try {
  ...
  async verifyCarouselTitle(key: number) {
  ...
  async getCarouselRect() {
    ...
    const element = (await this.screen) as WebdriverIO.Element;
