...
class CreateNewWalletScreen {
  ...
  async inputPasswordInFirstField(firstPassword: string) {
    await Gestures.typeText(this.newWalletPasswordField, firstPassword);
  }

  async inputConfirmPasswordField(secondPassword: string) {
  ...
  async inputConfirmResetPasswordField(secondPassword: string) {
    await Gestures.typeText(this.newWalletPasswordConfirm, secondPassword);
    await driver.hideKeyboard();
  }

  async tapSubmitButton() {
    const elements = await this.newWalletSubmitButton;
    await Gestures.waitAndTap(elements[0]);
  }
