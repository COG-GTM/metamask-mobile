...
class SecurityAndPrivacyScreen {
  ...
  async isChangePasswordTextVisible(text: string) {
  ...
  async tapRememberToggle() {
    const element = (await this.rememberMeToggle) as WebdriverIO.Element;
  ...
  async tapDeleteWalletButton() {
    const element = (await this.deleteWalletButton) as WebdriverIO.Element;
    while (!(await element.isDisplayed())) {
  ...
  async isRememberMeToggle(value: string) {
