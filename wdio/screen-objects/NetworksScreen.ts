...
class NetworksScreen {
  ...
  async waitForDisplayed() {
    const element = (await this.container) as WebdriverIO.Element;
    await element.waitForDisplayed();
  }

  async isPopularNetworksTabVisible() {
    const element = (await this.getPopularNetworksTab) as WebdriverIO.Element;
    await element.waitForDisplayed();
  }

  async isCustomNetworksTabVisible() {
    const element = (await this.getCustomNetworks) as WebdriverIO.Element;
    await element.waitForDisplayed();
  }
  ...
  async tapAndHoldNetwork(network: string) {
    await Gestures.tapTextByXpath(network);
  }
  ...
  async typeIntoNetworkName(text: string) {
  ...
  async typeIntoRPCURLField(text: string) {
  ...
  async typeIntoCHAINIDInputField(text: string) {
  ...
  async typeIntoNetworkSymbol(text: string) {
  ...
  async tapRemoveNetworkButton(text: string) {
    await Gestures.tapTextByXpath(text);
  }

  async isButtonTextVisibleByXpath(text: string) {
    expect(await Selectors.getXpathElementByText(text)).toBeDisplayed();
  }

  async isNetworkRemoved(network: string) {
    const element = (await Selectors.getXpathElementByText(network)) as WebdriverIO.Element;
    await element.waitForExist({ reverse: true });
  }

  async tapOnNetwork(network: string) {
    await Gestures.tapTextByXpath(network);
  }

  async isNetworkVisible(network: string) {
    const networkElement = (await Selectors.getXpathElementByText(network)) as WebdriverIO.Element;
    await networkElement.waitForDisplayed();
  }

  async isNetworkNotVisible(text: string) {
    const networkElement = (await Selectors.getXpathElementByText(text)) as WebdriverIO.Element;
    await networkElement.waitForExist({ reverse: true });
  }

  async tapOptionInSettings(text: string) {
    await Gestures.tapTextByXpath(text);
  }

  async isNetworknameDisplayed(network: string) {
