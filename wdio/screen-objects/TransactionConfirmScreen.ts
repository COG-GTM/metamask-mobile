...
class TransactionConfirmScreen {
  ...
  async isCorrectTokenConfirm(token: string) {
    const confirmAmount = (await this.confirmAmount) as WebdriverIO.Element;
    await confirmAmount.waitForDisplayed();
    expect(confirmAmount).toHaveTextContaining(token);
  }

  async isCorrectTokenAmountDisplayed(amount: string) {
    const confirmAmount = (await this.confirmAmount) as WebdriverIO.Element;
    await confirmAmount.waitForDisplayed();
  ...
  async waitEstimatedGasFeeToDisplay() {
    const estimatedGasFee = (await this.estimatedGasFee) as WebdriverIO.Element;
  ...
  async areSuggestedGasOptionsNotVisible() {
    const suggestedGasOptions = (await this.suggestedGasOptions) as WebdriverIO.Element;
