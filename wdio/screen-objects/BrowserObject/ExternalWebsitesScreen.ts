...
class ExternalWebsitesScreen {
  ...
  async tapHomeFavoritesButton() {
    const element = (await this.homeFavoriteButton) as WebdriverIO.Element;
  ...
  async isErrorPageTitle(title: string) {
    await expect(this.errorPageTitle).toHaveText(title);
  }

  async isErrorPageMessage(message: string) {
  ...
  async tapDappConnectButton() {
    ...
    const element = (await this.testDappConnectButton) as WebdriverIO.Element;
    await element.waitForEnabled();
    await Gestures.waitAndTap(this.testDappConnectButton);
  }

  async tapDappTransferTokens() {
    const element = (await this.testDappTransferTokens) as WebdriverIO.Element;
    await element.waitForEnabled();
    await Gestures.waitAndTap(this.testDappTransferTokens);
  }

  async tapDappTransferNft() {
    const element = (await this.testDappTransferNft) as WebdriverIO.Element;
    await element.waitForEnabled();
    await Gestures.waitAndTap(this.testDappTransferNft);
  }

  async tapDappApproveTokens() {
    const element = (await this.testDappApproveTokens) as WebdriverIO.Element;
    await element.waitForEnabled();
