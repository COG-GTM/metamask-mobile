...
class BrowserScreen {
  ...
  get accountIconButton() {
    // @ts-expect-error ACCOUNT_BUTTON was removed from BrowserViewSelectorsIDs (pre-existing stale reference)
  ...
  async isScreenContentDisplayed() {
    const screen = (await this.container) as WebdriverIO.Element;
    await screen.waitForDisplayed();
  }

  async tapUrlBar() {
    await driver.pause(500);
    const urlBarTitle = (await this.urlBarTitle) as WebdriverIO.Element;
  ...
  async tapOptionButton() {
    const element = (await this.optionButton) as WebdriverIO.Element;
    await element.waitForEnabled();
    await Gestures.waitAndTap(this.optionButton);
  }

  async numberOfTapsEqualsTo(expectedNumber: number) {
    const textFromElement = (await this.tabsButtonTextElement) as WebdriverIO.Element;
    const actualNumber = parseInt(await textFromElement.getText());
    await expect(await expectedNumber).toEqual(actualNumber);
  }

  async tapTabsButton() {
    const element = (await this.tabsButton) as WebdriverIO.Element;
    await element.waitForEnabled();
    await Gestures.waitAndTap(this.tabsButton);
  }

  async tapHomeButton() {
    const element = (await this.homeButton) as WebdriverIO.Element;
    await element.waitForEnabled();
    await Gestures.waitAndTap(this.homeButton);
  }

  async tapBackButton() {
    const element = (await this.backButton) as WebdriverIO.Element;
    await element.waitForEnabled();
